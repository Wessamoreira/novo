	...
	public void run() {		
		...
		try {
			...
			if(getRenovacaoMatriculaTurma().getSituacao().equals(SituacaoRenovacaoTurmaEnum.EM_PROCESSAMENTO)){
				try {
//					getFacadeFactory().getPermissaoFacade().getfacaverificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("Matricula_PermitirRealizarMatriculaDisciplinaPreRequisito", getUsuarioLogado());
			...
			for (RenovacaoMatriculaTurmaMatriculaPeriodoVO renovacaoMatriculaTurmaMatriculaPeriodoVO : renovacaoMatriculaTurmaMatriculaPeriodoVOs) {
				try {
					if (!getFacadeFactory().getRenovacaoMatriculaTurmaFacade().realizarVerificacaoRenovacaoTurmaInterrompida(getRenovacaoMatriculaTurma())) {
						renovacaoMatriculaTurmaMatriculaPeriodoVO.setNovaMatriculaPeriodoVO(getFacadeFactory().getRenovacaoMatriculaTurmaMatriculaPeriodoFacade().realizarRenovacaoAutomaticaAtravesRenovacaPorTurma(getRenovacaoMatriculaTurma(), renovacaoMatriculaTurmaMatriculaPeriodoVO,  getConfiguracaoGeralSistemaVO(), permitirRealizarMatriculaDisciplinaPreRequisito, getRenovacaoMatriculaTurma().getResponsavel(), false, gradeDisciplinaCompostaVOs));
