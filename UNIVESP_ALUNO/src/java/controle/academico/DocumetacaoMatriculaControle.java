...
public class DocumetacaoMatriculaControle extends SuperControle {
	...
	public void realizarCarregamentoDocumentacaoMatriculaVindoTelaFichaAluno() {
		...
		if (matriculaVO != null && !matriculaVO.getMatricula().equals("")) {
			try {
				...
				MatriculaPeriodoVO ultimaMatriculaPeriodo = (MatriculaPeriodoVO) getFacadeFactory().getMatriculaPeriodoFacade().consultaUltimaMatriculaPeriodoPorMatriculaConsultaBasica(matriculaVO.getMatricula(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado());
	...
	public String gravar() {
		try {
			getFacadeFactory().getControleAcessoFacade().alterar("EntregaDocumento", true, getUsuarioLogado());
...
//		}
	...
	public void executarVerificacaoUusuarioPodeAnexarImagemDocumentosEntregues() {
		...
		try {
			getFacadeFactory().getControleAcessoFacade().verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("EntregaDocumentoPermiteAnexarArquivo", getUsuarioLogado());
			liberar = true;
		} catch (Exception e) {
			liberar = false;
		}
	...
	public void excluirDocumentoGed() {
		try {
			getFacadeFactory().getControleAcessoFacade().excluir("EntregaDocumento", getUsuarioLogado());
	...
	public void persistirDocumentoGed() {
		try {
			...
			setOncompleteModal("PF('panelDocumentosEntreguesGED').hide()");
			setMensagemID("msg_dados_gravados");
			listaDocumentacaoGED = getFacadeFactory().getDocumentacaoGEDInterfaceFacade().consultarPorMatricula(getMatriculaVO().getMatricula(), Uteis.NIVELMONTARDADOS_TODOS, getUsuarioLogado());
		} catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage());
