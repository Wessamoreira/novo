...
public class ArquivoControle extends SuperControle {
	...
	/**
	 * Interface <code>ArquivoInterfaceFacade</code> responsvel pela interconexo
	 * da camada de controle com a camada de negcio. Criando uma independncia da
	 * camada de controle com relao a tenologia de persistncia dos dados
	 * (DesignPatter: Faade).
	 */
	...
	/**
	 * Rotina responsvel por disponibilizar um novo objeto da classe
	 * <code>Arquivo</code> para edio pelo usurio da aplicao.
	 */
	public void novo() {
		setArquivoVO(null);
	...
	/**
	 * Rotina responsvel por disponibilizar os dados de um objeto da classe
	 * <code>Arquivo</code> para alterao. O objeto desta classe  disponibilizado
	 * na session da pgina (request) para que o JSP correspondente possa
	 * disponibiliz-lo para edio.
	 */
	public void editar() {
		try {
	...
	/**
	 * Rotina responsvel por gravar no BD os dados editados de um novo objeto da
	 * classe <code>Arquivo</code>. Caso o objeto seja novo (ainda no gravado no
	 * BD)  acionado a operao <code>incluir()</code>. Caso contrrio  acionado o
	 * <code>alterar()</code>. Se houver alguma inconsistncia o objeto no 
	 * gravado, sendo re-apresentado para o usurio juntamente com uma mensagem de
	 * erro.
	 */
	public void persistir() throws Exception {
		...
		setOnCompletePanelUpload("PF('panelUploadArquivo').hide()");
...
//			getFacadeFactory().getGestaoEnvioMensagemAutomaticaFacade().executarEnvioMensagemPostagemMaterial(config, getArquivoVO());
	...
	public void verificarPermissaoParaVisualizacaoDataDisponibilizacaoMaterial() {
		try {
			getFacadeFactory().getControleAcessoFacade().alterar("ApresentarDataDisponibilizacaoMaterial", getUsuarioLogado());
	...
	/**
	 * Rotina responsvel por preencher a combo de consulta dos RichModal da telas.
	 */
	...
	public List<SelectItem> getTipoConsultarComboTurma() {
		if (tipoConsultarComboTurma == null) {
			...
			tipoConsultarComboTurma.add(new SelectItem("codigo", "Cdigo"));
	...
	/**
	 * Mtodo responsvel por processar a consulta na entidade
	 * <code>Disciplina</code> por meio dos parametros informados no richmodal. Esta
	 * rotina  utilizada fundamentalmente por requisies Ajax, que realizam busca
	 * pelos parmentros informados no richModal montando automaticamente o
	 * resultado da consulta para apresentao.
	 */
	...
	/**
	 * Mtodo responsvel por processar a consulta na entidade <code>Usuario</code>
	 * por meio de sua respectiva chave primria. Esta rotina  utilizada
	 * fundamentalmente por requisies Ajax, que realizam busca pela chave primria
	...
	/**
	 * Rotina responsvel por atribui um javascript com o mtodo de mascara para
	...
	/**
	 * Rotina responsvel por preencher a combo de consulta da telas.
	 */
	public List<SelectItem> getTipoConsultaCombo() {
		...
		itens.add(new SelectItem("codigoUsuario", "Responsvel Upload"));
		itens.add(new SelectItem("dataDisponibilizacao", "Data Disponibilizao"));
		itens.add(new SelectItem("nomeDisciplina", "Disciplina"));
		itens.add(new SelectItem("identificadorTurmaTurma", "Turma"));
		itens.add(new SelectItem("situacao", "Situao"));
		return itens;
	}

	/**
	 * Rotina responsvel por organizar a paginao entre as pginas resultantes de
	 * uma consulta.
	 */
	...
	public void upLoadArquivo(FileUploadEvent uploadEvent) {
		try {
			if (getUsuarioLogado().getVisaoLogar().equals("professor") || getUsuarioLogado().getVisaoLogar().equals("coordenador")) {
				if (uploadEvent.getFile() != null && uploadEvent.getFile().getSize() > (getConfiguracaoGeralPadraoSistema().getTamanhoMaximoUpload() * 1024 * 1024)) {
					setErroUpload("PF('panelMsgErroUpload').show()");
					setMsgErroUpload("Prezado professor/coordenador, seu arquivo excede o tamanho estipulado pela Instituio, por favor reduza o arquivo ou divida em partes antes de efetuar a postagem. Obrigado.");
				} else {
	...
	public void copiarArquivo(File origem, File destino) throws Exception {
		...
		// Copia contedo da origem no destino
	...
	public void organizarArquivosDiretorioCEAFI() {
		try {
			// Na base de dados do CEAFI existem arquivps que esto cadastrados
			// sem acentos,
			// contudo os mesmos existem nos diretrios com acento.
			...
			// System.out.println("=====================================================");
			// System.out.println(" FINALIZADA ALTERAO DE PASTA DE ARQUIVOS");
			// System.out.println("=====================================================");
			// getArquivo
...
//			getArquivoVO().setDataUpload(new Date());
	...
	public void novoVisaoProfessor() throws Exception {
		try {
			...
			setQtdeCaractereLimiteDownloadMaterial(getFacadeFactory().getConfiguracaoGeralSistemaFacade().consultarConfiguraoesQtdeCaractereLimiteDownloadMaterial());
			validarPermissao();
			setOnCompletePanelUpload("PF('panelUploadArquivo').show()");
			setMensagemID("msg_dados_consultados");
		} catch (Exception e) {
			setOnCompletePanelUpload("PF('panelUploadArquivo').hide()");
			setMensagemDetalhada("msg_erro", e.getMessage());
...
//		return getFacadeFactory().getTurmaFacade().consultarTurmaPorProfessorAnoSemestreNivelDadosCombobox(getUsuarioLogado().getPessoa().getCodigo(), getSemestre(), getAno(), "AT", getUnidadeEnsinoLogado().getCodigo(), getUsuarioLogado().getVisaoLogar().equals("professor"), true, true);
	...
	public void consultarTurma() {
		try {
			...
			if(getListaConsultaTurma().isEmpty()) {
				throw new Exception("No a uma Turma vinculada a Disciplina ( " + getDisciplinaVO().getNome() + " )");
			}
			setMensagemID("msg_dados_consultados");
		} catch (Exception e) {
	...
	public String getDownloadArquivo() {
		try {
			...
			if (getArquivoVO() != null && getUsuarioLogado().getIsApresentarVisaoAluno() && !getUsuarioLogado().getPermiteSimularNavegacaoAluno()) {
				getFacadeFactory().getDownloadFacade().registrarDownload(getArquivoVO(), getUsuarioLogado(), getVisaoAlunoControle().getMatriculaPeriodoVO(), getMatriculaVO());
			}
	...
	public void verificarPermissaoParaGravarUploadVisaoCoordenador() {
		try {
			if (getUsuarioLogado().getVisaoLogar().equals("coordenador")) {
				getFacadeFactory().getControleAcessoFacade().incluir("Upload", getUsuarioLogado());
	...
	public void verificarPermiteProfessorExcluirArquivosInstituicao() {
		Boolean liberar;
		try {
			getFacadeFactory().getControleAcessoFacade().verificarPermissaoUsuarioFuncionalidade("PermitirProfessorExcluirArquivoInstituicao", getUsuarioLogado());
			liberar = true;
		} catch (Exception e) {
			liberar = false;
		}
		...
		try {
			...
			if ((!getArquivosAnexadosProfessor().isEmpty() || !getArquivosAnexadosInstituicao().isEmpty()) && getMatriculaVO().getCurso().getNivelEducacionalPosGraduacao()) {
				...
				if (!getDentroDataMaximaLimiteDownload()) {
					throw new ConsistirException("Data limite para download de material foi ultrapassada. Favor entrar em contato com a Instituio de Ensino.");
				} else {
					setMensagemID("");
					setMensagem("");
				}
	...
	public  void verificarPermissaoUsuarioRealizarUpload(UsuarioVO usuario, String nomeEntidade) throws Exception {
		getFacadeFactory().getControleAcessoFacade().verificarPermissaoUsuarioFuncionalidadeComPerfilUsuarioVOEspecifico(nomeEntidade, usuario);
	}
...
//		}
	...
	public String getVerificarUltrapassouTamanhoMaximoUpload() {
		try {
			return "Arquivo no Enviado. Tamanho Mximo Permitido " + getConfiguracaoGeralPadraoSistema().getTamanhoMaximoUpload() + "MB.";
		} catch (Exception e) {
			return "";
		}
	}
	...
	public String getTamanhoMaximoUpload() {
		try {
			return "Tamanho Mximo Permitido: " + getConfiguracaoGeralPadraoSistema().getTamanhoMaximoUpload() + "MB.";
		} catch (Exception e) {
			return "Tamanho Mximo No Configurado";
		}
	}
	...
	public void alterarOrdemArquivoFilho() {
		try {
			...
			if (getSubir()) {
				...
				arquivoFilho2 = arquivoVO.getArquivoFilhoVOs()
						.get(arquivoVO.getArquivoFilhoVOs().indexOf(arquivoFilho1) - 1);
			} else {
			...
			getFacadeFactory().getArquivoFacade().alterarOrdemArquivoFilho(arquivoVO.getArquivoFilhoVOs(),
					arquivoFilho1, arquivoFilho2, getUsuarioLogado());
		} catch (Exception e) {
	...
	public void selecionarArquivoParaDownload() {
		try {
			...
			if (obj != null && getUsuarioLogado().getIsApresentarVisaoAluno() && !getUsuarioLogado().getPermiteSimularNavegacaoAluno()) {
				getFacadeFactory().getDownloadFacade().registrarDownload(obj, getUsuarioLogado(), getVisaoAlunoControle().getMatriculaPeriodoVO(), getMatriculaVO());
			}
			verificarQuantidadeDownloadsNaoRealizadosDownloadVisaoAluno();
...
//			}
	...
	public void verificarPostarMaterialComTurmaObrigatoriamenteInformado() {
		Boolean postarMaterialComTurmaObrigatoriamenteInformado;
		try {
			getFacadeFactory().getControleAcessoFacade().verificarPermissaoUsuarioFuncionalidade("PostarMaterialComTurmaObrigatoriamenteInformado", getUsuarioLogado());
