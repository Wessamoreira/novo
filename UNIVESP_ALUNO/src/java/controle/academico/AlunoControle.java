 ...
 * p�ginas pessoaForm.jsp pessoaCons.jsp) com as funcionalidades da classe
 * <code>Pessoa</code>. Implemta��o da camada controle (Backing Bean).
 * 
 * @see SuperControle
 * @see Pessoa
 * @see PessoaVO
...
;
	...
	/**
	 * Interface <code>PessoaInterfaceFacade</code> respons�vel pela
	...
	// Atributo para listar a(s) matricula(s) do(s) aluno(s) na visualiza��o da
	...
	/**
	 * Atributo utilizado para gera��o do relat�rio de ficha individual do
	 * aluno.
	 */
	...
	/**
	 * Atributo Utilizado para gera��o doa lista de Documentos 
	 *
	 */
	...
	/**
	 * 
	 * Atributo Utilizado para gera��o doa lista de Registro Atividade Complementar 
	 */
	...
	public void verificarPermissaoPodeMatricula() {
		try {
			getFacadeFactory().getControleAcessoFacade().incluir("Matricula", true, getUsuarioLogado());
...
//				inicializarListasSelectItemTodosComboBox();
	 ...
	 * <code>Pessoa</code> para edi��o pelo usu�rio da aplica��o.
	 */
...
//	}
	 ...
	 * <code>Pessoa</code> para altera��o. O objeto desta classe �
...
//		inicializarListasSelectItemTodosComboBox();
	 ...
	 * da classe <code>Pessoa</code>. Caso o objeto seja novo (ainda n�o gravado
	 * no BD) � acionado a opera��o <code>incluir()</code>. Caso contr�rio �
	 * acionado o <code>alterar()</code>. Se houver alguma inconsist�ncia o
	 * objeto n�o � gravado, sendo re-apresentado para o usu�rio juntamente com
	...
	public void gravar() throws Exception {
		try {
			...
			if (!Uteis.isAtributoPreenchido(getPessoaVO().getCPF()) && !Uteis.isAtributoPreenchido(getPessoaVO().getCertidaoNascimento())) {
				throw new Exception("� Obrigat�rio Preencher o Campo CPF ou Certid�o de Nascimento(Documentos Pessoais)");
			}
			...
			if (!getVerificarCpf() && !Uteis.isAtributoPreenchido(getPessoaVO().getCPF())) {
				...
				if (!Uteis.verificaCPF(getPessoaVO().getCPF())) {
					throw new Exception("CPF Inv�lido (Documentos Pessoais)");
				}
			} 
...
//			return Uteis.getCaminhoRedirecionamentoNavegacao("alunoForm.xhtml");
	...
	public String getAbrirModalConfirmacaoAlterarResponsavelContas(){
		if (getPossuiPermissaoAlterarResponsavelFinanceiroContas()) {
			...
			return "PF('panelConfirmacaoAlterarResponsavelContas').show()";
		}
		return "";
	}
	...
	public String getFecharRichModalCompromisso() {
		if (getPermitirAlterarCompromisso()) {
			return "PF('panelNovoCompromisso').hide();";
		}
		return "";
	}
...
//			}			
	 ...
	 * <code>PessoaVO</code> Ap�s a exclus�o ela automaticamente aciona a rotina
	...
	public void adicionarFiliacao() {
		try {
			...
			if (getPessoaVO().getCPF().equals(getFiliacaoVO().getCPF())) {
				throw new ConsistirException("O campo CPF do respons�vel deve ser diferente do aluno.");
			}
			...
			if (getFiliacaoVO().getResponsavelFinanceiro()) {
				if (!Uteis.isAtributoPreenchido(getFiliacaoVO().getCPF())) {
					throw new ConsistirException("O campo CPF do respons�vel financeiro deve ser informado!");
				}
				if (!Uteis.validaCPF(Uteis.removerAcentos(Uteis.removeCaractersEspeciais(getFiliacaoVO().getCPF())).replaceAll(" ", ""))) {
					throw new ConsistirException("O campo CPF do respons�vel financeiro � inv�lido (" + getFiliacaoVO().getCPF()+ ")!");
				}
	 ...
	 * <code>Filiacao</code> para edi��o pelo usu�rio.
	 */
	...
	/*
	 * M�todo respons�vel por disponibilizar dados de um objeto da classe
	 * <code>FormacaoAcademica</code> para edi��o pelo usu�rio.
	 */
	public void editarFormacaoAcademica() throws Exception {
		...
		montartListaSelectItemTitulo();
			...
			setMensagemID("Para Editar Libere a EDI��O ALUNO!" , Uteis.ALERTA, true);
		}
	}
...
//				mensagem += "Funcion�rio cadastrado";
//				setPessoaVO(pessoaExistente);
//				setEditarDados(true);
...
//			}
//			mensagem += " com este CPF. \n " + (pessoaExistente.getAluno() ? "Deseja editar o cadastro do aluno?" : "Deseja cadastr�-lo como aluno?");
//			throw new Exception(mensagem);
//		} else {
...
//	}
	...
	public void validarPessoaCadastradaPorCertidaoNascimento(Boolean certidaoInvalido) throws Exception {
		...
		if (!getPessoaVO().getCertidaoNascimento().equals("")) {
			...
			ajaxRedirecionarFocoCampo = "";
			throw new Exception("Certid�o inv�lida.");
		}
		if (pessoaExistente.getCodigo().intValue() != 0) {
			ajaxRedirecionarFocoCampo = "";
			mensagem = "J� existe um ";
	...
	public void removerFormacaoAcademica() throws Exception {
		try {
			...
			if (aptoRemover != null && !aptoRemover) {
				throw new Exception("N�o � poss�vel remover est� Forma��o Acad�mica.O Registro est� vinculado a uma MATR�CULA do Aluno.");
			}
	...
	/**
	 * M�todo respons�vel por consultar Cidade <code>Cidade/code>.
	 * Buscando todos os objetos correspondentes a entidade <code>Cidade</code>.
	 * Esta rotina n�o recebe par�metros para filtragem de dados, isto �
	 * importante para a inicializa��o dos dados da tela para o acionamento por
	 * meio requisi��es Ajax.
	 */
	...
	/**
	 * M�todo respons�vel por selecionar o objeto CidadeVO <code>Cidade/code>.
	 */
	...
	/**
	 * M�todo respons�vel por carregar umaCombobox com os tipos de pesquisa de
	...
	public List<SelectItem> getTipoConsultaComboSalario() {
		List<SelectItem> itens = new ArrayList<SelectItem>(0);
		itens.add(new SelectItem("At� R$999", "At� R$999,00"));
		itens.add(new SelectItem("R$1000 � R$1999", "De R$ 1000,00 at� R$ 1999,00"));
		itens.add(new SelectItem("R$2000 � R$2999", "De R$ 2000,00 at� R$ 2999,00"));
		itens.add(new SelectItem("R$3000 � R$3999", "De R$ 3000,00 at� R$ 3999,00"));
		itens.add(new SelectItem("R$4000 � R$4999", "De R$ 4000,00 at� R$ 4999,00"));
		itens.add(new SelectItem("R$5000 � R$5999", "De R$ 5000,00 at� R$ 5999,00"));
	...
	public List<SelectItem> getTipoComboNivelIngles() {
		...
		itens.add(new SelectItem("intermediario", "Intermedi�rio"));
		itens.add(new SelectItem("avancado", "Avan�ado"));
		return itens;
	}
	...
	public List<SelectItem> getTipoComboTipoDocumento() {
		...
		itens.add(new SelectItem("CN", "Certid�o Nascimento"));
		return itens;
	}
	...
	/**
	 * M�todo respons�vel por consultar Naturalidade <code>Cidade/code>.
	 * Buscando todos os objetos correspondentes a entidade <code>Cidade</code>.
	 * Esta rotina n�o recebe par�metros para filtragem de dados, isto �
	 * importante para a inicializa��o dos dados da tela para o acionamento por
	 * meio requisi��es Ajax.
	...
	/**
	 * M�todo respons�vel por selecionar o objeto CidadeVO em Naturalidade
	 * <code>Cidade/code>.
	 */
	 ...
	 * entidade <code>Paiz</code>. Esta rotina n�o recebe par�metros para
	 * filtragem de dados, isto � importante para a inicializa��o dos dados da
	 * tela para o acionamento por meio requisi��es Ajax.
	 */
	 ...
	 * <code><code> e montar o atributo <code>nome</code> Este atributo � uma
...
//	}
	...
	public boolean isInscricaoPermitida() {
		try {
    		return getFacadeFactory().getControleAcessoFacade().verificarPermissaoFuncionalidadeUsuario("AlunoIniciarMatricula", getUsuarioLogado());
		} catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage());
			return false;
		}
...
//	}
	 ...
	 * <code>Cidade</code>. Esta rotina n�o recebe par�metros para filtragem de
	 * dados, isto � importante para a inicializa��o dos dados da tela para o
...
//		itens.add(new SelectItem("matricula", "Matr�cula"));
		itens.add(new SelectItem("nome", "Nome"));
		itens.add(new SelectItem("nomeMae", "Nome da M�e"));
		itens.add(new SelectItem("nomePai", "Nome do Pai"));
		itens.add(new SelectItem("responsavelLegal", "Respons�vel Legal"));
		itens.add(new SelectItem("CPF", "CPF"));
		itens.add(new SelectItem("RG", "RG"));		
		itens.add(new SelectItem("registroAcademico", "Registro Acad�mico"));
	...
	/**
	 * Rotina respons�vel por organizar a pagina��o entre as p�ginas resultantes
	 * de uma consulta.
	 */
	...
	/**
	 * M�todo responsavel por consultar o Aluno pelo codigo no momento em que o
	 * usu�rio clicar no bot�o(fichaAluno) preenchendo a lista de Matricula.
	 * 
	 * @Autor Carlos
	 */
	...
	/**
	 * M�todo responsavel por invocar outro Controlador passando por par�metro o
	 * m�todo a ser executado e o obj.
	 * 
	 * @Autor Carlos
	 */
	...
	/**
	 * M�todo respons�vel por apresentar a aba Dados Acad�micos e consultar uma
	 * matricula pela chave prim�ria e seus filhos(MatriculaPeriodo no caso).
	 * 
	 * @Autor Carlos
	 */
...
//	}
	...
	/**
	 * M�todo respons�vel por apresentar a aba Turma Disciplina e consultar uma
	 * matriculaPeriodo pela chave prim�ria e seus
...
//			contratoPronto = planoFin.getTextoPadraoContratoMatricula().substituirTagsTextoPadraoContratoMatricula(null, getMatriculaVO(), getMatriculaPeriodoVO(), plano, new ArrayList<PlanoDescontoVO>(), dc, getUsuarioLogado());
//		}
//		if (tipoContrato.equals("FI")) {
//			contratoPronto = planoFin.getTextoPadraoContratoFiador().substituirTagsTextoPadraoContratoFiador(getMatriculaVO(),  getMatriculaPeriodoVO(), getMatriculaPeriodoVO(), plano, new ArrayList<PlanoDescontoVO>(), dc, getUsuarioLogado());
//		}
...
//			String enderecoUnidadeEnsino = unidadeEnsinoVO.getEndereco() + ", n� " + unidadeEnsinoVO.getNumero() + ", " + unidadeEnsinoVO.getSetor() + ", " + unidadeEnsinoVO.getCidade().getNome() + "-" + unidadeEnsinoVO.getCidade().getEstado().getSigla() + ", Fone:" + unidadeEnsinoVO.getTelComercial1();
//			getSuperParametroRelVO().adicionarParametro("endereco", enderecoUnidadeEnsino);
//			realizarImpressaoRelatorio();
//			removerObjetoMemoria(this);
//			registrarAtividadeUsuario(getUsuarioLogado(), "BoletimAcademicoRelControle", "Finalizando Gera��o de Relat�rio Boletim Acad�mico", "Emitindo Relat�rio");
...
//	}
	...
	public FuncionarioVO consultarFuncionarioPorMatricula(String matricula) throws Exception {
		...
		try {
			...
			if (Uteis.isAtributoPreenchido(funcionarioVO)) {
				...
				setMensagemDetalhada("msg_erro", "Funcion�rio de matr�cula " + matricula + " n�o encontrado. Verifique se o n�mero de matr�cula est� correto.");
			}
			setMensagemDetalhada("");
			setMensagemID("msg_dados_consultados");
		} catch (Exception e) {
	...
	public List<SelectItem> montarComboCargoFuncionario(List<FuncionarioCargoVO> cargos) throws Exception {
		try {
			if (cargos != null && !cargos.isEmpty()) {
				...
				setMensagemDetalhada("O Funcion�rio selecionado n�o possui cargo configurado");
			}
			return null;
		} catch (Exception e) {
			throw e;
	...
	public void montarListaAtividadeComplementarMatriculaVO(){
		try {
			setListaRegistroAtividadeComplementarMatriculaVOs(getFacadeFactory().getAcompanhamentoAtividadeComplementarFacade().consultar(this.getMatriculaVO().getCurso().getCodigo(), 0, "", "", "", this.getMatriculaVO().getMatricula(), 0, null, false,this.getMatriculaVO().getGradeCurricularAtual().getCodigo(), getControleConsultaOtimizado() ,this.getUsuarioLogado()));
		} catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
	}
	...
	public void verificarPermissaoLiberarEdicaoAlunoSomenteComSenha() {
		try {
			getFacadeFactory().getControleAcessoFacade().verificarPermissaoUsuarioFuncionalidadeComPerfilUsuarioVOEspecifico(PerfilAcessoPermissaoAcademicoEnum.LIBERAR_EDICAO_ALUNO_SOMENTE_COM_SENHA, getUsuarioLogadoClone());
	...
	public void persistirLiberacaoAlunoSomenteComSenha() {
		try {
			UsuarioVO usuarioVerif = getFacadeFactory().getControleAcessoFacade().verificarLoginUsuario(this.getUserNameLiberarFuncionalidade(), this.getSenhaLiberarFuncionalidade(), true, Uteis.NIVELMONTARDADOS_TODOS);
			getFacadeFactory().getControleAcessoFacade().verificarPermissaoUsuarioFuncionalidadeComPerfilUsuarioVOEspecifico(PerfilAcessoPermissaoAcademicoEnum.LIBERAR_EDICAO_ALUNO_SOMENTE_COM_SENHA, usuarioVerif);
			...
			setLiberarEdicaoAlunoSomenteComSenha(false);
			setMensagemID("Dados Pronto Para Edi��o" , Uteis.SUCESSO, true);
		} catch (Exception e) {
			setLiberarEdicaoAlunoSomenteComSenha(true);
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
...
//			setDisciplinaVO(obj);
	...
	public void verificarTornarCampoMidiaCaptacaoObrigatorio() throws Exception {
		try {
			getFacadeFactory().getControleAcessoFacade().verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("AlunoTornarCampoTipoMidiaCaptacaoObrigatorio", getUsuarioLogado());
	...
	public void verificarPermissaoLiberarEdicaoRegistroAcademicoAluno() {
		try {
			getFacadeFactory().getControleAcessoFacade().verificarPermissaoUsuarioFuncionalidadeComPerfilUsuarioVOEspecifico(PerfilAcessoPermissaoAcademicoEnum.LIBERAR_EDICAO_REGISTRO_ACADEMICO_ALUNO, getUsuarioLogadoClone());
