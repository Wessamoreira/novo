...
//			}
	...
	public void uploadArquivoAtividadeDiscursivaRespostaAluno(FileUploadEvent uploadEvent) {
		try {
			...
			setAbrirModalConfirmarSoliticaoAvaliacaoProfessor("PF('panelConfirmarSoliticaoAvaliacaoProfessor').show()");
	...
	private void montarListaSemestreAno() throws Exception {
		...
		if (matricula == null || matricula.getMatricula().equals("")) {
			matricula = getFacadeFactory().getMatriculaFacade()
					.consultarMatriculaPorCodigoPessoa(getUsuarioLogado().getPessoa().getCodigo(), 0, false, Uteis.NIVELMONTARDADOS_DADOSMINIMOS, getUsuarioLogado())
...
//			getFacadeFactory().getGestaoEnvioMensagemAutomaticaFacade().executarEnvioMensagemNotificacaoAlunoAvaliadoAtividadeDiscursiva(getAtividadeDiscursivaRespostaAlunoVO(), getUsuarioLogado());
			...
			setOncompleteModal("PF('panelNotaAluno').hide()");
	...
	public String editarAtvidadeDiscursivaRespostaAluno() {
		...
		try {
			...
			if (getUsuarioLogado().getIsApresentarVisaoAluno()) {
				...
				setMatriculaPeriodoVO(getFacadeFactory().getMatriculaPeriodoFacade().consultarPorChavePrimaria(getAtividadeDiscursivaRespostaAlunoVO().getMatriculaPeriodoTurmaDisciplinaVO().getMatriculaPeriodo(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado()));
			} else if (getUsuarioLogado().getIsApresentarVisaoProfessor()) {
...
//		}
	...
	void montarListaSelectItemNota() throws Exception {
		...
		if (Uteis.isAtributoPreenchido(getHistorico().getCodigo()) && Uteis.isAtributoPreenchido(getHistorico().getConfiguracaoAcademico().getCodigo())) {
			List<ConfiguracaoAcademicaNotaVO> configuracaoAcademicaNotaVOs = getFacadeFactory().getConfiguracaoAcademicoNotaFacade().consultarPorConfiguracaoAcademicoNotaPermiteLancamentoNota(getHistorico().getConfiguracaoAcademico().getCodigo(), false);
	...
	public void montarListaDeNotasDaConfiguracaoAcademico() {
		try {
			...
			getListaSelectItemVariavelNotaCfaVOs().addAll(getFacadeFactory().getConfiguracaoAcademicoFacade().consultarVariavelTituloConfiguracaoAcademicoPorTipoUsoNota( getUsuarioLogado()));
