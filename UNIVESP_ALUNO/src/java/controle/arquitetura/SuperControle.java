...
//import negocio.comuns.academico.ConfiguracaoLayoutBoletimVO;
...
//	}
	...
	public String tratarMensagemErroDetalhada(String mensagemDetalhada) {
		...
		if(context() != null && Uteis.isAtributoPreenchido(novaMensagem)){
			...
			// --- CORREÇÃO DE PROTEÇÃO DO FLASH ---
			try {
				context().getExternalContext().getFlash().setKeepMessages(true);
			} catch (Exception e) {
				// Ignora erro de Flash Scope para não travar a aplicação
			}
			// -------------------------------------
		}
		if (tratamentoErroVO != null) {
	...
	public void consultarRequerimento() {
		try {
			...
			if (getCampoConsultaRequerimento().equals("codigo")) {
				...
				objs = getFacadeFactory().getRequerimentoFacade().consultarPorCodigo(new Integer(valorInt), getTipoRequerimento(), this.getUnidadeEnsinoLogado().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			}
			if (getCampoConsultaRequerimento().equals("data")) {
				Date valorData = Uteis.getDate(getValorConsultaRequerimento());
				objs = getFacadeFactory().getRequerimentoFacade().consultarPorData(Uteis.getDateTime(valorData, 0, 0, 0), Uteis.getDateTime(valorData, 23, 59, 59), getTipoRequerimento(), this.getUnidadeEnsinoLogado().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			}
			if (getCampoConsultaRequerimento().equals("nomeTipoRequerimento")) {
				if (getValorConsultaRequerimento().length() < 2) {
					throw new Exception(getMensagemInternalizacao("msg_ParametroConsulta_vazio"));
				}
				objs = getFacadeFactory().getRequerimentoFacade().consultarPorNomeTipoRequerimento(getValorConsultaRequerimento(), this.getUnidadeEnsinoLogado().getCodigo(), true, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			}
			if (getCampoConsultaRequerimento().equals("situacao")) {
				objs = getFacadeFactory().getRequerimentoFacade().consultarPorSituacao(getValorConsultaRequerimento(), getTipoRequerimento(), this.getUnidadeEnsinoLogado().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			}
			if (getCampoConsultaRequerimento().equals("situacaoFinanceira")) {
				if (getValorConsultaRequerimento().length() < 2) {
					throw new Exception(getMensagemInternalizacao("msg_ParametroConsulta_vazio"));
				}
				objs = getFacadeFactory().getRequerimentoFacade().consultarPorSituacaoFinanceira(getValorConsultaRequerimento(), getTipoRequerimento(), this.getUnidadeEnsinoLogado().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			}
			if (getCampoConsultaRequerimento().equals("nomePessoa")) {
				if (getValorConsultaRequerimento().length() < 2) {
					throw new Exception(getMensagemInternalizacao("msg_ParametroConsulta_vazio"));
				}
				objs = getFacadeFactory().getRequerimentoFacade().consultarPorNomePessoa(getValorConsultaRequerimento(), getTipoRequerimento(), this.getUnidadeEnsinoLogado().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			}
			if (getCampoConsultaRequerimento().equals("cpfPessoa")) {
				if (getValorConsultaRequerimento().length() < 2) {
					throw new Exception(getMensagemInternalizacao("msg_ParametroConsulta_vazio"));
				}
				objs = getFacadeFactory().getRequerimentoFacade().consultarPorNomeCPFPessoa(getValorConsultaRequerimento(), getTipoRequerimento(), this.getUnidadeEnsinoLogado().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			}
			if (getCampoConsultaRequerimento().equals("matriculaMatricula")) {
				if (getValorConsultaRequerimento().length() < 2) {
					throw new Exception(getMensagemInternalizacao("msg_ParametroConsulta_vazio"));
				}
				objs = getFacadeFactory().getRequerimentoFacade().consultarPorMatriculaMatricula(getValorConsultaRequerimento(), getTipoRequerimento(), this.getUnidadeEnsinoLogado().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			}
...
//			System.out.println("INVESTLOGIN: exception: " + e.getMessage());
	...
	private boolean getAutenticacao(String idCliente, String url) throws Exception {
	    ...
	    Cliente autorizacao = target
	        .request(MediaType.APPLICATION_XML)
	        .post(Entity.entity(cliente, MediaType.APPLICATION_XML), Cliente.class);
	...
	public void removerControleMemoriaFlash(String idControle){
	    try {
	        if(context() != null && context().getExternalContext() != null){
	            ...
	            if (flash != null) {
	                // Tenta remover. Se der o NPE interno do Mojarra, o catch pega.
	                // Usar put(key, null) as vezes é mais seguro que remove(key) em algumas versões do JSF
	                flash.put(idControle, null); 
	                // Se realmente quiser usar remove:
	                // flash.remove(idControle);
	            }
	        }
	    } catch (Exception e) {
	        // Engole o erro de Flash. Não vale a pena quebrar a aplicação 
	        // só porque não conseguimos limpar uma memória temporária.
	        // System.out.println("Aviso: Falha ao limpar Flash Scope: " + e.getMessage());
	    }
	}
...
//			}
		...
		public void autenticarTokenCliente() throws Exception {
		    ...
		    String headerToken = request.getHeader(AUTHENTICATION_HEADER);
		    
		    if (headerToken != null) {
		        try {
		            // Decodifica Base64 usando java.util.Base64
		            byte[] decodedBytes = Base64.getDecoder().decode(headerToken);
		            token64 = new String(decodedBytes, StandardCharsets.UTF_8);
		        } catch (IllegalArgumentException e) {
		            System.err.println("Erro ao decodificar token Base64: " + e.getMessage());
		        }
		        
		        try {
		            tokenSha512 = headerToken;
		        } catch (Exception e) {
		            System.err.println("Erro ao processar token SHA-512: " + e.getMessage());
		        }
		    }
		    
		    String tokenSei = getFacadeFactory()
		        .getConfiguracaoGeralSistemaFacade()
		        .consultarTokenWebServicePadraoSistema();
		    ...
		    throw new SecurityException("Requisição não autorizada (Token Inválido)");
		}
	    ...
	    public void resetarMemoriaFlash() {
	        try {
	            if (context() != null && context().getExternalContext() != null) {
	                Flash flash = context().getExternalContext().getFlash();
	                if (flash != null) {
	                    // Tenta limpar de forma segura (padrão JSF)
	                    // O método .clear() pode não existir na interface Flash padrão dependendo da versão,
	                    // mas se você estiver usando ELFlash (Mojarra), ele existe.
	                    // Se o seu código original usava ELFlash.getFlash().clear(), mantenha mas com o try-catch.
	                    
	                    // Opção Genérica Segura:
	                    flash.clear(); // Se sua interface permitir
	                    
	                    // Se não tiver .clear(), você pode iterar e remover as chaves se necessário,
	                    // mas geralmente o try-catch abaixo já resolve o crash da chamada original.
	                    
	                    // Se o seu código original era algo como:
	                    // ((ELFlash) flash).clear(); 
	                    // Apenas envolva no try-catch abaixo.
	                }
	            }
	        } catch (Exception e) {
	            // Engole o erro.
	            // Se falhar ao limpar o flash, não é crítico para o sistema.
	             System.out.println("Aviso: Falha ao resetar Flash Scope: " + e.getMessage());
	        }
	    }
