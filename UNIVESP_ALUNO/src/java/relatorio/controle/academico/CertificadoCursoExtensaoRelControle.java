...
public class CertificadoCursoExtensaoRelControle extends SuperControleRelatorio {
    ...
    private boolean verificarPermissaoRegraEmissao() {
    	try {
getFacadeFactory().getControleAcessoFacade().verificarPermissaoUsuarioFuncionalidadeComPerfilUsuarioVOEspecifico(PerfilAcessoPermissaoAcademicoEnum.PERMITE_DEFINIR_REGRA_EMISSAO_CERTIFICADO, getUsuarioLogado());
...
public void imprimirPDF() {
	...
	public void montarRelatorioPorLayoutDiferenteDeTextoPadrao(List<CertificadoCursoExtensaoRelVO> certificadoCursoExtensaoRelVOs) throws Exception{
		...
		certificadoCursoExtensaoRelVOs = getFacadeFactory().getCertificadoCursoExtensaoRelFacade().criarObjeto(getCertificadoCursoExtensaoRelVO(), getMatriculaVO(), getPeriodoLetivoVO(), getTurmaVO(), getTipoLayout(), getFiltro(), getTipoDisciplinaPeriodoSelecionado().equals("DisciplinaDoPeriodoSelecionado"), getGradeDisciplinaVO().getDisciplina(),  getUsuarioLogado(), getTrazerTodasSituacoesAprovadas(), getProgramacaoFormaturaVO(),getListaCertificadoCursoExtensaoRelVOErros(), getListaCertificadoCursoExtensaoRelVOGerar());
    ...
    public void consultarAluno() {
        try {
            ...
            if (getCampoConsultaAluno().equals("matricula")) {
                objs = getFacadeFactory().getMatriculaFacade().consultarPorMatriculaNivelEducacional(getValorConsultaAluno(), getUnidadeEnsinoVO().getCodigo(), "", false, Uteis.NIVELMONTARDADOS_COMBOBOX, getUsuarioLogado());
            }
            if (getCampoConsultaAluno().equals("nomePessoa")) {
                objs = getFacadeFactory().getMatriculaFacade().consultarPorNomePessoaNivelEducacional(getValorConsultaAluno(), getUnidadeEnsinoVO().getCodigo(), "", false, Uteis.NIVELMONTARDADOS_COMBOBOX, getUsuarioLogado());
            }
            if (getCampoConsultaAluno().equals("nomeCurso")) {
                objs = getFacadeFactory().getMatriculaFacade().consultarPorNomeCursoNivelEducacional(getValorConsultaAluno(), getUnidadeEnsinoVO().getCodigo(), "", false, Uteis.NIVELMONTARDADOS_COMBOBOX, getUsuarioLogado());
            }
            if (getCampoConsultaAluno().equals("registroAcademico")) {
            	objs = getFacadeFactory().getMatriculaFacade().consultarPorRegistroAcademicoNivelEducacional(getValorConsultaAluno(), getUnidadeEnsinoVO().getCodigo(), "", false, Uteis.NIVELMONTARDADOS_COMBOBOX, getUsuarioLogado());
            }
...
//            setSelectItemsCargoFuncionarioSecundario(montarComboCargoFuncionario(getFacadeFactory().getFuncionarioCargoFacade().consultarCargoPorCodigoFuncionario(getCertificadoCursoExtensaoRelVO().getFuncionarioSecundarioVO().getCodigo(), false, Uteis.NIVELMONTARDADOS_COMBOBOX, getUsuarioLogado())));
    ...
    public void selecionarRequerimento() {
		try {
			...
			setRequerimentoVO(getFacadeFactory().getRequerimentoFacade().consultarPorChavePrimaria(obj.getCodigo(), Uteis.NIVELMONTARDADOS_TODOS, getUsuarioLogado()));
	...
	public String getExibirModalCertificadoErro() {
		if (!getListaCertificadoErro().isEmpty()) {
			return "PF('panelCertificadoErro').show()";
    	} 
		return "";
	}
	...
	public String getDownload() {
		if (getFazerDownload()) {
			...
			return "PF('panelImpressaoContratoExistente').show()";
		}
		return "";
	}
	...
	public List getTipoConsultaComboProgramacaoFormatura() {
	  ...
	  public List validarDataConsultaProgramacaoFormatura(List objs) throws Exception {
	    ...
	    private void montarListaCertificadoCursoExencaoPorTurma(TurmaVO turmaVO) {
        	try {
				List<MatriculaVO> listaMatricula = getFacadeFactory().getMatriculaFacade().executarConsultaPorTurmaCurso(turmaVO.getCodigo(), turmaVO.getCurso().getCodigo(), turmaVO.getUnidadeEnsino().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSMINIMOS, getUsuarioLogado());
