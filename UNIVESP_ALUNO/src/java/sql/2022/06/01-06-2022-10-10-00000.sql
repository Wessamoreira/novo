...
begin 
	...
	and exists (select configuracaogeralsistema.codigo from configuracaogeralsistema where configuracaogeralsistema.idautenticacaoservotimize = 'IPOG')
    	...
    	left join configuracaogedorigem on configuracaogedorigem.configuracaoged  = unidadeensino.configuracaoged and configuracaogedorigem.tipoorigemdocumentoassinado = 'CONTRATO' and coalesce(configuracaogedorigem.assinardocumento, false) is false
	 	...
	 	or (
	 		...
	 		and (unidadeensino.configuracaoged is null or configuracaogedorigem.codigo is null)
	 	))	 		 	
		and exists (select configuracaogeralsistema.codigo from configuracaogeralsistema where configuracaogeralsistema.idautenticacaoservotimize != 'IPOG')
    ) or exists(
