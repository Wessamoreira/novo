 ...
 */
public class TurmaUnidadeEnsinoSV_V2 extends HttpServlet {
	 ...
	 */
	public TurmaUnidadeEnsinoSV_V2() {
		super();
...
//			Integer codCurso = Integer.parseInt(curso);
			boolean trazerTurmaPos = true;
			if (codUnidadeEnsino == 28) {
				trazerTurmaPos = false;
			}
			List<TurmaVO> lista = getFacadeFactory().getTurmaFacade().consultaRapidaPorUnidadeEnsino("%", codUnidadeEnsino, false, false, false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			//List<TurmaVO> lista = getFacadeFactory().getTurmaFacade().consultarPorCurso(codCurso, codUnidadeEnsino, false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			// preenche o objeto
			Iterator i = lista.iterator();
			JSONObject my_obj_unid_nome = new JSONObject();
			while (i.hasNext()) {
				TurmaVO uni = (TurmaVO) i.next();
				if (uni.getSituacao().equals("AB")) {
					JSONObject my_obj_unid_cod = new JSONObject();
					my_obj_unid_cod.put("codigoTurma", uni.getCodigo());
					my_obj_unid_cod.put("identificacaoTurma", uni.getIdentificadorTurma());
					my_obj_unid_cod.put("situacao", uni.getSituacao());
					my_obj_unid_cod.put("codigoCurso", uni.getCurso().getCodigo());
					my_obj_unid_cod.put("nomeCurso", uni.getCurso().getNome());
					getFacadeFactory().getTurmaFacade().consultaRapidaDadosIntegracao(uni);
					my_obj_unid_cod.put("dataInicio", uni.getDataPrimeiraAulaProgramada());
					if (uni.getDataPrevisaoFinalizacao() != null) {
						my_obj_unid_cod.put("dataFim", Uteis.getData(uni.getDataPrevisaoFinalizacao()));
					} else {
						my_obj_unid_cod.put("dataFim", "");
					}
					my_obj_unid_cod.put("maxAlunos", uni.getNrMaximoMatricula());
					my_obj_unid_cod.put("matriculados", uni.getQtdMatriculados());
					my_obj_unid_cod.put("confirmados", uni.getQtdConfirmados());
					my_obj_unid_cod.put("preMatriculados", uni.getQtdPreMatriculados());
					my_obj_unid_nome.put(uni.getIdentificadorTurma(), my_obj_unid_cod.toString().replace("\\", "").replace("\"{", "{").replace("}\"", "}"));
				}
			}
