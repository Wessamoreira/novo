 ...
 */
public class DisciplinaUnidadeEnsinoSV extends HttpServlet {
	 ...
	 */
	public DisciplinaUnidadeEnsinoSV() {
		super();
...
//				codUnidadeEnsino = 18;
//			}
			List<TurmaDisciplinaVO> lista = getFacadeFactory().getTurmaDisciplinaFacade().consultarPorCodigoUnidadeEnsino(codUnidadeEnsino, false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			//List<TurmaDisciplinaVO> lista = getFacadeFactory().getTurmaDisciplinaFacade().consultarPorCodigoTurma(codTurma, false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			// preenche o objeto
			Iterator i = lista.iterator();
			JSONObject my_obj_unid_nome = new JSONObject();
			while (i.hasNext()) {
				TurmaDisciplinaVO uni = (TurmaDisciplinaVO) i.next();
				JSONObject my_obj_unid_cod = new JSONObject();
				my_obj_unid_cod.put("codigoTurma", uni.getTurma());				
				my_obj_unid_cod.put("codigoDisciplina", uni.getDisciplina().getCodigo());
				my_obj_unid_cod.put("descricaoDisciplina", uni.getDisciplina().getNome());
				my_obj_unid_nome.put(uni.getTurma() + "_" + uni.getDisciplina().getCodigo(), my_obj_unid_cod.toString().replace("\\", "").replace("\"{", "{").replace("}\"", "}"));				
			}
