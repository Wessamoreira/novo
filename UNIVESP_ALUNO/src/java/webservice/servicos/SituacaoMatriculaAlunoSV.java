...
//			ConfiguracaoFinanceiroVO confFinan = getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(matriculaPeriodo.getMatriculaVO().getUnidadeEnsino().getCodigo());
			getFacadeFactory().getMatriculaPeriodoFacade().carregarDados(matriculaPeriodo, NivelMontarDados.TODOS, new UsuarioVO());
...
//			}
			...
			// Contrato
            String diretorioContratoPdf = getFacadeFactory().getImpressaoContratoFacade().imprimirContratoRenovarMatricula("MA", matriculaPeriodo.getMatriculaVO(), matriculaPeriodo, configuracaoGeralSistema, usuResp);
