...
public class LogTrigger extends ControleAcesso implements LogTriggerInterfaceFacade {
	...
	public Integer executarConsultaTotalRegistrosLogTrigger(final String tabela, Date dataInicial, Date dataFinal, final Integer usuario, final List<CampoLogTriggerVO> filtros) throws Exception {
		...
		sql.append(sqlInterno);
		sql.append(") AS t ");
		return getConexao().getJdbcTemplate().queryForObject(sql.toString(), Integer.class);
	...
	public Integer executarConsultaTotalRegistroPorDataUsuarioTabela(Date dataInicial, Date dataFinal, final Integer usuario, final String tabela) throws Exception {
		...
		return getConexao().getJdbcTemplate().queryForObject("SELECT SUM(qtde) AS qtde FROM (" + sql.toString() + ") AS t", Integer.class);
...
//		}
	...
	public void realizarGeracaoTopoPadraoRelatorio(RegistroLogTriggerVO registroLogTriggerVO, String[] colunas, XSSFWorkbook workbook, XSSFSheet worksheet, String caminhoLogo) throws Exception {
	    ...
	    XSSFDrawing patriarch = worksheet.createDrawingPatriarch();
	    XSSFClientAnchor anchor = new XSSFClientAnchor(0, 0, 1023, 180, 0, 0, 1, 5);
	...
	private void realizarGeracaoCabecalhoRelatorio(XSSFSheet worksheet, String[] colunas) throws Exception {
	    ...
	    XSSFCellStyle styleOfHeader = getCss(worksheet.getWorkbook(), IndexedColors.GREY_25_PERCENT.getIndex(), (short) HorizontalAlignment.CENTER.getCode(), "", true, true, true, true, IndexedColors.BLACK.getIndex());
	...
	private void realizarGeracaoCorpoRelatorio(XSSFSheet worksheet, List<List<CampoLogTriggerVO>> campoLogTriggerVOs) throws Exception {
	    ...
	    for (List<CampoLogTriggerVO> campoLogTriggers : campoLogTriggerVOs) {
	        ...
	        cellColuna.setCellType(CellType.STRING);
	        cellColuna.setCellValue(campoLogTriggers.get(0).getLinha());
	        cellColuna.setCellStyle(getCss(worksheet.getWorkbook(), IndexedColors.WHITE.getIndex(), (short) HorizontalAlignment.LEFT.getCode(), "", true, true, true, true, IndexedColors.BLACK.getIndex()));
	...
	public void realizarGeracaoFiltrosRelatoriosRelatorio(RegistroLogTriggerVO registroLogTriggerVO, XSSFWorkbook workbook, XSSFSheet worksheet) throws Exception {
	    ...
	    cellCabecalho.setCellType(CellType.STRING);
	    cellCabecalho.setCellValue("Script");
	    cellCabecalho.setCellStyle(getCss(workbook, IndexedColors.GREY_25_PERCENT.getIndex(), (short) HorizontalAlignment.LEFT.getCode(), "", true, true, true, true, IndexedColors.BLACK.getIndex()));

	    cellCabecalho = cabecalho.createCell(1);
	    cellCabecalho.setCellType(CellType.STRING);
	    cellCabecalho.setCellStyle(getCss(workbook, IndexedColors.GREY_25_PERCENT.getIndex(), (short) HorizontalAlignment.LEFT.getCode(), "", true, true, true, true, IndexedColors.BLACK.getIndex()));

	    XSSFCellStyle styleOfString = getCss(worksheet.getWorkbook(), IndexedColors.WHITE.getIndex(), (short) HorizontalAlignment.LEFT.getCode(), "", true, true, true, false, IndexedColors.BLACK.getIndex());
	...
	public XSSFCellStyle getCss(XSSFWorkbook workbook, short corFundo, short alinhamentoHorizontal, String pattner, boolean bordaTopo, boolean bordaRodape, boolean bordaEsquerda, boolean bordaDireita, short corBorda) {
	    ...
	    for (int x = 0; x < workbook.getNumCellStyles(); x++) {
	        style = workbook.getCellStyleAt(x);
	        if (style.getFillForegroundColor() == corFundo 
	            && style.getAlignment().getCode() == alinhamentoHorizontal 
	            && ((!pattner.trim().isEmpty() && style.getDataFormat() == workbook.createDataFormat().getFormat(pattner)) || (pattner.trim().isEmpty() && style.getDataFormat() == 0)) 
	            && ((bordaRodape && style.getBorderBottom() == BorderStyle.THIN && style.getBottomBorderColor() == corBorda) || (!bordaRodape && style.getBorderBottom() == BorderStyle.NONE)) 
	            && ((bordaTopo && style.getBorderTop() == BorderStyle.THIN && style.getTopBorderColor() == corBorda) || (!bordaTopo && style.getBorderTop() == BorderStyle.NONE)) 
	            && ((bordaEsquerda && style.getBorderLeft() == BorderStyle.THIN && style.getLeftBorderColor() == corBorda) || (!bordaEsquerda && style.getBorderLeft() == BorderStyle.NONE))
	            && ((bordaDireita && style.getBorderRight() == BorderStyle.THIN && style.getRightBorderColor() == corBorda) || (!bordaDireita && style.getBorderRight() == BorderStyle.NONE))) {
	            return style;
	        }
	    }
	    ...
	    style.setAlignment(HorizontalAlignment.forInt(alinhamentoHorizontal));
