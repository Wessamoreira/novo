...
public class OfertaDisciplina extends ControleAcesso implements OfertaDisciplinaInterfaceFacade {
	...
	public void upload(FileUploadEvent uploadEvent, List<OfertaDisciplinaVO> ofertaDisciplinaVOs, List<OfertaDisciplinaVO> listaErro, String ano, String semestre) throws Exception {
		...
		InputStream is = uploadEvent.getFile().getInputStream();
		int rowMax = 0;
...
//			PARA XLS UTILIZA HSSFWorkbook
		...
		while (linha <= rowMax) {
			...
			}else if(row.getCell(0) != null && row.getCell(0).getCellType() == CellType.STRING && !row.getCell(0).getStringCellValue().trim().isEmpty()) {
			...
			}else if(row.getCell(1) != null &&  row.getCell(1).getCellType() == CellType.STRING && !row.getCell(1).getStringCellValue().trim().isEmpty()) {
			...
			else if(row.getCell(2) != null && row.getCell(2).getCellType() == CellType.STRING && !row.getCell(2).getStringCellValue().trim().isEmpty()) {
