...
public class RelatorioFinalFacilitador extends ControleAcesso implements RelatorioFinalFacilitadorInterfaceFacade {
	...
	private void consultarRelatoriosNaoEnviados(RelatorioFinalFacilitadorVO relatorioFinalFacilitador, List<UnidadeEnsinoVO> listaUnidadeEnsino, Date dataFinal, Date dataInicial, CalendarioRelatorioFinalFacilitadorVO calendarioRelatorioFinalFacilitadorVO, StringBuilder sqlstr, boolean trazerSomenteContador) {
		...
		sqlstr.append(" matriculaperiodoturmadisciplina.matricula, matriculaperiodoturmadisciplina.disciplina disciplina, salaaulablackboard.idsalaaulablackboard,");
		...
		sqlstr.append(" SELECT pei.codigo FROM pessoaemailinstitucional pei WHERE pei.pessoa = matricula.aluno");
		sqlstr.append(" ORDER BY pei.statusativoinativoenum,");
		sqlstr.append(" case when pei.email ilike ('%'||configuracaoldap.dominio) then 1 else 2 end");
		sqlstr.append(" LIMIT 1)");
		sqlstr.append(" INNER JOIN salaaulablackboardpessoa ON pessoaemailinstitucional.codigo = salaaulablackboardpessoa.pessoaemailinstitucional");
		sqlstr.append(" INNER join salaaulablackboard on salaaulablackboardpessoa.salaaulablackboard = salaaulablackboard.codigo ");
		sqlstr.append(" INNER JOIN salaaulablackboardpessoa as supervisor on supervisor.salaaulablackboard = salaaulablackboard.codigo ");
		sqlstr.append(" INNER JOIN pessoaemailinstitucional ps on ps.codigo = supervisor.pessoaemailinstitucional ");
		sqlstr.append(" INNER JOIN pessoa s on s.codigo = ps.pessoa ");
		...
		sqlstr.append(" AND salaaulablackboardpessoa.tiposalaaulablackboardpessoaenum = 'FACILITADOR'");
		sqlstr.append(" AND salaaulablackboardpessoa.salaaulablackboard IS NOT NULL");
		if(Uteis.isAtributoPreenchido(relatorioFinalFacilitador.getSupervisor())) {
			sqlstr.append(" AND (supervisor.tiposalaaulablackboardpessoaenum = 'ORIENTADOR' and s.codigo = '").append(relatorioFinalFacilitador.getSupervisor().getCodigo()).append("') ");
		}
		...
		sqlstr.append(" matriculaperiodoturmadisciplina.matricula, matriculaperiodoturmadisciplina.disciplina, salaaulablackboard.idsalaaulablackboard,");
	...
	public List<RelatorioFinalFacilitadorVO> consultarRelatorioFacilitador(RelatorioFinalFacilitadorVO relatorioFinalFacilitador, List<UnidadeEnsinoVO> listaUnidadeEnsino, Date dataInicial, Date dataFinal, CalendarioRelatorioFinalFacilitadorVO calendarioRelatorioFinalFacilitadorVO, String multiplasSituacoes, int nivelMontarDados, DataModelo dataModelo, UsuarioVO usuario) throws Exception {
		...
		sql.append(" INNER JOIN pessoaemailinstitucional  on pessoaemailinstitucional.pessoa = matricula.aluno");
		sql.append(" AND pessoaemailinstitucional.codigo  = (");
		sql.append(" SELECT pei.codigo FROM pessoaemailinstitucional pei WHERE pei.pessoa = matricula.aluno");
		sql.append(" ORDER BY pei.statusativoinativoenum,");
		sql.append(" case when pei.email ilike ('%'||configuracaoldap.dominio) then 1 else 2 end");
		sql.append(" LIMIT 1)");
		sql.append(" inner join pessoa on matricula.aluno = pessoa.codigo");
		sql.append(" inner join disciplina on disciplina.codigo = matriculaperiodoturmadisciplina.disciplina ");
	...
	public File realizarGeracaoExcel(List<RelatorioFinalFacilitadorVO> listaRelatorioFinalFacilitadorVO,  String urlLogoPadraoRelatorio,UsuarioVO usuario) throws Exception {
		...
		uteisExcel.realizarGeracaoTopoPadraoRelatorio(workbook, sheet, urlLogoPadraoRelatorio, tamanhoTitulo, "");
