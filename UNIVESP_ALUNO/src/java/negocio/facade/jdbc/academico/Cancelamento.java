...
public class Cancelamento extends ControleAcesso implements CancelamentoInterfaceFacade {
	...
	public void incluir(final CancelamentoVO obj, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO,  UsuarioVO usuario, Boolean verificarPermissao) throws Exception {
		try {
			...
			alterarSituacaoAcademicaMatriculaCancelamento(obj, usuario);
//			getFacadeFactory().getGestaoEnvioMensagemAutomaticaFacade().executarEnvioMensagemCancelamentoMatricula(obj.getMatricula().getAluno(), obj.getMatricula(), usuario);
	 ...
	 */
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	private void alterarSituacaoAcademicaMatriculaCancelamento(CancelamentoVO obj,  UsuarioVO usuario) throws Exception {
		...
		if (Uteis.isAtributoPreenchido(obj.getMatriculaPeriodoVO())) {
			...
			getFacadeFactory().getTrancamentoFacade().alterarSituacaoAcademicaMatricula(obj.getMatriculaPeriodoVO(), obj.getHistoricoVOs(), obj.getCodigoRequerimento(), obj.getData(), OrigemFechamentoMatriculaPeriodoEnum.CANCELAMENTO, obj.getCodigo(), false, false, obj.getInativarUsuarioLdapAoCancelarMatriculaPorOutroCursoMesmoNivelEducacional() , obj.getInativarUsuarioBlackBoardAoCancelarMatriculaPorOutroCursoMesmoNivelEducacional() ,usuario);
		} else {
			if (!obj.getMatricula().getCurso().getIntegral()) {
				MatriculaPeriodoVO matriculaPeriodoVO = getFacadeFactory().getMatriculaPeriodoFacade().consultarUltimaMatriculaPeriodoPorMatriculaSituacoes(obj.getMatricula().getMatricula(), "'AT', 'PR', 'TR'", false, Uteis.NIVELMONTARDADOS_COMBOBOX, usuario);
				...
				getFacadeFactory().getTrancamentoFacade().alterarSituacaoAcademicaMatricula(matriculaPeriodoVO, obj.getHistoricoVOs(), obj.getCodigoRequerimento(), obj.getData(), OrigemFechamentoMatriculaPeriodoEnum.CANCELAMENTO, obj.getCodigo(), false, false, obj.getInativarUsuarioLdapAoCancelarMatriculaPorOutroCursoMesmoNivelEducacional() , obj.getInativarUsuarioBlackBoardAoCancelarMatriculaPorOutroCursoMesmoNivelEducacional() ,usuario);
			} else {
				List listaMatriculaPeriodoVO = getFacadeFactory().getMatriculaPeriodoFacade().consultarMatriculaPeriodoPorMatriculaSituacoes(obj.getMatricula().getMatricula(), "'AT', 'PR', 'TR'", false, Uteis.NIVELMONTARDADOS_COMBOBOX, usuario);
...
//	}
	...
	public CancelamentoVO consultarPorCodigoRequerimento(Integer codigoPrm, int nivelMontarDados,  UsuarioVO usuario) throws Exception {
...
//	}
	 ...
	 *                Caso haja problemas de conexão ou restrição de acesso.
	 */
	public List<CancelamentoVO> consultarPorSituacao(String valorConsulta, boolean controlarAcesso, int nivelMontarDados,  UsuarioVO usuario) throws Exception {
		getFacadeFactory().getControleAcessoFacade().consultar(getIdEntidade(), controlarAcesso, usuario);
...
//	}
	...
	public List<CancelamentoVO> consultarPorDescricao(String valorConsulta, boolean controlarAcesso, int nivelMontarDados,  UsuarioVO usuario) throws Exception {
		getFacadeFactory().getControleAcessoFacade().consultar(getIdEntidade(), controlarAcesso, usuario);
...
//	public List<CancelamentoVO> consultarPorData(Date prmIni, Date prmFim, boolean controlarAcesso, int nivelMontarDados,  UsuarioVO usuario) throws Exception {
//		getFacadeFactory().getControleAcessoFacade().consultar(getIdEntidade(), controlarAcesso, usuario);
...
//	}
	...
	public List<CancelamentoVO> consultarPorDataUnidadeEnsino(Date prmIni, Date prmFim, String unidadeEnsino, boolean controlarAcesso, int nivelMontarDados,  UsuarioVO usuario) throws Exception {
		getFacadeFactory().getControleAcessoFacade().consultar(getIdEntidade(), controlarAcesso, usuario);
	 ...
	 *                Caso haja problemas de conexão ou restrição de acesso.
	 */
	public List<CancelamentoVO> consultarPorCodigo(Integer valorConsulta, boolean controlarAcesso, int nivelMontarDados,  UsuarioVO usuario) throws Exception {
		getFacadeFactory().getControleAcessoFacade().consultar(getIdEntidade(), controlarAcesso, usuario);
	 ...
	 *         <code>CancelamentoVO</code> resultantes da consulta.
	 */
	public  List<CancelamentoVO> montarDadosConsulta(SqlRowSet tabelaResultado, int nivelMontaDados,  UsuarioVO usuario) throws Exception {
		List<CancelamentoVO> vetResultado = new ArrayList<CancelamentoVO>(0);
		while (tabelaResultado.next()) {
			vetResultado.add(montarDados(tabelaResultado, nivelMontaDados, usuario));
		}
		return vetResultado;
	}
	 ...
	 */
	public  CancelamentoVO montarDados(SqlRowSet dadosSQL, int nivelMontarDados,  UsuarioVO usuario) throws Exception {
		CancelamentoVO obj = new CancelamentoVO();
		...
		if (nivelMontarDados == Uteis.NIVELMONTARDADOS_DADOSMINIMOS) {
			...
			montarDadosCodigoRequerimento(obj, usuario);
			return obj;
		}
	 ...
	 */
	public  void montarDadosResponsavelAutorizacao(CancelamentoVO obj, UsuarioVO usuario) throws Exception {
	...
	public  void montarDadosResponsavelEstorno(CancelamentoVO obj, UsuarioVO usuario) throws Exception {
	 ...
	 *            Objeto no qual será montado os dados consultados.
	 */
	public  void montarDadosCodigoRequerimento(CancelamentoVO obj,  UsuarioVO usuario) throws Exception {
	 ...
	 *            Objeto no qual será montado os dados consultados.
	 */
	public  void montarDadosMatricula(CancelamentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	 ...
	 *                procurado.
	 */
	public CancelamentoVO consultarPorChavePrimaria(Integer codigoPrm, int nivelMontarDados,  UsuarioVO usuario) throws Exception {
		getFacadeFactory().getControleAcessoFacade().consultar(getIdEntidade(), false, usuario);
	...
	public void carregarDados(CancelamentoVO obj,  UsuarioVO usuario) throws Exception {
		carregarDados(obj, NivelMontarDados.TODOS, usuario);
	}

	public void carregarDados(CancelamentoVO obj, NivelMontarDados nivelMontarDados,  UsuarioVO usuario) throws Exception {
	...
	private void montarDadosCompleto(CancelamentoVO obj, SqlRowSet dadosSQL,  UsuarioVO usuario) throws Exception {
		obj.setCodigo(dadosSQL.getInt("codigo"));
		obj.setData(dadosSQL.getDate("data"));
		obj.setDescricao(dadosSQL.getString("descricao"));
		obj.setSituacao(dadosSQL.getString("situacao"));
	...
	public List<CancelamentoVO> montarDadosConsultaCompleta(SqlRowSet tabelaResultado,  UsuarioVO usuario) throws Exception {
		List<CancelamentoVO> vetResultado = new ArrayList<CancelamentoVO>(0);
		while (tabelaResultado.next()) {
			CancelamentoVO obj = new CancelamentoVO();
...
//			}
	 ...
	 */
	@Override
	public void persistir(final CancelamentoVO cancelamentoVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO,  UsuarioVO usuarioVO) throws Exception {
		if (cancelamentoVO.isNovoObj()) {
			incluir(cancelamentoVO, configuracaoGeralSistemaVO, usuarioVO, true);
		} else {
...
//			jobSincronizarCancelamento.start();
	...
	public CancelamentoVO consultarPorMatricula(String valorConsulta, int nivelMontarDados,  UsuarioVO usuario) throws Exception {
		getFacadeFactory().getControleAcessoFacade().consultar(getIdEntidade(), false, usuario);
		StringBuffer sql = getSQLPadraoConsultaCompleta();
		sql.append(" WHERE UPPER(c.matricula) LIKE '" + valorConsulta.toUpperCase() + "' ");
	...
	@Override
	public void realizarCancelamentoMatriculaAtivaPorOutroCursoMesmoNivelEducacional(MatriculaVO matriculaVO , ConfiguracaoGeralSistemaVO configuracaoGeralSistema,  UsuarioVO usuarioVO) throws Exception {
		try {
				...
				getFacadeFactory().getCancelamentoFacade().incluir(cancelamentoVO, configuracaoGeralSistema, usuarioVO, false);
	...
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void persistirCancelamentoMapaRegistroAbandonoCursoTrancamentoIndividualmente(MapaRegistroEvasaoCursoMatriculaPeriodoVO mrecmp, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO,  UsuarioVO usuarioVO) throws Exception {
		...
		persistir(mrecmp.getCancelamentoVO(), configuracaoGeralSistemaVO, usuarioVO);
		if (Uteis.isAtributoPreenchido(mrecmp.getMatriculaPeriodoVO())) {
			mrecmp.setMatriculaPeriodoVO(getFacadeFactory().getMatriculaPeriodoFacade().consultarPorChavePrimaria(mrecmp.getMatriculaPeriodoVO().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSMINIMOS, usuarioVO));
		}
	}
	
	private void realizarMontagemMatriculaPeriodoEvasao(MapaRegistroEvasaoCursoMatriculaPeriodoVO mrecmp,  UsuarioVO usuarioVO) throws Exception {
