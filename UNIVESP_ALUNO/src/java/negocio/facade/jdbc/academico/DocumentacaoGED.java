...
public class DocumentacaoGED extends ControleAcesso implements DocumentacaoGEDInterfaceFacade {
	...
	public  synchronized void processarArquivosDocumentacaoGED(Boolean validarAcesso, UsuarioVO usuarioVO) throws Exception {
		if (AplicacaoControle.registrarProcessamentoDigitalizacaoGED()) {
			try {
				...
				String caminhoBaseFinal = PastaBaseArquivoEnum.DIGITALIZACAO_GED.getValue() + File.separator + LocalDate.now() + File.separator + LocalDate.now();
				if (filesOrigem != null) {
	...
	private  synchronized void executarProcessamentoLoteDocumentacaoGED(File lote, String caminhoBaseFinal, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO, Date hoje, Boolean validarAcesso, UsuarioVO usuarioVO) throws Exception {
...
//			}			
	...
	private ArquivoVO montarDadosArquivo(ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO, String[] dadosSeparadosArquivo, String caminhoBaseFinal, Date hoje, UsuarioVO usuario) throws Exception {
		ArquivoVO arquivoVO = new ArquivoVO();
		arquivoVO.setPastaBaseArquivo(PastaBaseArquivoEnum.DIGITALIZACAO_GED.getValue() + File.separator + LocalDate.now() + File.separator + LocalDate.now());
		String nomeArquivo = dadosSeparadosArquivo[0].substring(dadosSeparadosArquivo[0].lastIndexOf('\\') + 1, dadosSeparadosArquivo[0].length()).replaceAll("\"", "");
	...
	public void incluir(DocumentacaoGEDVO obj, Boolean validarAcesso, UsuarioVO usuario) throws Exception {
		try {
			incluir(getIdEntidade(), validarAcesso, usuario);
	...
	private  Date consultarDataUltimoProcessamento() {
