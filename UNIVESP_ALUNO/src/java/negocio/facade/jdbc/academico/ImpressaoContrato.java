...
//			}
	...
	public String imprimirContratoRenovarMatricula(String tipoContrato, MatriculaVO matricula, MatriculaPeriodoVO matriculaPeriodoVO,  ConfiguracaoGeralSistemaVO config, UsuarioVO usuarioLogado) throws Exception {
		ImpressaoContratoVO impressaoContratoVO =  new ImpressaoContratoVO();		 
		impressaoContratoVO.setMatriculaVO(matricula);
		impressaoContratoVO.setMatriculaPeriodoVO(matriculaPeriodoVO);
		...
		return imprimirContrato(tipoContrato, impressaoContratoVO, config, usuarioLogado);
	
	}
	
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public String imprimirContratoVisaoAluno(MatriculaVO matricula, MatriculaPeriodoVO matriculaPeriodoVO,  ConfiguracaoGeralSistemaVO config, UsuarioVO usuarioLogado) throws Exception {		
		...
		return carregarDadosParaImpressaoContratoVisaoAluno(texto, matricula, matriculaPeriodoVO, config, usuarioLogado);
	}
...
//		}
	...
	public String carregarDadosParaImpressaoContratoVisaoAluno(TextoPadraoVO texto, MatriculaVO matricula, MatriculaPeriodoVO matriculaPeriodoVO,  ConfiguracaoGeralSistemaVO config, UsuarioVO usuarioLogado) throws Exception {
		...
		return imprimirContratoRenovarMatricula(texto.getTipo(), matricula, matriculaPeriodoVO, config, usuarioLogado);
	}
...
//			getFacadeFactory().getMatriculaPeriodoFacade().carregarDados(impressaoContratoVO.getMatriculaPeriodoVO(), NivelMontarDados.TODOS, usuarioLogado);
...
//		return executarValidacaoTipoImpressaoContrato(texto, contratoPronto, impressaoContratoVO, config, impressaoContratoVO.getGerarNovoArquivoAssinado(), usuarioLogado);
	...
	private  void realizarGravacaoContrato(ImpressaoContratoVO impressaoContratoVO) throws Exception {
	...
	private  TextoPadraoVO getTextoPadraoVOTipoContratoNormal_Matricula_Extencao_Fiador(String tipoContrato, ImpressaoContratoVO impressaoContratoVO, UsuarioVO usuarioLogado, TextoPadraoVO texto) throws Exception {
...
//		}
	...
	public String imprimirContratoInclusaoReposicao(ImpressaoContratoVO impressaoContratoVO,  TextoPadraoVO textoPadraoContratoInclusao, UsuarioVO usuarioLogado, ConfiguracaoGeralSistemaVO config) throws Exception {
		...
		getFacadeFactory().getMatriculaPeriodoFacade().carregarDados(impressaoContratoVO.getMatriculaPeriodoVO(), NivelMontarDados.BASICO, usuarioLogado);
//		impressaoContratoVO.getMatriculaPeriodoVO().setProcessoMatriculaCalendarioVO(getFacadeFactory().getProcessoMatriculaCalendarioFacade().consultarPorMatriculaPeriodoUnidadeEnsinoCurso(impressaoContratoVO.getMatriculaPeriodoVO().getCodigo(), null, null, null, false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioLogado));
...
//		listaOrdemDesconto = (matri.getPlanoFinanceiroAluno().obterOrdemAplicacaoDescontosPadraoAtual());
		...
		getFacadeFactory().getMatriculaFacade().calcularTotalDesconto(matri, impressaoContratoVO.getMatriculaPeriodoVO(), listaOrdemDesconto, usuarioLogado);
...
//				List<BoletimAcademicoRelVO> boletimAcademicoRelVOs = getFacadeFactory().getBoletimAcademicoRelFacade().criarObjeto(
//					    impressaoContratoVO.getMatriculaVO(), 
//					    "BoletimAcademicoEnsinoMedioRel", 
//					    true, 
//					    impressaoContratoVO.getMatriculaPeriodoVO().getAno() + impressaoContratoVO.getMatriculaPeriodoVO().getSemestre(), 
//					    impressaoContratoVO.getMatriculaPeriodoVO().getTurma(), 
//					    impressaoContratoVO.getMatriculaVO().getUnidadeEnsino(), 
//					    null, 
//					    usuario, 
//					    false, 
//					    false, 
