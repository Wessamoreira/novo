...
//		}
	...
	public  synchronized void alterarDocumentoMatriculaAprovaPeloDepSincronizado(DocumetacaoMatriculaVO obj, UsuarioVO usuario) throws Exception {
		getFacadeFactory().getDocumetacaoMatriculaFacade().alterarDocumentoMatriculaAprovaPeloDep(obj, usuario);
	}
	...
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void alterarDocumentoMatriculaAprovaPeloDep(DocumetacaoMatriculaVO obj, MatriculaVO matricula , UsuarioVO usuario) throws Exception {
		validarPermissaoPermiteUploadDocumentoIndeferidoForaPrazoParaMatriculaProcessoSeletivo(matricula, obj ,usuario);
		getFacadeFactory().getDocumetacaoMatriculaFacade().alterarDocumentoMatriculaAprovaPeloDep(obj, usuario);
		if (obj.getValidarPdfA() && Uteis.isAtributoPreenchido(obj.getCaminhoPreviewPdfA())) {
			return;
		}
		getFacadeFactory().getMatriculaPeriodoFacade().realizarAtivacaoMatriculaValidandoRegrasEntregaDocumentoAssinaturaContratoMatricula(matricula.getMatricula() , Boolean.TRUE, usuario);
	}
...
//		
//		 String ids = listaDocumentacaoMatricula.stream()
//		                  .map(DocumentacaoCurso::getCodigo) 
//		                  .map(String::valueOf)                   
...
//		}
	...
	public  void montarDadosArquivo(DocumetacaoMatriculaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public  void montarDadosArquivoVerso(DocumetacaoMatriculaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public  void montarDadosArquivoAssinado(DocumetacaoMatriculaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public  void montarDadosTipoDeDocumento(DocumetacaoMatriculaVO obj, UsuarioVO usuario) throws Exception {
	...
	public  void montarDadosUsuario(DocumetacaoMatriculaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
...
//        	}
	...
	public  void montarDadosRespAprovacaoDocDep(DocumetacaoMatriculaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public void realizarUploadArquivo(FileUploadEvent uploadEvent, FileItem fileItem, DocumetacaoMatriculaVO documetacaoMatriculaVO, PessoaVO aluno, boolean frente, boolean ged, DocumentacaoGEDVO documentacaoGEDVO, UsuarioVO usuarioVO, String ambiente) throws Exception {
		...
		getFacadeFactory().getMatriculaFacade().validarExtensaoArquivo(uploadEvent != null ? uploadEvent.getFile().getFileName() : fileItem.getName(), documetacaoMatriculaVO, ged);
		if(frente) {
			getFacadeFactory().getDocumetacaoMatriculaFacade().validarExtensaoArquivoFrente(uploadEvent != null ? uploadEvent.getFile().getFileName() : fileItem.getName(), documetacaoMatriculaVO);
		}else if(!ged) {
			getFacadeFactory().getDocumetacaoMatriculaFacade().validarExtensaoArquivoVerso(uploadEvent != null ? uploadEvent.getFile().getFileName() : fileItem.getName(), documetacaoMatriculaVO);
		}
		if (uploadEvent != null) {
			String nomeArquivo = uploadEvent.getFile().getFileName();
			if (isPDF(nomeArquivo)) {
				inpuStream = ArquivoHelper.criarByteArrayOutputStream(uploadEvent.getFile().getInputStream());
		...
		try {
			if(ged) {
				...
				getFacadeFactory().getArquivoHelper().upLoad(uploadEvent, Objects.nonNull(inpuStream) ? inpuStream : uploadEvent.getFile().getInputStream(), documentacaoGEDVO.getArquivo(), configuracaoGeralSistemaVO, PastaBaseArquivoEnum.DIGITALIZACAO_GED_TMP, usuarioVO);
...
//					}
	...
	private  void montarDadosMotivoIndeferimentoDocumentoAluno(DocumetacaoMatriculaVO obj, UsuarioVO usuario) throws Exception {
	...
	private  void montarDadosUsuarioRespNegarDocumento(DocumetacaoMatriculaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public void validarPermissaoPermiteUploadDocumentoIndeferidoForaPrazoParaMatriculaProcessoSeletivo(MatriculaVO matricula, DocumetacaoMatriculaVO doc, UsuarioVO usuario) throws Exception {
		try {
			getFacadeFactory().getControleAcessoFacade().verificarPermissaoUsuarioFuncionalidadeComPerfilUsuarioVOEspecifico(PerfilAcessoPermissaoAcademicoEnum.ENTREGA_DOCUMENTO_PERMITE_UPLOAD_DEFERIMENTO_INDEFERIMENTO_DOCUMENTO_CALOURODENTROPRAZOCHAMADAPROCESSOSELETIVO.getValor(), usuario);
			} catch (Exception e) {
			return ;
		}
