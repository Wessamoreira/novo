...
public class ProgressBarVO extends SuperVO {
		...
		public void run() {
			if(controle != null && !metodo.trim().isEmpty()){				
				try{
					...
					controle.setMensagemDetalhada("msg_erro", "Mtodo "+metodo+" no encontrado no controle "+controle.getClass().getSimpleName(), Uteis.ERRO);
				}catch(Exception e){
					setForcarEncerramento(true);					
	...
	public String getPreencherStatusProgressBarVO(ProgressBarVO progressBarVO, String entidade, String chave) {
		...
		sb.append(" - (").append(entidade).append(" Atual N = ").append(chave).append(")");
		return sb.toString();		
	}
