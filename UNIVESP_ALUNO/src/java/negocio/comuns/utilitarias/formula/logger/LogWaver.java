...
class LogWaver extends ChainLinkTemplate  {
	...
	private void montarStringLogContexto(StringBuilder logger, String parte) {
		 if (parte.contains(CONTEXTO)) {
		        ...
		        int quantidadePalavrasRepetidas = 0;
		        int index = 0;
		        while ((index = parte.indexOf(VAR, index)) != -1) {
		            quantidadePalavrasRepetidas++;
		            index += VAR.length();
		        }
...
//	}
	...
	private void montarStringLogFuncoes(StringBuilder logger, String parte) {
	    if (parte.contains(FUNCOES)) {
	        ...
	        int quantidadePalavrasRepetidas = 0;
	        int index = 0;
	        while ((index = parte.indexOf(FUNCOES, index)) != -1) {
	            quantidadePalavrasRepetidas++;
	            index += FUNCOES.length();
	        }
	...
	public String[] getEventosString(String texto) {
		 int quantidade = StringUtils.countMatches(texto, "eventos");
		    String[] eventos = new String[quantidade];
...
//	}
	...
	public String[] getMapaString(String texto) {
	    int quantidade = StringUtils.countMatches(texto, "mapa");
	    String[] mapas = new String[quantidade];
	    String[] arrayString = texto.split(" ");
	    int contadorEventos = 0;
	    
	    for(String palavra : arrayString) {
	        if(palavra.contains("mapa")) {
	            mapas[contadorEventos] = recuperaStringAPartirValor(palavra, "mapa");
	            contadorEventos++;
	        }
	    }
