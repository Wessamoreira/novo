...
//	}
    ...
    public void substituirTag(ImpressaoContratoVO impressaoContrato, Boolean contrato, UsuarioVO usuario) throws Exception {
//		MarcadorVO mar = new MarcadorVO();
//		TextoPadraoVO t = new TextoPadraoVO();
//		PlanoFinanceiroAlunoVO planoFinanceiroVO = new PlanoFinanceiroAlunoVO();
//		List<PlanoDescontoVO> planoDescontoVOs = new ArrayList<PlanoDescontoVO>(0);
...
//			.findFirst()
//			.orElse(impressaoContrato.getProfessor().getPessoa().getDadosComerciaisVOs().stream().findFirst().orElse(null));
//		planoFinanceiroVO = impressaoContrato.getMatriculaVO().getPlanoFinanceiroAluno(); 
//		planoDescontoVOs = planoFinanceiroVO.getItemPlanoFinanceiroAlunoVOs().stream().map(ItemPlanoFinanceiroAlunoVO::getPlanoDesconto).collect(Collectors.toList());
//		Pattern p = Pattern.compile("\\[(.*?)\\]");
//		Matcher m = p.matcher(getTexto());
//		while (m.find()) {
//			mar.setTag(m.group());
...
//			} else if (mar.getTag().contains("_UnidadeEnsino")) {
//				setTexto(t.substituirTagUnidadeEnsino(getTexto(), impressaoContrato, mar));
//			} else if (mar.getTag().contains("_Matricula")) {
//				setTexto(t.substituirTagMatricula(getTexto(), impressaoContrato, mar));
//			} else if (mar.getTag().contains("_Curso")) {
//				setTexto(t.substituirTagCurso(getTexto(), impressaoContrato, mar, planoFinanceiroVO, planoDescontoVOs, usuario));
//			} else if (mar.getTag().contains("_Disciplina")) {
...
//			else if (mar.getTag().contains("_ProgramacaoFormaturaAluno")) {
//				setTexto(t.substituirTagProgramacaoFormatura(getTexto(), impressaoContrato, mar));
//			} else if (mar.getTag().contains("_Microcertificacao")){
//				setTexto(t.substituirTagMicrocertificacao(texto, impressaoContrato, mar));
//			} else if (mar.getTag().contains("_AtividadeComplementar")) {
//				setTexto(t.substituirTagAtividadeComplementar(getTexto(), impressaoContrato, mar));
//			} else if (mar.getTag().contains("_Polo")) {
//				setTexto(t.substituirTagPoloUnidadeEnsino(getTexto(), impressaoContrato, mar));
//			}else {
//				setTexto(t.substituirTagAluno(getTexto(), impressaoContrato, mar, dcAluno));
//				setTexto(t.substituirTagProfessor(getTexto(), impressaoContrato, mar, dcProfessor));
//				setTexto(t.substituirTagUnidadeEnsino(getTexto(), impressaoContrato, mar));
...
//			}
//		}
//		if (Uteis.isAtributoPreenchido(impressaoContrato.getTextoPadraoVO().getCodigo()) && impressaoContrato.getTextoPadraoVO().getTipoDocumentoPDF().equals(TipoDocumentoPDFEnum.DOC)) {
//			t.getParametrosRel().putAll(impressaoContrato.getTextoPadraoVO().getParametrosRel());
//			getParametrosRel().putAll(impressaoContrato.getTextoPadraoVO().getParametrosRel());
//		} else {
//			getParametrosRel().putAll(t.getParametrosRel());
//		}
//		
...
//		planoDescontoVOs = null;
//		
//		if (!Uteis.isAtributoPreenchido(impressaoContrato.getFileDeclaracaoDocWord())) {
//			return;
//		}
//		XWPFDocument document = new XWPFDocument(new FileInputStream(impressaoContrato.getFileDeclaracaoDocWord()));
//		realizarAlteracaoTagDocumentoWord(impressaoContrato.getTextoPadraoDeclaracaoVO(), t, document, usuario);
//		for (XWPFTable tbl : document.getTables()) {
//			realizarCriacaoAlteracaoTagTableDocumentoWord(contrato ? impressaoContrato.getTextoPadraoVO().getTexto() : impressaoContrato.getTextoPadraoDeclaracaoVO().getTexto(), t, document, tbl, usuario);
//		}
//		document.write(new FileOutputStream(impressaoContrato.getFileDeclaracaoDocWord()));
	}
    ...
    public void substituirTag(ImpressaoContratoVO impressaoContrato, UsuarioVO usuario) throws Exception {
    	substituirTag(impressaoContrato, Boolean.FALSE, usuario);
    }
