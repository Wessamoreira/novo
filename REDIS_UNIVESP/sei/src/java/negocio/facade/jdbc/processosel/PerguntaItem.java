...
public class PerguntaItem extends ControleAcesso implements PerguntaItemInterfaceFacade {
	...
	public List consultarPerguntaItens(Integer perguntaPrm, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		...
		while (resultado.next()) {
			...
			novoObj = PerguntaItem.montarDados(resultado, nivelMontarDados, usuario);
			objetos.add(novoObj);
		}
		return objetos;
	}
	 ...
	 */
	public static PerguntaItemVO montarDados(SqlRowSet dadosSQL, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		PerguntaItemVO obj = new PerguntaItemVO();
	 ...
	 */
	public static void montarDadosPergunta(PerguntaItemVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getPerguntaVO().getCodigo().intValue() == 0) {
	...
	public static void montarDadosPerguntaPrincipal(PerguntaItemVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getPerguntaPrincipalVO().getCodigo().intValue() == 0) {
