...
public class PessoaPreInscricaoCurso extends ControleAcesso implements PessoaPreInscricaoCursoInterfaceFacade{
    ...
    public static void montarDadosCurso(PessoaPreInscricaoCursoVO obj, UsuarioVO usuario) throws Exception {
        if (obj.getCurso().getCodigo().intValue() == 0) {
     ...
     * @exception Exception  Erro de conexão com o BD ou restrição de acesso a esta operação.
     */
    public static List consultarPessoaPreInscricaoCursos(Integer pessoa, boolean controlarAcesso, UsuarioVO usuario) throws Exception {
        PessoaPreInscricaoCurso.consultar(getIdEntidade(), controlarAcesso, usuario);
