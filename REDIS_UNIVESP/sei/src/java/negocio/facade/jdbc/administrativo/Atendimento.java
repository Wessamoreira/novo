...
@Scope("singleton")
@Lazy
public class Atendimento extends ControleAcesso implements AtendimentoInterfaceFacade {
...
//		setIdEntidade("");
	...
	public void incluir(final AtendimentoVO obj, String idEntidade, UsuarioVO usuario,
		...
		try {
			if (usuario.getCodigo() != 0) {
				Atendimento.incluir(idEntidade, true, usuario);
			}
	...
	public void alterar(final AtendimentoVO obj, String idEntidade, UsuarioVO usuario,
		...
		try {
			if (Uteis.isAtributoPreenchido(usuario)) {
				Atendimento.alterar(idEntidade, true, usuario);
			}
			validarDados(obj, usuario);
			realizarValidacaoParaAtualizacaoDadosPessoa(obj, usuario, configuracaoGeralSistemaVO);
			realizarUpperCaseDados(obj);
	...
	public void excluir(AtendimentoVO obj, String idEntidade, UsuarioVO usuario) throws Exception {
		try {
			Atendimento.excluir(idEntidade, true, usuario);
	...
	public List consultarPorCodigoPessoa(Integer valorConsulta, TipoAtendimentoEnum tipagemAtedimento,
		...
		ControleAcesso.consultar(idEntidade, controlarAcesso, usuario);
	 ...
	 *         resultantes da consulta.
	 */
	public static List<AtendimentoVO> montarDadosConsulta(SqlRowSet tabelaResultado, int nivelMontarDados,
			UsuarioVO usuario) throws Exception {
	 ...
	 */
	public static AtendimentoVO montarDados(SqlRowSet dadosSQL, int nivelMontarDados, UsuarioVO usuario)
			throws Exception {
...
//			obj.setSituacaoAtendimentoEnum(SituacaoAtendimentoEnum.getEnumPorValor(Integer.parseInt(dadosSQL.getString("situacaoAtendimentoEnum"))));			
	...
	public static void montarDadosPessoaVO(AtendimentoVO obj, int nivelMontarDados, UsuarioVO usuario)
			throws Exception {
		if (obj.getPessoaVO().getCodigo().intValue() == 0) {
	...
	public static void montarDadosResponsavelFechamento(AtendimentoVO obj, int nivelMontarDados, UsuarioVO usuario)
			throws Exception {
		if (obj.getResponsavelFechamento().getCodigo().intValue() == 0) {
	...
	public static void montarDadosOuvidor(AtendimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getResponsavelAtendimento().getCodigo().intValue() == 0) {
	...
	public static void montarDadosResponsavelCadastro(AtendimentoVO obj, int nivelMontarDados, UsuarioVO usuario)
			throws Exception {
		if (obj.getResponsavelCadastro().getCodigo().intValue() == 0) {
	...
	public static void montarDadosTipagemOuvidoria(AtendimentoVO obj, int nivelMontarDados, UsuarioVO usuario)
			throws Exception {
		if (obj.getTipagemOuvidoriaVO().getCodigo().intValue() == 0) {
	...
	public AtendimentoVO consultarPorChavePrimaria(Integer codigoPrm, int nivelMontarDados, String idEntidade,
			UsuarioVO usuario) throws Exception {
		ControleAcesso.consultar(idEntidade, false, usuario);
...
//        obj.adicionarObjUsuarioPerfilAcessoAPartirMatriculaVOs(usuarioPerfilAcesso);
	...
	public void realizarConsultarPessoaExistePorCPF(AtendimentoVO atendimentoVO, TipoAtendimentoEnum tipoAtendimento,
		...
		ControleAcesso.consultar(idEntidade, false, usuarioLogado);
