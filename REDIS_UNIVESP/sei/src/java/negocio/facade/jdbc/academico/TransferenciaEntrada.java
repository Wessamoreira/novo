...
public class TransferenciaEntrada extends ControleAcesso implements TransferenciaEntradaInterfaceFacade {
	...
	public void incluir(final TransferenciaEntradaVO obj, ConfiguracaoGeralSistemaVO configuracaoGeralSistema, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, Boolean criarHistoricoTransferenciaEntrada, Boolean controleAcesso, UsuarioVO usuario) throws Exception {
		try {
			...
			if(obj.getTipoTransferenciaEntrada().equals(TipoTransferenciaEntrada.INTERNA.getValor())){
				TransferenciaEntrada.incluir(getIdEntidadeTransferenciaInterna(), controleAcesso, usuario);
			}else {
				TransferenciaEntrada.incluir(getIdEntidade(), controleAcesso, usuario);
			}
			...
			try {
				...
				if(criarHistoricoTransferenciaEntrada) {
					...
					criarHistoricoTransferenciaEntrada(obj, configuracaoFinanceiroVO, usuario);
					obj.setNovoObj(Boolean.FALSE);
					if (obj.getTipoTransferenciaEntrada().equals(TipoTransferenciaEntrada.INTERNA.getValor())) {
	...
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void realizarCancelamentoMatricula(TransferenciaEntradaVO obj, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		 ...
		 */
		if (!obj.getCodigoRequerimento().getCodigo().equals(0)) {
			getFacadeFactory().getMatriculaFacade().alterarSituacaoMatriculaEMatriculaPeriodoTransferenciaInterna(obj.getCodigoRequerimento().getMatricula(), OrigemFechamentoMatriculaPeriodoEnum.TRANSFERENCIA_INTERNA, obj.getCodigo(), obj.getData(), configuracaoFinanceiroVO, usuario);
		} else {
			getFacadeFactory().getMatriculaFacade().alterarSituacaoMatriculaEMatriculaPeriodoTransferenciaInterna(obj.getMatricula(), OrigemFechamentoMatriculaPeriodoEnum.TRANSFERENCIA_INTERNA, obj.getCodigo(), obj.getData(), configuracaoFinanceiroVO, usuario);
		}
	}
...
//			verificarAlunoAprovadoDisciplina(historicoVO.getDisciplina().getCodigo(), obj.getMatricula().getMatricula(), obj.getMatriculaPeriodo().getCodigo(), configuracaoFinanceiroVO, usuario);
	...
	public void executarEfetivacaoSituacaoTransferenciaEntrada(TransferenciaEntradaVO transferenciaEntradaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		...
		alterar(transferenciaEntradaVO, configuracaoFinanceiroVO, usuario);
	}

	public void verificarAlunoAprovadoDisciplina(Integer disciplina, String matricula, Integer matriculaPeriodo, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		List obj = consultarHistoricoAlunoDisciplina(matricula, disciplina, false, configuracaoFinanceiroVO, usuario);
	...
	public List consultarHistoricoAlunoDisciplina(String matricula, Integer disciplina, boolean controlarAcesso, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
		List hist = getFacadeFactory().getHistoricoFacade().consultarPorMatriculaDisciplinaSituacao(matricula, disciplina, false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, configuracaoFinanceiroVO, usuario);
		return hist;
	}
	 ...
	 */
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void alterar(final TransferenciaEntradaVO obj, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		try {
			...
			if(obj.getTipoTransferenciaEntrada().equals(TipoTransferenciaEntrada.INTERNA.getValor())){
				TransferenciaEntrada.alterar(getIdEntidadeTransferenciaInterna(), true, usuario);
			}else {
				TransferenciaEntrada.alterar(getIdEntidade(), true, usuario);
			}
	 ...
	 */
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void alterarTransferenciaEntradaEfetivada(final TransferenciaEntradaVO obj, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		try {
	...
	public void excluir(TransferenciaEntradaVO obj, boolean controlarAcesso, UsuarioVO usuarioVO) throws Exception {
		try {			
			if(obj.getTipoTransferenciaEntrada().equals(TipoTransferenciaEntrada.INTERNA.getValor())){
				TransferenciaEntrada.excluir(getIdEntidadeTransferenciaInterna(), true, usuarioVO);
			}else {
				TransferenciaEntrada.excluir(getIdEntidade(), true, usuarioVO);
			}
	...
	public TransferenciaEntradaVO montarDadosTransferenciaInternaPeloCodigoRequerimento(Integer requerimento, Integer unidadeEnsino, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, Boolean possuiPermissaoTransferirInternamenteMesmoCursoMatriculaIntegral, UsuarioVO usuario) throws Exception {
		RequerimentoVO requerimentoVO = getFacadeFactory().getRequerimentoFacade().consultarPorChavePrimariaFiltrandoPorUnidadeEnsino(requerimento, "TI", unidadeEnsino, false, Uteis.NIVELMONTARDADOS_TODOS, usuario, configuracaoFinanceiroVO);
...
//		}
	...
	public List<TransferenciaEntradaVO> consultarPorCodigoRequerimento(Integer valorConsulta, String situacaoMatriculado, TipoTransferenciaEntrada tipoTransferenciaEntrada, boolean controlarAcesso, int nivelMontarDados, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		ControleAcesso.consultar(tipoTransferenciaEntrada.equals(TipoTransferenciaEntrada.INTERNA) ? getIdEntidadeTransferenciaInterna() : getIdEntidade(), controlarAcesso, usuario);
	...
	@Override
	public List<TransferenciaEntradaVO> consultarPorMatriculaETipo(String valorConsulta, String situacaoMatriculado, TipoTransferenciaEntrada tipoTransferenciaEntrada, boolean controlarAcesso, int nivelMontarDados, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		ControleAcesso.consultar(tipoTransferenciaEntrada.equals(TipoTransferenciaEntrada.INTERNA) ? getIdEntidadeTransferenciaInterna() : getIdEntidade(), controlarAcesso, usuario);
	...
	@Override
	public List<TransferenciaEntradaVO> consultarPorRegistroAcademicoAluno(String valorConsulta, String situacaoMatriculado, TipoTransferenciaEntrada tipoTransferenciaEntrada, boolean controlarAcesso, int nivelMontarDados, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		ControleAcesso.consultar(tipoTransferenciaEntrada.equals(TipoTransferenciaEntrada.INTERNA) ? getIdEntidadeTransferenciaInterna() : getIdEntidade(), controlarAcesso, usuario);
	...
	public List consultarPorDataUnidadeEnsino(Date prmIni, Date prmFim, String unidadeEnsino, boolean matriculado, TipoTransferenciaEntrada tipoTransferenciaEntrada, boolean controlarAcesso, int nivelMontarDados, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		ControleAcesso.consultar(tipoTransferenciaEntrada.equals(TipoTransferenciaEntrada.INTERNA) ? getIdEntidadeTransferenciaInterna() : getIdEntidade(), controlarAcesso, usuario);
		String sqlStr = "";
		if (matriculado) {
	...
	public List consultarTodosTiposPorData(Date prmIni, Date prmFim, String unidadeEnsino, boolean controlarAcesso, int nivelMontarDados, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
	...
	public List<TransferenciaEntradaDisciplinasAproveitadasVO> buscarDisciplinaSeremAproveitadaTransferenciaInterna(Integer aluno, Integer curso, Integer gradeCurricular, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		...
		List<HistoricoVO> historicoVOs = getFacadeFactory().getHistoricoFacade().consultarHistoricoParaAproveitamentoNaTransferenciaInternaPorCursoGrade(aluno, curso, gradeCurricular, Uteis.NIVELMONTARDADOS_DADOSMINIMOS, configuracaoFinanceiroVO, usuario, null);
	...
	public void verificarTransferenciaEntrada(TransferenciaEntradaVO transferenciaEntrada, String situacao, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		...
		if ((transferenciaEntrada.getCodigo() != null && transferenciaEntrada.getCodigo() != 0) && situacao.equals("EF") && !transferenciaEntrada.getTipoTransferenciaEntrada().equals("EX")) {
			realizarCancelamentoMatricula(transferenciaEntrada, configuracaoFinanceiroVO, usuario);
		}
	}
	 ...
	 *         <code>TransferenciaEntradaVO</code> resultantes da consulta.
	 */
	public static List<TransferenciaEntradaVO> montarDadosConsulta(SqlRowSet tabelaResultado, int nivelMontarDados, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
	 ...
	 */
	public static TransferenciaEntradaVO montarDados(SqlRowSet dadosSQL, int nivelMontarDados, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		TransferenciaEntradaVO obj = new TransferenciaEntradaVO();
		...
		if (nivelMontarDados == Uteis.NIVELMONTARDADOS_DADOSCONSULTA) {
			montarDadosRequerimento(obj, Uteis.NIVELMONTARDADOS_DADOSBASICOS, configuracaoFinanceiroVO, usuario);
		...
		obj.setTransferenciaEntradaDisciplinasAproveitadasVOs(TransferenciaEntradaDisciplinasAproveitadas.consultarTransferenciaEntradaDisciplinasAproveitadass(obj.getCodigo(), nivelMontarDados, usuario));
		...
		montarDadosRequerimento(obj, nivelMontarDados, configuracaoFinanceiroVO, usuario);
		montarDadosPessoa(obj, Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuario);
	 ...
	 */
	public static void montarDadosPessoa(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getPessoa().getCodigo().intValue() == 0) {
			return;
		}
	...
	public static void montarDadosResponsavelAutorizacao(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getResponsavelAutorizacao().getCodigo().intValue() == 0) {
			return;
		}
	...
	public static void montarDadosResponsavelEstorno(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosTipoMidiaCaptacao(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getTipoMidiaCaptacao().getCodigo().intValue() == 0) {
			return;
		}
	...
	public static void montarDadosRequerimento(TransferenciaEntradaVO obj, int nivelMontarDados, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		if (obj.getCodigoRequerimento().getCodigo().intValue() == 0) {
			return;
		}
	 ...
	 *            Objeto no qual será montado os dados consultados.
	 */
	public static void montarDadosMatricula(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosCidade(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosCurso(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosTurno(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosTurma(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosUnidadeEnsino(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosGradeCurricular(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosPeriodoLetivo(TransferenciaEntradaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	 ...
	 *                procurado.
	 */
	public TransferenciaEntradaVO consultarPorChavePrimaria(Integer codigoPrm, int nivelMontarDados, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
	...
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void realizarEstornoTransferenciaInterna(TransferenciaEntradaVO transferenciaEntradaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuarioVO) throws Exception{
		TransferenciaEntrada.verificarPermissaoFuncionalidadeUsuario("PermiteEstornarTransferenciaInterna", usuarioVO);		
		try{ 
			...
			MatriculaPeriodoVO matriculaPeriodoVO = getFacadeFactory().getMatriculaPeriodoFacade().consultarUltimaMatriculaPeriodoPorMatriculaAnoSemestre(transferenciaEntradaVO.getMatricula().getMatricula(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, configuracaoFinanceiroVO, usuarioVO);
	...
	private void alterarSituacaoEstorno(final TransferenciaEntradaVO obj, UsuarioVO usuario) throws Exception {
			TransferenciaSaida.alterar(obj.getTipoTransferenciaEntrada().equals(TipoTransferenciaEntrada.INTERNA.getValor()) ? getIdEntidadeTransferenciaInterna() : getIdEntidade() , true, usuario);
	...
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void realizarVinculoTransferenciaInternaMatriculaExistente(TransferenciaEntradaVO transferenciaEntrada, MatriculaVO matriculaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuarioVO) throws Exception {		
		...
		verificarTransferenciaEntrada(transferenciaEntrada, SituacaoTransferenciaEntrada.EFETIVADO.getValor(), configuracaoFinanceiroVO, usuarioVO);
	...
	@Override	
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void realizarCriacaoTranferenciaInternaGerandoNovaMatriculaAproveitandoDisciplinasAprovadasProximoPeriodoLetivoPorRequerimento(RequerimentoVO obj, ConfiguracaoGeralSistemaVO configuracaoGeralSistema,  ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, Boolean apresentarVisaoAluno , UsuarioVO usuario) throws Exception {
			...
			if (Uteis.isAtributoPreenchido(obj.getCodigo())) {	
				...
				if (!Uteis.isAtributoPreenchido(ultimaMP)) {
					ultimaMP = getFacadeFactory().getMatriculaPeriodoFacade().consultaUltimaMatriculaPeriodoPorMatriculaConsultaBasica(transferenciaEntradaVO.getMatricula().getMatricula(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, configuracaoFinanceiroVO, usuario);
				}
				...
				getFacadeFactory().getTransferenciaEntradaFacade().incluir(transferenciaEntradaVO,configuracaoGeralSistema, configuracaoFinanceiroVO, false, false ,usuario );				
				getFacadeFactory().getMatriculaFacade().realizarCriacaoMatriculaPorTransferenciaInternaOrigemRequerimento(transferenciaEntradaVO,configuracaoGeralSistema, configuracaoFinanceiroVO, apresentarVisaoAluno, processoMatriculaVOs.get(0), usuario);
				}catch (Exception e) {
					if(ultimaMP != null) {
