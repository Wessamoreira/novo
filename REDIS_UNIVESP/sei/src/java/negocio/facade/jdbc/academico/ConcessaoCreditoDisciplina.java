...
import negocio.interfaces.academico.ConcessaoCreditoDisciplinaInterfaceFacade;
...
@Scope("singleton")
@Lazy
public class ConcessaoCreditoDisciplina extends ControleAcesso implements ConcessaoCreditoDisciplinaInterfaceFacade {

    public ConcessaoCreditoDisciplina() {
    }
    ...
    public void validarDados(ConcessaoCreditoDisciplinaVO obj) throws Exception {
        ...
        if (obj.getQtdeCreditoConcedido() == 0) {
            throw new Exception("O campo QTDE DE CRÉDITO A CONCEDER deve ser informado");
        }
    ...
    public void incluir(final ConcessaoCreditoDisciplinaVO obj, UsuarioVO usuario) throws Exception {
        try {
            validarDados(obj);
            final String sql = "INSERT INTO ConcessaoCreditoDisciplina( disciplina, qtdeCreditoConcedido, descricaoComplementacaoCH, aproveitamentoDisciplina, ano, semestre ) "
            ...
            obj.setCodigo((Integer) getConexao().getJdbcTemplate().query(new PreparedStatementCreator() {
                ...
                public PreparedStatement createPreparedStatement(Connection arg0) throws SQLException {
                    ...
                    sqlInserir.setInt(2, obj.getQtdeCreditoConcedido().intValue());
    ...
    public void alterar(final ConcessaoCreditoDisciplinaVO obj, UsuarioVO usuario) throws Exception {
        try {
            validarDados(obj);
            final String sql = "UPDATE ConcessaoCreditoDisciplina set disciplina=?, qtdeCreditoConcedido=?, descricaoComplementacaoCH=?, aproveitamentoDisciplina=?, ano=?, semestre=? "
            ...
            getConexao().getJdbcTemplate().update(new PreparedStatementCreator() {
                ...
                public PreparedStatement createPreparedStatement(Connection arg0) throws SQLException {
                    ...
                    sqlAlterar.setInt(2, obj.getQtdeCreditoConcedido());
    ...
    public void excluir(ConcessaoCreditoDisciplinaVO obj, UsuarioVO usuario) throws Exception {
        try {
            ConcessaoCreditoDisciplina.excluir(getIdEntidade());
            String sql = "DELETE FROM ConcessaoCreditoDisciplina WHERE ((codigo = ?))"+ adicionarUsuarioLogadoComoComentarioParaLogTrigger(usuario);
    ...
    public void excluirPorAproveitamentoCreditoDisciplina(Integer aproveitamentoDisciplina, UsuarioVO usuario) throws Exception {
		try {
			ConcessaoCreditoDisciplina.excluir(getIdEntidade());
			String sql = "DELETE FROM ConcessaoCreditoDisciplina WHERE (aproveitamentoDisciplina = ?)"+ adicionarUsuarioLogadoComoComentarioParaLogTrigger(usuario);
    ...
    public List<ConcessaoCreditoDisciplinaVO> consultarPorDescricaoAproveitamentoDisciplina(String valorConsulta, int nivelMontarDados, UsuarioVO usuario) throws Exception {
        ...
        String sqlStr = "SELECT ConcessaoCreditoDisciplina.* FROM AproveitamentoDisciplinaDisciplinasAproveitadas, AproveitamentoDisciplina WHERE AproveitamentoDisciplinaDisciplinasAproveitadas.AproveitamentoDisciplina = AproveitamentoDisciplina.codigo and upper( AproveitamentoDisciplina.descricao ) like('" + valorConsulta.toUpperCase() + "%') ORDER BY AproveitamentoDisciplina.descricao";
    ...
    public List<ConcessaoCreditoDisciplinaVO> consultarConcessaoCreditoDisciplinaPorAproveitamento(Integer aproveitamentoDisciplina, int nivelMontarDados, UsuarioVO usuario) throws Exception {
    	ConcessaoCreditoDisciplina.consultar(getIdEntidade());
        String sql = "SELECT * FROM ConcessaoCreditoDisciplina WHERE aproveitamentoDisciplina = ?";
        SqlRowSet tabelaResultado = getConexao().getJdbcTemplate().queryForRowSet(sql, new Object[]{aproveitamentoDisciplina});
        return montarDadosConsulta(tabelaResultado, nivelMontarDados, usuario);
    }

    public static List<ConcessaoCreditoDisciplinaVO> montarDadosConsulta(SqlRowSet tabelaResultado, int nivelMontarDados, UsuarioVO usuario) throws Exception {
        List vetResultado = new ArrayList(0);
        while (tabelaResultado.next()) {
            ConcessaoCreditoDisciplinaVO obj = new ConcessaoCreditoDisciplinaVO();
     ...
     */
    public static ConcessaoCreditoDisciplinaVO montarDados(SqlRowSet dadosSQL, int nivelMontarDados, UsuarioVO usuario) throws Exception {
        ConcessaoCreditoDisciplinaVO obj = new ConcessaoCreditoDisciplinaVO();
    ...
    public static void montarDadosDisciplina(ConcessaoCreditoDisciplinaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
    ...
    public static void montarDadosAproveitamentoDisciplina(ConcessaoCreditoDisciplinaVO obj, int nivelMontarDados, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
        if (obj.getAproveitamentoDisciplinaVO().getCodigo().intValue() == 0) {
    ...
    public void incluirConcessaoCreditoDisciplina(Integer aproveitamentoDisciplina, List objetos, UsuarioVO usuario) throws Exception {
        ...
        while (e.hasNext()) {
            ConcessaoCreditoDisciplinaVO obj = (ConcessaoCreditoDisciplinaVO) e.next();
    ...
    public void alterarConcessaoCreditoDisciplina(Integer aproveitamentoDisciplina, List objetos, UsuarioVO usuario) throws Exception {
        try {
            ...
            while (i.hasNext()) {
                ConcessaoCreditoDisciplinaVO obj = (ConcessaoCreditoDisciplinaVO) i.next();
    ...
    public boolean consultaExistenciaCreditoConcedidoDisciplinaPreRequisito(String matricula, Integer disciplina, Integer gradeCurricular) throws Exception {
    	...
    	sb.append(" select codigo from historico where situacao in ('AA', 'CC', 'CH', 'IS', 'AB') ");
    	sb.append(" and matricula = ? ");
    	sb.append(" and matrizcurricular = ? ");
    	sb.append(" and disciplina = ? ");
    	SqlRowSet tabelaResultado = getConexao().getJdbcTemplate().queryForRowSet(sb.toString(), matricula, gradeCurricular, disciplina);
