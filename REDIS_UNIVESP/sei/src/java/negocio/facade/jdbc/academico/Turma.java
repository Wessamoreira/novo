...
public class Turma extends ControleAcesso implements TurmaInterfaceFacade {
	...
	public void incluir(final TurmaVO obj, UsuarioVO usuario) throws Exception {
		...
		try {
			...
			Turma.incluir(getIdEntidade(), true, usuario);
	...
	public void alterar(final TurmaVO obj, UsuarioVO usuario) throws Exception {
		try {
			validarCentroResultadoExistentePorTurma(obj, usuario);
			TurmaVO.validarDados(obj);
			Turma.alterar(getIdEntidade(), true, usuario);
			validarUnicidadeIdentificadorTurma(obj);
	...
	public void excluir(TurmaVO obj, UsuarioVO usuarioVO) throws Exception {
		try {
			Turma.excluir(getIdEntidade(), true, usuarioVO);
	...
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void realizarAtualizacaoGradeCurricularCursoIntegral(TurmaVO turmaAtual, TurmaVO turmaNovaGradeCurricular, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuarioVO, List<MatriculaPeriodoVO> listaMatriculaPeriodo) {
		...
		try {
			...
			if (turmaAtual.isExisteAlunosMatriculados()) {
				adicionarGradeCurricularCursoIntegralParaTurma(listaMatriculaPeriodo, turmaAtual, turmaNovaGradeCurricular, listaAtualGradeCurricular, configuracaoFinanceiroVO, usuarioVO);
	...
	private void adicionarGradeCurricularCursoIntegralParaTurma(List<MatriculaPeriodoVO> listaMatriculaPeriodo, TurmaVO turmaAtual, TurmaVO turmaNovaGradeCurricular, List<TurmaDisciplinaVO> listaAtualGradeCurricular, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuarioVO) throws Exception {
			...
			if (td.isAlterarMatrizCurricular()) {
				...
				if(Uteis.isAtributoPreenchido(listaMpAptarIncluir)){
					for (MatriculaPeriodoVO mp : listaMpAptarIncluir) {
						...
						getFacadeFactory().getMatriculaPeriodoTurmaDisciplinaFacade().incluir(mptd, configuracaoFinanceiroVO, turmaNovaGradeCurricular.getGradeCurricularVO(), usuarioVO);
					}
				...
				if (Uteis.isAtributoPreenchido(listaMptd)) {
					for (MatriculaPeriodoTurmaDisciplinaVO matriculaPeriodoTurmaDisciplinaVO : listaMptd) {
						if (matriculaPeriodoTurmaDisciplinaVO.getDisciplinaEquivale() || matriculaPeriodoTurmaDisciplinaVO.getDisciplinaPorEquivalencia()) {
							...
							getFacadeFactory().getHistoricoFacade().excluirHistoricoForaPrazoVerificandoHistoricoPorEquivalencia(obj, null, usuarioVO);
						} else {
...
//		}
	...
	public List<TurmaVO> consultaRapidaPorNrPeriodoLetivoUnidadeEnsinoCursoTurno(Integer nrPeridoLetivo, Integer unidadeEnsino, Integer curso, Integer turno, Integer gradeCurricular, boolean novaMatricula, boolean renovandoMatricula, boolean editandoMatricula, boolean trazerApenasTurmaComAulaProgramada, String ano, String semestre, boolean controlarAcesso, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiro, Boolean trazerApenasTurmasApresentarRenovacaoOnline) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
		StringBuilder sql = getSQLPadraoConsultaBasica();
...
//			sqlStr.append(" < (case when vagaturmadisciplina.nrVagasMatriculaReposicao is not null then vagaturmadisciplina.nrVagasMatriculaReposicao else turma.nrVagasInclusaoReposicao end )");
	 ...
	 * @return List Contendo vÃ¯Â¿Â½rios objetos da classe <code>TurmaVO</code> resultantes da consulta.
	 */
	public static List<TurmaVO> montarDadosConsulta(SqlRowSet tabelaResultado, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		List<TurmaVO> vetResultado = new ArrayList<TurmaVO>(0);
		while (tabelaResultado.next()) {
	 ...
	 */
	public static TurmaVO montarDados(SqlRowSet tabelaResultado, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		TurmaVO obj = new TurmaVO();
		obj.setNovoObj(Boolean.FALSE);
		obj.setCodigo(tabelaResultado.getInt("codigo"));
		obj.setIdentificadorTurma(tabelaResultado.getString("identificadorTurma"));
		...
		if (nivelMontarDados == Uteis.NIVELMONTARDADOS_PROCESSAMENTO) {
			obj.setTurmaAgrupadaVOs(TurmaAgrupada.consultarTurmaAgrupadas(obj.getCodigo(), false, nivelMontarDados, usuario));
			return obj;
		}
	...
	public static void montarDadosContaCorrente(TurmaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getContaCorrente().getCodigo().intValue() == 0) {
	...
	public static void montarDadosChancela(TurmaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getChancelaVO().getCodigo().intValue() == 0) {
			obj.setChancelaVO(new ChancelaVO());
			return;
		}
		obj.setChancelaVO(getFacadeFactory().getChancelaFacade().consultarPorChavePrimaria(obj.getChancelaVO().getCodigo(), false, nivelMontarDados, usuario));
	}

	public static void montarDadosTurno(TurmaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getTurno().getCodigo().intValue() == 0) {
	...
	public static void montarDadosGrupoDestinatarios(TurmaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getGrupoDestinatarios().getCodigo().intValue() == 0) {
	...
	public static void montarDadosPlanoFinanceiroCurso(TurmaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getPlanoFinanceiroCurso().getCodigo().intValue() == 0) {
			return;
		}
	 ...
	 */
	public static void montarDadosUnidadeEnsino(TurmaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getUnidadeEnsino().getCodigo().intValue() == 0) {
	 ...
	 *            Objeto no qual serÃ¯Â¿Â½ montado os dados consultados.
	 */
	public static void montarDadosCurso(TurmaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getCurso().getCodigo().intValue() == 0) {
	...
	public static void montarDadosPeriodoLetivo(TurmaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getPeridoLetivo().getCodigo().intValue() == 0) {
	...
	public static void montarDadosGradeCurricular(TurmaVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getGradeCurricularVO().getCodigo().intValue() == 0) {
...
//		sql.append("or (turma.turmaAgrupada and curso.codigo = (select t.curso from turmaagrupada inner join turma as t on t.codigo = turmaagrupada.turma where turmaagrupada.turmaorigem = turma.codigo limit 1)) )");
	...
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void executarAtualizacaoDisciplinaAlunosTurma(Boolean atualizarDisciplinaAlunos, List<MatriculaPeriodoVO> listaMatriculaPeriodoVO, TurmaVO turmaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UnidadeEnsinoVO unidadeEnsinoVO, UsuarioVO usuario) throws Exception {
		try {
			if (atualizarDisciplinaAlunos) {
				...
				// true, usuario);
				if (!listaMatriculaPeriodoVO.isEmpty()) {
					executarCriacaoMatriculaPeriodoTurmaDisciplina(turmaVO, listaMatriculaPeriodoVO, configuracaoFinanceiroVO, usuario);
				}
	...
	// turmaVO, ConfiguracaoFinanceiroVO
	// configuracaoFinanceiroVO, UnidadeEnsinoVO unidadeEnsinoVO, UsuarioVO
	...
	// }
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void executarCriacaoMatriculaPeriodoTurmaDisciplina(TurmaVO turmaVO, List<MatriculaPeriodoVO> listaMatriculaPeriodoVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		try {
			...
			for (MatriculaPeriodoTurmaDisciplinaVO mptd : listaMatriculaPeriodoTurmaDisciplinaVO) {
				if (hashMapMatriculaPeriodoVO.containsKey(mptd.getMatricula()) && hashMapDisciplinaVO.containsKey(mptd.getDisciplina().getCodigo())) {
					...
					if (!mptd.getMatriculaPeriodo().equals(0)) {
						getFacadeFactory().getMatriculaPeriodoTurmaDisciplinaFacade().incluir(mptd, configuracaoFinanceiroVO, turmaVO.getGradeCurricularVO(), usuario);
					}
				}
			}
		} catch (Exception e) {
	...
	public Double consultarReceitaPrevistaPeriodo(Date dataInicio, Date dataFim, List<UnidadesPlanoOrcamentarioVO> unidadesPlanoOrcamentarioVOs) {
	...
	public Double consultarDespesaPrevistaPeriodo(Date dataInicio, Date dataFim, List<UnidadesPlanoOrcamentarioVO> unidadesPlanoOrcamentarioVOs) {
	...
	public Integer consultarQuantidadeNovasTurmasPrevistaPeriodo(Date dataInicio, Date dataFim, List<UnidadesPlanoOrcamentarioVO> unidadesPlanoOrcamentarioVOs) {
...
//			sqlStr.append(" 	limit 1)");
	...
	@Override
	public void processarDadosPermitinentesTurmaSelecionada(MatriculaVO matriculaVO, MatriculaPeriodoVO matriculaPeriodoVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuarioVO) throws Exception {
		...
		getFacadeFactory().getMatriculaPeriodoFacade().atualizarSituacaoMatriculaPeriodo(matriculaVO, matriculaPeriodoVO, configuracaoFinanceiroVO, usuarioVO);
	}
...
//		sqlStr.append(" and de.disciplina = ").append(codDisciplina).append(" ");
	...
	public List<TurmaVO> consultarPorIdentificadorTurmaUnidadeEnsinoCondicaoRenegociacao(String valorConsulta, List<CondicaoRenegociacaoUnidadeEnsinoVO> listaCondicaoUnidadeEnsinoVOs, Integer curso, Integer turno, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
		StringBuilder sql = getSQLPadraoConsultaBasica();
		sql.append("WHERE sem_acentos(lower (turma.identificadorturma)) like(sem_acentos('").append(valorConsulta.toLowerCase()).append("%'))  ");
		// if (unidadeEnsino != null && unidadeEnsino.intValue() != 0) {
	...
	public TurmaVO realizarCriacaoTurmaPadraoSemDisciplinaParaMatriculaProcessoSeletivoEmPeriodoLetivoEscolhaAutomatica(MatriculaVO matriculaVO , MatriculaPeriodoVO matriculaPeriodoVO , boolean controlarAcesso, UsuarioVO usuario) throws Exception {
	    ...
	    try {
		     ...
		     StringBuilder sql = getSqlInsertCriacaoTurmaSemDisciplinaParaMatriculaProcessoSeletivoEmPeriodoLetivoEscolhaAutomatica(
		    		 ...
		    		 matriculaPeriodoVO.getUnidadeEnsinoCursoVO().getPlanoFinanceiroCurso().getCodigo());
		...
		try {
			...
			if(turmaVOs.isEmpty() && numeroPeriodoletivo != null &&  numeroPeriodoletivo > 1) {
				...
				for (TurmaVO turmaVO : turmaVOs) {
					...
					turmaRSVO.setNome(turmaVO.getIdentificadorTurma());				
					turmaRSVO.setPossuiDisciplinasNaTurma(validarExistenciaDisplinasTurmaDisponivelEHabilitadaParaEstudar(turmaVO,aluno ,codigoGradeCurricular, configuracaoFinanceiroVO ,usuarioVO));
					turmaRSVOs.add(turmaRSVO);
	...
	private Boolean validarExistenciaDisplinasTurmaDisponivelEHabilitadaParaEstudar(TurmaVO  turmaVO , Integer aluno,  Integer gradeCurricular ,ConfiguracaoFinanceiroVO  configuracaoFinanceiroVO ,UsuarioVO usuarioVO) {
		...
		try {
		   ...
		   getFacadeFactory().getMatriculaFacade().realizarCriacaoHistoricoApartirDisciplinasAprovadasMesmaGradeCurricularMatriculaOnline("",aluno ,gradeCurricular, turmaVO ,configuracaoFinanceiroVO  , NivelMontarDados.TODOS ,usuarioVO);
