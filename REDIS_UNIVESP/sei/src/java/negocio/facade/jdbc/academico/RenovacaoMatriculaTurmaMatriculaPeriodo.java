...
//		obj.setInformacaoFinanceira(rs.getString("informacaoFinanceira"));
		...
		if (nivelMontarDados == Uteis.NIVELMONTARDADOS_TODOS) {
			obj.setMatriculaPeriodoVO(getFacadeFactory().getMatriculaPeriodoFacade().consultarPorChavePrimaria(obj.getMatriculaPeriodoVO().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSMINIMOS, new ConfiguracaoFinanceiroVO(), usuarioVO));
			if(rs.getInt("novamatriculaperiodo") != 0) {
				obj.setNovaMatriculaPeriodoVO(getFacadeFactory().getMatriculaPeriodoFacade().consultarPorChavePrimaria(obj.getNovaMatriculaPeriodoVO().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSMINIMOS, new ConfiguracaoFinanceiroVO(), usuarioVO));	
			}
		}
		return obj;
	}
...
//		}
	...
	public void realizarConsultarMatriculaPeriodoAptaRenovar(RenovacaoMatriculaTurmaVO renovacaoMatriculaTurmaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO, Boolean permitirRealizarMatriculaDisciplinaPreRequisito, UsuarioVO usuarioVO, List<GradeDisciplinaCompostaVO> gradeDisciplinaCompostaVOs) throws Exception {
		...
		if (!renovacaoMatriculaTurmaVO.getTurmaVO().getCurso().getConfiguracaoAcademico().getPermiteEvoluirPeriodoLetivoCasoReprovado() 
			...
			renovacaoMatriculaTurmaVO.setQtdeRenovacaoAGerada(getFacadeFactory().getMatriculaPeriodoFacade().consultaQuantidadeAlunoPorTurmaTurmaSituacaoMatriculaPeriodoESituacao(renovacaoMatriculaTurmaVO, false, 0, configuracaoFinanceiroVO, usuarioVO));
		}
	}
	...
	public void realizarConsultarMatriculaPeriodoAptaRenovarPorTurma(RenovacaoMatriculaTurmaVO renovacaoMatriculaTurmaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO, Boolean permitirRealizarMatriculaDisciplinaPreRequisito, UsuarioVO usuarioVO, List<GradeDisciplinaCompostaVO> gradeDisciplinaCompostaVOs, ProgressBarVO progressBarVO) throws Exception {
		...
		if (!renovacaoMatriculaTurmaVO.getTurmaVO().getCurso().getConfiguracaoAcademico().getPermiteEvoluirPeriodoLetivoCasoReprovado() && renovacaoMatriculaTurmaVO.getTurmaVO().getCurso().getConfiguracaoAcademico().getIsPossuiControleDisciplinaReprovacao()) {
			...
			matriculaPeriodoVOs = getFacadeFactory().getMatriculaPeriodoFacade().consultaRapidaPorTurmaTurmaSituacaoMatriculaPeriodoESituacao(renovacaoMatriculaTurmaVO, false, 0, configuracaoFinanceiroVO, usuarioVO);
		}
		...
		for (MatriculaPeriodoVO matriculaPeriodoVO : matriculaPeriodoVOs) {
			...
			try {				
				...
				obj.setNovaMatriculaPeriodoVO(realizarRenovacaoAutomaticaAtravesRenovacaPorTurma(renovacaoMatriculaTurmaVO, obj, configuracaoFinanceiroVO, configuracaoGeralSistemaVO, permitirRealizarMatriculaDisciplinaPreRequisito, usuarioVO, progressBarVO != null, gradeDisciplinaCompostaVOs));
...
//		}
	...
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public MatriculaPeriodoVO realizarRenovacaoAutomaticaAtravesRenovacaPorTurma(RenovacaoMatriculaTurmaVO rmt, RenovacaoMatriculaTurmaMatriculaPeriodoVO rmtmp, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO, Boolean permitirRealizarMatriculaDisciplinaPreRequisito, UsuarioVO usuarioVO, boolean simularRenovacao, List<GradeDisciplinaCompostaVO> gradeDisciplinaCompostaVOs) throws Exception {
		...
		MatriculaPeriodoVO novaMatriculaPeriodo = getFacadeFactory().getMatriculaFacade().realizarGeracaoNovaMatriculaPeriodoParaRenovacao(matriculaVO, configuracaoFinanceiroVO, usuarioVO);
		...
		getFacadeFactory().getMatriculaPeriodoFacade().definirSituacaoMatriculaPeriodoComBaseProcesso(matriculaVO, novaMatriculaPeriodo, configuracaoFinanceiroVO, usuarioVO);
...
//		realizarDefinicaoDadosFinanceiroMatriculaPeriodo(rmt, rmtmp, matriculaVO, novaMatriculaPeriodo, configuracaoFinanceiroVO, configuracaoGeralSistemaVO, permitirRealizarMatriculaDisciplinaPreRequisito, usuarioVO, simularRenovacao, gradeDisciplinaCompostaVOs);		
		if (!simularRenovacao) {
			getFacadeFactory().getMatriculaFacade().persistir(matriculaVO, novaMatriculaPeriodo, configuracaoGeralSistemaVO, configuracaoFinanceiroVO, usuarioVO);
		}
