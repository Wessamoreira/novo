...
public class RegistroAulaNota extends ControleAcesso implements RegistroAulaNotaInterfaceFacade {
    ...
    public void persistir(List<RegistroAulaVO> registroAulaVOs, ConfiguracaoAcademicoVO configuracaoAcademicoVO, Boolean permiteLancamentoAulaFutura, UsuarioVO usuarioLogado) throws Exception {
        ...
        for (FrequenciaAulaVO freqAulaVO : regAuxVO.getFrequenciaAulaVOs()) {
            //verificar se a disciplina faz parte de uma disciplina Composta
            if (!freqAulaVO.getFrequenciaOculta()) {
//                if (freqAulaVO.getHistoricoVO().getHistoricoDisciplinaFazParteComposicao()) {
//                    getFacadeFactory().getHistoricoFacade().executarAtualizacaoDisciplinaComposta(freqAulaVO.getHistoricoVO(), usuarioLogado);
//                }
//                getFacadeFactory().getHistoricoFacade().alterarInformacoesInseridasRegistroAulaNota(freqAulaVO.getHistoricoVO());
            	if (Uteis.isAtributoPreenchido(freqAulaVO.getHistoricoVO().getCodigo())) {
            		historicoVOs.add(freqAulaVO.getHistoricoVO());
            	}
            }
        }
        ...
        NotificacaoRegistroAulaNotaVO n = new NotificacaoRegistroAulaNotaVO();
        n.setTurma(turma.getCodigo());
        n.setProfessor(professor);
        n.setDisciplina(disciplina);
        n.setUnidadeEnsino(0);
        n.setUsuario(usuarioLogado);
        new NotificacaoRegistroAulaNota().incluir(n);
    }
...
//				}
    ...
    @Override
    public Boolean setarNotasDeAcordoComMedia(List<HistoricoVO> listaNotas, FrequenciaAulaVO frAulaVO, ConfiguracaoAcademicoVO configuracaoAcademicoVO, Boolean listasIguais) throws Exception {
        try {
