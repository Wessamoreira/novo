...
public class ConteudoUnidadePagina extends ControleAcesso implements ConteudoUnidadePaginaInterfaceFacade {
	...
	public void upLoadIconeConteudoUnidadePagina(FileUploadEvent upload, ConteudoUnidadePaginaVO conteudoUnidadePagina, boolean iconeVoltar, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO, UsuarioVO usuarioVO) throws Exception {
		...
		try {
			...
			ArquivoHelper.salvarIconeNaPastaTemp(new File(upload.getUploadedFile().getName()), nomeIcone, configuracaoGeralSistemaVO, usuarioVO);
		} catch (Exception e) {
			throw e;
		} finally {
	...
	public void upLoadArquivoConteudoUnidadePaginaHtml(FileUploadEvent upload, Integer disciplina, ConteudoUnidadePaginaVO conteudoUnidadePagina, Boolean publicarImagem, String nomeImagem, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO, UsuarioVO usuarioVO) throws Exception {
		...
		String nomeFisico = String.valueOf(new Date().getTime()) + upload.getUploadedFile().getName().substring(upload.getUploadedFile().getName().lastIndexOf("."), upload.getUploadedFile().getName().length());
		...
		if (publicarImagem) {
			...
			recursoEducacionalVO.setNomeRealArquivo(upload.getUploadedFile().getName());
	...
	public void upLoadArquivoConteudoUnidadePagina(FileUploadEvent upload, Integer disciplina, ConteudoUnidadePaginaVO conteudoUnidadePagina, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO, UsuarioVO usuarioVO) throws Exception {
		try {
			...
			if (!conteudoUnidadePagina.getTipoRecursoEducacional().equals(TipoRecursoEducacionalEnum.SLIDE_IMAGEM)) {
				...
				conteudoUnidadePagina.setNomeRealArquivo(upload.getUploadedFile().getName());
				if (conteudoUnidadePagina.getNomeFisicoArquivo().isEmpty()) {
					conteudoUnidadePagina.setNomeFisicoArquivo(String.valueOf(new Date().getTime()) + upload.getUploadedFile().getName().substring(upload.getUploadedFile().getName().lastIndexOf("."), upload.getUploadedFile().getName().length()));
				} else {
					conteudoUnidadePagina.setNomeFisicoArquivo(conteudoUnidadePagina.getNomeFisicoArquivo().substring(0, conteudoUnidadePagina.getNomeFisicoArquivo().indexOf(".")) + upload.getUploadedFile().getName().substring(upload.getUploadedFile().getName().lastIndexOf("."), upload.getUploadedFile().getName().length()));
				}
				...
				String nomeFisico = "";
				if (!conteudoUnidadePagina.getNomeRealArquivo().contains(upload.getUploadedFile().getName())) {
					nomeFisico = String.valueOf(new Date().getTime()) + upload.getUploadedFile().getName().substring(upload.getUploadedFile().getName().lastIndexOf("."), upload.getUploadedFile().getName().length());
					if (!conteudoUnidadePagina.getNomeFisicoArquivo().trim().isEmpty()) {
						...
						conteudoUnidadePagina.setNomeRealArquivo(conteudoUnidadePagina.getNomeRealArquivo() + ", " + upload.getUploadedFile().getName());
					} else {
						conteudoUnidadePagina.setCaminhoBaseRepositorio(PastaBaseArquivoEnum.EAD_TMP.getValue() + File.separator + PastaBaseArquivoEnum.EAD_CONTEUDO_TMP.getValue() + File.separator + disciplina + File.separator + conteudoUnidadePagina.getTipoRecursoEducacional().name());
