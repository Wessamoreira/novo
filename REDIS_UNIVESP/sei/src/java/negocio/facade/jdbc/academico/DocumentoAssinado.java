...
//            realizarEnvioNotificacaoLembreteDocumentoPendenteAssinatura(doc.getDocumentoAssinadoVO(), configGedVO);
	...
	public void executarProcessamentoDocumentosAssinadoEletronicaMenteValidandoSituacaoAssinaturaPorProvedorCertiSign(DocumentoAssinadoVO doc, UsuarioVO usuarioLogado) throws Exception {
		...
		if (Uteis.isAtributoPreenchido(doc) && doc.getTipoOrigemDocumentoAssinadoEnum().isContrato()) {
			getFacadeFactory().getMatriculaPeriodoFacade().realizarAtivacaoMatriculaValidandoRegrasEntregaDocumentoAssinaturaContratoMatricula(doc.getMatricula().getMatricula(), getAplicacaoControle().getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(doc.getUnidadeEnsinoVO().getCodigo()), Boolean.FALSE, usuarioLogado);
		}
	}
    ...
    public void executarProcessamentoDocumentosAssinadoEletronicaMenteValidandoSituacaoAssinaturaPorProvedorTechCert(DocumentoAssinadoVO doc, UsuarioVO usuarioLogado) throws Exception {
        ...
        if (Uteis.isAtributoPreenchido(doc) && doc.getTipoOrigemDocumentoAssinadoEnum().isContrato()) {
            getFacadeFactory().getMatriculaPeriodoFacade().realizarAtivacaoMatriculaValidandoRegrasEntregaDocumentoAssinaturaContratoMatricula(doc.getMatricula().getMatricula(), getAplicacaoControle().getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(doc.getUnidadeEnsinoVO().getCodigo()), Boolean.TRUE, usuarioLogado);
        }
    }
    ...
    public String executarAssinaturaProvedorCertiSign(DocumentoAssinadoVO obj, String arquivoOrigem, ConfiguracaoGEDVO configGEDVO, ConfiguracaoGeralSistemaVO config, UsuarioVO usuarioVO) throws Exception {
        ...
        for (DocumentoAssinadoPessoaVO dap : obj.getListaDocumentoAssinadoPessoa()) {
            ...
            if (Uteis.isAtributoPreenchido(obj.getProgramacaoFormaturaVO().getDataLimiteAssinaturaAta())) {
                ...
                deadlineRSVO.setTime(UteisData.getDataCompletaFormatoUTC(obj.getProgramacaoFormaturaVO().getDataLimiteAssinaturaAta()));
    ...
    public String executarAssinaturaProvedorTechCert(DocumentoAssinadoVO documentoAssinadoVO, String arquivoOrigem,
        ...
        for (DocumentoAssinadoPessoaVO dap : documentoAssinadoVO.getListaDocumentoAssinadoPessoa()) {
            ...
            if (Uteis.isAtributoPreenchido(documentoAssinadoVO.getProgramacaoFormaturaVO().getDataLimiteAssinaturaAta())) {
                ...
                deadlineRSVO.setTime(UteisData.getDataCompletaFormatoUTC(documentoAssinadoVO.getProgramacaoFormaturaVO().getDataLimiteAssinaturaAta()));
                deadlineRSVO.setTimeType(TimeType.PERSONALIZED_TIME.getValor());
                deadlineRSVO.setTimezoneId(ZoneId.systemDefault().getId());
                dpRSVO.setBlockDocumentAfterLimit(true);
                dpRSVO.setDeadline(deadlineRSVO);
	...
	public void incluir(final DocumentoAssinadoVO obj, boolean verificarAcesso, UsuarioVO usuario, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO) throws Exception {
		...
		DocumentoAssinado.incluir(getIdEntidade(), verificarAcesso, usuario);
	...
	public void alterar(final DocumentoAssinadoVO obj, boolean verificarAcesso, UsuarioVO usuario, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO) throws Exception {
		try {
			DocumentoAssinado.alterar(getIdEntidade(), verificarAcesso, usuario);
    ...
    public void excluir(final DocumentoAssinadoVO obj, final boolean verificarAcesso, UsuarioVO usuario, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO) throws Exception {
        DocumentoAssinado.excluir(getIdEntidade(), verificarAcesso, usuario);
...
//					realizarEscritaDebugSeiSignature(obj, "(7) Realizando download xml da AMAZON para verificar assinaturas apos upar arquivo no servidor amazon. [Arquivo: " + obj.getArquivo().getPastaBaseArquivo() + File.separator + obj.getArquivo().getNome());
	...
	public DocumentoAssinadoVO montarDadosArquivoAssinado(SqlRowSet dadosSQL, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		...
		if (Uteis.isAtributoPreenchido(obj.getMatricula())) {
			obj.setMatricula(getFacadeFactory().getMatriculaFacade().consultarPorChavePrimaria(obj.getMatricula().getMatricula(), obj.getMatricula().getUnidadeEnsino().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSMINIMOS, new ConfiguracaoFinanceiroVO(), usuario));
		}
...
//			obj.getArquivo().setExtensao(nomeArquivo.substring((nomeArquivo.lastIndexOf(".") + 1), nomeArquivo.length()));
    ...
    public String realizarInclusaoDocumentoAssinadoPorEstagio(EstagioVO estagioVO, TipoOrigemDocumentoAssinadoEnum tipoOrigemDocumentoAssinadoEnum, String nomeArquivoOrigem, ConfiguracaoEstagioObrigatorioVO configEstagio, ConfiguracaoGeralSistemaVO config, UsuarioVO usuarioVO) throws Exception {
        ...
        estagioVO.getDocumentoAssinadoVO().getArquivo().setPastaBaseArquivo(PastaBaseArquivoEnum.ESTAGIO.getValue() + File.separator + UteisData.getAnoData(new Date()) + File.separator + UteisData.getMesData(new Date()));
        estagioVO.getDocumentoAssinadoVO().getArquivo().setPastaBaseArquivoEnum(PastaBaseArquivoEnum.ESTAGIO);
...
//		}
	...
	public void excluirDocumentoAssinadoVinculadoDocumentacaoGED(Integer codigo, UsuarioVO usuario) throws Exception {
		DocumentoAssinado.excluir(getIdEntidade(), false, usuario);
		String sql = "DELETE FROM documentoassinado WHERE ((codigo = ?))" + adicionarUsuarioLogadoComoComentarioParaLogTrigger(usuario);
		...
		for (DocumentoAssinadoPessoaVO dap : documentoAssinado.getListaDocumentoAssinadoPessoa()) {
			try {
				if (dap.getAssinarDocumento() && dap.getSituacaoDocumentoAssinadoPessoaEnum().equals(SituacaoDocumentoAssinadoPessoaEnum.PENDENTE)) {
					...
					dap.setDataAssinatura(new Date());
					getFacadeFactory().getDocumentoAssinadoPessoaFacade().atualizarDadosAssinatura(dap, null, null, null);
					getFacadeFactory().getMatriculaPeriodoFacade().realizarAtivacaoMatriculaValidandoRegrasEntregaDocumentoAssinaturaContratoMatricula(documentoAssinado.getMatricula().getMatricula() ,   getAplicacaoControle().getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(documentoAssinado.getUnidadeEnsinoVO().getCodigo()), Boolean.FALSE,  usuarioVO);
    ...
    @Override
    public List<DocumentoAssinadoVO> verificarGeracaoDocumentoAssinado(Integer codigoPessoa, MatriculaVO matriculaVO, MatriculaPeriodoVO matriculaPeriodoVO, UnidadeEnsinoVO unidadeEnsinoVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO, int nivelMontarDados, UsuarioVO usuario) throws Exception {
        verificarGeracaoDocumentoContratoMatricula(matriculaVO, matriculaPeriodoVO, unidadeEnsinoVO, configuracaoFinanceiroVO != null ? configuracaoFinanceiroVO : new ConfiguracaoFinanceiroVO(), configuracaoGeralSistemaVO, nivelMontarDados, usuario);
        ...
        try {
            if (Uteis.isAtributoPreenchido(usuario) && (matriculaVO.getSituacao().equals(SituacaoVinculoMatricula.ATIVA.getValor())
                ...
                if (((!Uteis.isAtributoPreenchido(documentoAssinadoPessoa)) && getFacadeFactory().getMatriculaPeriodoFacade().verificarMatriculaDeCalouro(matriculaVO.getMatricula(), matriculaPeriodoVO.getCodigo(), false, usuario))
                    ...
                    getFacadeFactory().getImpressaoContratoFacade().imprimirContratoVisaoAluno(matriculaVO, matriculaPeriodoVO, configuracaoFinanceiroVO, configuracaoGeralSistemaVO, usuario);
                }
                if (((!Uteis.isAtributoPreenchido(documentoAssinadoPessoa)) && getFacadeFactory().getMatriculaPeriodoFacade().verificarMatriculaDeCalouro(matriculaVO.getMatricula(), matriculaPeriodoVO.getCodigo(), false, usuario))
                        && getFacadeFactory().getTextoPadraoFacade().verificarTextoPadraoAssinaDigitalmentePorMatricula(matriculaVO.getMatricula(), false, 0, usuario)
                        && getAplicacaoControle().getConfiguracaoGEDPorUnidadeEnsino(matriculaVO.getUnidadeEnsino().getCodigo(), usuario).getConfiguracaoGedContratoVO().getAssinarDocumento()
	...
	public void realizarVerificacaoDocumentoAssinadoPendenteUsuarioLogado(UsuarioVO usuarioVO, MatriculaVO matriculaVO, MatriculaPeriodoVO matriculaPeriodoVO, Boolean verificarAtaColacao) throws Exception {
		try {
			...
			List<DocumentoAssinadoVO> listaDocumento = getFacadeFactory().getDocumentoAssinadoFacade().verificarGeracaoDocumentoAssinado(usuarioVO.getPessoa().getCodigo(), matriculaVO, matriculaPeriodoVO, matriculaVO.getUnidadeEnsino(), getAplicacaoControle().getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(matriculaVO.getUnidadeEnsino().getCodigo()), getAplicacaoControle().getConfiguracaoGeralSistemaVO(null, usuarioVO), Uteis.NIVELMONTARDADOS_TODOS, usuarioVO);
...
//			getFacadeFactory().getMatriculaPeriodoRevalidacaoInterfaceFacade().recusarRevalidacaoMatricula(documentoAssinadoVO.getMatriculaPeriodoRevalidacaoVO(), documentoAssinadoVO.getMatricula(), true, longitude, latitude, usuarioVO, configuracaoGeralSistemaVO);
    ...
    public Boolean verificarPermissaoAssinaturaDiarioPeloProfessorAposAssinaturaCoordenadorCurso(UsuarioVO usuario) {
        try {
            DocumentoAssinado.verificarPermissaoUsuarioFuncionalidade("PermitirAssinaturaDiarioPeloProfessorAposAssinaturaCoordenadorCurso", usuario);
            return true;
        } catch (Exception e) {
            return false;
        }
...
//		ControleLivroRegistroDiplomaVO livroRegistroDiplomaVO = new ControleLivroRegistroDiplomaVO();
    ...
    public void realizarGeracaoCodigoValidacaoDiplomaDigital(DocumentoAssinadoVO documentoAssinadoVO, UsuarioVO usuarioVO, Document doc, String localArquivo, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO) throws Exception {
        try {
            ...
            if (Uteis.isAtributoPreenchido(expedicaoDiplomaVO) && !Uteis.isAtributoPreenchido(expedicaoDiplomaVO.getCodigoValidacaoDiplomaDigital())) {
                expedicaoDiplomaVO.setMatricula(getFacadeFactory().getMatriculaFacade().consultarPorChavePrimaria(expedicaoDiplomaVO.getMatricula().getMatricula(), expedicaoDiplomaVO.getMatricula().getUnidadeEnsino().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSMINIMOS, new ConfiguracaoFinanceiroVO(), usuarioVO));
	...
	private ArquivoVO criarArquivoCurriculoEscolar(GestaoXmlGradeCurricularVO gestaoXmlGradeCurricularVO, File arquivo, TipoOrigemDocumentoAssinadoEnum tipoOrigemDocumentoAssinadoEnum, ConfiguracaoGeralSistemaVO config, UsuarioVO usuarioVO) throws IOException {
		ArquivoVO obj = new ArquivoVO();
		obj.setDataUpload(new Date());
		obj.setNome(arquivo.getName());
		obj.setControlarDownload(true);
...
//        }
	...
	public List<DocumentoAssinadoVO> consultarAtaColacaoGrauDataLimiteAssinaturaVencido() {
		StringBuilder sql = new StringBuilder();
		sql.append(" SELECT documentoAssinado.codigo AS \"documentoAssinado\", documentoAssinadoPessoa.codigo AS \"documentoAssinadoPessoa\", ");
		sql.append(" programacaoFormaturaAluno.codigo AS \"programacaoFormaturaAluno\", programacaoFormaturaAluno.colouGrau AS \"colouGrau\", ");
		sql.append(" programacaoFormatura.dataLimiteAssinaturaAta AS \"dataLimiteAssinaturaAta\" ");
		sql.append(" FROM documentoAssinado ");
		sql.append(" INNER JOIN documentoAssinadoPessoa ON documentoAssinado.codigo = documentoAssinadoPessoa.documentoAssinado ");
		sql.append(" INNER JOIN programacaoFormatura ON programacaoFormatura.codigo = documentoAssinado.programacaoFormatura ");
		sql.append(" INNER JOIN programacaoFormaturaAluno ON programacaoFormaturaAluno.programacaoFormatura = programacaoFormatura.codigo ");
		sql.append(" INNER JOIN matricula ON matricula.matricula = programacaoFormaturaAluno.matricula ");
		sql.append(" WHERE documentoAssinado.tipoOrigemDocumentoAssinado = 'ATA_COLACAO_GRAU' ");
		sql.append(" AND documentoAssinadoPessoa.tipoPessoa = 'ALUNO' ");
		sql.append(" AND documentoAssinadoPessoa.dataAssinatura IS NULL ");
		sql.append(" AND matricula.aluno = documentoAssinadoPessoa.pessoa ");
		sql.append(" AND documentoAssinadoPessoa.dataRejeicao IS NULL ");
		sql.append(" AND programacaoFormatura.dataLimiteAssinaturaAta < CURRENT_TIMESTAMP ");
		sql.append(" AND documentoAssinado.documentoAssinadoInvalido = FALSE ");
		sql.append(" AND (documentoAssinadoPessoa.situacaoDocumentoAssinadoPessoa IS NULL OR documentoAssinadoPessoa.situacaoDocumentoAssinadoPessoa <> 'REJEITADO') ");
		sql.append(" AND matricula.situacao <> 'FO' ");
		
		List<DocumentoAssinadoVO> documentos = new ArrayList<DocumentoAssinadoVO>();
		SqlRowSet tabelaResultado = getConexao().getJdbcTemplate().queryForRowSet(sql.toString());
		while (tabelaResultado.next()) {
			DocumentoAssinadoVO documento = new DocumentoAssinadoVO();
			documento.setCodigo(new Integer(tabelaResultado.getInt("documentoAssinado")));
			//documento.getDocumentoAssinadoPessoaVO().setCodigo(tabelaResultado.getInt("documentoAssinadoPessoa"));
			documento.setCodigoRejeitarAtaColacaoGrau(tabelaResultado.getInt("documentoAssinadoPessoa"));
			documento.getProgramacaoFormaturaVO().setDataLimiteAssinaturaAta(tabelaResultado.getTimestamp("dataLimiteAssinaturaAta"));
			documento.getProgramacaoFormaturaVO().getProgramacaoFormaturaAlunoVO().setCodigo(tabelaResultado.getInt("programacaoFormaturaAluno"));
			documento.getProgramacaoFormaturaVO().getProgramacaoFormaturaAlunoVO().setColouGrau(tabelaResultado.getString("colouGrau"));
