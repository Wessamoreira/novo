...
public class DocumentoAssinadoPessoa extends ControleAcesso implements DocumentoAssinadoPessoaInterfaceFacade {
	...
	public void incluir(final DocumentoAssinadoPessoaVO obj, boolean verificarAcesso, UsuarioVO usuario) throws Exception {
		DocumentoAssinadoPessoa.incluir(getIdEntidade(), verificarAcesso, usuario);
	...
	public void alterar(final DocumentoAssinadoPessoaVO obj, boolean verificarAcesso, UsuarioVO usuario) throws Exception {
		DocumentoAssinadoPessoa.alterar(getIdEntidade(), verificarAcesso, usuario);
	...
	public void excluir(DocumentoAssinadoPessoaVO obj, UsuarioVO usuario) throws Exception {
		try {
			ControleAcesso.excluir("MapaDocumentoAssinadoPessoa", true, usuario);
	...
	@Override
	public void realizarRejeicaoDocumentosAssinados(GestaoXmlGradeCurricularVO gestaoXmlGradeCurricularVO, UsuarioVO usuarioVO) throws Exception {
		List<DocumentoAssinadoVO> documentoAssinadoVOs = new ArrayList<DocumentoAssinadoVO>();
		DocumentoAssinadoVO obj = new DocumentoAssinadoVO();
		...
		if (Uteis.isAtributoPreenchido(documentoAssinadoVOs)) {
			...
			if (Uteis.isAtributoPreenchido(documentoAssinadoVOs)) {
				...
				for (DocumentoAssinadoVO documentoAssinadoVO : documentoAssinadoVOs) {
					getFacadeFactory().getDocumentoAssinadoFacade().atualizarDocumentoAssinadoInvalido(documentoAssinadoVO.getCodigo(), Boolean.TRUE, gestaoXmlGradeCurricularVO.getMotivoRejeicao(), usuarioVO);
				}
			}
		}
	}
...
//				}
	...
	@Override
	public void rejeitarAtaColacaoGrauDataLimiteAssinaturaVencido(Integer codigo, Date dataLimiteAssinaturaAta, UsuarioVO usuario) {
		StringBuilder sql = new StringBuilder();
		sql.append(" UPDATE documentoAssinadoPessoa SET dataRejeicao = CURRENT_DATE, situacaoDocumentoAssinadoPessoa = 'REJEITADO', ");
		sql.append(" motivoRejeicao = 'O aluno não assinou a ata dentro do prazo estipulado ' || TO_CHAR(TIMESTAMP '").append(Uteis.getDataJDBCTimestamp(dataLimiteAssinaturaAta)).append("', 'DD/MM/YYYY') ");
		sql.append(" WHERE codigo = ? ").append(adicionarUsuarioLogadoComoComentarioParaLogTrigger(usuario));
		getConexao().getJdbcTemplate().update(sql.toString(), codigo.intValue());
	}
}
