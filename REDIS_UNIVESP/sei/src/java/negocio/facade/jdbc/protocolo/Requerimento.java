...
public class Requerimento extends ControleAcesso implements RequerimentoInterfaceFacade {
	...
	public void incluir(final RequerimentoVO obj, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistema) throws Exception {
		try {

			Requerimento.incluir(getIdEntidade(), true, usuario);
...
//				emitirBoletoParcela(obj, usuario, configuracaoFinanceiroVO);
//			}
...
//			}
			...
			if (obj.getIsDeferido()) {
				realizarTrancamentoAlunoAutomatico(obj, configuracaoFinanceiroVO, configuracaoGeralSistema, usuario);
			 ...
			 if (obj.getTipoRequerimento().getDeferirAutomaticamente() && obj.getTipoRequerimento().getIsTipoTransferenciaInterna() && obj.getSituacao().equals(SituacaoRequerimento.FINALIZADO_DEFERIDO.getValor())) {
				 ...
				 getFacadeFactory().getTransferenciaEntradaFacade().realizarCriacaoTranferenciaInternaGerandoNovaMatriculaAproveitandoDisciplinasAprovadasProximoPeriodoLetivoPorRequerimento(obj, configuracaoGeralSistema , configuracaoFinanceiroVO , apresentarVisaoAluno ,usuario);
	...
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void registrarRecebimentoDoc(final RequerimentoVO obj, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistema) throws Exception {
		try {
			if (obj.getCodigo().intValue() == 0) {
				incluir(obj, usuario, configuracaoFinanceiroVO, configuracaoGeralSistema);
			} else {
	...
	public void alterar(final RequerimentoVO obj, UsuarioVO usuario, ConfiguracaoGeralSistemaVO configuracaoGeralSistema) throws Exception {
		try {
			...
			Requerimento.alterar(getIdEntidade(), true, usuario);
	...
	public void deferirRequerimento(RequerimentoVO requerimentoVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, RequerimentoHistoricoVO requerimentoHistoricoAtualVO, UsuarioVO usuarioVO) throws Exception {
		Requerimento.verificarPermissaoFuncionalidadeUsuario("Requerimento_PermitirDeferir", usuarioVO);
		...
		if(requerimentoVO.getAguardandoAutorizacaoPagamento() && requerimentoVO.getTipoRequerimento().getPermiteDeferirAguardandoAutorizacaoPagamento()) {
			...
			realizarAutorizacaoPagamentoRequerimento(requerimentoVO, configuracaoFinanceiroVO, usuarioVO);
		}
		RequerimentoHistoricoVO reqHistVO = requerimentoVO.consultarUltimoRequerimentoHistoricoDepartamentoAtualVOs();
		if (reqHistVO != null) {
		...
		realizarTrancamentoAlunoAutomatico(requerimentoVO, configuracaoFinanceiroVO, getConfiguracaoGeralSistemaPrivilegiandoACfgDaUnidade(usuarioVO), usuarioVO);
		...
		realizarDeferimentoRequerimentoTCC(requerimentoVO, usuarioVO);
		 if ( requerimentoVO.getTipoRequerimento().getIsTipoTransferenciaInterna() ) {
		    	getFacadeFactory().getTransferenciaEntradaFacade().realizarCriacaoTranferenciaInternaGerandoNovaMatriculaAproveitandoDisciplinasAprovadasProximoPeriodoLetivoPorRequerimento(requerimentoVO, getConfiguracaoGeralSistemaPrivilegiandoACfgDaUnidade(usuarioVO) , configuracaoFinanceiroVO , false , usuarioVO);
	...
	public void indeferirRequerimento(RequerimentoVO requerimentoVO, boolean validarAcesso, boolean indeferimentoAutomatico, UsuarioVO usuarioVO) throws Exception {
		if (validarAcesso) {
			Requerimento.verificarPermissaoFuncionalidadeUsuario("Requerimento_PermitirIndeferir", usuarioVO);
		}
	...
	public void emitirBoletoParcela(RequerimentoVO requerimentoVO, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiro) throws Exception {
		...
		if (valorConta > 0) {
			...
			getFacadeFactory().getContaReceberFacade().alterar(contaReceberVO, configuracaoFinanceiro, false, usuario);
		} else {
	...
	public void receberBoleto(RequerimentoVO requerimento, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistema) throws Exception {
		try {
			if (requerimento.isNovoObj()) {
				incluir(requerimento, usuario, configuracaoFinanceiroVO, configuracaoGeralSistema);
			} else {
				// alterarSemComit(requerimento);
			}
			ContaReceberVO contaReceber = getFacadeFactory().getContaReceberFacade().consultarPorChavePrimaria(requerimento.getContaReceber(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, configuracaoFinanceiroVO, usuario);
			RecebimentoVO recebimento = requerimento.receberBoleto(contaReceber);
			getFacadeFactory().getRecebimentoFacade().incluir(recebimento, usuario, configuracaoFinanceiroVO);
	...
	public void excluir(RequerimentoVO obj, ConfiguracaoFinanceiroVO configuracaoFinanceiro, UsuarioVO usuario, ConfiguracaoGeralSistemaVO configuracaoGeralSistema, boolean validarPermissao) throws Exception {
		try {
			if(validarPermissao){
				Requerimento.excluir(getIdEntidade(), true, usuario);				
			}
	...
	public void excluirContaReceber(RequerimentoVO obj, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiro) throws Exception {
		if (Uteis.isAtributoPreenchido(obj.getContaReceber())) {
			...
			try {
				contaReceberVO = getFacadeFactory().getContaReceberFacade().consultarPorChavePrimaria(obj.getContaReceber(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, configuracaoFinanceiro, usuario);
				...
				getFacadeFactory().getContaReceberFacade().excluir(contaReceberVO, configuracaoFinanceiro, false, usuario);
			} catch (Exception e) {
				throw e;
			}
		}
	...
	public void verificarExisteSolicitacaoRequerimento(RequerimentoVO requerimentoVO, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		if (requerimentoVO.getCodigo() != null && !requerimentoVO.getCodigo().equals(0)) {
			if (requerimentoVO.getTipoRequerimento().getTipo().equals("CA")) {
				CancelamentoVO cancelamentoVO = getFacadeFactory().getCancelamentoFacade().consultarPorCodigoRequerimento(requerimentoVO.getCodigo(), Uteis.NIVELMONTARDADOS_COMBOBOX, configuracaoFinanceiroVO, usuario);
			...
			if (requerimentoVO.getTipoRequerimento().getTipo().equals("TR")) {
				TrancamentoVO trancamentoVO = getFacadeFactory().getTrancamentoFacade().consultarPorCodigoRequerimento(requerimentoVO.getCodigo(), Uteis.NIVELMONTARDADOS_COMBOBOX, configuracaoFinanceiroVO, usuario);
			...
			if (requerimentoVO.getTipoRequerimento().getTipo().equals("TI")) {
				TransferenciaEntradaVO transferenciaEntradaVO = getFacadeFactory().getTransferenciaEntradaFacade().consultarPorCodigoRequerimento(requerimentoVO.getCodigo(), Uteis.NIVELMONTARDADOS_COMBOBOX, configuracaoFinanceiroVO, usuario);
	 ...
	 *                Caso haja problemas de conexão ou restrição de acesso.
	 */
	public List<RequerimentoVO> consultarPorMatriculaMatricula(String valorConsulta, String tipoRequerimento, Integer unidadeEnsino, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
	...
	public List<RequerimentoVO> consultarPorMatriculaMatriculaSemProgramacaoFormatura(String valorConsulta, String tipoRequerimento, List<ProgramacaoFormaturaUnidadeEnsinoVO> proFormaturaUnidadeEnsinoVOs, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
	 ...
	 *                Caso haja problemas de conexão ou restrição de acesso.
	 */
	public List<RequerimentoVO> consultarPorNomePessoa(String valorConsulta, String tipoRequerimento, Integer unidadeEnsino, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
	...
	public List<RequerimentoVO> consultarPorNomePessoaSemProgramacaoFormatura(String valorConsulta, String tipoRequerimento, Integer unidadeEnsino, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
	 ...
	 *                Caso haja problemas de conexão ou restrição de acesso.
	 */
	public List<RequerimentoVO> consultarPorSituacaoFinanceira(String valorConsulta, String tipoRequerimento, Integer unidadeEnsino, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
	...
	public List<RequerimentoVO> consultarRequerimentoAtrasadosParaExclusao(UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceira) throws Exception {
		StringBuilder sqlStr = new StringBuilder("");
		...
		SqlRowSet tabelaResultado = getConexao().getJdbcTemplate().queryForRowSet(sqlStr.toString());
		return (montarDadosConsulta(tabelaResultado, Uteis.NIVELMONTARDADOS_TODOS, usuario, configuracaoFinanceira));
	}
	 ...
	 *                Caso haja problemas de conexão ou restrição de acesso.
	 */
	public List<RequerimentoVO> consultarPorSituacao(String valorConsulta, String tipoRequerimento, Integer unidadeEnsino, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
	...
	public List<RequerimentoVO> consultarPorMatriculaSituacaoDiferenteDe(String valorConsulta, String situacao, String tipoRequerimento, Integer unidadeEnsino, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
	 ...
	 *                Caso haja problemas de conexão ou restrição de acesso.
	 */
	public List<RequerimentoVO> consultarPorDataFinalizacao(Date prmIni, Date prmFim, String tipoRequerimento, Integer unidadeEnsino, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
	...
	public List<RequerimentoVO> consultarPorCodigo(String listaCodigos, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
	...
	public List<RequerimentoVO> consultarPorDataSemProgramacaoFormatura(Date prmIni, Date prmFim, String tipo, List<ProgramacaoFormaturaUnidadeEnsinoVO> proFormaturaUnidadeEnsinoVOs, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
	...
	public List<RequerimentoVO> consultarPorCodigoSemProgramacaoFormatura(Integer valorConsulta, String tipo, List<ProgramacaoFormaturaUnidadeEnsinoVO> proFormaturaUnidadeEnsinoVOs, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
	 ...
	 *         <code>RequerimentoVO</code> resultantes da consulta.
	 */
	public static List<RequerimentoVO> montarDadosConsulta(SqlRowSet tabelaResultado, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
	 ...
	 */
	public static RequerimentoVO montarDados(SqlRowSet dadosSQL, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		RequerimentoVO obj = new RequerimentoVO();
		obj.setNovoObj(Boolean.FALSE);
		...
		montarDadosMatricula(obj, Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuario, configuracaoFinanceiroVO);
	 ...
	 */
	public static void montarDadosMatricula(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
	...
	public static void montarDadosPessoa(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosResponsavelEmissaoBoleto(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosResponsavelIsencaoTaxa(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosUnidadeEnsino(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	 ...
	 *            Objeto no qual será montado os dados consultados.
	 */
	public static void montarDadosDepartamentoResponsavel(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	 ...
	 *            Objeto no qual será montado os dados consultados.
	 */
	public static void montarDadosTipoRequerimento(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosFuncionario(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosCidade(RequerimentoVO obj, UsuarioVO usuario) throws Exception {
		if (obj.getCidade().getCodigo().intValue() == 0) {
	...
	public static void montarDadosArquivo(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		if (obj.getArquivoVO().getCodigo().intValue() == 0) {
	...
	public static void montarDadosCurso(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosTurno(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	 ...
	 *                procurado.
	 */
	public RequerimentoVO consultarPorChavePrimaria(Integer codigoPrm, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
	...
	public RequerimentoVO consultarPorChavePrimariaFiltrandoPorUnidadeEnsino(Integer codigoPrm, String tipoRequerimento, Integer unidadeEnsino, boolean controlarAcesso, int nivelMontarDados, UsuarioVO usuario, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO) throws Exception {
		ControleAcesso.consultar(getIdEntidade(), controlarAcesso, usuario);
...
//		System.out.println(sqlStr.toString());
	...
	private void montarDadosCompleto(RequerimentoVO obj, SqlRowSet dadosSQL, UsuarioVO usuario) throws Exception {
		...
		if(dadosSQL.getInt("requerimento.codigonovorequerimento") != 0) {
			obj.setRequerimentoAntigo(consultarPorChavePrimaria(dadosSQL.getInt("requerimento.codigonovorequerimento"), Uteis.NIVELMONTARDADOS_DADOSMINIMOS, usuario, null));
		}
	...
	public List<RequerimentoVO> consultaRapidaRequerimentoPorCodigoPai(Integer codigoPai, String matriculaAluno, Integer unidadeEnsino, boolean controlarAcesso, UsuarioVO usuario, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO) throws Exception {
		...
		if (usuario.getIsApresentarVisaoAluno() || usuario.getIsApresentarVisaoPais()) {
			ControleAcesso.consultar("RequerimentoAluno", controlarAcesso, usuario);
		}else {
...
//		}
	...
	public synchronized void persistirSicronizadoRequerimento(RequerimentoVO requerimento, RequerimentoHistoricoVO requerimentoHistoricoVO, Boolean visaoAluno, Boolean exigePagamento, ConfiguracaoGeralSistemaVO configuracaoGeralSistema, ConfiguracaoFinanceiroVO configuracaoFinanceira, UnidadeEnsinoVO unidadeEnsinoLogado, UsuarioVO usuarioLogado) throws Exception {
		persistirRequerimento(requerimento, requerimentoHistoricoVO, visaoAluno, exigePagamento, configuracaoGeralSistema, configuracaoFinanceira, unidadeEnsinoLogado, usuarioLogado);	
	}
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void persistirRequerimento(RequerimentoVO requerimento, RequerimentoHistoricoVO requerimentoHistoricoVO, Boolean visaoAluno, Boolean exigePagamento, ConfiguracaoGeralSistemaVO configuracaoGeralSistema, ConfiguracaoFinanceiroVO configuracaoFinanceira, UnidadeEnsinoVO unidadeEnsinoLogado, UsuarioVO usuarioLogado) throws Exception {
		try {
			...
			if ((requerimento.getSituacaoFinanceira().equals("PE")) || (requerimento.getSituacaoFinanceira().equals("IS") && (requerimento.getSituacao().equals("PE") || requerimento.getSituacao().equals("EX") || requerimento.getSituacao().equals("FD"))) || (requerimento.getSituacaoFinanceira().equals("AP") && (requerimento.getSituacao().equals("PE") || requerimento.getSituacao().equals("EX")))) {
				...
				if (requerimento.isNovoObj().booleanValue()) {
					...
					incluir(requerimento, usuarioLogado, configuracaoFinanceira, configuracaoGeralSistema);
...
//			requerimento.setSituacao(SituacaoRequerimento.AGUARDANDO_PAGAMENTO.getValor());
	...
	public static void montarDadosTurma(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public static void montarDadosDisciplina(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
	...
	public String executarVerificacaoProfessorMinistrouAula(String matricula, Integer disciplina, boolean controlarAcesso, UsuarioVO usuarioVO) throws Exception {
		Requerimento.consultar(getIdEntidade(), controlarAcesso, usuarioVO);
		StringBuilder sqlStr = new StringBuilder();
	...
	public void executarVerificacaoUsuarioPossuiPermissaoInformarDescontoAcrescimo(RequerimentoVO requerimentoVO, boolean valorDescontoSelecionado, String tipoDescontoTemp, Double valorAcrescimoDescontoTemp, String usuarioLiberarOperacaoFuncionalidade, String senhaLiberarOperacaoFuncionalidade, List<OperacaoFuncionalidadeVO> operacaoFuncionalidadeVOs) throws Exception {
		String str = "Desconto";
		UsuarioVO usuarioVO = Requerimento.verificarLoginUsuario(usuarioLiberarOperacaoFuncionalidade, senhaLiberarOperacaoFuncionalidade, true, Uteis.NIVELMONTARDADOS_TODOS);
		try {
			String observacao = "";
			if (valorDescontoSelecionado) {
				Requerimento.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("PermitirInformarDesconto", usuarioVO);
				...
				str = "Valor Adicional";
				Requerimento.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("PermitirInformarAcrescimo", usuarioVO);
	...
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void excluirRequerimentoPorMatricula(String matricula, Integer unidadeEnsino, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO,  UsuarioVO usuarioVO) throws Exception{
		...
		for(RequerimentoVO requerimentoVO: requerimentoVOs){
			...
			excluir(requerimentoVO, configuracaoFinanceiroVO, usuarioVO, configuracaoGeralSistemaVO, false);
		}
	}
	
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void excluirRequerimentoPorMatriculaPeriodo(Integer matriculaPeriodo, Integer unidadeEnsino, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO,  UsuarioVO usuarioVO) throws Exception{
...
//		}
	...
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void realizarAutorizacaoPagamentoRequerimento(RequerimentoVO requerimentoVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception{
		if (requerimentoVO.getValorTotalFinal() > 0.0 && requerimentoVO.getSituacaoFinanceira().equals("AP")) {
			try {
				...
				if (!Uteis.isAtributoPreenchido(requerimentoVO.getContaReceberVO().getCodigo())) {
					emitirBoletoParcela(requerimentoVO, usuario, configuracaoFinanceiroVO);
				}
			}catch (Exception e) {
				requerimentoVO.setSituacaoFinanceira("AP");
				throw e;
	...
	@Override
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void realizarRegistroDeferimentoIndeferimentoSolicitacaoIsencaoTaxa(final RequerimentoVO requerimentoVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, final UsuarioVO usuarioVO) throws Exception {
			...
			try {
				...
				if(requerimentoVO.getSituacaoIsencaoTaxa().equals(SituacaoSolicitacaoIsencaoTaxaRequerimentoEnum.DEFERIDO)) {
					...
					if(Uteis.isAtributoPreenchido(requerimentoVO.getContaReceber())) {
						excluirContaReceber(requerimentoVO, usuarioVO, configuracaoFinanceiroVO);
	...
	@Override
	public void realizarValidacaoChoqueHorarioEVagaTurmaRequerimentoReposicao(RequerimentoVO requerimentoVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuarioVO) throws Exception{
		...
		try {
			matriculaPeriodoTurmaDisciplinaVO = getFacadeFactory().getInclusaoHistoricoForaPrazoFacade().realizarPreenchimentoDadosMatriculaPeriodoTurmaDisciplinaAdicionarPorRequerimento(requerimentoVO, true, true, configuracaoFinanceiroVO, usuarioVO);
	...
	public static void montarDadosMapaEquivalenciaDisciplina(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception{
	...
	public static void montarDadosMapaEquivalenciaDisciplinaCursada(RequerimentoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception{
	...
	public String realizarImpressaoDeclaracaoRequerimento(RequerimentoVO requerimentoVO, ImpressaoContratoVO impressaoContratoFiltro, ImpressaoContratoVO contratoGravar, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, TextoPadraoDeclaracaoVO texto, Boolean persistirDocumentoAssinado, UsuarioVO usuarioVO) throws Exception {
		...
		caminhoArquivo = getFacadeFactory().getImpressaoDeclaracaoFacade().imprimirDeclaracao(texto.getCodigo(), impressaoContratoFiltro, contratoGravar, "AL", impressaoContratoFiltro.getTurmaVO(), impressaoContratoFiltro.getDisciplinaVO(), configuracaoGeralSistemaVO, configuracaoFinanceiroVO, usuarioVO, persistirDocumentoAssinado);
	...
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void realizarTrancamentoAlunoAutomatico(RequerimentoVO requerimentoVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistema, UsuarioVO usuario) throws Exception {
	try {
		if (requerimentoVO.getTipoRequerimento().getTipo().equals("TR") && requerimentoVO.getIsDeferido().booleanValue() && requerimentoVO.getTipoRequerimento().getDeferirAutomaticamenteTrancamento().booleanValue()) {
			...
			getFacadeFactory().getMatriculaPeriodoFacade().carregarDados(requerimentoVO.getMatriculaPeriodoVO(), configuracaoFinanceiroVO, usuario);
...
//									&& (new BigDecimal(requerimentoVO.getMatriculaPeriodoVO().getSemestre()).compareTo(new BigDecimal(Uteis.getSemestreAtual())) > 0))));
			...
			if(requerimentoVO.getTipoRequerimento().getRegistrarTrancamentoProximoSemestre().booleanValue() &&  
				...
				getFacadeFactory().getTrancamentoFacade().executarVerificacaoEGeracaoMatriculaPeriodoSemestrePosteriorMapaRegistroAbandonoCursoTrancamento(trancamentoVO, requerimentoVO.getMatriculaPeriodoVO(), configuracaoFinanceiroVO, usuario);
				if(!Uteis.isAtributoPreenchido(trancamentoVO.getMatriculaPeriodoVO())) {
					trancamentoVO.setUltimaMatriculaPeriodoVO(getFacadeFactory().getMatriculaPeriodoFacade().consultarUltimaMatriculaPeriodoPorMatriculaSituacoes(trancamentoVO.getMatricula().getMatricula(), "'AT', 'PR', 'PC', 'AC', 'TR', 'ER'", false, Uteis.NIVELMONTARDADOS_COMBOBOX, configuracaoFinanceiroVO, usuario));
			...
			getFacadeFactory().getTrancamentoFacade().incluirEDeferirRequerimento(trancamentoVO, configuracaoGeralSistema, configuracaoFinanceiroVO, usuario, true);
