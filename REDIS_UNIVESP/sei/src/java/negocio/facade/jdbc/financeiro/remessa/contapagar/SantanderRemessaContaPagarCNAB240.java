 ...
 */
public class SantanderRemessaContaPagarCNAB240 extends ControleRemessaContaPagarLayoutImpl implements ControleRemessaContaPagarLayoutInterfaceFacade {
	...
	private void gerarDadosHeaderArquivo(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		...
		String convenio = "0033"+controleRemessaVO.getContaCorrenteVO().getAgencia().getNumeroAgencia()+Uteis.preencherComZerosPosicoesVagas(controleRemessaVO.getContaCorrenteVO().getCodigoComunicacaoRemessaCP(), 12);
		linha = editorOC.adicionarCampoLinhaVersao2(linha, convenio, 33, 52, "0", false, false);
		// Agencia da conta
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "060", 164, 166, " ", false, false);
		// Densidade de Gravacao Arquivo
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 167, 171, " ", false, false);
		...
		// Filler espaco em branco
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 212, 230, " ", false, false);
	...
	private void gerarDadosHeaderLote(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, MapaRemessaLoteContaPagarVO mapaRemessaContaPagar, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, mapaRemessaContaPagar.getNumeroVersaoLote(), 14, 16, " ", false, false);
		// filler espaco em branco
		...
		// Opcional
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 143, 240, " ", false, false);
		linha += "\r";
		cmd.adicionarLinhaComando(linha, 0);
	...
	private void gerarDetalheSegmentoA(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		if(dadosRemessaVO.getTipoLancamentoContaPagar().isTransferencia() && (dadosRemessaVO.getModalidadeTransferenciaBancariaEnum().isTed() || (dadosRemessaVO.getModalidadeTransferenciaBancariaEnum().isDoc() && !dadosRemessaVO.getFinalidadeDocEnum().isDocPoupanca()))){
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "CC", 225, 226, " ", false, false);
		}else if(dadosRemessaVO.getTipoLancamentoContaPagar().isTransferencia() && dadosRemessaVO.getModalidadeTransferenciaBancariaEnum().isDoc() && dadosRemessaVO.getFinalidadeDocEnum().isDocPoupanca()){
	...
	private void gerarDetalheSegmentoB(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.copiarDelimitandoTamanhoDoTexto(Uteis.removeCaractersEspeciais(Uteis.removerAcentuacao(dadosRemessaVO.getNumeroEnderecoFavorecido())), 5), 63, 67, " ", true, false);
		...
		//0013 = CRÉDITO DE DIVIDENDOS
		//0091 = PAGAMENTO DE SINISTRO - SEGURADO
		//0109 = PGTO DESPESAS VIAJANTE EM TRANSITO
		//0137 = PGTO REPRES/VENDEDORES AUTORIZADOS
		//0149 = PAGAMENTO DE BENEFÍCIOS DO INSS
		//0183 = PAGAMENTO A FORNECEDORES
		//0197 = DEVOLUÇÃO DE LANÇAMENTO
		//0295 = PGTO CREDENCIADO DE EMPR CONV
		//2060 = TRANSFERÊNCIA MESMA TITULARIDADE
		//0367 = PAGAMENTO DE PENSÃO ALIMENTÍCIA
		//0491 = RESGATE PREVIDÊNCIA
		//0493 = PAGAMENTO BENEFÍCIO PREVIDÊNCIA
		//0495 = ESTORNO CONTRIBUIÇÃO PREVIDÊNCIA
		//1070 = TRANSFERÊNCIA DE SALDO
		//2214 = CRÉDITOS DIVERSOS
		...
		if(dadosRemessaVO.getTipoLancamentoContaPagar().isCreditoContaCorrente() || dadosRemessaVO.getTipoLancamentoContaPagar().isCreditoContaPoupanca()){
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 226, 229, "0", false, false);
		}else{
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 226, 229, "0", false, false);
		}
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "0", 230, 230, " ", false, false);
		//filter espaco em branco
		linha = editorOC.adicionarCampoLinhaVersao2(linha, " ", 231, 231, " ", false, false);
		//TED para Instituição Financeira 
		//S = Utilizar quando o favorecido/beneficiário de uma TED é uma Instituição Financeira. Neste caso, as informações da agência e conta do favorecido são opcionais (Nota G003)
		//N =Utilizar quando o favorecido/beneficiário de uma TED não é uma Instituição Financeira. Neste caso, as informações da agência e conta do favorecido são obrigatórias (Nota G003)
		if(dadosRemessaVO.getTipoLancamentoContaPagar().isTransferencia() && dadosRemessaVO.getModalidadeTransferenciaBancariaEnum().isTed() && dadosRemessaVO.isTipoSacadoBanco()){
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "S", 232, 232, " ", false, false);
			//Identificação da IF no SPB 
			Bancos layout = Bancos.getEnum(dadosRemessaVO.getBancoRecebimento().getNrBanco());
			linha = editorOC.adicionarCampoLinhaVersao2(linha, layout.getNumeroISPB(), 233, 240, " ", false, false);
		}else if(dadosRemessaVO.getTipoLancamentoContaPagar().isTransferencia() && dadosRemessaVO.getModalidadeTransferenciaBancariaEnum().isTed() && !dadosRemessaVO.isTipoSacadoBanco()){
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "N", 232, 232, " ", false, false);
			//Identificação da IF no SPB 
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 233, 240, " ", false, false);
		}else{
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 232, 232, " ", false, false);
			//Identificação da IF no SPB 
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 233, 240, " ", false, false);
		}
		linha += "\r";
	...
	private void gerarDetalheSegmentoJ(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		/*// Valor Desconto + Abatimento
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.preencherComZerosPosicoesVagas(Uteis.removeCaractersEspeciais(Uteis.arrendondarForcando2CadasDecimaisStr(dadosRemessaVO.getDesconto())), 13), 115, 129, "0", false, false);
		// Valor Multa + Juros
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.preencherComZerosPosicoesVagas(Uteis.removeCaractersEspeciais(Uteis.arrendondarForcando2CadasDecimaisStr(dadosRemessaVO.getMulta() + dadosRemessaVO.getJuro())), 13), 130, 144, "0", false, false);*/
	...
	private void gerarDetalheSegmentoJ52(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		// nome Cedente
		linha = editorOC.adicionarCampoLinhaVersao2(linha,  Uteis.copiarDelimitandoTamanhoDoTexto(dadosRemessaVO.getNomeFavorecido(), 40), 92, 131, " ", true, false);
		
		// Cedente
		// Tipo de Inscrição do Cedente
		linha = editorOC.adicionarCampoLinhaVersao2(linha, dadosRemessaVO.getTipoInscricaoFavorecido(), 132, 132, " ", false, false);
		// Numero de Inscricao da Cedente
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.removerMascara(dadosRemessaVO.getCnpjOuCpfFavorecido()), 133, 147, "0", false, false);
		// nome Cedente
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.copiarDelimitandoTamanhoDoTexto(dadosRemessaVO.getNomeFavorecido(), 40), 148, 187, " ", true, false);
	...
	private void processarDetalheSegmentoARetorno(String linha, RegistroDetalhePagarVO registroDetalhe) throws Exception {
		...
		if(Uteis.getValorString(linha.substring(90, 93)).matches("[A-Z]*")) {		
			...
			//registroDetalhe.setCodigoTransmissaoRemessaNossonumero(linha.substring(73, 93));
		}
