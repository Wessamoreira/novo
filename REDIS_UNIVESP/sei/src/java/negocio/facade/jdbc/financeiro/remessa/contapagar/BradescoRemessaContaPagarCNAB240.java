 ...
 * @author Otimize-04
 */
public class BradescoRemessaContaPagarCNAB240 extends ControleRemessaContaPagarLayoutImpl implements ControleRemessaContaPagarLayoutInterfaceFacade {
	...
	public BradescoRemessaContaPagarCNAB240() {
	}
	...
	public EditorOC executarGeracaoDadosArquivoRemessa(List<ContaPagarControleRemessaContaPagarVO> listaDadosRemessaVOs, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		...
		controleRemessaVO.setIncrementalMXCP(getFacadeFactory().getControleRemessaMXFacade().consultarIncrementalCPPorContaCorrente(controleRemessaVO.getContaCorrenteVO().getCodigo(), usuario));
     	Boolean eArquivoPix = executarVerificacaoExistenciaPixListaRemessa(listaDadosRemessaVOs);
		gerarDadosHeaderArquivo(editorOC, cmd, controleRemessaVO, configuracaoFinanceiroVO, eArquivoPix ,usuario);     	
	...
	private void separarDadosRemessaPorLote(List<ContaPagarControleRemessaContaPagarVO> listaDadosRemessaVOs) throws ConsistirException {
		listaMapaRemessaContaPagarVO.clear();
		for (ContaPagarControleRemessaContaPagarVO obj : listaDadosRemessaVOs) {
			validarDadosParaTodosSegmento(obj);
	...
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.SUPPORTS)
	private void gerarDadosHeaderArquivo(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO,Boolean eArquivoPix, UsuarioVO usuario) throws Exception {
		String linha = "";
		// Codigo do banco
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "237", 1, 3, " ", false, false);
		// Lote do servico
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, controleRemessaVO.getContaCorrenteVO().getConvenio(), 33, 52, "0", false, false);
		// Agencia da conta
		...
		// Nome da Empresa
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.copiarDelimitandoTamanhoDoTexto(Uteis.removerAcentuacao(controleRemessaVO.getUnidadeEnsinoVO().getRazaoSocial()), 30), 73, 102, " ", false, false);
		// Nome do Banco
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "BANCO BRADESCO", 103, 132, " ", true, false);
		// filler espaco em branco
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.getData(controleRemessaVO.getDataGeracao(), PATTERN), 144, 151, " ", false, false);
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "089", 164, 166, " ", false, false);
		...
		if(eArquivoPix) {
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "Pix", 172, 174, " ", false, false);
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 175, 191, " ", false, false);
		}else {
		...
		// Filler espaco em branco
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 212, 240, " ", false, false);		
		linha += "\r";
	...
	private void gerarDadosHeaderLote(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, MapaRemessaLoteContaPagarVO mapaRemessaContaPagar, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuario) throws Exception {
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, getNumeroVersaoLote(mapaRemessaContaPagar.getTipoLancamentoContaPagar()), 14, 16, " ", false, false);
		// filler espaco em branco
		...
		//Logradouro
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.copiarDelimitandoTamanhoDoTexto(Uteis.removerAcentuacao(controleRemessaVO.getUnidadeEnsinoVO().getEndereco()), 30), 143, 172, " ", true, false);
		//Número
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 173, 177, "0", true, false);
		//Complemento
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.copiarDelimitandoTamanhoDoTexto(Uteis.removerAcentuacao(controleRemessaVO.getUnidadeEnsinoVO().getEnderecoCompleto()), 15), 178, 192, " ", true, false);
		//Cidade
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.copiarDelimitandoTamanhoDoTexto(Uteis.removerAcentuacao(controleRemessaVO.getUnidadeEnsinoVO().getCidade().getNome()), 20), 193, 212, " ", true, false);
		//CEP
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.removerMascara(Uteis.removerAcentuacao(controleRemessaVO.getUnidadeEnsinoVO().getCEP())), 213, 217, " ", true, false);
		//Complemento do CEP
		linha = editorOC.adicionarCampoLinhaVersao2(linha,"", 218, 220, "0", true, false);
		//Estado do Favorecido
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.copiarDelimitandoTamanhoDoTexto(controleRemessaVO.getUnidadeEnsinoVO().getCidade().getEstado().getSigla(), 2), 221, 222, " ", true, false);
		
		// Opcional
		
		 if(getNumeroVersaoLote(mapaRemessaContaPagar.getTipoLancamentoContaPagar()).equals("040")) {			
				linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 223, 240, " ", false, false);			
		}else if(getNumeroVersaoLote(mapaRemessaContaPagar.getTipoLancamentoContaPagar()).equals("045")){
			   linha = editorOC.adicionarCampoLinhaVersao2(linha, "01", 223, 240, " ", true, false);		
		}else {
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "00", 223, 224, " ", false, false);
	...
	private void gerarDetalheSegmentoA(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		if(dadosRemessaVO.getTipoLancamentoContaPagar().isCreditoContaCorrente() || dadosRemessaVO.getTipoLancamentoContaPagar().isCreditoContaPoupanca() ){
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, !Uteis.removeCaractersEspeciais(dadosRemessaVO.getBancoRecebimento().getNrBanco().trim()).equals(Bancos.BRADESCO.getNumeroBanco()) ? Uteis.removeCaractersEspeciais(dadosRemessaVO.getBancoRecebimento().getNrBanco().trim()) : "", 21, 23, "0", false, false);
		// Código da Agência Favorecido
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.removeCaractersEspeciais(dadosRemessaVO.getNumeroAgenciaRecebimento().trim()), 24, 28, "0", false, false);
		// Dígito Verificador da Agência
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.removeCaractersEspeciais(dadosRemessaVO.getDigitoAgenciaRecebimento().trim()), 29, 29, " ", false, false);
		// Conta Corrente do Favorecido
		linha = editorOC.adicionarCampoLinhaVersao2(linha,Uteis.removeCaractersEspeciais( dadosRemessaVO.getContaCorrenteRecebimento().trim()), 30, 41, "0", false, false);
		// Dígito Verificador da Conta
		linha = editorOC.adicionarCampoLinhaVersao2(linha,Uteis.removeCaractersEspeciais( dadosRemessaVO.getDigitoCorrenteRecebimento().trim()), 42, 42, "0", false, false);
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, dadosRemessaVO.getCodigoTransmissaoRemessanossonumero(), 74,93, " ", false, false);
		
		
		// Data do Pagamento
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.getData(dadosRemessaVO.getDataVencimento(), PATTERN), 94, 101, " ", false, false);
		// Tipo da Moeda
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "BRL", 102, 104, " ", false, false);
		// Quantidade de Moeda
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 105, 119, "0", false, false);
		// Valor do Pagamento
		...
		// Nro. do Documento Banco   deixar branco 
		linha = editorOC.adicionarCampoLinhaVersao2(linha, " ", 135, 154, " ", false, false);
		// Data Real do Pagamento (Retorno)
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.getData(dadosRemessaVO.getDataVencimento(), PATTERN), 155, 162, " ", false, false);
		...
		// PP = Tipo de Conta de Crédito é Poupança
		if (dadosRemessaVO.getTipoLancamentoContaPagar().isTransferencia()) {
			if ((dadosRemessaVO.getModalidadeTransferenciaBancariaEnum().isTed() && (!Uteis
					.isAtributoPreenchido(dadosRemessaVO.getTipoContaEnum())
					|| (Uteis.isAtributoPreenchido(dadosRemessaVO.getTipoContaEnum())
							&& dadosRemessaVO.getTipoContaEnum().equals(TipoContaEnum.CREDITO_EM_CONTA_CORRENTE))))
					|| (dadosRemessaVO.getModalidadeTransferenciaBancariaEnum().isDoc()
							&& !dadosRemessaVO.getFinalidadeDocEnum().isDocPoupanca())) {
				linha = editorOC.adicionarCampoLinhaVersao2(linha, "CC", 225, 226, " ", false, false);

			} else if ((dadosRemessaVO.getModalidadeTransferenciaBancariaEnum().isDoc()
					&& dadosRemessaVO.getFinalidadeDocEnum().isDocPoupanca())
					|| (dadosRemessaVO.getModalidadeTransferenciaBancariaEnum().isTed()
							&& Uteis.isAtributoPreenchido(dadosRemessaVO.getTipoContaEnum())
							&& dadosRemessaVO.getTipoContaEnum().equals(TipoContaEnum.CREDITO_EM_CONTA_POUPANCA))) {

				linha = editorOC.adicionarCampoLinhaVersao2(linha, "PP", 225, 226, " ", false, false);
			}
		} else {
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 225, 226, " ", false, false);
		}	
		
		// Filler espaco em branco
		linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 227, 229, " ", false, false);
	...
	private void gerarDetalheSegmentoB(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		if(Uteis.isAtributoPreenchido(dadosRemessaVO.getModalidadeTransferenciaBancariaEnum()) && dadosRemessaVO.getModalidadeTransferenciaBancariaEnum().isPix()){
			...
			//Valor do Documento
			linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.preencherComZerosPosicoesVagas(Uteis.removeCaractersEspeciais(Uteis.arrendondarForcando2CadasDecimaisStr(dadosRemessaVO.getPrevisaoValorPagoDescontosMultas())), 13), 136, 150, "0", false, false);
			//Valor do Abatimento
			...
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 211, 225, "0", false, false);
			//Aviso ao Favorecido
			linha = editorOC.adicionarCampoLinhaVersao2(linha, "", 226, 226, "0", false, false);
		}
		//Código UG Centralizadora
		linha = editorOC.adicionarCampoLinhaVersao2(linha, " ", 227, 232, " ", false, false);	
		//Ocorrências para o Retorno
	...
	private void gerarDetalheSegmentoJ(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		//Nome do Cedente
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.copiarDelimitandoTamanhoDoTexto(Uteis.removeCaractersEspeciais(dadosRemessaVO.getNomeFavorecido()), 30), 62, 91, " ", true, false);
		// Data Vencimento
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.getData(dadosRemessaVO.getDataVencimento(), PATTERN), 92, 99, " ", false, false);
		...
		// Data do Pagamento
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.getData(dadosRemessaVO.getDataVencimento(), PATTERN), 145, 152, " ", false, false);
		// Valor do Pagamento
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.removerMascara(dadosRemessaVO.getNrDocumento()), 203, 222, "0", false, false);
		// Tipo da Moeda
	...
	private void gerarDetalheSegmentoJ52(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		//CNPJ/CPF do Sacado
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.removerMascara(controleRemessaVO.getUnidadeEnsinoVO().getCNPJ()), 21, 35, "0", false, false);
		//Nome do Sacado
		linha = editorOC.adicionarCampoLinhaVersao2(linha,Uteis.copiarDelimitandoTamanhoDoTexto(Uteis.removeCaractersEspeciais(controleRemessaVO.getUnidadeEnsinoVO().getRazaoSocial()), 40), 36, 75, " ", true, false);
		// Cedente
		// Tipo de Inscrição do Cedente
		linha = editorOC.adicionarCampoLinhaVersao2(linha, dadosRemessaVO.getTipoInscricaoFavorecido(), 76, 76, " ", false, false);
		// Numero de Inscricao da Cedente
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.removerMascara(dadosRemessaVO.getCnpjOuCpfFavorecido()), 77, 91, "0", false, false);
		// nome Cedente
		linha = editorOC.adicionarCampoLinhaVersao2(linha,  Uteis.copiarDelimitandoTamanhoDoTexto(Uteis.removeCaractersEspeciais(dadosRemessaVO.getNomeFavorecido()), 40), 92, 131, " ", true, false);
		
		if(Uteis.isAtributoPreenchido(dadosRemessaVO.getModalidadeTransferenciaBancariaEnum())  && dadosRemessaVO.getModalidadeTransferenciaBancariaEnum().isPix() &&   dadosRemessaVO.getTipoLancamentoContaPagar().isPixTransferencia()) {
			// URL / CHAVE PIX
			linha = editorOC.adicionarCampoLinhaVersao2(linha, dadosRemessaVO.getChaveEnderecamentoPix(), 132, 210, " ", false, false);
			// CÓDIGO DE IDENTIFICAÇÃO DO QR-CODE
			linha = editorOC.adicionarCampoLinhaVersao2(linha, dadosRemessaVO.getIdentificacaoQRCODE(), 211, 240, " ", false, false);	
		}else {
		// Cedente
		// Tipo de Inscrição do Cedente
	...
	private void gerarDetalheSegmentoN(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		// Nro. do Documento Banco
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.removerMascara(dadosRemessaVO.getNrDocumento()), 38, 57, "0", false, false);
		
		// Nome do contribuiente
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.copiarDelimitandoTamanhoDoTexto(Uteis.removeCaractersEspeciais(dadosRemessaVO.getNomeFavorecido()), 30), 58, 87, " ", true, false);
		// Data do Pagamento
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.getData(dadosRemessaVO.getDataVencimento(), PATTERN), 88, 95, " ", false, false);
		// Valor do Pagamento
	...
	private void gerarDetalheSegmentoN2(String linha, EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		// Período de Apuração 
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.getData(dadosRemessaVO.getDataFatoGerador(), PATTERN), 135, 142, " ", false, false);
		...
		// Data de Vencimento
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.getData(dadosRemessaVO.getDataVencimento(), PATTERN), 205, 212, " ", false, false);
		// Ocorrências para o Retorno
	...
	private void gerarDetalheSegmentoN4(String linha, EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		// Data Vencimento
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.getData(dadosRemessaVO.getDataVencimento(), PATTERN), 135, 142, " ", false, false);
	...
	private void gerarDetalheSegmentoO(EditorOC editorOC, Comando cmd, ControleRemessaContaPagarVO controleRemessaVO, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, ContaPagarControleRemessaContaPagarVO dadosRemessaVO) throws Exception {
		...
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.getData(dadosRemessaVO.getDataVencimento(), PATTERN), 92, 99, " ", false, false);
		// Data do Pagamento
		linha = editorOC.adicionarCampoLinhaVersao2(linha, Uteis.getData(dadosRemessaVO.getDataVencimento(), PATTERN), 100, 107, " ", false, false);
		// Valor do Pagamento
	...
	public  void validarDadosSegmentoA(ContaPagarControleRemessaContaPagarVO contaPagar) throws ConsistirException{
		if(Uteis.isAtributoPreenchido(contaPagar.getModalidadeTransferenciaBancariaEnum()) &&  contaPagar.getModalidadeTransferenciaBancariaEnum().isPix()) {
			...
			if(contaPagar.getDigitoCorrenteRecebimento().isEmpty()){
				throw  new ConsistirException("O campo Digito da Conta corrente recebimento Favorecido deve ser informado. Para a conta de número: " + contaPagar.getContaPagar().getNossoNumero());
			}
	...
	public String getNumeroVersaoLote(TipoLancamentoContaPagarEnum tipoLancamentoContaPagar) {
		if (tipoLancamentoContaPagar.isCreditoContaCorrente() ||
			tipoLancamentoContaPagar.isCreditoContaPoupanca() ||
			tipoLancamentoContaPagar.isTransferencia()) {
			return "045";
		} else if ( tipoLancamentoContaPagar.isLiquidacaoTituloProprioBanco() || tipoLancamentoContaPagar.isLiquidacaoTituloOutroBanco()) {
			return "040";
		} else if (tipoLancamentoContaPagar.isDarfNormalSemCodigoBarra()
				|| tipoLancamentoContaPagar.isDarfSimplesSemCodigoBarra()
				|| tipoLancamentoContaPagar.isGareDrSemCodigoBarra()
				|| tipoLancamentoContaPagar.isGareIcmsSemCodigoBarra()
				|| tipoLancamentoContaPagar.isGareItcmdSemCodigoBarra()
				|| tipoLancamentoContaPagar.isGpsSemCodigoBarra() || tipoLancamentoContaPagar.isDpvatRenavam()
				|| tipoLancamentoContaPagar.isIpvaComRenavam()
				|| tipoLancamentoContaPagar.isLicenciamentoComRenavam()
				|| tipoLancamentoContaPagar.isPagamentoContasTributosComCodigoBarra()) {
			return "012";
		}
		return "";
	}
