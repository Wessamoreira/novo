...
import negocio.interfaces.financeiro.ConvenioTurnoInterfaceFacade;

/**
 * Classe de persistência que encapsula todas as operações de manipulação dos dados da classe <code>ConvenioTurnoVO</code>.
 * Responsável por implementar operações como incluir, alterar, excluir e consultar pertinentes a classe <code>ConvenioTurnoVO</code>.
...
@Scope("singleton")
@Lazy 
public class ConvenioTurno extends ControleAcesso implements ConvenioTurnoInterfaceFacade{

    protected static String idEntidade;

    public ConvenioTurno() throws Exception {
        super();
    ...
    public ConvenioTurnoVO novo() throws Exception {
        ConvenioTurno.incluir(getIdEntidade());
        ConvenioTurnoVO obj = new ConvenioTurnoVO();
        return obj;
    }
    ...
    public void incluir(ConvenioTurnoVO obj) throws Exception {
        ...
        String sql = "INSERT INTO ConvenioTurno( convenio, turno ) VALUES ( ?, ? )";
        getConexao().getJdbcTemplate().update(sql, new Object[]{obj.getConvenio(), obj.getTurno().getCodigo()});
     ...
     */
    @Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
    public void alterar(final ConvenioTurnoVO obj) throws Exception {
        ConvenioTurnoVO.validarDados(obj);
        ConvenioTurno.alterar(getIdEntidade());
        final String sql = "UPDATE ConvenioTurno set  WHERE ((convenio = ?) and (turno = ?))";
    ...
    public void excluir(ConvenioTurnoVO obj) throws Exception {
        ConvenioTurno.excluir(getIdEntidade());
        String sql = "DELETE FROM ConvenioTurno WHERE ((convenio = ?) and (turno = ?))";
        getConexao().getJdbcTemplate().update(sql, new Object[]{obj.getConvenio(), obj.getTurno().getCodigo()});
    }
    ...
    public List consultarPorNomeTurno(String valorConsulta, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        ...
        String sqlStr = "SELECT ConvenioTurno.* FROM ConvenioTurno, Turno WHERE ConvenioTurno.turno = Turno.codigo and upper( Turno.nome ) like('" + valorConsulta.toUpperCase() + "%') ORDER BY Turno.nome";
    ...
    public List consultarPorDescricaoConvenio(String valorConsulta, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        ControleAcesso.consultar(getIdEntidade(), true,usuario);
        String sqlStr = "SELECT ConvenioTurno.* FROM ConvenioTurno, Convenio WHERE ConvenioTurno.convenio = Convenio.codigo and upper( Convenio.descricao ) like('" + valorConsulta.toUpperCase() + "%') ORDER BY Convenio.descricao";
        SqlRowSet tabelaResultado = getConexao().getJdbcTemplate().queryForRowSet(sqlStr);
        return montarDadosConsulta(tabelaResultado, nivelMontarDados,usuario);
    }
     ...
     * em um objeto da classe <code>ConvenioTurnoVO</code>.
     * @return  O objeto da classe <code>ConvenioTurnoVO</code> com os dados devidamente montados.
     */
    public static ConvenioTurnoVO montarDados(SqlRowSet dadosSQL, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        ConvenioTurnoVO obj = new ConvenioTurnoVO();
    ...
    /**
     * Operação responsável por montar os dados de um objeto da classe <code>TurnoVO</code> relacionado ao objeto <code>ConvenioTurnoVO</code>.
     * Faz uso da chave primária da classe <code>TurnoVO</code> para realizar a consulta.
     * @param obj  Objeto no qual será montado os dados consultados.
     */
    public static void montarDadosTurno(ConvenioTurnoVO obj, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        if (obj.getTurno().getCodigo().intValue() == 0) {
    ...
    public void excluirConvenioTurnos(Integer convenio) throws Exception {
        String sql = "DELETE FROM ConvenioTurno WHERE (convenio = ?)";
    ...
    public void alterarConvenioTurnos(Integer convenio, List objetos) throws Exception {
        excluirConvenioTurnos(convenio);
        incluirConvenioTurnos(convenio, objetos);
    }
    ...
    public void incluirConvenioTurnos(Integer convenioPrm, List objetos) throws Exception {
        ...
        while (e.hasNext()) {
            ConvenioTurnoVO obj = (ConvenioTurnoVO) e.next();
    ...
    /**
     * Operação responsável por consultar todos os <code>ConvenioTurnoVO</code> relacionados a um objeto da classe <code>financeiro.Convenio</code>.
     * @param convenio  Atributo de <code>financeiro.Convenio</code> a ser utilizado para localizar os objetos da classe <code>ConvenioTurnoVO</code>.
     * @return List  Contendo todos os objetos da classe <code>ConvenioTurnoVO</code> resultantes da consulta.
     * @exception Exception  Erro de conexão com o BD ou restrição de acesso a esta operação.
     */
    public static List consultarConvenioTurnos(Integer convenio, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        ConvenioTurno.consultar(getIdEntidade());
        List objetos = new ArrayList(0);
        String sql = "SELECT * FROM ConvenioTurno WHERE convenio = ?";
        SqlRowSet resultado = getConexao().getJdbcTemplate().queryForRowSet(sql, new Object[]{convenio});
        while (resultado.next()) {
            ConvenioTurnoVO novoObj = new ConvenioTurnoVO();
            novoObj = ConvenioTurno.montarDados(resultado, nivelMontarDados,usuario);
            objetos.add(novoObj);
        }
        return objetos;
    }
    ...
    public ConvenioTurnoVO consultarPorChavePrimaria(Integer convenioPrm, Integer turnoPrm, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        ...
        String sql = "SELECT * FROM ConvenioTurno WHERE convenio = ? and turno = ?";
        SqlRowSet tabelaResultado = getConexao().getJdbcTemplate().queryForRowSet(sql, new Object[]{convenioPrm, turnoPrm});
        if (!tabelaResultado.next()) {
            throw new ConsistirException("Dados Não Encontrados ( ConvenioTurno ).");
        }
    ...
    public static String getIdEntidade() {
        return ConvenioTurno.idEntidade;
    }
    ...
    public void setIdEntidade(String idEntidade) {
        ConvenioTurno.idEntidade = idEntidade;
    }
}
