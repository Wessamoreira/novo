...
import negocio.interfaces.financeiro.CondicaoRenegociacaoFuncionarioInterfaceFacade;
...
@Scope("singleton")
@Lazy
public class CondicaoRenegociacaoFuncionario extends ControleAcesso implements CondicaoRenegociacaoFuncionarioInterfaceFacade {

	private static final long serialVersionUID = 1L;
	protected static String idEntidade;

	public CondicaoRenegociacaoFuncionario() throws Exception {
		super();
		setIdEntidade("CondicaoRenegociacaoFuncionario");
	}
	...
	public void incluir(final CondicaoRenegociacaoFuncionarioVO obj) throws Exception {
		validarDados(obj);

		final String sql = "INSERT INTO CondicaoRenegociacaoFuncionario( condicaoRenegociacao, funcionario) VALUES ( ?, ?) returning codigo";
		obj.setCodigo((Integer) getConexao().getJdbcTemplate().query(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection arg0) throws SQLException {
				...
				if (obj.getFuncionarioVO().getCodigo().intValue() != 0) {
					sqlInserir.setInt(2, obj.getFuncionarioVO().getCodigo().intValue());
				} else {
	 ...
	 */
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void alterar(final CondicaoRenegociacaoFuncionarioVO obj) throws Exception {
		validarDados(obj);

		final String sql = "UPDATE CondicaoRenegociacaoFuncionario set condicaoRenegociacao=?, funcionario=? WHERE ((codigo = ?))";
		getConexao().getJdbcTemplate().update(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection arg0) throws SQLException {
				...
				if (obj.getFuncionarioVO().getCodigo().intValue() != 0) {
					sqlAlterar.setInt(2, obj.getFuncionarioVO().getCodigo().intValue());
				} else {
	 ...
	 */
	public void excluir(CondicaoRenegociacaoFuncionarioVO obj) throws Exception {
		CondicaoRenegociacaoFuncionario.excluir(getIdEntidade());
		String sql = "DELETE FROM CondicaoRenegociacaoFuncionario WHERE ((codigo = ?))";
	 ...
	 * da pagina.
	 */
	public List<CondicaoRenegociacaoFuncionarioVO> consultar(String valorConsulta, String campoConsulta, boolean controlarAcesso, UsuarioVO usuario) throws Exception {
	...
	public List consultarPorCodigo(Integer valorConsulta, boolean controlarAcesso, UsuarioVO usuario, int nivelMontarDados) throws Exception {
		CondicaoRenegociacaoFuncionario.consultar(getIdEntidade(), controlarAcesso, usuario);
		String sqlStr = "SELECT * FROM CondicaoRenegociacaoFuncionario WHERE codigo >= ?  ORDER BY codigo";
	 ...
	 */
	public static CondicaoRenegociacaoFuncionarioVO montarDados(SqlRowSet dadosSQL, int nivelMontarDados, UsuarioVO usuarioVO) throws Exception {
		CondicaoRenegociacaoFuncionarioVO obj = new CondicaoRenegociacaoFuncionarioVO();
		...
		montarDadosFuncionario(obj, nivelMontarDados, usuarioVO);
		return obj;
	}
	
	public static void montarDadosFuncionario(CondicaoRenegociacaoFuncionarioVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
        if (obj.getFuncionarioVO().getCodigo().intValue() == 0) {
            obj.setFuncionarioVO(new FuncionarioVO());
            return;
        }
        obj.setFuncionarioVO(getFacadeFactory().getFuncionarioFacade().consultaRapidaPorChavePrimaria(obj.getFuncionarioVO().getCodigo(), false, usuario));
    }
	 ...
	 */
	@Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public void excluirCondicaoRenegociacaoFuncionarioVOs(Integer condicaoRenegociacao, List<CondicaoRenegociacaoFuncionarioVO> objetos) throws Exception {
		if (objetos != null && !objetos.isEmpty()) {
			String sql = "DELETE FROM CondicaoRenegociacaoFuncionario WHERE (condicaoRenegociacao = ?)";
			sql += " and codigo not in(";
			boolean primeiro = true;
			for (CondicaoRenegociacaoFuncionarioVO obj : objetos) {
				if (primeiro) {
					sql += obj.getCodigo();
					primeiro = false;
				} else {
	 ...
	 */
	@Override
	public void alterarCondicaoRenegociacaoFuncionarioVOs(Integer condicaoRenegociacao, List<CondicaoRenegociacaoFuncionarioVO> objetos) throws Exception {
		excluirCondicaoRenegociacaoFuncionarioVOs(condicaoRenegociacao, objetos);
		for (CondicaoRenegociacaoFuncionarioVO obj : objetos) {
	 ...
	 */
	@Override
	public void incluirCondicaoRenegociacaoFuncionarioVOs(Integer condicaoRenegociacaoPrm, List<CondicaoRenegociacaoFuncionarioVO> objetos) throws Exception {
		for (CondicaoRenegociacaoFuncionarioVO obj : objetos) {
	...
	@Override
	public List consultarCondicaoRenegociacaoFuncionarioPorCondicaoRenegociacao(Integer condicaoRenegociacao, int nivelMontarDados, UsuarioVO usuarioVO) throws Exception {
		CondicaoRenegociacaoFuncionario.consultar(getIdEntidade());
		List objetos = new ArrayList();
		String sql = "SELECT * FROM CondicaoRenegociacaoFuncionario WHERE condicaoRenegociacao = " + condicaoRenegociacao;
		SqlRowSet resultado = getConexao().getJdbcTemplate().queryForRowSet(sql);
		while (resultado.next()) {
			CondicaoRenegociacaoFuncionarioVO novoObj = new CondicaoRenegociacaoFuncionarioVO();
	 ...
	 *                procurado.
	 */
	public CondicaoRenegociacaoFuncionarioVO consultarPorChavePrimaria(Integer codigoPrm, int nivelMontarDados, UsuarioVO usuario) throws Exception {
		CondicaoRenegociacaoFuncionario.consultar(getIdEntidade(), false, usuario);
		String sql = "SELECT * FROM CondicaoRenegociacaoFuncionario WHERE codigo = " + codigoPrm;
		SqlRowSet tabelaResultado = getConexao().getJdbcTemplate().queryForRowSet(sql);
		if (!tabelaResultado.next()) {
			throw new ConsistirException("Dados Não Encontrados ( CondicaoRenegociacaoFuncionario ).");
		}
	...
	public static String getIdEntidade() {
		return CondicaoRenegociacaoFuncionario.idEntidade;
	}
	...
	public void setIdEntidade(String idEntidade) {
		CondicaoRenegociacaoFuncionario.idEntidade = idEntidade;
	}
}