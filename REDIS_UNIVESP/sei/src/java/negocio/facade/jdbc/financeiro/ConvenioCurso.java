...
import negocio.interfaces.financeiro.ConvenioCursoInterfaceFacade;

/**
 * Classe de persistência que encapsula todas as operações de manipulação dos dados da classe <code>ConvenioCursoVO</code>.
 * Responsável por implementar operações como incluir, alterar, excluir e consultar pertinentes a classe <code>ConvenioCursoVO</code>.
...
@Scope("singleton")
@Lazy 
public class ConvenioCurso extends ControleAcesso implements ConvenioCursoInterfaceFacade{

    protected static String idEntidade;

    public ConvenioCurso() throws Exception {
        super();
    ...
    public ConvenioCursoVO novo() throws Exception {
        ConvenioCurso.incluir(getIdEntidade());
        ConvenioCursoVO obj = new ConvenioCursoVO();
        return obj;
    }
    ...
    public void incluir(final ConvenioCursoVO obj) throws Exception {
        ConvenioCursoVO.validarDados(obj);
        final String sql = "INSERT INTO ConvenioCurso( convenio, curso ) VALUES ( ?, ? )";
        getConexao().getJdbcTemplate().update(sql, new Object[]{obj.getConvenio(), obj.getCurso().getCodigo()});
     ...
     */
    @Transactional(readOnly = false, isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
    public void alterar(final ConvenioCursoVO obj) throws Exception {
        ConvenioCursoVO.validarDados(obj);
        ConvenioCurso.alterar(getIdEntidade());
        final String sql = "UPDATE ConvenioCurso set  WHERE ((convenio = ?) and (curso = ?))";
    ...
    public void excluir(ConvenioCursoVO obj) throws Exception {
        ConvenioCurso.excluir(getIdEntidade());
        String sql = "DELETE FROM ConvenioCurso WHERE ((convenio = ?) and (curso = ?))";
        getConexao().getJdbcTemplate().update(sql, new Object[]{obj.getConvenio(), obj.getCurso().getCodigo()});
    }
    ...
    public List consultarPorNomeCurso(String valorConsulta, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        ...
        String sqlStr = "SELECT ConvenioCurso.* FROM ConvenioCurso, Curso WHERE ConvenioCurso.curso = Curso.codigo and upper( Curso.nome ) like('" + valorConsulta.toUpperCase() + "%') ORDER BY Curso.nome";
    ...
    public List consultarPorDescricaoConvenio(String valorConsulta, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        ControleAcesso.consultar(getIdEntidade(), true,usuario);
        String sqlStr = "SELECT ConvenioCurso.* FROM ConvenioCurso, Convenio WHERE ConvenioCurso.convenio = Convenio.codigo and upper( Convenio.descricao ) like('" + valorConsulta.toUpperCase() + "%') ORDER BY Convenio.descricao";
        SqlRowSet tabelaResultado = getConexao().getJdbcTemplate().queryForRowSet(sqlStr);
        return montarDadosConsulta(tabelaResultado, nivelMontarDados,usuario);
    }
     ...
     * em um objeto da classe <code>ConvenioCursoVO</code>.
     * @return  O objeto da classe <code>ConvenioCursoVO</code> com os dados devidamente montados.
     */
    public static ConvenioCursoVO montarDados(SqlRowSet dadosSQL, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        ConvenioCursoVO obj = new ConvenioCursoVO();
    ...
    /**
     * Operação responsável por montar os dados de um objeto da classe <code>CursoVO</code> relacionado ao objeto <code>ConvenioCursoVO</code>.
     * Faz uso da chave primária da classe <code>CursoVO</code> para realizar a consulta.
     * @param obj  Objeto no qual será montado os dados consultados.
     */
    public static void montarDadosCurso(ConvenioCursoVO obj, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        if (obj.getCurso().getCodigo().intValue() == 0) {
    ...
    public void excluirConvenioCursos(Integer convenio) throws Exception {
        String sql = "DELETE FROM ConvenioCurso WHERE (convenio = ?)";
    ...
    public void alterarConvenioCursos(Integer convenio, List objetos) throws Exception {
        excluirConvenioCursos(convenio);
        incluirConvenioCursos(convenio, objetos);
    }
    ...
    public void incluirConvenioCursos(Integer convenioPrm, List objetos) throws Exception {
        ...
        while (e.hasNext()) {
            ConvenioCursoVO obj = (ConvenioCursoVO) e.next();
    ...
    /**
     * Operação responsável por consultar todos os <code>ConvenioCursoVO</code> relacionados a um objeto da classe <code>financeiro.Convenio</code>.
     * @param convenio  Atributo de <code>financeiro.Convenio</code> a ser utilizado para localizar os objetos da classe <code>ConvenioCursoVO</code>.
     * @return List  Contendo todos os objetos da classe <code>ConvenioCursoVO</code> resultantes da consulta.
     * @exception Exception  Erro de conexão com o BD ou restrição de acesso a esta operação.
     */
    public static List consultarConvenioCursos(Integer convenio, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        ConvenioCurso.consultar(getIdEntidade());
        List objetos = new ArrayList(0);
        String sql = "SELECT * FROM ConvenioCurso WHERE convenio = ?";
        SqlRowSet resultado = getConexao().getJdbcTemplate().queryForRowSet(sql, new Object[]{convenio});
        while (resultado.next()) {
            ConvenioCursoVO novoObj = new ConvenioCursoVO();
            novoObj = ConvenioCurso.montarDados(resultado, nivelMontarDados,usuario);
            objetos.add(novoObj);
        }
        return objetos;
    }
    ...
    public ConvenioCursoVO consultarPorChavePrimaria(Integer convenioPrm, Integer cursoPrm, int nivelMontarDados,UsuarioVO usuario) throws Exception {
        ...
        String sql = "SELECT * FROM ConvenioCurso WHERE convenio = ? and curso = ?";
        SqlRowSet tabelaResultado = getConexao().getJdbcTemplate().queryForRowSet(sql, new Object[]{convenioPrm, cursoPrm});
        if (!tabelaResultado.next()) {
            throw new ConsistirException("Dados Não Encontrados ( ConvenioCurso ).");
        }
    ...
    public static String getIdEntidade() {
        return ConvenioCurso.idEntidade;
    }
    ...
    public void setIdEntidade(String idEntidade) {
        ConvenioCurso.idEntidade = idEntidade;
    }
}
