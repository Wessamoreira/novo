...
//        String sqlStr = "SELECT * FROM Endereco WHERE upper( cep ) like('" + valorConsulta.toUpperCase() + "%') ORDER BY cep";
     ...
     */
    public static EnderecoVO montarDados(SqlRowSet dadosSQL, int nivelMontarDados, UsuarioVO usuario) throws Exception {
        EnderecoVO obj = new EnderecoVO();
     ...
     */
    public static void montarDadosBairrocodigo(EnderecoVO obj, int nivelMontarDados, UsuarioVO usuario) throws Exception {
        if (obj.getBairrocodigo().getCodigo().intValue() == 0) {
