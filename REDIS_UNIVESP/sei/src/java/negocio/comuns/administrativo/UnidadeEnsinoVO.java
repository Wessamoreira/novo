...
public class UnidadeEnsinoVO extends EmpresaVO implements PossuiEndereco {
	...
	// Atributo usado no momento que esta fazendo uma cotação para universidade
	// não e persistido no banco
	...
	@ExcluirJsonAnnotation @JsonIgnore
	@Expose(deserialize = false, serialize = false)
	private CentroResultadoVO centroResultadoRequerimentoVO;
	@ExcluirJsonAnnotation @JsonIgnore
	@Expose(deserialize = false, serialize = false)
	...
	/**
	 * Construtor padrão da classe <code>UnidadeEnsino</code>. Cria uma nova
	 * instância desta entidade, inicializando automaticamente seus atributos
	 * (Classe VO).
	 */

	// CAMPO DESTINADO PARA SELECIONAR AS INSTITUIÇÕES ONDE SE DESEJA ADICIONAR
	// O CURSO EM QUESTÃO ( TELA DE CURSO CURSO.FORM)
	@ExcluirJsonAnnotation @JsonIgnore
	@Expose(deserialize = false, serialize = false)
	 ...
	 * <code>UnidadeEnsinoVO</code>. Todos os tipos de consistência de dados são
	...
	public static void validarDados(UnidadeEnsinoVO obj) throws ConsistirException {
		if (obj.getNome().equals("")) {
			throw new ConsistirException("O campo NOME (Dados Básicos) deve ser informado.");
		}
		if (obj.getAbreviatura().equals("")) {
			throw new ConsistirException("O campo ABREVIATURA(Dados Básicos) deve ser informado.");
		}
		if (obj.getTipoEmpresa().equals("JU")) {
			if (obj.getCNPJ().equals("")) {
				throw new ConsistirException("O campo CNPJ (Dados Básicos) deve ser informado.");
			}
		}
		if (obj.getCEP().equals("")) {
			throw new ConsistirException("O campo CEP (Dados Básicos) deve ser informado.");
		}
		if (obj.getEndereco().equals("")) {
			throw new ConsistirException("O campo ENDEREÇO (Dados Básicos) deve ser informado.");
		}
		if (obj.getSetor().equals("")) {
			throw new ConsistirException("O campo BAIRRO/SETOR (Dados Básicos) deve ser informado.");
		}
		if ((obj.getCidade() == null) || (obj.getCidade().getCodigo().intValue() == 0)) {
			throw new ConsistirException("O campo CIDADE (Dados Básicos) deve ser informado.");
		}

		if (obj.getTelComercial1().equals("")) {
			throw new ConsistirException("O campo TELEFONE (Dados Básicos) deve ser informado.");
		}
		if (obj.getConfiguracoes().getCodigo() == 0) {
			throw new ConsistirException("O campo Configuração (Dados Básicos) deve ser informado ");
		}

		if (obj.getTipoEmpresa().equals("FI")) {
			if (obj.getRG().equals("")) {
				throw new ConsistirException("O campo RG (Dados Básicos) deve ser informado.");
			}
			if (obj.getCPF().equals("")) {
				throw new ConsistirException("O campo CPF (Dados Básicos) deve ser informado.");
			}
		}
		...
		if(obj.getLocalizacaoZonaEscola().equals(LocalizacaoZonaEscolaEnum.URBANA.getValor()) && obj.getLocalizacaoDiferenciadaEscola().equals(LocalizacaoDiferenciadaEscolaEnum.AREA_ASSENTAMENTO.getValor())) {
			throw new ConsistirException("O campo Localização Diferenciada da Escola (Dados Censo) não pode ser Área de Assentamento quando o campo Localização Zona da escola for Urbana.");
		}
		if(obj.getUnidadeVinculadaEscolaEducacaoBasica().equals(UnidadeVinculadaEscolaEducacaoBasicaEnum.UNIDADE_VINCULADA_ESCOLA_EDUCACAO_BASICA.getValor())) {
			throw new ConsistirException("O campo Código Escola Sede (Dados Básicos) deve ser preenchido.");
		}
		if(obj.getUnidadeVinculadaEscolaEducacaoBasica().equals(UnidadeVinculadaEscolaEducacaoBasicaEnum.UNIDADE_OFERTANTE_EDUCACAO_SUPERIOR.getValor())) {
			throw new ConsistirException("O campo Código da IES Sede (Dados Básicos) deve ser preenchido.");
		}
		 ...
		 * ConsistirException(
		 * "Nenhuma configuração definida, selecione uma configuração ou marque a opção Configuração Padrão."
		 * ); }
		 */
	}
	...
	public void adicionarObjUnidadeEnsinoCursoVOs(UnidadeEnsinoCursoVO objEditar, UnidadeEnsinoCursoVO obj) throws Exception {
		...
		while (i.hasNext()) {
			...
			if ((objExistente.getCurso().getCodigo().equals(obj.getCurso().getCodigo())) && (objExistente.getTurno().getCodigo().equals(obj.getTurno().getCodigo())) && (objExistente.getUnidadeEnsino().equals(obj.getUnidadeEnsino()))) {
				...
				throw new Exception("Já existe um registro deste curso para o turno (" + objExistente.getTurno().getNome() + ").");
			}
			index++;
		}
		index = 0;
...
//	}
	...
	public void setCentroResultadoVO(CentroResultadoVO centroResultatoVO) {
		this.centroResultadoVO = centroResultatoVO;
	}
	...
	public CentroResultadoVO getCentroResultadoRequerimentoVO() {
		centroResultadoRequerimentoVO = Optional.ofNullable(centroResultadoRequerimentoVO).orElse(new CentroResultadoVO());
		return centroResultadoRequerimentoVO;
	}

	public void setCentroResultadoRequerimentoVO(CentroResultadoVO centroResultadoRequerimentoVO) {
		this.centroResultadoRequerimentoVO = centroResultadoRequerimentoVO;
	}
	...
	public CidadeVO getCidadeNFSe() {
		if (Uteis.isAtributoPreenchido(getConfiguracaoNotaFiscalVO().getCidade())) {
			cidade = getConfiguracaoNotaFiscalVO().getCidade();
		} else {
			cidade = getCidade();
		}
		return cidade;
