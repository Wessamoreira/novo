    ...
    public static void validarDados(UsuarioVO obj) throws ConsistirException {
        ...
        if (!obj.getTipoUsuario().equals("PA")) {
            ...
            if ((obj.getParceiro() == null) || (obj.getParceiro().getCodigo().intValue() == 0)) {
                throw new ConsistirException("O campo Parceiro Referente ao Usuário deve ser informado.");
            }
        }
...
//            if ((objExistente.getUnidadeEnsinoVO().getCodigo().intValue() == obj.getUnidadeEnsinoVO().getCodigo().intValue())) {
    ...
    public String getNome() {
        if (!getPessoa().getNome().equals("")) {
            nome = getPessoa().getNome();
            return getPessoa().getNome();
        } else if (!getParceiro().getNome().equals("")) {
            nome = getParceiro().getNome();
