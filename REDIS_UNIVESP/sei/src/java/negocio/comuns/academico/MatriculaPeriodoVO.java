...
public class MatriculaPeriodoVO extends SuperVO {
   ...
   /** Atributos TRANSIENT utilizadas em controles especificos
     ...
     */
    private List<ConvenioVO> listaConvenioRemoverContaPagarRestituicaoAluno;
    /**
    ...
    private List<MatriculaPeriodoVencimentoVO> matriculaPeriodoVencimentoVOs;
    /*
     ...
     */
    private List<MatriculaPeriodoVencimentoVO> listaParcelasGeradasManualmenteAnteriormenteAptasParaRegeracao;
    private MatriculaPeriodoPreMatriculaVO matriculaPeriodoPreMatricula;
    ...
    private List<PlanoFinanceiroAlunoDescricaoDescontosVO> listaDescontosMatricula;
    private List<PlanoFinanceiroAlunoDescricaoDescontosVO> listaDescontosMensalidade;
    private List<PlanoFinanceiroAlunoDescricaoDescontosVO> listaDescontosMaterialDidatico;
	...
	private List<ContaReceberVO> contaReceberNotaFiscalEmitidaVOs;
	
	private ContaPagarVO contaPagarRestituicaoVO;
	private List<MatriculaPeriodoContaReceberLogVO> matriculaPeriodoContaReceberLogVOs;
	private List<MatriculaPeriodoContaReceberLogVO> matriculaPeriodoContaReceberAlterarLogVOs;
    ...
    public void adicionarObjMatriculaPeriodoVencimentoVO(MatriculaPeriodoVencimentoVO vcto) throws Exception {
        this.matriculaPeriodoVencimentoVOs.add(vcto);
    }
    ...
    public ContaReceberVO montarDadosContaReceber(Integer centroReceita, Integer contaCorrente, String tipoOrigem, MatriculaVO matriculaVO, String nrDocumento, String descricao, String parcela, Double valor, String tipoDesconto, Double desconto, Double percDesconto, Date dataVencimento, Date dataCompetencia, Integer descontoProgressivo, Integer convenio, Double descontoInstituicao, Double descontoConvenio,
            Integer codigoMatriculaPeriodo, ParceiroVO parceiroVO, int ordemConvenio, Boolean ordemConvenioValorCheio, int ordemDescontoAluno, Boolean ordemDescontoAlunoValorCheio, int ordemDescontoProgressivo, Boolean ordemDescontoProgressivoValorCheio, int ordemPlanoDesconto, Boolean ordemPlanoDescontoValorCheio, Double valorDescontoInclusaoExclusao, boolean abaterValorRateiroNoDescontoLancadoRecebimento, Double acrescimoExtra) throws Exception {
        ContaReceberVO contaReceberVO = new ContaReceberVO();
        contaReceberVO.verificarEReplicarLiberacaoBloqueioEntidadePrincipal(this);
        contaReceberVO.getCentroReceita().setCodigo(centroReceita);
        contaReceberVO.setData(new Date());
        contaReceberVO.setMatriculaAluno(matriculaVO);
        contaReceberVO.setSituacao("AR");
        contaReceberVO.setMatriculaPeriodo(codigoMatriculaPeriodo);
        if (tipoOrigem.equals("BCC")) {
            contaReceberVO.setTipoPessoa("PA");
        } else {
        ...
        if (!tipoOrigem.equals("BCC")) {
            contaReceberVO.setPessoa(matriculaVO.getAluno());
            contaReceberVO.setBeneficiario(new PessoaVO());
        } else {
            contaReceberVO.setPessoa(new PessoaVO());
            contaReceberVO.setParceiroVO(parceiroVO);
            contaReceberVO.setBeneficiario(matriculaVO.getAluno());
        }
        contaReceberVO.getUnidadeEnsino().setCodigo(matriculaVO.getUnidadeEnsino().getCodigo());
        contaReceberVO.getUnidadeEnsinoFinanceira().setCodigo(matriculaVO.getUnidadeEnsino().getCodigo());
        contaReceberVO.setDescricaoPagamento(descricao);
        if (!valorDescontoInclusaoExclusao.equals(0.0)) {
        	if(abaterValorRateiroNoDescontoLancadoRecebimento){
        		contaReceberVO.setValorDescontoRateio(valorDescontoInclusaoExclusao);        
        	}else{
        		if(valorDescontoInclusaoExclusao >= valor){
        			contaReceberVO.setValorDescontoRateio(valorDescontoInclusaoExclusao);       
        		}else{        
        			contaReceberVO.setValorDescontoRateio(valorDescontoInclusaoExclusao);
        		}
        ...
        contaReceberVO.setValorBaseContaReceber(valor);
        
        if (tipoOrigem.equals(TipoOrigemContaReceber.MATRICULA.getValor())) {
            contaReceberVO.setTipoBoleto(TipoBoletoBancario.MATRICULA.getValor());
        } else if (tipoOrigem.equals(TipoOrigemContaReceber.MENSALIDADE.getValor())) {
            contaReceberVO.setTipoBoleto(TipoBoletoBancario.MENSALIDADE.getValor());
        } else if (tipoOrigem.equals(TipoOrigemContaReceber.MATERIAL_DIDATICO.getValor())) {
            contaReceberVO.setTipoBoleto(TipoBoletoBancario.MATERIAL_DIDATICO.getValor());
        } else if (tipoOrigem.equals(TipoOrigemContaReceber.INSCRICAO_PROCESSO_SELETIVO.getValor())) {
            contaReceberVO.setTipoBoleto(TipoBoletoBancario.PROCESSO_SELETIVO.getValor());
        } else if (tipoOrigem.equals(TipoOrigemContaReceber.NEGOCIACAO.getValor())) {
            contaReceberVO.setTipoBoleto(TipoBoletoBancario.RENEGOCIACAO.getValor());
        } else if (tipoOrigem.equals(TipoOrigemContaReceber.REQUERIMENTO.getValor())) {
            contaReceberVO.setTipoBoleto(TipoBoletoBancario.REQUERIMENTO.getValor());
        } else {
            contaReceberVO.setTipoBoleto(TipoBoletoBancario.OUTROS.getValor());
        }
        contaReceberVO.setTipoDesconto(tipoDesconto);
        if (tipoDesconto.equals("PO")) {
            contaReceberVO.setValorDesconto(percDesconto);
        } else {
            contaReceberVO.setValorDesconto(desconto);
        }
        contaReceberVO.setAcrescimo(acrescimoExtra);
        contaReceberVO.setValorDescontoInclusaoExclusao(valorDescontoInclusaoExclusao);
        contaReceberVO.setDataVencimento(dataVencimento);
        contaReceberVO.setDataCompetencia(dataCompetencia);
        contaReceberVO.setNrDocumento(nrDocumento);
        contaReceberVO.getDescontoProgressivo().setCodigo(descontoProgressivo);
        contaReceberVO.getConvenio().setCodigo(convenio);
        contaReceberVO.setValorDescontoInstituicao(descontoInstituicao);
        contaReceberVO.setValorDescontoConvenio(descontoConvenio);
        contaReceberVO.setOrdemConvenio(ordemConvenio);
        contaReceberVO.setOrdemConvenioValorCheio(ordemConvenioValorCheio);
        contaReceberVO.setOrdemDescontoAluno(ordemDescontoAluno);
        contaReceberVO.setOrdemDescontoAlunoValorCheio(ordemDescontoAlunoValorCheio);
        contaReceberVO.setOrdemDescontoProgressivo(ordemDescontoProgressivo);
        contaReceberVO.setOrdemDescontoProgressivoValorCheio(ordemDescontoProgressivoValorCheio);
        contaReceberVO.setOrdemPlanoDesconto(ordemPlanoDesconto);
        contaReceberVO.setOrdemPlanoDescontoValorCheio(ordemPlanoDescontoValorCheio);


        // buscar na config do sistema...
    ...
    //TODOCOPIAR
    public ContaReceberVO criarContaReceberMatriculaBaseadoMatriculaPeriodoVencimento(MatriculaVO matriculaVO, MatriculaPeriodoVO matriculaPeriodoVO, MatriculaPeriodoVencimentoVO parcelaMatricula, Boolean contaLiberadaPagamento,
            ConfiguracaoFinanceiroVO configFinan, Boolean simularContaReceberParaConvenio, UsuarioVO usuario) throws Exception {
        String nrDoc = (String.valueOf(matriculaVO.getUnidadeEnsino().getCodigo()) + "." + String.valueOf(matriculaVO.getInscricao().getCodigo()) + "." + getCodigo());
        setNrDocumento(nrDoc);
        String descricao = ("Taxa de Matrícula para o curso " + matriculaVO.getCurso().getNome().toUpperCase() + ", período " + matriculaVO.getTurno().getNome().toUpperCase());
        if (!descricao.equals(parcelaMatricula.getDescricaoPagamento())) {
        ...
        if(Uteis.isAtributoPreenchido(matriculaPeriodoVO.getDataVencimentoMatriculaEspecifico())) {
        	dataVencimento = matriculaPeriodoVO.getDataVencimentoMatriculaEspecifico();
            ...
            if (matriculaPeriodoVO.getProcessoMatriculaCalendarioVO().getQtdeDiasAvancarDataVencimentoMatricula() > 0) {
                dataVencimento = (Uteis.obterDataAvancada(matriculaPeriodoVO.getData(), matriculaPeriodoVO.getProcessoMatriculaCalendarioVO().getQtdeDiasAvancarDataVencimentoMatricula()));
            } else {
                dataVencimento = matriculaPeriodoVO.getData();
            }
        } else {
            dataVencimento = processoMatriculaCalendarioVO.getDataVencimentoMatricula();
        }
        ...
        Double percDesconto = matriculaVO.getPlanoFinanceiroAluno().getPercDescontoMatricula();
        Double valorDesconto = matriculaVO.getPlanoFinanceiroAluno().getValorDescontoMatricula();
        Integer descontoProgressivo = 0;
        getValorMatriculaCheio();
        if (contaLiberadaPagamento) {
            valorDesconto = valorDesconto + parcelaMatricula.getCalcularValorFinal();
            this.adicionarHistoricoOperacao("  Pgto matrícula liberado. Valor desconto concedido para liberação: " + valorDesconto, this.getDataProcessamentoAtual());
        } else {
            descontoProgressivo = matriculaPeriodoVO.getCondicaoPagamentoPlanoFinanceiroCurso().getDescontoProgressivoPadraoMatricula().getCodigo();
        }

        Integer contaCorrente = 0;
        if (!getTurma().getContaCorrente().getCodigo().equals(0)) {
            contaCorrente = getTurma().getContaCorrente().getCodigo();
        } else if (!matriculaVO.getUnidadeEnsino().getContaCorrentePadraoMatricula().equals(0)) {
            contaCorrente = matriculaVO.getUnidadeEnsino().getContaCorrentePadraoMatricula();
        } else {
            contaCorrente = configuracaoFinanceiro.getContaCorrentePadraoMatricula();
        }
        Integer centroReceita = matriculaVO.getPlanoFinanceiroAluno().getCondicaoPagamentoPlanoFinanceiroCursoVO().getCentroReceita().getCodigo() > 0 ? matriculaVO.getPlanoFinanceiroAluno().getCondicaoPagamentoPlanoFinanceiroCursoVO().getCentroReceita().getCodigo() : configuracaoFinanceiro.getCentroReceitaMatriculaPadrao().getCodigo();
        ContaReceberVO contaReceberGerada = montarDadosContaReceber(centroReceita, contaCorrente, "MAT", matriculaVO, nrDoc, descricao, "Matrícula", valor, tipoDesconto, valorDesconto, percDesconto, dataVencimento, parcelaMatricula.getDataCompetencia(), descontoProgressivo, 0, 0.0, 0.0, matriculaPeriodoVO.getCodigo(), null, matriculaVO.getPlanoFinanceiroAluno().getOrdemConvenio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemConvenioValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoAluno(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoAlunoValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoProgressivo(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoProgressivoValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemPlanoDesconto(), matriculaVO.getPlanoFinanceiroAluno().getOrdemPlanoDescontoValorCheio(), parcelaMatricula.getValorDesconto(), matriculaPeriodoVO.getCondicaoPagamentoPlanoFinanceiroCurso().getAbaterValorRateiroComoDescontoRateio(), parcelaMatricula.getAcrescimoExtra());
//        contaReceberGerada.setValorBaseContaReceber(parcelaMatricula.getContaReceber().getValorBaseContaReceber());
        contaCorrente = null;
        descontoProgressivo = null;
        Date dataBaseSimularBaixaParcela = getData();
        if (simularContaReceberParaConvenio) {
            // Neste caso vamos simular a conta a receber com todos os descontos iniciais
            // ao qual o aluno tem direito. Ou seja, nao podemos gerar um convenio com base
            // em um valor onde descontos (como o progressivo) foram perdidos, pois isto poderia
            // implicar em gerar um valor de convênio para um parcela (que já estava vencida) e 
            // outro valor de convenio (de uma parcela futura) para uma parcela que ainda nao venceu.
            // Assim, vamos simular uma data de baixa antiga - que garanta que a simulaçao da conta
            // a receber com todos os descontos originais - não vencidos. Definimos a data do passad
            dataBaseSimularBaixaParcela = Uteis.obterDataPassada(contaReceberGerada.getDataVencimento(), PlanoFinanceiroAlunoDescricaoDescontosVO.NRDIASCOMECARACONTARVALIDADEPLANODESCONTO / 2);
        }
        contaReceberGerada.adicionarListaPlanoDescontoContaReceberVOComBaseItensPlanoDescontoMatricula(matriculaVO.getPlanoFinanceiroAluno(), matriculaPeriodoVO, parcelaMatricula);
        contaReceberGerada.atualizarSituacaoContaReceberDeAcordoComDescontos(configFinan, dataBaseSimularBaixaParcela, usuario);
        parcelaMatricula.setValorDescontoCalculadoPrimeiraFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoPrimeiraFaixaDescontos()-contaReceberGerada.getValorDescontoRateio());
        parcelaMatricula.setValorDescontoCalculadoSegundaFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoSegundaFaixaDescontos()-contaReceberGerada.getValorDescontoRateio());
        parcelaMatricula.setValorDescontoCalculadoTerceiraFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoTerceiraFaixaDescontos()-contaReceberGerada.getValorDescontoRateio());
        parcelaMatricula.setValorDescontoCalculadoQuartaFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoQuartaFaixaDescontos()-contaReceberGerada.getValorDescontoRateio());
        contaReceberGerada.setValorDescontoCalculadoPrimeiraFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoPrimeiraFaixaDescontos());
        contaReceberGerada.setValorDescontoCalculadoSegundaFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoSegundaFaixaDescontos());
        contaReceberGerada.setValorDescontoCalculadoTerceiraFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoTerceiraFaixaDescontos());
        contaReceberGerada.setValorDescontoCalculadoQuartaFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoQuartaFaixaDescontos());
        if (contaLiberadaPagamento) {
            contaReceberGerada.setSituacao("RE");
        }
        if(matriculaPeriodoVO.getBloqueioPorFechamentoMesLiberado()) {
        	contaReceberGerada.liberarVerificacaoBloqueioFechamentoMes();
		}
        return contaReceberGerada;
    }

    public ContaReceberVO criarContaReceberMensalidadeBaseadoMatriculaPeriodoVencimento(MatriculaVO matriculaVO, MatriculaPeriodoVO matriculaPeriodoVO, MatriculaPeriodoVencimentoVO parcelaMatricula, ConfiguracaoFinanceiroVO conf, Boolean simularContaReceberParaConvenio, UsuarioVO usuario) throws Exception {
    	String descricao = null;
    	String nrDoc = (String.valueOf(matriculaVO.getUnidadeEnsino().getCodigo()) + "." + String.valueOf(matriculaVO.getInscricao().getCodigo()) + "." + getCodigo());
        setNrDocumento(nrDoc);
        if(parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().isMaterialDidatico()){
        	descricao = ("Material Didático " + parcelaMatricula.getParcela() + " para o curso " + matriculaVO.getCurso().getNome().toUpperCase() + ", período " + matriculaVO.getTurno().getNome().toUpperCase());	
        }else if(parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().isMensalidade()){
        	descricao = ("Parcela " + parcelaMatricula.getParcela() + " para o curso " + matriculaVO.getCurso().getNome().toUpperCase() + ", período " + matriculaVO.getTurno().getNome().toUpperCase());	
        }
        if (!descricao.equals(parcelaMatricula.getDescricaoPagamento())) {
            descricao = descricao + '\n' + parcelaMatricula.getDescricaoPagamento();
        }
        ...
        Double percDesconto = parcelaMatricula.getParcela().contains("EX") ? 0.0 : matriculaVO.getPlanoFinanceiroAluno().getPercDescontoParcela();
        Double valorDesconto = parcelaMatricula.getParcela().contains("EX") ? 0.0 : matriculaVO.getPlanoFinanceiroAluno().getValorDescontoParcela();
        if(parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().isMaterialDidatico() && !matriculaPeriodoVO.getCondicaoPagamentoPlanoFinanceiroCurso().isAplicarDescontoMaterialDidaticoDescontoAluno()){
        	percDesconto = 0.0;
			valorDesconto = 0.0;
        }
        Integer descontoProgressivo = 0;
        if (parcelaMatricula.getParcela().contains("EX")) {
			descontoProgressivo = 0;
		}else if(((parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().isMensalidade() && !parcelaMatricula.isUsaValorPrimeiraParcela()) || 
        		(parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().isMaterialDidatico() && matriculaPeriodoVO.getCondicaoPagamentoPlanoFinanceiroCurso().isAplicarDescontoMaterialDidaticoDescontoProgressivo()))
           ){
	        if (!matriculaVO.getPlanoFinanceiroAluno().getDescontoProgressivoPrimeiraParcela().getCodigo().equals(0) 
	        		&& parcelaMatricula.getParcela().matches("0?1/(.*)")) {
	            descontoProgressivo = matriculaVO.getPlanoFinanceiroAluno().getDescontoProgressivoPrimeiraParcela().getCodigo();
	        } else {
	            descontoProgressivo = matriculaVO.getPlanoFinanceiroAluno().getDescontoProgressivo().getCodigo();
	        }        
        }
		Integer contaCorrente = obterContaCorrentePadraoParaContaReceberParcela(matriculaVO, matriculaPeriodoVO, parcelaMatricula, conf);
        Integer centroReceita = obterCentroReceitaPadraoParaContaReceberParcela(matriculaVO, matriculaPeriodoVO, parcelaMatricula, conf);
        ContaReceberVO contaReceberGerada = montarDadosContaReceber(centroReceita, contaCorrente, parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().getValor(), matriculaVO, nrDoc, descricao, parcelaMatricula.getParcela(), valor, tipoDesconto, valorDesconto, percDesconto, dataVencimento, parcelaMatricula.getDataCompetencia(), descontoProgressivo, 0, 0.0, 0.0, matriculaPeriodoVO.getCodigo(),
                null, matriculaVO.getPlanoFinanceiroAluno().getOrdemConvenio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemConvenioValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoAluno(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoAlunoValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoProgressivo(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoProgressivoValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemPlanoDesconto(), matriculaVO.getPlanoFinanceiroAluno().getOrdemPlanoDescontoValorCheio(), 
                parcelaMatricula.getValorDesconto(), matriculaPeriodoVO.getCondicaoPagamentoPlanoFinanceiroCurso().getAbaterValorRateiroComoDescontoRateio(), parcelaMatricula.getAcrescimoExtra());
        contaReceberGerada.setValorBaseContaReceber(parcelaMatricula.getContaReceber().getValorBaseContaReceber());        
//        ContaReceberVO contaReceberGerada = montarDadosContaReceber(configuracaoFinanceiro.getCentroReceitaMatriculaPadrao().getCodigo(), contaCorrente, "MEN", matriculaVO, nrDoc, descricao, parcelaMatricula.getParcela(), valor, tipoDesconto, valorDesconto, percDesconto, dataVencimento, descontoProgressivo, 0, 0.0, 0.0, matriculaPeriodoVO.getCodigo(),
//	                null, matriculaVO.getPlanoFinanceiroAluno().getOrdemConvenio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemConvenioValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoAluno(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoAlunoValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoProgressivo(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoProgressivoValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemPlanoDesconto(), matriculaVO.getPlanoFinanceiroAluno().getOrdemPlanoDescontoValorCheio(), parcelaMatricula.getValorDesconto());
        contaCorrente = null;
        descontoProgressivo = null;
        ...
        if (simularContaReceberParaConvenio) {
            ...
            // outro valor de convenio (de uma parcela futura) para uma parcela que ainda nao venceu.
            // Assim, vamos simular uma data de baixa antiga - que garanta que a simulaçao da conta
            // a receber com todos os descontos originais - não vencidos.
            dataBaseSimularBaixaParcela = Uteis.obterDataPassada(contaReceberGerada.getDataVencimento(), PlanoFinanceiroAlunoDescricaoDescontosVO.NRDIASCOMECARACONTARVALIDADEPLANODESCONTO / 2);
        }
        if (!contaReceberGerada.getParcela().contains("EX") && !parcelaMatricula.isUsaValorPrimeiraParcela()) {
			contaReceberGerada.adicionarListaPlanoDescontoContaReceberVOComBaseItensPlanoDescontoMatricula(matriculaVO.getPlanoFinanceiroAluno(), matriculaPeriodoVO, parcelaMatricula);
		}
        contaReceberGerada.atualizarSituacaoContaReceberDeAcordoComDescontos(conf, dataBaseSimularBaixaParcela, usuario);
        validarDescontosDesconsiderandosVencimentoMaterialDidatico(contaReceberGerada, matriculaPeriodoVO, parcelaMatricula,conf,dataBaseSimularBaixaParcela);
        parcelaMatricula.setValorDescontoCalculadoPrimeiraFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoPrimeiraFaixaDescontos()-contaReceberGerada.getValorDescontoRateio());
        parcelaMatricula.setValorDescontoCalculadoSegundaFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoSegundaFaixaDescontos()-contaReceberGerada.getValorDescontoRateio());
        parcelaMatricula.setValorDescontoCalculadoTerceiraFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoTerceiraFaixaDescontos()-contaReceberGerada.getValorDescontoRateio());
        parcelaMatricula.setValorDescontoCalculadoQuartaFaixaDescontos(parcelaMatricula.getValorDescontoCalculadoQuartaFaixaDescontos()-contaReceberGerada.getValorDescontoRateio());
        ...
        if(parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().isMaterialDidatico() && Uteis.isAtributoPreenchido(matriculaPeriodoVO.getCondicaoPagamentoPlanoFinanceiroCurso().getUnidadeEnsinoFinanceira())){
        	contaReceberGerada.setUnidadeEnsinoFinanceira(matriculaPeriodoVO.getCondicaoPagamentoPlanoFinanceiroCurso().getUnidadeEnsinoFinanceira());
        }else{
        	contaReceberGerada.setUnidadeEnsinoFinanceira(contaReceberGerada.getUnidadeEnsino());
        }
        if(matriculaPeriodoVO.getBloqueioPorFechamentoMesLiberado()) {
        	contaReceberGerada.liberarVerificacaoBloqueioFechamentoMes();
		}
    ...
    private void validarDescontosDesconsiderandosVencimentoMaterialDidatico(ContaReceberVO contaReceberGerada, MatriculaPeriodoVO matriculaPeriodoVO, MatriculaPeriodoVencimentoVO parcelaMatricula, ConfiguracaoFinanceiroVO conf, Date dataBaseSimularBaixaParcela) throws Exception{
    	if(parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().isMaterialDidatico() && matriculaPeriodoVO.getCondicaoPagamentoPlanoFinanceiroCurso().isAplicarDescontosDesconsiderandosVencimento()){
    		Iterator<PlanoDescontoContaReceberVO> j = contaReceberGerada.getPlanoDescontoContaReceberVOs().iterator();
    		while (j.hasNext()) {
    			PlanoDescontoContaReceberVO planoDescontoContaReceberVO = (PlanoDescontoContaReceberVO) j.next();
    			if(planoDescontoContaReceberVO.getIsPlanoDescontoInstitucional()){
    				j.remove();
    			}
    		}
    		boolean removerPlanoFinanceiroAluno = false;
    		Iterator<PlanoFinanceiroAlunoDescricaoDescontosVO> i = contaReceberGerada.getListaDescontosAplicavesContaReceber().iterator();
    		while (i.hasNext()) {
				PlanoFinanceiroAlunoDescricaoDescontosVO objExistente = (PlanoFinanceiroAlunoDescricaoDescontosVO) i.next();
				if(removerPlanoFinanceiroAluno){
					i.remove();
				}else{
					objExistente.setValorDescontoAluno(objExistente.getValorDescontoAluno() + objExistente.getValorDescontoInstituicao() + objExistente.getValorDescontoProgressivo());
					objExistente.setValorDescontoInstituicao(0.0); 
					objExistente.setDiaNrAntesVencimento(0);
					objExistente.setValorDescontoProgressivo(0.0);
					objExistente.setPercentualDescontoProgressivo(0.0);
					objExistente.setReferentePlanoFinanceiroAposVcto(false);
					objExistente.setReferentePlanoFinanceiroAteVencimento(false);
					objExistente.setVencimentoDescontoProgressivoDiaUtil(false);
					objExistente.getListaDescontosPlanoDesconto().clear();
					objExistente.setTipoOrigemDesconto(PlanoFinanceiroAlunoDescricaoDescontosVO.TIPODESCONTOPADRAO);
					removerPlanoFinanceiroAluno = true;
				}
			}
    		contaReceberGerada.setValorDescontoAlunoJaCalculado(contaReceberGerada.getValorDescontoAlunoJaCalculado() +  contaReceberGerada.getValorDescontoInstituicao() + contaReceberGerada.getValorDescontoProgressivo());
    		contaReceberGerada.setTipoDesconto("VA");
    		contaReceberGerada.setValorDesconto(contaReceberGerada.getValorDescontoAlunoJaCalculado());
    		contaReceberGerada.setValorDescontoInstituicao(0.0);
    		contaReceberGerada.setValorDescontoProgressivo(0.0);
    		contaReceberGerada.setDescontoProgressivo(new DescontoProgressivoVO());
    	}
    	
    }
    
    public Integer obterContaCorrentePadraoParaContaReceberParcela(MatriculaVO matricula, MatriculaPeriodoVO matriculaPeriodoVO, MatriculaPeriodoVencimentoVO parcelaMatricula, ConfiguracaoFinanceiroVO conf) throws Exception{
    	Integer contaCorrente = 0;
    	if(parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().isMensalidade()){
    		if (!getTurma().getContaCorrente().getCodigo().equals(0)) {
    			contaCorrente = getTurma().getContaCorrente().getCodigo();
    		} else if (!matricula.getUnidadeEnsino().getContaCorrentePadraoMensalidade().equals(0)) {
    			contaCorrente = matricula.getUnidadeEnsino().getContaCorrentePadraoMensalidade();
    		} else {
    			contaCorrente = conf.getContaCorrentePadraoMensalidade();
    		}
    		if (contaCorrente == 0) {    		
        		throw new Exception("A Conta corrente não foi localizada na configuração padrão do sistema ou na unidade ensino, para a geração das parcelas");
        	}
    	}else if(parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().isMaterialDidatico()){
    		if (Uteis.isAtributoPreenchido(matriculaPeriodoVO.getCondicaoPagamentoPlanoFinanceiroCurso().getUnidadeEnsinoFinanceira().getContaCorrentePadraoMaterialDidatico())) {
    			contaCorrente = matriculaPeriodoVO.getCondicaoPagamentoPlanoFinanceiroCurso().getUnidadeEnsinoFinanceira().getContaCorrentePadraoMaterialDidatico();
    		} else if (!matricula.getUnidadeEnsino().getContaCorrentePadraoMaterialDidatico().equals(0)) {
    			contaCorrente = matricula.getUnidadeEnsino().getContaCorrentePadraoMaterialDidatico();
    		} else {
    			contaCorrente = conf.getContaCorrentePadraoMaterialDidatico();
    		}
    		if (contaCorrente == 0) {    		
        		throw new Exception("A Conta corrente não foi localizada na configuração padrão do sistema ou na unidade ensino, para a geração dos Material Didático ");
        	}
    	}
    ...
    public Integer obterCentroReceitaPadraoParaContaReceberParcela(MatriculaVO matricula, MatriculaPeriodoVO matriculaPeriodoVO, MatriculaPeriodoVencimentoVO parcelaMatricula, ConfiguracaoFinanceiroVO conf) throws Exception{
    	Integer centroReceita = 0;
    	if(parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().isMensalidade()){
    		centroReceita = matricula.getPlanoFinanceiroAluno().getCondicaoPagamentoPlanoFinanceiroCursoVO().getCentroReceita().getCodigo() > 0 ? matricula.getPlanoFinanceiroAluno().getCondicaoPagamentoPlanoFinanceiroCursoVO().getCentroReceita().getCodigo() : conf.getCentroReceitaMensalidadePadrao().getCodigo();
    		if (centroReceita == 0) {    		
        		throw new Exception("O Centro de Receita não foi localizada na configuração padrão do sistema ou na Condição de Pagamento do Plano financeiro, para a geração das parcelas");
        	}
    	}else if(parcelaMatricula.getTipoOrigemMatriculaPeriodoVencimento().isMaterialDidatico()){
    		centroReceita = matricula.getPlanoFinanceiroAluno().getCondicaoPagamentoPlanoFinanceiroCursoVO().getCentroReceita().getCodigo() > 0 ? matricula.getPlanoFinanceiroAluno().getCondicaoPagamentoPlanoFinanceiroCursoVO().getCentroReceita().getCodigo() : conf.getCentroReceitaMaterialDidaticoPadrao().getCodigo();
    		if (centroReceita == 0) {    		
        		throw new Exception("O Centro de Receita não foi localizada na configuração padrão do sistema ou na Condição de Pagamento do Plano financeiro, para a geração dos Material Didático");
        	}
    	}
     ...
     * @param contaLiberadaPagamento
     * @return
     * @throws Exception
     * @deprecated 26-09-2010
     */
    public ContaReceberVO criarContaReceberMatricula(MatriculaVO matriculaVO, MatriculaPeriodoVO matriculaPeriodoVO, ProcessoMatriculaCalendarioVO processoMatriculaCalendarioVO, ConfiguracaoFinanceiroVO configuracaoFinanceiro, Boolean contaLiberadaPagamento) throws Exception {
        String nrDoc = (String.valueOf(matriculaVO.getUnidadeEnsino().getCodigo()) + "." + String.valueOf(matriculaVO.getInscricao().getCodigo()) + "." + getCodigo());
        setNrDocumento(nrDoc);
        String descricao = ("Taxa de Matrícula para o curso " + matriculaVO.getCurso().getNome().toUpperCase() + ", período " + matriculaVO.getTurno().getNome().toUpperCase());
        Date dataVencimento = processoMatriculaCalendarioVO.getDataVencimentoMatricula();
        Date dataCompetencia = processoMatriculaCalendarioVO.getDataCompetenciaMatricula();
        Double desconto = 0.0;
        Integer descontoProgressivo = 0;
        Double descontoInstituicao = 0.0;
        Double descontoConvenio = 0.0;
        Double valor = getValorMatriculaCheio();
        if (contaLiberadaPagamento) {
            desconto = getValorMatriculaCheio();
        } else {
            desconto = getValorDescontoMatricula();
            descontoProgressivo = matriculaVO.getPlanoFinanceiroAluno().getDescontoProgressivo().getCodigo();
            descontoInstituicao = getValorDescontoInstituicaoMatricula();
            descontoConvenio = getValorDescontoConvenioMatricula();
        }
        Integer centroReceita = matriculaVO.getPlanoFinanceiroAluno().getCondicaoPagamentoPlanoFinanceiroCursoVO().getCentroReceita().getCodigo() > 0 ? matriculaVO.getPlanoFinanceiroAluno().getCondicaoPagamentoPlanoFinanceiroCursoVO().getCentroReceita().getCodigo() : configuracaoFinanceiro.getCentroReceitaMensalidadePadrao().getCodigo();
        ContaReceberVO contaReceberGerar = montarDadosContaReceber(centroReceita, configuracaoFinanceiro.getContaCorrentePadraoMatricula(), "MAT", matriculaVO, nrDoc, descricao, "1/1", valor, "VA", desconto, 0.0, dataVencimento, dataCompetencia, descontoProgressivo, 0, descontoInstituicao, descontoConvenio, matriculaPeriodoVO.getCodigo(), null, matriculaVO.getPlanoFinanceiroAluno().getOrdemConvenio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemConvenioValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoAluno(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoAlunoValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoProgressivo(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoProgressivoValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemPlanoDesconto(), matriculaVO.getPlanoFinanceiroAluno().getOrdemPlanoDescontoValorCheio(), 0.0, false, 0.0);
        if (contaLiberadaPagamento) {
            contaReceberGerar.setSituacao("RE");
        }
        return contaReceberGerar;
    }

//    public List criarContaReceberBolsaCusteadaMatricula(MatriculaVO matriculaVO, MatriculaPeriodoVO matriculaPeriodoVO, ProcessoMatriculaCalendarioVO processoMatriculaCalendarioVO, ConfiguracaoFinanceiroVO configuracaoFinanceiro, ContaReceberVO contaReceberAlunoBaseGeracaoConvenio) throws Exception {
...
//                        }
//                        listaMatriculaBolsaCusteada.add(montarMatriculaBolsaCusteada(planoFinCurso, configuracaoFinanceiro, matriculaVO,
...
//    }
    ...
    public ContaReceberVO montarMatriculaBolsaCusteada(PlanoFinanceiroCursoVO planoCursoVO, ConfiguracaoFinanceiroVO configuracaoFinanceiro, MatriculaVO matriculaVO, MatriculaPeriodoVO matriculaPeriodoVO, ProcessoMatriculaCalendarioVO processoMatriculaCalendarioVO, Double valor, Double desconto, Integer descontoProgressivo, Integer convenio, Double descontoFaculdade, Double descontoConvenio,
            ParceiroVO parceiroVO) throws Exception {
        ContaReceberVO obj = new ContaReceberVO();
        String nrDoc = (String.valueOf(matriculaVO.getUnidadeEnsino().getCodigo()) + "." + String.valueOf(matriculaVO.getInscricao().getCodigo()) + "." + getCodigo());
        String descricao = ("Bolsa Custeada referente a Matrícula para o curso " + matriculaVO.getCurso().getNome().toUpperCase() + ", período " + matriculaVO.getTurno().getNome().toUpperCase() + " referente ao aluno " + matriculaVO.getAluno().getNome());
        Date dataVencimento = processoMatriculaCalendarioVO.getDataVencimentoMatricula();
        if(Uteis.isAtributoPreenchido(matriculaPeriodoVO.getDataVencimentoMatriculaEspecifico())) {
        	dataVencimento = matriculaPeriodoVO.getDataVencimentoMatriculaEspecifico();
        }else if (matriculaPeriodoVO.getProcessoMatriculaCalendarioVO().getUsarDataVencimentoDataMatricula()) {
        ...
        if (processoMatriculaCalendarioVO.getConsiderarCompetenciaVctoMatricula()) {
            dataCompetencia = dataVencimento;
        } else {
            processoMatriculaCalendarioVO.getDataCompetenciaMatricula();
        }

        setNrDocumento(nrDoc);
        Integer contaCorrente = 0;
        if (!matriculaPeriodoVO.getTurma().getContaCorrente().getCodigo().equals(0)) {
            contaCorrente = matriculaPeriodoVO.getTurma().getContaCorrente().getCodigo();
        } else if (!matriculaVO.getUnidadeEnsino().getContaCorrentePadraoMatricula().equals(0)) {
            contaCorrente = matriculaVO.getUnidadeEnsino().getContaCorrentePadraoMatricula();
        } else {
            contaCorrente = configuracaoFinanceiro.getContaCorrentePadraoMatricula();
        ...
        if (contaCorrente == 0) {
            throw new Exception("Conta corrente não foi localizada na configuração padrão do sistema, para a geração das parcelas");
        }
        obj = montarDadosContaReceber(configuracaoFinanceiro.getCentroReceitaMatriculaPadrao().getCodigo(), contaCorrente, "BCC", matriculaVO, nrDoc, descricao, "Matrícula", valor, "VA", desconto, 0.0, dataVencimento, dataCompetencia, descontoProgressivo, convenio, descontoFaculdade, descontoConvenio, matriculaPeriodoVO.getCodigo(), parceiroVO, matriculaVO.getPlanoFinanceiroAluno().getOrdemConvenio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemConvenioValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoAluno(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoAlunoValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoProgressivo(), matriculaVO.getPlanoFinanceiroAluno().getOrdemDescontoProgressivoValorCheio(), matriculaVO.getPlanoFinanceiroAluno().getOrdemPlanoDesconto(), matriculaVO.getPlanoFinanceiroAluno().getOrdemPlanoDescontoValorCheio(), 0.0, false, 0.0);
        return obj;
    }

//	public List criarContaReceberBolsaCusteadaMensalidade(ConfiguracaoFinanceiroVO configFinan, MatriculaVO matriculaVO,
...
//	}
//    public ContaReceberVO montarMensalidade(PlanoFinanceiroCursoVO planoCursoVO, ConfiguracaoFinanceiroVO configuracaoFinanceiro, MatriculaVO matriculaVO, MatriculaPeriodoVO matriculaPeriodoVO, ProcessoMatriculaCalendarioVO processoMatriculaCalendarioVO, int nrMensalidade, Double valor, Double desconto, Integer descontoProgressivo, Integer convenio, Double descontoFaculdade,
...
//        return obj;
//    }
//    public ContaReceberVO montarMensalidadeBolsaCusteada(PlanoFinanceiroCursoVO planoCursoVO, ConfiguracaoFinanceiroVO configuracaoFinanceiro, MatriculaVO matriculaVO, MatriculaPeriodoVO matriculaPeriodoVO, ProcessoMatriculaCalendarioVO processoMatriculaCalendarioVO, String parcela, Double valor, Double desconto, Integer descontoProgressivo, Integer convenio, Double descontoFaculdade,
//            Double descontoConvenio, CondicaoPagamentoPlanoFinanceiroCursoVO condicao, ParceiroVO parceiroVO) throws Exception {
//        ContaReceberVO obj = new ContaReceberVO();
...
//    }
    ...
    /**
     * Responsï¿½vel para retornar o valor em R$ do total de desconto a ser
     * aplicado na Matricula
     *
     * @param planoFinanceiroAlunoVO
     * @return
     * @deprecated
     */
    public Double descontoTotalMatricula(PlanoFinanceiroCursoVO planoCurso, PlanoFinanceiroAlunoVO planoAluno, Double valorMatricula, List ordemDesconto, CondicaoPagamentoPlanoFinanceiroCursoVO condicao) {
        setValorDescontoMatricula(0.0);
        setValorDescontoInstituicaoMatricula(0.0);
        setValorDescontoConvenioMatricula(0.0);
        setValorMatriculaBolsaCusteada(0.0);
        setValorTotalTodosDescontosMatricula(0.0);
        setValorResidualMatricula(valorMatricula);
        for (int posicao = 0; posicao < ordemDesconto.size(); posicao++) {
            OrdemDescontoVO ordem = (OrdemDescontoVO) ordemDesconto.get(posicao);
            if (ordem.getDescricaoDesconto().equals("Desconto Aluno")) {
                calcularDescontoAlunoMatricula(planoCurso, planoAluno, valorMatricula, ordemDesconto, ordem, posicao);
            }
            if (ordem.getDescricaoDesconto().equals("Plano Desconto")) {
                calcularPlanoDescontoInstitucionalMatricula(planoAluno, valorMatricula, ordemDesconto, ordem, posicao);
            }
            if (ordem.getDescricaoDesconto().equals("Convênio")) {
                calcularDescontoConvenioMatricula(planoAluno, valorMatricula, ordemDesconto, ordem, posicao);
            }
        }
        // setValorDescontoMatricula(getValorDescontoInstituicaoMatricula() +
        // getValorDescontoConvenioMatricula());
        // if (getValorDescontoMatricula() > valorMatricula) {
        // setValorDescontoInstituicaoMatricula(getValorDescontoInstituicaoMatricula()
        // - (getValorDescontoMatricula() - valorMatricula));
        // setValorDescontoMatricula(valorMatricula);
        // }
        calcularValorMatricula(getValorMatriculaCheio(), getValorTotalTodosDescontosMatricula());
        return getValorDescontoMatricula();
    }
     ...
     * @param posicao
     */
    public void calcularDescontoAlunoMatricula(PlanoFinanceiroCursoVO planoCurso, PlanoFinanceiroAlunoVO planoAluno, Double valorMatricula, List ordemDesconto, OrdemDescontoVO ordem, Integer posicao) {
        if (ordem.getValorCheio().booleanValue()) {
            if (planoAluno.getTipoDescontoMatricula().equals("PO")) {
                setValorDescontoMatricula((valorMatricula * planoAluno.getPercDescontoMatricula()) / 100.0);

            } else {
                setValorDescontoMatricula(planoAluno.getValorDescontoMatricula());
            }
            setValorTotalTodosDescontosMatricula(getValorTotalTodosDescontosMatricula() + this.getValorDescontoMatricula());
            setValorResidualMatricula(valorMatricula - getValorTotalTodosDescontosMatricula());
            // planoAluno.setOrdemDescontoAluno(posicao + 3);
        } else {
            if (planoAluno.getTipoDescontoMatricula().equals("PO")) {
                setValorDescontoMatricula((getValorResidualMatricula() * planoAluno.getPercDescontoMatricula()) / 100.0);
            } else {
                setValorDescontoMatricula(planoAluno.getValorDescontoMatricula());
            }
            setValorTotalTodosDescontosMatricula(getValorTotalTodosDescontosMatricula() + this.getValorDescontoMatricula());
            setValorResidualMatricula(valorMatricula - getValorTotalTodosDescontosMatricula());
            // planoAluno.setOrdemDescontoAluno(posicao);
        }
    }

    public void processarDescontoInstitucionalMatriculaAluno(PlanoFinanceiroAlunoVO planoAluno, Double valorMatricula, List ordemDesconto, OrdemDescontoVO ordem, Integer posicao, ItemPlanoFinanceiroAlunoVO itemDescontoPlanoInstitucional) {
        Double valorBaseCalculo = 0.0;
        if (ordem.getValorCheio().booleanValue()) {
            valorBaseCalculo = valorMatricula;
        } else {
            valorBaseCalculo = getValorResidualMatricula();
        }
        Double valorTotalDescontoInstitucionalCalculado = 0.0;
        if (itemDescontoPlanoInstitucional.getPlanoDesconto().getSomente1PeriodoLetivoMatricula().equals(true)) {
            if (getPeridoLetivo().getPeriodoLetivo().intValue() == 1) {
                if (itemDescontoPlanoInstitucional.getPlanoDesconto().getTipoDescontoMatricula().equals("VA")) { // desconto
                    // é
                    // um
                    // valor
                    // fixo
                    ...
                    // um
                    // percentual
                    valorTotalDescontoInstitucionalCalculado = itemDescontoPlanoInstitucional.getPlanoDesconto().getPercDescontoMatricula();
                } else {
                    valorTotalDescontoInstitucionalCalculado = ((valorBaseCalculo * itemDescontoPlanoInstitucional.getPlanoDesconto().getPercDescontoMatricula()) / 100.0);
                }
            }
        } else {
            if (itemDescontoPlanoInstitucional.getPlanoDesconto().getTipoDescontoMatricula().equals("VA")) { // desconto
                ...
                valorTotalDescontoInstitucionalCalculado = (valorTotalDescontoInstitucionalCalculado + ((valorBaseCalculo * itemDescontoPlanoInstitucional.getPlanoDesconto().getPercDescontoMatricula()) / 100.0));
            }
        }
        setValorDescontoInstituicaoMatricula(this.getValorDescontoInstituicaoMatricula() + valorTotalDescontoInstitucionalCalculado);

        // Iterator i = planoAluno.getItemPlanoFinanceiroAlunoVOs().iterator();
        // while (i.hasNext()) {
        // ItemPlanoFinanceiroAlunoVO obj = (ItemPlanoFinanceiroAlunoVO)
        // i.next();
        // if
        // (obj.getPlanoDesconto().getSomente1PeriodoLetivoMatricula().equals(new
        // Boolean(true))) {
        // if (getPeridoLetivo().getPeriodoLetivo().intValue() == 1) {
        // setValorDescontoInstituicaoMatricula(getValorDescontoInstituicaoMatricula()
        // + ((getValorResidualMatricula() *
        // obj.getPlanoDesconto().getPercDescontoMatricula()) / 100.0));
        // }
        // } else {
        // setValorDescontoInstituicaoMatricula(getValorDescontoInstituicaoMatricula()
        // + ((getValorResidualMatricula() *
        ...
        // }
        // setValorTotalTodosDescontosMatricula(getValorTotalTodosDescontosMatricula()
        // + ((valorMatricula *
        // condicao.getPlanoDescontoPadrao().getPercDescontoMatricula()) /
        // 100));
        // setValorResidualMatricula(valorMatricula -
        // (getValorDescontoInstituicaoMatricula() +
        // getValorDescontoConvenioMatricula()));
    }
     ...
     * @param ordem
     * @param posicao
     * @param itemDescontoPlanoInstitucional
     * @deprecated
     */
    public void processarDescontoConvenioMatriculaAluno(PlanoFinanceiroAlunoVO planoAluno, Double valorMatricula, List ordemDesconto, OrdemDescontoVO ordem, Integer posicao, ItemPlanoFinanceiroAlunoVO itemDescontoPlanoInstitucional) {
        Double valorBaseCalculo = 0.0;
        if (ordem.getValorCheio().booleanValue()) {
            valorBaseCalculo = valorMatricula;
        } else {
        ...
        Double valorTotalDescontoConvenioCalculado = 0.0;

        if ((itemDescontoPlanoInstitucional.getConvenio().getDataFinalVigencia() == null) || (itemDescontoPlanoInstitucional.getConvenio().getDataFinalVigencia().compareTo(new Date()) <= 0)) {
            if (itemDescontoPlanoInstitucional.getConvenio().getTipoDescontoMatricula().equals("VA")) {
                valorTotalDescontoConvenioCalculado = itemDescontoPlanoInstitucional.getConvenio().getDescontoMatricula();
            } else {
                valorTotalDescontoConvenioCalculado = ((valorBaseCalculo * itemDescontoPlanoInstitucional.getConvenio().getDescontoMatricula()) / 100.0);
            }
        }
        setValorDescontoConvenioMatricula(this.getValorDescontoConvenioMatricula() + valorTotalDescontoConvenioCalculado);
    }
     ...
     * @deprecated
     */
    public void calcularPlanoDescontoInstitucionalMatricula(PlanoFinanceiroAlunoVO planoAluno, Double valorMatricula, List ordemDesconto, OrdemDescontoVO ordem, Integer posicao) {
        Iterator i = planoAluno.getItemPlanoFinanceiroAlunoVOs().iterator();
        while (i.hasNext()) {
            ItemPlanoFinanceiroAlunoVO itemDescontoPlanoInstitucional = (ItemPlanoFinanceiroAlunoVO) i.next();
            if (itemDescontoPlanoInstitucional.getTipoPlanoFinanceiroDescontoInstitucional()) {
                processarDescontoInstitucionalMatriculaAluno(planoAluno, valorMatricula, ordemDesconto, ordem, posicao, itemDescontoPlanoInstitucional);
            }
        }
        setValorTotalTodosDescontosMatricula(this.getValorTotalTodosDescontosMatricula() + this.getValorDescontoInstituicaoMatricula());
        setValorResidualMatricula(valorMatricula - getValorTotalTodosDescontosMatricula());
        // if (ordem.getValorCheio().booleanValue()) {
        // planoAluno.setOrdemPlanoDesconto(posicao + 3);
        // } else {
        // planoAluno.setOrdemPlanoDesconto(posicao);
        // }
    }
     ...
     * @deprecated
     */
    public void calcularDescontoConvenioMatricula(PlanoFinanceiroAlunoVO planoAluno, Double valorMatricula, List ordemDesconto, OrdemDescontoVO ordem, Integer posicao) {
        Iterator i = planoAluno.getItemPlanoFinanceiroAlunoVOs().iterator();
        while (i.hasNext()) {
            ItemPlanoFinanceiroAlunoVO itemDescontoPlanoInstitucional = (ItemPlanoFinanceiroAlunoVO) i.next();
            if (itemDescontoPlanoInstitucional.getTipoPlanoFinanceiroConvenio()) {
                processarDescontoConvenioMatriculaAluno(planoAluno, valorMatricula, ordemDesconto, ordem, posicao, itemDescontoPlanoInstitucional);
            }
        }
        setValorTotalTodosDescontosMatricula(this.getValorTotalTodosDescontosMatricula() + this.getValorDescontoConvenioMatricula());
        setValorResidualMatricula(valorMatricula - getValorTotalTodosDescontosMatricula());
        // if (ordem.getValorCheio().booleanValue()) {
        // planoAluno.setOrdemPlanoDesconto(posicao + 3);
        // } else {
    ...
    // public void calcularDescontoConvenioMatricula(PlanoFinanceiroCursoVO
    // planoCurso, PlanoFinanceiroAlunoVO planoAluno, Double valorMatricula,
    // List ordemDesconto, OrdemDescontoVO ordem, Integer posicao) {
    // if (ordem.getValorCheio().booleanValue()) {
    // Iterator i = planoAluno.getItemPlanoFinanceiroAlunoVOs().iterator();
    // while (i.hasNext()) {
    // ItemPlanoFinanceiroAlunoVO obj = (ItemPlanoFinanceiroAlunoVO) i.next();
    // if (obj.getTipoItemPlanoFinanceiro().equals("CO")) {
    // String dataFinalVigencia = "";
    // if (obj.getConvenio().getDataFinalVigencia() != null) {
    // dataFinalVigencia =
    // String.valueOf(obj.getConvenio().getDataFinalVigencia().getDate()) + " "
    // + String.valueOf(obj.getConvenio().getDataFinalVigencia().getMonth()) +
    // " " + String.valueOf(obj.getConvenio().getDataFinalVigencia().getYear());
    // }
    // String dataComparacao = String.valueOf(new Date().getDate()) + " " +
    // String.valueOf(new Date().getMonth()) + " " + String.valueOf(new
    // Date().getYear());
    // if (obj.getConvenio().getDataFinalVigencia() == null ||
    // obj.getConvenio().getDataFinalVigencia().after(new Date()) ||
    // dataFinalVigencia.equals(dataComparacao)) {
    // if (obj.getConvenio().getTipoDescontoMatricula().equals("VA")) {
    // setValorDescontoConvenioMatricula(getValorDescontoConvenioMatricula() +
    // obj.getConvenio().getDescontoMatricula());
    // } else {
    // setValorDescontoConvenioMatricula(getValorDescontoConvenioMatricula() +
    // ((valorMatricula * obj.getConvenio().getDescontoMatricula()) / 100.0));
    // }
    // }
    // }
    // }
    ...
    // (getValorDescontoInstituicaoMatricula() +
    // getValorDescontoConvenioMatricula()));
    // planoAluno.setOrdemConvenio(posicao + 3);
    // } else {
    // setValorBaseDescontoConvenioMatricula(getValorResidualMatricula());
    // Iterator i = planoAluno.getItemPlanoFinanceiroAlunoVOs().iterator();
    // while (i.hasNext()) {
    // ItemPlanoFinanceiroAlunoVO obj = (ItemPlanoFinanceiroAlunoVO) i.next();
    // if (obj.getTipoItemPlanoFinanceiro().equals("CO")) {
    ...
    // } else {
    // setValorDescontoConvenioMatricula(getValorDescontoConvenioMatricula() +
    // ((getValorResidualMatricula() * obj.getConvenio().getDescontoMatricula())
    // / 100.0));
    // }
    // }
    // }
    ...
    // (getValorDescontoInstituicaoMatricula() +
    // getValorDescontoConvenioMatricula()));
    // planoAluno.setOrdemConvenio(posicao);
    // }
    // }
    /**
     * Responsï¿½vel para retornar o valor em R$ do total de desconto a ser
     * aplicado na Mensalidade
     *
     * @param planoFinanceiroAlunoVO
     * @return
     * @deprecated
     */
    public Double descontoTotalMensalidade(PlanoFinanceiroCursoVO planoCurso, PlanoFinanceiroAlunoVO planoAluno, Double valorMensalidade, List ordemDesconto, CondicaoPagamentoPlanoFinanceiroCursoVO condicao) {
        setValorDescontoMensalidade(0.0);
        setValorDescontoInstituicaoMensalidade(0.0);
        setValorDescontoConvenioMensalidade(0.0);
        setValorMensalidadeBolsaCusteada(0.0);
        setValorTotalTodosDescontosMensalidade(0.0);
        setValorResidualMensalidade(valorMensalidade);
        for (int posicao = 0; posicao < ordemDesconto.size(); posicao++) {
            OrdemDescontoVO ordem = (OrdemDescontoVO) ordemDesconto.get(posicao);
            if (ordem.getDescricaoDesconto().equals("Desconto Aluno")) {
                calcularDescontoAlunoMensalidade(planoCurso, planoAluno, valorMensalidade, ordemDesconto, ordem, posicao);
            }
            if (ordem.getDescricaoDesconto().equals("Plano Desconto")) {
                calcularPlanoDescontoInstitucionalMensalidade(planoAluno, valorMensalidade, ordemDesconto, ordem, posicao);
            }
            if (ordem.getDescricaoDesconto().equals("Convênio")) {
                calcularDescontoConvenioMensalidade(planoAluno, valorMensalidade, ordemDesconto, ordem, posicao);
            }
        }
        // setValorDescontoMensalidade(getValorDescontoInstituicaoMensalidade()
        // + getValorDescontoConvenioMensalidade());
        // if (getValorDescontoMensalidade() > valorMensalidade) {
        // setValorDescontoInstituicaoMensalidade(getValorDescontoInstituicaoMensalidade()
        // - (getValorDescontoMensalidade() - valorMensalidade));
        // setValorDescontoMensalidade(valorMensalidade);
        // }
        calcularValorMensalidade(getValorMensalidadeCheio(), getValorTotalTodosDescontosMensalidade());
        return getValorDescontoMensalidade();
    }

    public void calcularDescontoAlunoMensalidade(PlanoFinanceiroCursoVO planoCurso, PlanoFinanceiroAlunoVO planoAluno, Double valorMensalidade, List ordemDesconto, OrdemDescontoVO ordem, Integer posicao) {
        if (ordem.getValorCheio().booleanValue()) {
            if (planoAluno.getTipoDescontoParcela().equals("PO")) {
                setValorDescontoMensalidade(getValorDescontoMensalidade() + (valorMensalidade * planoAluno.getPercDescontoParcela()) / 100.0);
            } else {
                setValorDescontoMensalidade(getValorDescontoMensalidade() + planoAluno.getValorDescontoParcela());
            }
            setValorTotalTodosDescontosMensalidade(getValorTotalTodosDescontosMensalidade() + this.getValorDescontoMensalidade());
            setValorResidualMatricula(valorMensalidade - getValorTotalTodosDescontosMensalidade());
        } else {
            if (planoAluno.getTipoDescontoParcela().equals("PO")) {
                setValorDescontoMensalidade(getValorDescontoMensalidade() + (getValorResidualMensalidade() * planoAluno.getPercDescontoParcela()) / 100.0);
            } else {
                setValorDescontoMensalidade(getValorDescontoMensalidade() + planoAluno.getValorDescontoParcela());
            }
            setValorTotalTodosDescontosMensalidade(getValorTotalTodosDescontosMensalidade() + this.getValorDescontoMensalidade());
    ...
    public void processarDescontoInstitucionalMensalidadeAluno(PlanoFinanceiroAlunoVO planoAluno, Double valorMensalidade, List ordemDesconto, OrdemDescontoVO ordem, Integer posicao, ItemPlanoFinanceiroAlunoVO itemDescontoPlanoInstitucional) {
        Double valorBaseCalculo = 0.0;
        if (ordem.getValorCheio().booleanValue()) {
            valorBaseCalculo = valorMensalidade;
        } else {
            valorBaseCalculo = getValorResidualMensalidade();
        }
        Double valorTotalDescontoInstitucionalCalculado = 0.0;
        if (itemDescontoPlanoInstitucional.getPlanoDesconto().getSomente1PeriodoLetivoMatricula().equals(true)) {
            if (getPeridoLetivo().getPeriodoLetivo().intValue() == 1) {
                if (itemDescontoPlanoInstitucional.getPlanoDesconto().getTipoDescontoParcela().equals("VA")) {
                    valorTotalDescontoInstitucionalCalculado = itemDescontoPlanoInstitucional.getPlanoDesconto().getPercDescontoParcela();
                } else {
                    valorTotalDescontoInstitucionalCalculado = ((valorBaseCalculo * itemDescontoPlanoInstitucional.getPlanoDesconto().getPercDescontoParcela()) / 100.0);
                }
            }
        } else {
            if (itemDescontoPlanoInstitucional.getPlanoDesconto().getTipoDescontoParcela().equals("VA")) {
                ...
                valorTotalDescontoInstitucionalCalculado = (valorTotalDescontoInstitucionalCalculado + ((valorBaseCalculo * itemDescontoPlanoInstitucional.getPlanoDesconto().getPercDescontoParcela()) / 100.0));
            }
        }
        setValorDescontoInstituicaoMensalidade(this.getValorDescontoInstituicaoMensalidade() + valorTotalDescontoInstitucionalCalculado);
    }

    public void calcularPlanoDescontoInstitucionalMensalidade(PlanoFinanceiroAlunoVO planoAluno, Double valorMensalidade, List ordemDesconto, OrdemDescontoVO ordem, Integer posicao) {
        Iterator i = planoAluno.getItemPlanoFinanceiroAlunoVOs().iterator();
        while (i.hasNext()) {
            ItemPlanoFinanceiroAlunoVO itemDescontoPlanoInstitucional = (ItemPlanoFinanceiroAlunoVO) i.next();
            if (itemDescontoPlanoInstitucional.getTipoPlanoFinanceiroDescontoInstitucional()) {
                processarDescontoInstitucionalMensalidadeAluno(planoAluno, valorMensalidade, ordemDesconto, ordem, posicao, itemDescontoPlanoInstitucional);
            }
        }
        setValorTotalTodosDescontosMensalidade(this.getValorTotalTodosDescontosMensalidade() + this.getValorDescontoInstituicaoMensalidade());
        setValorResidualMensalidade(valorMensalidade - getValorTotalTodosDescontosMensalidade());
        // if (ordem.getValorCheio().booleanValue()) {
        // planoAluno.setOrdemPlanoDesconto(posicao + 3);
        // } else {
        // planoAluno.setOrdemPlanoDesconto(posicao);
        ...
        // setValorDescontoInstituicaoMensalidade(getValorDescontoInstituicaoMensalidade()
        // + ((valorMensalidade *
        // condicao.getPlanoDescontoPadrao().getPercDescontoParcela()) / 100));
        // Iterator i = planoAluno.getItemPlanoFinanceiroAlunoVOs().iterator();
        // while (i.hasNext()) {
        // ItemPlanoFinanceiroAlunoVO obj = (ItemPlanoFinanceiroAlunoVO)
        // i.next();
        // if
        // (obj.getPlanoDesconto().getSomente1PeriodoLetivoParcela().equals(new
        // Boolean(true))) {
        // if (getPeridoLetivo().getPeriodoLetivo().intValue() == 1) {
        // setValorDescontoInstituicaoMensalidade(getValorDescontoInstituicaoMensalidade()
        // + ((valorMensalidade *
        // obj.getPlanoDesconto().getPercDescontoParcela()) / 100.0));
        // }
        // } else {
        // setValorDescontoInstituicaoMensalidade(getValorDescontoInstituicaoMensalidade()
        // + ((valorMensalidade *
        ...
        // }
        // setValorResidualMensalidade(valorMensalidade -
        // (getValorDescontoInstituicaoMensalidade() +
        // getValorDescontoConvenioMensalidade()));
        // } else {
        // setValorDescontoInstituicaoMensalidade(getValorDescontoInstituicaoMensalidade()
        // + ((getValorResidualMensalidade() *
        // condicao.getPlanoDescontoPadrao().getPercDescontoParcela()) / 100));
        // Iterator i = planoAluno.getItemPlanoFinanceiroAlunoVOs().iterator();
        // while (i.hasNext()) {
        // ItemPlanoFinanceiroAlunoVO obj = (ItemPlanoFinanceiroAlunoVO)
    ...
    public void processarDescontoConvenioMensalidadeAluno(PlanoFinanceiroAlunoVO planoAluno, Double valorMensalidade, List ordemDesconto, OrdemDescontoVO ordem, Integer posicao, ItemPlanoFinanceiroAlunoVO itemDescontoPlanoInstitucional) {
        Double valorBaseCalculo = 0.0;
        if (ordem.getValorCheio().booleanValue()) {
            valorBaseCalculo = valorMensalidade;
        } else {
        ...
        if ((itemDescontoPlanoInstitucional.getConvenio().getDataFinalVigencia() == null) || (itemDescontoPlanoInstitucional.getConvenio().getDataFinalVigencia().compareTo(new Date()) <= 0)) {
            if (itemDescontoPlanoInstitucional.getConvenio().getTipoDescontoMatricula().equals("VA")) {
                valorTotalDescontoConvenioCalculado = itemDescontoPlanoInstitucional.getConvenio().getDescontoParcela();
            } else {
                valorTotalDescontoConvenioCalculado = ((valorBaseCalculo * itemDescontoPlanoInstitucional.getConvenio().getDescontoParcela()) / 100.0);
            }
        }
        setValorDescontoConvenioMensalidade(this.getValorDescontoConvenioMensalidade() + valorTotalDescontoConvenioCalculado);
    }

    /*
     * @de
     */
    public void calcularDescontoConvenioMensalidade(PlanoFinanceiroAlunoVO planoAluno, Double valorMensalidade, List ordemDesconto, OrdemDescontoVO ordem, Integer posicao) {
        Iterator i = planoAluno.getItemPlanoFinanceiroAlunoVOs().iterator();
        while (i.hasNext()) {
            ItemPlanoFinanceiroAlunoVO itemDescontoPlanoInstitucional = (ItemPlanoFinanceiroAlunoVO) i.next();
            if (itemDescontoPlanoInstitucional.getTipoPlanoFinanceiroConvenio()) {
                processarDescontoConvenioMensalidadeAluno(planoAluno, valorMensalidade, ordemDesconto, ordem, posicao, itemDescontoPlanoInstitucional);
            }
        }
        setValorTotalTodosDescontosMensalidade(this.getValorTotalTodosDescontosMensalidade() + this.getValorDescontoConvenioMensalidade());
        setValorResidualMensalidade(valorMensalidade - getValorTotalTodosDescontosMensalidade());
        // if (ordem.getValorCheio().booleanValue()) {
        // planoAluno.setOrdemPlanoDesconto(posicao + 3);
        // } else {
...
//    }
    ...
    public MatriculaPeriodoVencimentoVO getMatriculaPeriodoVencimentoReferenteMatricula() {
        return (this.getMatriculaPeriodoVencimentoEspecifico("MA"));
    }

    public void excluirMatriculaPeriodoVencimentoVOPorIdentificador(String identificadorParcela) throws Exception {
        MatriculaPeriodoVencimentoVO vcto = this.getMatriculaPeriodoVencimentoEspecifico("MA");
        if (vcto == null) {
            return;
        }
        excluirObjMatriculaPeriodoVencimentoVOs(vcto);
    }
    
    public MatriculaPeriodoVencimentoVO obterMatriculaPeriodoVencimentoCorrespondente(MatriculaPeriodoVencimentoVO mpvctoVO) {
    	for(MatriculaPeriodoVencimentoVO matriculaPeriodoVencimentoVO: getMatriculaPeriodoVencimentoVOs()) {
			if((matriculaPeriodoVencimentoVO.getParcelaReferenteMatricula() && mpvctoVO.getParcelaReferenteMatricula())
					|| (!matriculaPeriodoVencimentoVO.getVencimentoReferenteMatricula() 
							&& !matriculaPeriodoVencimentoVO.getParcela().contains("EX")
							&& !mpvctoVO.getVencimentoReferenteMatricula() 
							&& !mpvctoVO.getParcela().contains("EX")
							&& matriculaPeriodoVencimentoVO.verificarParcelaCorrespondente(mpvctoVO))) {
    ...
    public SituacaoVencimentoMatriculaPeriodo obterSituacaoParcelasReferenteMatricula() {
        MatriculaPeriodoVencimentoVO vctoMatricula = this.getMatriculaPeriodoVencimentoEspecifico("MA");
        if (vctoMatricula == null) {
            return SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_NAO_GERADA;
        } else {
            if (vctoMatricula.getContaReceber().getCodigo() > 0 && vctoMatricula.getContaReceber().getSituacao().equals("NE")) {
                return SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_RENEGOCIADA;
            }
            if (vctoMatricula.getContaReceber().getCodigo() > 0 && 
            	vctoMatricula.getContaReceber().getContaEditadaManualmente()) {
            	if (vctoMatricula.getContaReceber().getSituacaoEQuitada()) {
            		return SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_GERADA_EPAGA;
            	}
            	// Caso a conta a receber tenha sido editata manualmente. Então tratamos a mesma como se tivesse sido renegociada.
            	// Ou seja, nao iremos entrar no mérito se a mesma está quitada ou não. Pois como a mesma foi editada manualmente,
            	// teremos que ignorar o valor atual (que foi modificado), tratando a mesma como uma parcela renegociada. 
                return SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_RENEGOCIADA_EDITADA_MANUALMENTE;
            }
            if (!this.getCondicaoPagamentoPlanoFinanceiroCurso().getNaoControlarMatricula() && vctoMatricula.getSituacao().equals(SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_NAO_DEVE_SER_GERADA)) {
            	return SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_NAO_GERADA;
            }
    ...
    public Boolean getIsExisteParcelasReferenteMatricula() {
        MatriculaPeriodoVencimentoVO vctoMatricula = this.getMatriculaPeriodoVencimentoEspecifico("MA");
        if (vctoMatricula == null) {
            return false;
        } else {
    ...
    public Boolean verificarExisteAlgumaParcelaPagaMatriculaPeriodo() {
        for (MatriculaPeriodoVencimentoVO vcto : this.getMatriculaPeriodoVencimentoVOs()) {
            if (vcto.getSituacao().equals(SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_GERADA_EPAGA) && Uteis.isAtributoPreenchido(vcto.getContaReceber().getCodigo()) && vcto.getContaReceber().getSituacao().equals("RE")) {
                return true;
            }
        }
        return false;
    ...
    public Boolean verificarTodasAsParcelasJaPagas() {
        Iterator i = this.getMatriculaPeriodoVencimentoVOs().iterator();
        Boolean algumaParcelaGerada = false;
        Boolean algumaParcelaNaoPaga = false;
        while (i.hasNext()) {
            MatriculaPeriodoVencimentoVO vctoMensalidades = (MatriculaPeriodoVencimentoVO) i.next();
            if (!vctoMensalidades.getVencimentoReferenteMatricula()) {
                if (!vctoMensalidades.getSituacao().equals(SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_NAO_GERADA)) {
                    algumaParcelaGerada = true;
                    if (!vctoMensalidades.getSituacao().equals(SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_GERADA_EPAGA)) {
                        algumaParcelaNaoPaga = true;
                    }
                } else {
                    algumaParcelaNaoPaga = true;
                }
        ...
        if (!algumaParcelaGerada) {
            return false;
        }
        if (algumaParcelaNaoPaga) {
            return false;
        }
        return true;
    }

    public Boolean existemParcelasReferenteMensalidadesParaSeremGeradas() {
        Iterator i = this.getMatriculaPeriodoVencimentoVOs().iterator();
        while (i.hasNext()) {
            MatriculaPeriodoVencimentoVO vctoMensalidades = (MatriculaPeriodoVencimentoVO) i.next();
            if ((!vctoMensalidades.getVencimentoReferenteMatricula()) && (vctoMensalidades.getSituacao().equals(SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_NAO_GERADA))) {
                return true;
            }
        }
        return false;
    ...
    public SituacaoVencimentoMatriculaPeriodo obterSituacaoParcelasReferenteMensalidades() throws Exception {
        Iterator<MatriculaPeriodoVencimentoVO> i = this.getMatriculaPeriodoVencimentoVOs().iterator();
        Boolean algumaParcelaGerada = false;
        Boolean algumaParcelaPaga = false;
        // TODO Mudanças para identificar problemas com a MatriculaPeriodoVencimento e geração de ContaReceber MURILLO 14/12/2010
        Boolean parcelasGeradasParcialmente = false;
        while (i.hasNext()) {
            MatriculaPeriodoVencimentoVO vctoMensalidades = (MatriculaPeriodoVencimentoVO) i.next();            
            if (!vctoMensalidades.getVencimentoReferenteMatricula()) {
                if (vctoMensalidades.getContaReceber().getSituacao().equals(SituacaoContaReceber.CANCELADO_FINANCEIRO.getValor())) {
                    throw new Exception("Existem contas a receber para esta matrícula período que foram canceladas. As mesmas devem ser reativadas para que esta matrícula possa ser editada (Parcela Cancelada: " + vctoMensalidades.getContaReceber().getParcela() + " - " + vctoMensalidades.getContaReceber().getNossoNumero() + ")");
                }
                if (!vctoMensalidades.getSituacao().equals(SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_NAO_GERADA)) {
                    algumaParcelaGerada = true;
                    if (vctoMensalidades.getSituacao().equals(SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_GERADA_EPAGA)) {
                        algumaParcelaPaga = true;
                    }
                }
                if ((algumaParcelaGerada || algumaParcelaPaga) && vctoMensalidades.getSituacao().equals(SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_NAO_GERADA)
                        && !this.getPlanoFinanceiroCurso().getModeloGeracaoParcelas().equals("FC")) {
                    parcelasGeradasParcialmente = true;
                }
            }
        }
        
        
        if (parcelasGeradasParcialmente) {
            return SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_GERADA_E_PAGA_PARCIALMENTE;
        }
        // TODO Mudanças para identificar problemas com a MatriculaPeriodoVencimento e geração de ContaReceber MURILLO 14/12/2010
        if (!algumaParcelaGerada) {
            return SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_NAO_GERADA;
        ...
        if (algumaParcelaPaga) {
            ...
            return SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_GERADA;
        }
    }
    ...
    /**
     * @return the matriculaPeriodoVencimentoVOs
     */
    public List<MatriculaPeriodoVencimentoVO> getMatriculaPeriodoVencimentoVOs() {
        if (matriculaPeriodoVencimentoVOs == null) {
            matriculaPeriodoVencimentoVOs = new ArrayList<MatriculaPeriodoVencimentoVO>(0);
        }
        return matriculaPeriodoVencimentoVOs;
    ...
    /**
     * @param matriculaPeriodoVencimentoVOs
     *            the matriculaPeriodoVencimentoVOs to set
     */
    public void setMatriculaPeriodoVencimentoVOs(List<MatriculaPeriodoVencimentoVO> matriculaPeriodoVencimentoVOs) {
        this.matriculaPeriodoVencimentoVOs = matriculaPeriodoVencimentoVOs;
    }

    /**
     * @return the matriculaPeriodoPreMatricula
     */
    public MatriculaPeriodoPreMatriculaVO getMatriculaPeriodoPreMatricula() {
        if (matriculaPeriodoPreMatricula == null) {
            matriculaPeriodoPreMatricula = new MatriculaPeriodoPreMatriculaVO();
        }
        return matriculaPeriodoPreMatricula;
    }
    ...
    public Boolean verificarExistePreMatriculaParaPeriodo() {
        if (this.getMatriculaPeriodoPreMatricula().getCodigo().intValue() != 0) {
            return true;
        } else {
            return false;
        }
    ...
    public Boolean getMatriculaJaPaga() {
        if ((this.getMatriculaPeriodoVencimentoReferenteMatricula() != null) && (this.getMatriculaPeriodoVencimentoReferenteMatricula().getSituacao().equals(SituacaoVencimentoMatriculaPeriodo.CONTARECEBER_GERADA_EPAGA))) {
            return true;
        }
        return false;
    }
    ...
    public List<PlanoFinanceiroAlunoDescricaoDescontosVO> getListaDescontosMatricula() {
        if (listaDescontosMatricula == null) {
            listaDescontosMatricula = new ArrayList<PlanoFinanceiroAlunoDescricaoDescontosVO>(0);
        }
        return listaDescontosMatricula;
    }
    ...
    /**
     * @param listaDescontosMatricula
     *            the listaDescontosMatricula to set
     */
    public void setListaDescontosMatricula(List<PlanoFinanceiroAlunoDescricaoDescontosVO> listaDescontosMatricula) {
        this.listaDescontosMatricula = listaDescontosMatricula;

    }

    /**
     * @return the listaDescontosMensalidade
     */
    ...
    /**
     * @param listaDescontosMensalidade
     *            the listaDescontosMensalidade to set
     */
     ...
     */
    public List<MatriculaPeriodoVencimentoVO> getListaParcelasGeradasManualmenteAnteriormenteAptasParaRegeracao() {
        if (listaParcelasGeradasManualmenteAnteriormenteAptasParaRegeracao == null) {
            listaParcelasGeradasManualmenteAnteriormenteAptasParaRegeracao = new ArrayList<MatriculaPeriodoVencimentoVO>(1);
        }
        return listaParcelasGeradasManualmenteAnteriormenteAptasParaRegeracao;
    }

    /**
     * @param listaParcelasGeradasManualmenteAnteriormenteAptasParaRegeracao the listaParcelasGeradasManualmenteAnteriormenteAptasParaRegeracao to set
     */
    public void setListaParcelasGeradasManualmenteAnteriormenteAptasParaRegeracao(List<MatriculaPeriodoVencimentoVO> listaParcelasGeradasManualmenteAnteriormenteAptasParaRegeracao) {
        this.listaParcelasGeradasManualmenteAnteriormenteAptasParaRegeracao = listaParcelasGeradasManualmenteAnteriormenteAptasParaRegeracao;
    }
    ...
    public List<LocalPeriodoVO> getListaLocalPeriodoVO() {
        if (listaLocalPeriodoVO == null) {
            listaLocalPeriodoVO = new ArrayList<LocalPeriodoVO>(0);
        }
        return listaLocalPeriodoVO;
    }

    public void setListaLocalPeriodoVO(List<LocalPeriodoVO> listaLocalPeriodoVO) {
        this.listaLocalPeriodoVO = listaLocalPeriodoVO;
    }
...
//        }
    ...
    /**
     * @return the listaConvenioRemoverContaPagarRestituicaoAluno
     */
    public List<ConvenioVO> getListaConvenioRemoverContaPagarRestituicaoAluno() {
        if (listaConvenioRemoverContaPagarRestituicaoAluno == null) {
            listaConvenioRemoverContaPagarRestituicaoAluno = new ArrayList<ConvenioVO>(0);
        }
        return listaConvenioRemoverContaPagarRestituicaoAluno;
    }

    /**
     * @param listaConvenioRemoverContaPagarRestituicaoAluno the listaConvenioRemoverContaPagarRestituicaoAluno to set
     */
    public void setListaConvenioRemoverContaPagarRestituicaoAluno(List<ConvenioVO> listaConvenioRemoverContaPagarRestituicaoAluno) {
        this.listaConvenioRemoverContaPagarRestituicaoAluno = listaConvenioRemoverContaPagarRestituicaoAluno;
    }
	...
	public List<ContaReceberVO> getContaReceberNotaFiscalEmitidaVOs() {
		if(contaReceberNotaFiscalEmitidaVOs == null){
			contaReceberNotaFiscalEmitidaVOs = new ArrayList<ContaReceberVO>(0);
		}
		return contaReceberNotaFiscalEmitidaVOs;
	}

	public void setContaReceberNotaFiscalEmitidaVOs(List<ContaReceberVO> contaReceberNotaFiscalEmitidaVOs) {
		this.contaReceberNotaFiscalEmitidaVOs = contaReceberNotaFiscalEmitidaVOs;
	}
	
	private MatriculaPeriodoMensalidadeCalculadaVO matriculaPeriodoMatriculaCalculadaVO;

	public MatriculaPeriodoMensalidadeCalculadaVO getMatriculaPeriodoMatriculaCalculadaVO() {
		if (matriculaPeriodoMatriculaCalculadaVO == null) {
			matriculaPeriodoMatriculaCalculadaVO = new MatriculaPeriodoMensalidadeCalculadaVO();
		}
		return matriculaPeriodoMatriculaCalculadaVO;
	}

	public void setMatriculaPeriodoMatriculaCalculadaVO(MatriculaPeriodoMensalidadeCalculadaVO matriculaPeriodoMatriculaCalculadaVO) {
		this.matriculaPeriodoMatriculaCalculadaVO = matriculaPeriodoMatriculaCalculadaVO;
	}

	private List<MatriculaPeriodoMensalidadeCalculadaVO> matriculaPeriodoMensalidadeCalculadaVOs;
	private List<MatriculaPeriodoMensalidadeCalculadaVO> matriculaPeriodoMensalidadeEntradaCalculadaVOs;

	public List<MatriculaPeriodoMensalidadeCalculadaVO> getMatriculaPeriodoMensalidadeCalculadaVOs() {
		if(matriculaPeriodoMensalidadeCalculadaVOs == null){
			matriculaPeriodoMensalidadeCalculadaVOs = new ArrayList<MatriculaPeriodoMensalidadeCalculadaVO>(0);
		}
		return matriculaPeriodoMensalidadeCalculadaVOs;
	}

	public void setMatriculaPeriodoMensalidadeCalculadaVOs(
			List<MatriculaPeriodoMensalidadeCalculadaVO> matriculaPeriodoMensalidadeCalculadaVOs) {
		this.matriculaPeriodoMensalidadeCalculadaVOs = matriculaPeriodoMensalidadeCalculadaVOs;
	}
	
	
	public List<MatriculaPeriodoMensalidadeCalculadaVO> getMatriculaPeriodoMensalidadeEntradaCalculadaVOs() {
		if(matriculaPeriodoMensalidadeEntradaCalculadaVOs == null){
			matriculaPeriodoMensalidadeEntradaCalculadaVOs = new ArrayList<MatriculaPeriodoMensalidadeCalculadaVO>(0);
		}
		return matriculaPeriodoMensalidadeEntradaCalculadaVOs;
	}

	public void setMatriculaPeriodoMensalidadeEntradaCalculadaVOs(
			List<MatriculaPeriodoMensalidadeCalculadaVO> matriculaPeriodoMensalidadeEntradaCalculadaVOs) {
		this.matriculaPeriodoMensalidadeEntradaCalculadaVOs = matriculaPeriodoMensalidadeEntradaCalculadaVOs;
	}

	private List<MatriculaPeriodoMensalidadeCalculadaVO> matriculaPeriodoMaterialDidaticoCalculadaVOs;
	
	public List<MatriculaPeriodoMensalidadeCalculadaVO> getMatriculaPeriodoMaterialDidaticoCalculadaVOs() {
		if (matriculaPeriodoMaterialDidaticoCalculadaVOs == null) {
			matriculaPeriodoMaterialDidaticoCalculadaVOs = new ArrayList<MatriculaPeriodoMensalidadeCalculadaVO>(0);
		}
		return matriculaPeriodoMaterialDidaticoCalculadaVOs;
	}

	public void setMatriculaPeriodoMaterialDidaticoCalculadaVOs(List<MatriculaPeriodoMensalidadeCalculadaVO> matriculaPeriodoMaterialDidaticoCalculadaVOs) {
		this.matriculaPeriodoMaterialDidaticoCalculadaVOs = matriculaPeriodoMaterialDidaticoCalculadaVOs;
	}
	
	
	public boolean isApresentarValoresMatriculaParcelaCalculadas(){
		return Uteis.isAtributoPreenchido(getMatriculaPeriodoMatriculaCalculadaVO().getParcelaInicial())  && (Uteis.isAtributoPreenchido(getMatriculaPeriodoMaterialDidaticoCalculadaVOs()) || Uteis.isAtributoPreenchido(getMatriculaPeriodoMensalidadeCalculadaVOs())); 
	}
	
	public Double getTotalMatriculaPeriodoMatriculaParcelaCalculada() {
		Double valor = getMatriculaPeriodoMatriculaCalculadaVO().getTotalMensalidadeCheio();
		for (MatriculaPeriodoMensalidadeCalculadaVO mpmc : getMatriculaPeriodoMensalidadeEntradaCalculadaVOs()) {
			valor = valor + mpmc.getTotalMensalidadeCheio();
		}
		for (MatriculaPeriodoMensalidadeCalculadaVO mpmc : getMatriculaPeriodoMensalidadeCalculadaVOs()) {
			valor = valor + mpmc.getTotalMensalidadeCheio();
		}
		for (MatriculaPeriodoMensalidadeCalculadaVO mpmc : getMatriculaPeriodoMaterialDidaticoCalculadaVOs()) {
			valor = valor + mpmc.getTotalMensalidadeCheio();
		}
		return Uteis.arrendondarForcando2CasasDecimais(valor);
	}

	public List<PlanoFinanceiroAlunoDescricaoDescontosVO> getListaDescontosMaterialDidatico() {
		if (listaDescontosMaterialDidatico == null) {
			listaDescontosMaterialDidatico = new ArrayList<PlanoFinanceiroAlunoDescricaoDescontosVO>();
		}
		return listaDescontosMaterialDidatico;
	}

	public void setListaDescontosMaterialDidatico(List<PlanoFinanceiroAlunoDescricaoDescontosVO> listaDescontosMaterialDidatico) {
		this.listaDescontosMaterialDidatico = listaDescontosMaterialDidatico;
	}
	...
	public ContaPagarVO getContaPagarRestituicaoVO() {
		if(contaPagarRestituicaoVO == null){
			contaPagarRestituicaoVO = new ContaPagarVO();
		}
		return contaPagarRestituicaoVO;
	}

	public void setContaPagarRestituicaoVO(ContaPagarVO contaPagarRestituicaoVO) {
		this.contaPagarRestituicaoVO = contaPagarRestituicaoVO;
	}

	public List<MatriculaPeriodoContaReceberLogVO> getMatriculaPeriodoContaReceberLogVOs() {
		if(matriculaPeriodoContaReceberLogVOs == null){
			matriculaPeriodoContaReceberLogVOs = new ArrayList<MatriculaPeriodoContaReceberLogVO>(0);
		}
		return matriculaPeriodoContaReceberLogVOs;
	}

	public void setMatriculaPeriodoContaReceberLogVOs(
			List<MatriculaPeriodoContaReceberLogVO> matriculaPeriodoContaReceberLogVOs) {
		this.matriculaPeriodoContaReceberLogVOs = matriculaPeriodoContaReceberLogVOs;
	}
	...
	private List<ConvenioVO> listaConvenioNaoRestituiAluno;

	public List<ConvenioVO> getListaConvenioNaoRestituiAluno() {
		if(listaConvenioNaoRestituiAluno == null){
			listaConvenioNaoRestituiAluno = new ArrayList<ConvenioVO>(0);
		}
		return listaConvenioNaoRestituiAluno;
	}

	public void setListaConvenioNaoRestituiAluno(List<ConvenioVO> listaConvenioNaoRestituiAluno) {
		this.listaConvenioNaoRestituiAluno = listaConvenioNaoRestituiAluno;
	}
	
	private String descricaoDescontoConvenioNaoRestituirAluno;
	
	public String getDescricaoDescontoConvenioNaoRestituirAluno() {
		if(descricaoDescontoConvenioNaoRestituirAluno == null){
			descricaoDescontoConvenioNaoRestituirAluno = "";
			for(ConvenioVO desc: getListaConvenioNaoRestituiAluno()){				
				descricaoDescontoConvenioNaoRestituirAluno += "<div>"+desc.getDescricao()+"</div>";
			}
		}
		return descricaoDescontoConvenioNaoRestituirAluno;
	}
	

	public List<MatriculaPeriodoContaReceberLogVO> getMatriculaPeriodoContaReceberAlterarLogVOs() {
		if(matriculaPeriodoContaReceberAlterarLogVOs == null){
			matriculaPeriodoContaReceberAlterarLogVOs =  new ArrayList<MatriculaPeriodoContaReceberLogVO>(0);
		}
		return matriculaPeriodoContaReceberAlterarLogVOs;
	}

	public void setMatriculaPeriodoContaReceberAlterarLogVOs(
			List<MatriculaPeriodoContaReceberLogVO> matriculaPeriodoContaReceberAlterarLogVOs) {
		this.matriculaPeriodoContaReceberAlterarLogVOs = matriculaPeriodoContaReceberAlterarLogVOs;
	}
	...
	private List<ContaReceberVO> listaContaReceberEditadaManualmente;
	private Boolean existemContaReceberComEdicaoManualAtiva;
	private Boolean regerarContaReceberComEdicaoManualAtiva;	
	
	public List<ContaReceberVO> getListaContaReceberEditadaManualmente() {
		if (listaContaReceberEditadaManualmente == null) {
			listaContaReceberEditadaManualmente = new ArrayList<ContaReceberVO>();
		}
		return listaContaReceberEditadaManualmente;
	}

	public void setListaContaReceberEditadaManualmente(List<ContaReceberVO> listaContaReceberEditadaManualmente) {
		this.listaContaReceberEditadaManualmente = listaContaReceberEditadaManualmente;
	}
...
//		}
	...
	public String getObservacaoParaLiberacaoDiferencaValorRateio() {
		StringBuilder sb = new StringBuilder("");
		if(Uteis.isAtributoPreenchido(getMatriculaPeriodoContaReceberLogVOs()) && getMatriculaPeriodoContaReceberLogVOs().stream().anyMatch(p-> p.isLiberadoDiferencaValorRateio())) {
			Optional<MatriculaPeriodoContaReceberLogVO> mpcr = getMatriculaPeriodoContaReceberLogVOs().stream().filter(p-> p.isLiberadoDiferencaValorRateio()).findFirst();
			sb.append(" As seguintes Parcelas foram desconsiderada do valor de diferença do Rateio pelo Usuário - ").append(mpcr.get().getResponsavelLiberadoDiferencaValorRateio().getCodigo());
			sb.append(" - ").append(mpcr.get().getResponsavelLiberadoDiferencaValorRateio().getUsername());
			sb.append(System.lineSeparator());
			sb.append(" Segue as Parcelas abaixo ");
			sb.append(System.lineSeparator());
			getMatriculaPeriodoContaReceberLogVOs().stream().filter(p-> p.isLiberadoDiferencaValorRateio()).forEach(p->{
				sb.append(" Parcela - ").append(p.getParcelaAntiga());
				sb.append(" Valor Diferença - ").append(p.getDiferencaParcela());
				sb.append(System.lineSeparator());
			});
		}
		return sb.toString();
