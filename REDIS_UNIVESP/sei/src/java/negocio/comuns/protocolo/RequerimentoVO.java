...
//	private List<CidTipoRequerimentoVO> listaCidTipoRequerimentoVOs;
 ...
 // Atributo utilizado apenas no relatório Requerimento por Responsável Sintético
	...
	// }
	public RecebimentoVO receberBoleto(ContaReceberVO contaReceber) throws Exception {
		RecebimentoVO recebimento = new RecebimentoVO();
		recebimento.setContaReceber(contaReceber);
		recebimento.getCentroReceita().setCodigo(contaReceber.getCentroReceita().getCodigo());
		recebimento.getMatriculaAluno().setMatricula(contaReceber.getMatriculaAluno().getMatricula());
		recebimento.getUnidadeEnsino().setCodigo(contaReceber.getUnidadeEnsino().getCodigo());
		recebimento.setCandidato(contaReceber.getCandidato());
		recebimento.setFuncionario(contaReceber.getFuncionario());
		recebimento.setMatriculaAluno(contaReceber.getMatriculaAluno());
		recebimento.getContaCorrente().setCodigo(contaReceber.getContaCorrente());
		recebimento.setValor(contaReceber.getValor());
		recebimento.setValorRecebido(contaReceber.getValor());
		recebimento.setCodigoBarra(contaReceber.getCodigoBarra());
		recebimento.setData(contaReceber.getDataVencimento());
		recebimento.setDescricao(contaReceber.getDescricaoPagamento());
		recebimento.setNrDocumento(contaReceber.getNrDocumento());
		recebimento.setTipoOrigem(contaReceber.getTipoOrigem());
		recebimento.setTipoPessoa(contaReceber.getTipoPessoa());
		recebimento.setCodigoOrigem(contaReceber.getCodOrigem());
		recebimento.getPessoa().setCodigo(contaReceber.getPessoa().getCodigo());
		return recebimento;
	}
...
//		}
	...
	@XmlElement(name = "permitirImpressaoBoleto")
	public boolean getIsApresentarEmitirBoleto() {
		try {
			if ((!getCodigo().equals(0)) 
					&& (getContaReceberVO().getSituacao().equals("AR")) 
					&& (!getContaReceberVO().getCodigo().equals(0) 
					&& (getContaReceberVO().getPermiteImprimirBoleto() || getContaReceberVO().getPermiteImprimirBoletoLinkBanco()))) {
				return true;
			}
			return false;
		} catch (Exception e) {
