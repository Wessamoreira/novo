...
public class ScriptBuilderChain extends ChainRunnerTemplate {

	public ScriptBuilderChain(List<ChainLink> chainList) {
		super(chainList);
	}

	@Override
	protected StringFormula getStringFormula(String script) {
		return new StringFormula(script, "\n", "function main(contexto, mapa, funcoes, logger, eventos){", "}", true);
	}
