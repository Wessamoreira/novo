...
/**
 * Implementacao do layout do HSBC
 * 
 * @author Murillo
 * 
 */
...
public class HSBCLayout extends LayoutBancosImpl implements LayoutBancos, Serializable {
    ...
    public HSBCLayout(RegistroArquivoVO registroArquivo) {
    ...
    private void processarRegistroHeader(String linha, RegistroHeaderVO header) throws Exception {
        header.setTipoRegistro(Uteis.getValorInteiro(linha.substring(0, 1)));
        header.setNumeroAgencia(Uteis.getValorInteiro(linha.substring(27, 31)));
        header.setNumeroConta(Uteis.getValorInteiro(linha.substring(34, 44)));
        header.setNomeEmpresa(linha.substring(47, 76));
        header.setCodigoBanco(linha.substring(76, 79));
        header.setDataGeracaoArquivo(Uteis.getData(linha.substring(95, 100), PATTERN));
        header.setCodigoConvenioBanco(linha.substring(109, 118));
        header.setNomeBanco(linha.substring(118, 137));
    ...
    private RegistroDetalheVO processarRegistroDetalhe(String linha, RegistroDetalheVO detalhe, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuarioVO, RegistroHeaderVO header) throws Exception {
        ...
        detalhe.setCedenteNumeroInscricaoEmpresa(linha.substring(4, 17));
        detalhe.setCedenteNumeroAgencia(linha.substring(17, 22));
        detalhe.setCedenteNumeroConta(linha.substring(24, 35));
        detalhe.setIdentificacaoTituloEmpresa(linha.substring(37, 53));
        detalhe.setDataCredito(Uteis.getData(linha.substring(82, 88), PATTERN));
        detalhe.setCodigoMoeda(Uteis.getValorInteiro(linha.substring(88, 89)));
...
//        detalhe.setSacadoBancoCodigo(linha.substring(165, 168));
        ...
        detalhe.setValorIOF(Uteis.getValorDoubleComCasasDecimais(linha.substring(175, 186)));
...
//        getRegistroArquivoVO().getContaReceberVOs().add(executarCriacaoContaReceberVOs(detalhe, configuracaoFinanceiroVO, usuarioVO));
        return detalhe;
    }
    ...
    private void mapearArquivo(ControleCobrancaVO controleCobrancaVO, String caminho, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuarioVO) throws Exception {
        try {
            ...
            while ((linha = reader.readLine()) != null) {
                ...
                switch (registro) {
                    case 0:
                        processarRegistroHeader(linha, getRegistroArquivoVO().getRegistroHeader());
                        break;
                    case 1:
                        getRegistroArquivoVO().getRegistroDetalheVOs().add(processarRegistroDetalhe(linha, new RegistroDetalheVO(), configuracaoFinanceiroVO, usuarioVO, getRegistroArquivoVO().getRegistroHeader()));
                        break;
                    case 9:
