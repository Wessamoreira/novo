...
public class GeradorDeDigitoPadrao implements GeradorDeDigito {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 765839456946083150L;
	...
	public int geraDigitoMod(String codigoDeBarras, int inicio, int fim, int numMOD) {
		return Integer.valueOf(new DigitoPara(codigoDeBarras).comMultiplicadoresDeAte(inicio, fim).complementarAoModulo().trocandoPorSeEncontrar("1", 0, 1, 10, 11).mod(numMOD).calcula());
	}

	@Override
	public int geraDigitoMod10(String campo) {
		return Integer.valueOf(new DigitoPara(campo).comMultiplicadores(2, 1).somandoIndividualmente().trocandoPorSeEncontrar("0", 10).complementarAoModulo().mod(10).calcula());
	}
	...
	public int geraDigitoModAceitandoRestoZero(String codigoDeBarras, int inicio, int fim, int numMod) {
		return Integer.valueOf(new DigitoPara(codigoDeBarras).comMultiplicadoresDeAte(inicio, fim).complementarAoModulo().trocandoPorSeEncontrar("0", 0, 10, 11).mod(numMod).calcula());
	}
