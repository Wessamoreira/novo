...
public class DaycovalLayout extends LayoutBancosImpl implements LayoutBancos, Serializable {

    private RegistroArquivoVO registroArquivoVO;
    private static final String PATTERN = "ddMMyy";

    public DaycovalLayout() {
    }

    public DaycovalLayout(RegistroArquivoVO registroArquivo) {
    ...
    private RegistroDetalheVO processarRegistroTransacao(String linha, RegistroDetalheVO detalhe, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuarioVO, RegistroHeaderVO header) throws Exception {
        ...
        //detalhe.setIdentificacaoTituloEmpresa(linha.substring(62, 70));
        //detalhe.setIdentificacaoTituloEmpresa(linha.substring(85, 94));
        detalhe.setIdentificacaoTituloBanco(linha.substring(85, 94));
        //detalhe.setIndicadorRateioCredito(linha.substring(104, 105));
        detalhe.setCarteira(Uteis.getValorInteiro(linha.substring(82, 85)));
        
        String nossonumero = linha.substring(94, 107);
        String primeiroNr = nossonumero.substring(0, 1);
        if (!primeiroNr.equals("0")) {
        	nossonumero = linha.substring(94, 105);
        }
        primeiroNr = nossonumero.substring(0, 2);
        if (primeiroNr.equals("01")) {
        	nossonumero = linha.substring(95, 106);
        }
        detalhe.setIdentificacaoTituloEmpresa(nossonumero);
...
//        detalhe.setSacadoBancoCodigo(linha.substring(165, 168));
        ...
        if (detalhe.getIdentificacaoOcorrencia() == 3 || detalhe.getIdentificacaoOcorrencia() == 15 || detalhe.getIdentificacaoOcorrencia() ==16) {
...
//        header.setCodigoEmpresa(Uteis.getValorInteiro(linha.substring(26, 38)));
