...
public class BancoDoBrasilLayout extends LayoutBancosImpl implements LayoutBancos {

	private RegistroArquivoVO registroArquivoVO;
	private static final String PATTERN = "ddMMyy";

	public BancoDoBrasilLayout() {
	}

	public BancoDoBrasilLayout(RegistroArquivoVO registroArquivo) {
	...
	private void processarHeader(String linha, RegistroHeaderVO header) throws Exception {
		...
		header.setNumeroConta(Uteis.getValorInteiro(linha.substring(31, 39)));
		header.setDigitoConta(linha.substring(39, 40));
		header.setDigitoAgenciaConta(Uteis.getValorInteiro(linha.substring(30, 31)));
		...
		header.setCodigoConvenioBanco(Uteis.getValorInteiro(linha.substring(149, 156)).toString());
	...
	private RegistroDetalheVO processarRegistroDetalhe(String linha, RegistroDetalheVO detalhe, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuarioVO, RegistroHeaderVO header) throws Exception {
		...
		detalhe.setIdentificacaoTituloEmpresa(linha.substring(70, 80));
		// detalhe.setValorDesconto(Uteis.getValorDoubleComCasasDecimais(linha.substring(95,
		// 99)));
...
//		detalhe.setSacadoBancoCodigo(linha.substring(165, 168));
		...
		// detalhe.setTarifaCobranca(Uteis.getValorDoubleComCasasDecimais(linha.substring(210,
		// 214)));
		detalhe.setJurosMora(Uteis.getValorDoubleComCasasDecimais(linha.substring(266, 279)));
		detalhe.setValorLancamento(Uteis.getValorDoubleComCasasDecimais(linha.substring(306, 317)));
		detalhe.setIndicativoCreditoDebito(Uteis.getValorInteiro(linha.substring(317, 318)));
		detalhe.setIndicativoValor(Uteis.getValorInteiro(linha.substring(318, 319)));
		...
		// Método responsável por realizar a Criação da conta receber para o
		// Cliente verificado atualmente.
		// getRegistroArquivoVO().getContaReceberRegistroArquivoVOs().add(executarCriacaoContaReceberRegistroArquivoVOs(detalhe,
		// configuracaoFinanceiroVO, usuarioVO));
		// getRegistroArquivoVO().getContaReceberVOs().add(executarCriacaoContaReceberVOs(detalhe,
		// configuracaoFinanceiroVO, usuarioVO));
		return detalhe;
	}
	...
	private void processarRegistroTrailler(String linha, RegistroTrailerVO trailler) {
		...
		trailler.setQuantidadeTitulosEmCobranca(Uteis.getValorInteiro(linha.substring(18, 25)));
		trailler.setNumeroAvisoBancario(Uteis.getValorInteiro(linha.substring(40, 47)));
		...
		try {
			...
			while ((linha = reader.readLine()) != null) {
				...
				case 0:
					...
					break;
				case 7:
					RegistroDetalheVO detalhe = processarRegistroDetalhe(linha, new RegistroDetalheVO(), configuracaoFinanceiroVO, usuarioVO, getRegistroArquivoVO().getRegistroHeader());
					if (!detalhe.getIdentificacaoTituloEmpresa().substring(0, 1).equals("3")) {
						getRegistroArquivoVO().getRegistroDetalheVOs().add(detalhe);
					} else {
						detalhe.setContaReceberAgrupada(Boolean.TRUE);
                		getRegistroArquivoVO().getRegistroDetalheContaAgrupadaVOs().add(detalhe);
                		List<RegistroDetalheVO> listaRegistroDetalheVOs = realizarCriacaoRegistroDetalheMedianteContaAgrupada(detalhe);
