...
/**
 * job com a finalidade de executar a operação de envio de mensagem recebida do
 * moodle em {@link IntegracaoMoodleRS} no POST /mensagens
 * 
 * @author Felipi Alves
 * @chamado 42530
 */
...
@Component
public class JobOperacaoMensagemMoodle extends SuperControle implements Runnable {
	...
	public void run() {
		if (!JobOperacaoMensagemMoodle.emExecucao) {
			JobOperacaoMensagemMoodle.emExecucao = true;
			Stopwatch stopwatch = new Stopwatch();
			RegistroExecucaoJobVO registroExecucaoJob = new RegistroExecucaoJobVO();
			try {
				stopwatch.start();
				registroExecucaoJob.setNome(JobsEnum.JOB_OPERACAO_MENSAGEM_MOODLE.getName());
				...
				getFacadeFactory().getOperacaoMoodleInterfaceFacade().realizarOperacaoMoodle(TipoOperacaoMoodleEnum.MENSAGENS, registroExecucaoJob);
			} catch (Exception e) {
			...
			JobOperacaoMensagemMoodle.emExecucao = false;
		}
	}
}