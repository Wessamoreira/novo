...
public class ComprovanteRenovacaoMatriculaSV extends SuperRelatorioSV {
    ...
    private void imprimirPDF(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            apresentarRelatorioObjetos(nomeRelatorio, titulo, unidadeEnsinoVO.getNome(), "", "PDF", "", design, "", getComprovanteRenovacaoMatriculaRel().getDescricaoFiltros(), getComprovanteRenovacaoMatriculaRel().criarObjeto(matriculaPeriodo, matricula, null, null), getComprovanteRenovacaoMatriculaRel().getCaminhoBaseRelatorio(), request, response);
        } catch (Exception e) {
            e.getMessage();
        }
    }
