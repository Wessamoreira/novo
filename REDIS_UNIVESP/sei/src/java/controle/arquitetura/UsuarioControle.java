...
//                }           
    ...
    public void scrollerListenerEspecificoUsuarioAlunoProfessorCoordenador(DataScrollEvent DataScrollEvent) throws Exception {
        getControleConsultaOtimizado().setPaginaAtual(DataScrollEvent.getPage());
        getControleConsultaOtimizado().setPage(DataScrollEvent.getPage());
...
//        setListaConsultaPessoa(new ArrayList(0));
    ...
    public void consultarParceiro() {
        try {
            ...
            if (getCampoConsultaParceiro().equals("nome")) {
                objs = getFacadeFactory().getParceiroFacade().consultarPorNome(getValorConsultaParceiro(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado());
            }
    ...
    public void selecionarParceiro() {
        ...
        getUsuarioVO().setParceiro(obj);
        getUsuarioVO().setNome(obj.getNome());
//        setListaConsultaParceiro(new ArrayList(0));
    }

    public void limparUsuario() {
        getUsuarioVO().setNome("");
        getUsuarioVO().setPessoa(new PessoaVO());
        getUsuarioVO().setParceiro(new ParceiroVO());
        getUsuarioVO().getUsuarioPerfilAcessoVOs().clear();
        getListaConsultaPessoa().clear();
    }
    
    public void limparCampoUsuario() {
    	if(getUsuarioVO().getTipoParcerio()) {
    		limparUsuario();
    	}
    }
...
//					return "/visaoAdministrativo/administrativo/homeAdministrador.xhtml";		
    ...
    public String loginComSimulacaoAcesso(UsuarioVO obj) throws Exception {
		...
		try {
			usuarioVO = ControleAcesso.verificarLoginUsuarioSimulacaoVisaoAluno(obj.getPessoa().getCodigo(), false);
