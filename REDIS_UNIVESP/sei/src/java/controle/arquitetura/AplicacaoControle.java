...
public class AplicacaoControle {
	...
	public void limparRegistros() {
	    // 1. Limpa listas locais do ADM (Lógica antiga)
	    if (mapaControleUsuarios != null) mapaControleUsuarios.clear();
	    setMapaControleUsuarios(new ArrayList(0));
	    
	    if (mapaAtividadesUsuarioPorEntidade != null) mapaAtividadesUsuarioPorEntidade.clear();
	    setMapaAtividadesUsuarioPorEntidade(new ArrayList(0));
	    
	    if (mapaAtividadesUsuarioPorUsuario != null) mapaAtividadesUsuarioPorUsuario.clear();
	    setMapaAtividadesUsuarioPorUsuario(new ArrayList(0));
	    
	    // 2. Limpa o Redis usando o Cliente Legado (NOVO)
	    // Isso garante que o Portal do Aluno também pare de ver esses dados
	    try {
	        // Precisamos adicionar um método 'del' no RedisLegacyClient ou usar o invalidarChave
	        // Como o 'invalidarChave' também manda Pub/Sub (o que é bom!), podemos usá-lo.
	        RedisLegacyClient.invalidarChave("app:monitoramento:atividades:por_entidade");
	        RedisLegacyClient.invalidarChave("app:monitoramento:atividades:por_usuario");
	        RedisLegacyClient.invalidarChave("app:monitoramento:sessoes:ativas");
	    } catch (Exception e) {
	        System.err.println("Aviso ADM: Erro ao limpar Redis: " + e.getMessage());
	    }
	    
	    setMensagem("Limpeza de Recursos realizada e Caches invalidados!");
	}
	...
	public void atualizar() {
	    // No ADM, o atualizar apenas chama os métodos de montagem.
	    // Como já alteramos o 'montarLista...' para tentar ler do Redis primeiro,
	    // isso vai puxar os dados mais recentes processados pelo Aluno.
...
//	}
	...
	public List<MapaControleAtividadesUsuarioVO> getMapaControleUsuarios() {
        if (mapaControleUsuarios == null) {
            // 1. Inicializa vazio primeiro (segurança contra NullPointer)
            mapaControleUsuarios = new ArrayList<>();

            // 2. Tenta carregar do Redis (Onde o Portal do Aluno salvou)
            try {
                // CORREÇÃO: O nome do método na classe utilitária é 'lerJson'
                // Certifique-se que o Portal do Aluno está salvando EXATAMENTE nesta chave:
                String json = RedisLegacyClient.lerJson("app:monitoramento:sessoes:ativas");
                
                if (json != null && !json.trim().isEmpty()) {
                    // CORREÇÃO: No legado sem Spring, instanciamos o Gson manualmente
                    Gson gson = new Gson();
                    
                    // CORREÇÃO: Sintaxe correta do TypeToken para Lista Genérica
                    Type tipoLista = new TypeToken<List<MapaControleAtividadesUsuarioVO>>(){}.getType();
                    
                    List<MapaControleAtividadesUsuarioVO> listaRedis = gson.fromJson(json, tipoLista);
                    
                    if (listaRedis != null) {
                        mapaControleUsuarios = listaRedis;
                    }
                }
            } catch (Exception e) { 
                // Log simples para não travar o ADM se o Redis falhar ou o JSON estiver incompatível
                System.err.println("Aviso ADM: Falha ao carregar sessões do Redis: " + e.getMessage());
            }
        }
...
//	
	// =====================================================================
    // MÉTODO 1: ATIVIDADES POR ENTIDADE (ADM)
    // Estratégia: Tenta Redis -> Se falhar, calcula localmente
    // =====================================================================
    public void montarListaMapaAtividadesUsuarioPorEntidade() {
        // --- TENTATIVA 1: BUSCAR DO REDIS (Rápido) ---
        try {
            String json = RedisLegacyClient.lerJson("app:monitoramento:atividades:por_entidade");
            
            if (json != null && !json.trim().isEmpty()) {
                Gson gson = new Gson();
                // Desserializa como Array para garantir compatibilidade
                ControleAtividadeUsuarioVO[] array = gson.fromJson(json, ControleAtividadeUsuarioVO[].class);
                
                if (array != null) {
                    setMapaAtividadesUsuarioPorEntidade(new ArrayList<>(Arrays.asList(array)));
                    return; // Sucesso! Sai do método e economiza CPU.
                }
            }
        } catch (Exception e) {
            // Apenas loga e continua para o fallback. O ADM não pode parar.
            System.err.println("Aviso ADM: Não foi possível ler monitoramento do Redis: " + e.getMessage());
        }

        // --- TENTATIVA 2: FALLBACK (Lógica Original Legada) ---
        // Executa se o Redis estiver vazio ou offline
        try {
            setMapaAtividadesUsuarioPorEntidade(new ArrayList<>(0));
            Hashtable<String, ControleAtividadeUsuarioVO> hash = new Hashtable<>();
            
            List<MapaControleAtividadesUsuarioVO> listaUsuarios = getMapaControleUsuarios();
            
            if (listaUsuarios != null) {
                Iterator<MapaControleAtividadesUsuarioVO> i = listaUsuarios.iterator();
                while (i.hasNext()) {
                    MapaControleAtividadesUsuarioVO mapa = i.next();
                    if (mapa.getAtividadesUsuario() != null) {
                        Iterator<ControleAtividadeUsuarioVO> j = mapa.getAtividadesUsuario().iterator();
                        while (j.hasNext()) {
                            ControleAtividadeUsuarioVO contr = j.next();
                            String key = contr.getEntidade() + "-" + contr.getDescricao() + "-" + contr.getTipo();
                            
                            if (!hash.containsKey(key)) {
                                
                                // ControleAtividadeUsuarioVO clone = (ControleAtividadeUsuarioVO) Uteis.clonar(contr);
                                ...
                                ControleAtividadeUsuarioVO controle = hash.get(key);
                                controle.setMomento(contr.getMomento());
                                controle.setNrOperacoes(controle.getNrOperacoes() + 1);
                            }
                        }
                ...
                while (e.hasMoreElements()) {
                    String controle = e.nextElement();
                    getMapaAtividadesUsuarioPorEntidade().add(hash.get(controle));
                }
            }
        } catch (Exception e) {
            setMapaAtividadesUsuarioPorEntidade(new ArrayList<>(0));
    ...
    // =====================================================================
    // MÉTODO 2: ATIVIDADES POR USUÁRIO (ADM)
    // Estratégia: Tenta Redis -> Se falhar, calcula localmente
    // =====================================================================
    public void montarListaMapaAtividadesUsuarioPorUsuario() {
        // --- TENTATIVA 1: BUSCAR DO REDIS (Rápido) ---
        try {
            String json = RedisLegacyClient.lerJson("app:monitoramento:atividades:por_usuario");
            ...
            if (json != null && !json.trim().isEmpty()) {
                ...
                if (array != null) {
                    setMapaAtividadesUsuarioPorUsuario(new ArrayList<>(Arrays.asList(array)));
                    return; // Sucesso!
                }
            }
        } catch (Exception e) {
            System.err.println("Aviso ADM: Não foi possível ler monitoramento por usuário do Redis: " + e.getMessage());
        }
        ...
        try {
            ...
            // Garante que a lista base está populada (caso o fallback anterior tenha rodado)
            if (getMapaAtividadesUsuarioPorEntidade() == null || getMapaAtividadesUsuarioPorEntidade().isEmpty()) {
                 // Se a lista base estiver vazia, tentamos popular ela primeiro (Fallback Recursivo)
                 montarListaMapaAtividadesUsuarioPorEntidade();
            }

            Iterator<ControleAtividadeUsuarioVO> i = getMapaAtividadesUsuarioPorEntidade().iterator();
            while (i.hasNext()) {
                ControleAtividadeUsuarioVO mapa = i.next();
            ...
            while (e.hasMoreElements()) {
                String controle = e.nextElement();
                getMapaAtividadesUsuarioPorUsuario().add(hash.get(controle));
            }
        } catch (Exception e) {
            setMapaAtividadesUsuarioPorUsuario(new ArrayList<>(0));
            e.printStackTrace();
...
//	}
	 ...
	 public MapaControleAtividadesUsuarioVO obterMapaControleAtividadesUsuarioEspecifico(
	        ...
	        for (MapaControleAtividadesUsuarioVO mapa : getMapaControleUsuarios()) {
	            if (mapa.getUsuarioVO() != null && 
	                usuarioVO.getCodigo().equals(mapa.getUsuarioVO().getCodigo())) {
	                return mapa;
	            }
	        }
	        return null;
...
//	}
	 ...
	 public MapaControleAtividadesUsuarioVO obterMapaControleAtividadesUsuarioEspecifico(
	            UsuarioVO usuarioVO, String idSession) throws Exception {
	        ...
	        for (MapaControleAtividadesUsuarioVO mapa : getMapaControleUsuarios()) {
	            if (idSession.equals(mapa.getIdSessao())) {
	                return mapa;
	            }
	        }
	        return null;
...
//	
	...
	private synchronized ConfiguracaoGeralSistemaVO manterConfiguracaoGeralSistemaEmNivelAplicacao(
        ...
        if (remover) {
            ...
            if (Uteis.isAtributoPreenchido(configuracaoGeralSistemaVO)) {
                ...
                while (i.hasNext()) {
                    ...
                    if (configuracaoVerificarVO.getCodigo().equals(configuracaoGeralSistemaVO.getCodigo())) {
                        ...
                        // [NOVO] Se removeu do ADM, avisa o Aluno para remover do cache também
                        try {
                            util.RedisLegacyClient.invalidarChave("app:configuracaogeral:unidade:" + (unidadeEnsino != null ? unidadeEnsino : 0));
                        } catch (Exception e) { System.err.println("Erro silencioso Redis ADM (Remove): " + e.getMessage()); }
            ...
            if (configuracaoGeralSistemaVO == null && unidadeEnsino == null) {
                ...
                // [NOVO] Limpeza geral
                try {
                    util.RedisLegacyClient.invalidarChave("app:configuracaogeral:unidade:0");
                } catch (Exception e) { /* ignora */ }
            }
            
        } else {
            // --- LÓGICA ORIGINAL DE BUSCA/ATUALIZAÇÃO ---
            Integer codigo = 0;
            if (mapConfiguracaoGeralSistemaVOPorUnidadeEnsino.containsKey(unidadeEnsino == null ? 0 : unidadeEnsino) 
                    && Uteis.isAtributoPreenchido(mapConfiguracaoGeralSistemaVOPorUnidadeEnsino.get(unidadeEnsino == null ? 0 : unidadeEnsino))) {
                codigo = mapConfiguracaoGeralSistemaVOPorUnidadeEnsino.get(unidadeEnsino == null ? 0 : unidadeEnsino);
            } else {
                codigo = getFacadeFactory().getConfiguracaoGeralSistemaFacade().consultarCodigoConfiguracaoReferenteUnidadeEnsino(unidadeEnsino);
            }
            ...
            // Busca do banco (Se não achou na lista em memória)
            configuracaoGeralSistemaVO = getFacadeFactory().getConfiguracaoGeralSistemaFacade()
            ...
            // --- BLOCO ONDE ACONTECE A ATUALIZAÇÃO ---
            if (Uteis.isAtributoPreenchido(configuracaoGeralSistemaVO)) {
                boolean existeAdd = false;
                for (ConfiguracaoGeralSistemaVO configuracaoGeralSistema : listaConfiguracaoGeralSistemaVOAtiva) {
                    if (configuracaoGeralSistemaVO.getCodigo().equals(configuracaoGeralSistema.getCodigo())) {
                        existeAdd = true;
                        // Se já existe, poderíamos atualizar a referência aqui, mas seguindo sua lógica original:
                        break;
                    }                
                }
                ...
                // Aqui detectamos que uma configuração foi carregada/atualizada no ADM.
                // Mandamos o sinal para o Redis limpar o cache do Portal do Aluno.
                try {
                    // Monta a chave padrão que o Portal do Aluno usa
                    String chaveRedis = "app:configuracaogeral:unidade:" + (unidadeEnsino != null ? unidadeEnsino : 0);
                    
                    // Chama nosso cliente utilitário (que usa Jedis puro)
                    util.RedisLegacyClient.invalidarChave(chaveRedis);
                    
                } catch (Exception e) {
                    System.err.println("Erro ao notificar Redis no ADM (não crítico): " + e.getMessage());
                }
            }   
        }
        return configuracaoGeralSistemaVO;
