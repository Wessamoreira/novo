...
public class VisaoCandidatoControle extends SuperControleRelatorio implements Serializable {
	...
	public void inicializarApresentaInscricao() {
		try {
			...
			if (getUnidadeEnsinoVO() != null && getUnidadeEnsinoVO().getCodigo() != null && getUnidadeEnsinoVO().getCodigo() != 0 && getProcessoSeletivoVO() != null && getProcessoSeletivoVO().getCodigo() != null && getProcessoSeletivoVO().getCodigo() != 0 && getUnidadeEnsinoCursoVO() != null && getUnidadeEnsinoCursoVO().getCodigo() != null && getUnidadeEnsinoCursoVO().getCodigo() != 0 && getInscricaoVO() != null && getInscricaoVO().getItemProcessoSeletivoDataProva().getCodigo() > 0) {
				getInscricaoVO().setItemProcessoSeletivoDataProva(getFacadeFactory().getItemProcSeletivoDataProvaFacade().consultarPorChavePrimaria(getInscricaoVO().getItemProcessoSeletivoDataProva().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado()));
				if ((getInscricaoVO().getFormaIngresso().equals("PD") && getInscricaoVO().getProcSeletivo().getUploadComprovantePortadorDiploma()) || (getInscricaoVO().getFormaIngresso().equals("EN") && getInscricaoVO().getProcSeletivo().getUploadComprovanteEnem()) || (getInscricaoVO().getFormaIngresso().equals("TR") && getInscricaoVO().getProcSeletivo().getObrigarUploadComprovanteTransferencia()) ) {
					if (!getArquivoVO().getDescricao().equals("")) {
				...
				if (this.getProcessoSeletivoVO().getNrOpcoesCurso().equals("2")) {
					if (getCursoOpcao2() != null && getCursoOpcao2().getCodigo() != null && getCursoOpcao2().getCodigo() != 0) {
						if ((getInscricaoVO().getFormaIngresso().equals("PD") && getInscricaoVO().getProcSeletivo().getUploadComprovantePortadorDiploma()) || (getInscricaoVO().getFormaIngresso().equals("EN") && getInscricaoVO().getProcSeletivo().getUploadComprovanteEnem()) || (getInscricaoVO().getFormaIngresso().equals("TR") && getInscricaoVO().getProcSeletivo().getObrigarUploadComprovanteTransferencia()) ) {
							if (!getArquivoVO().getDescricao().equals("")) {
								setApresentarInscricao(true);
								setApresentarBotaoEtapa2(Boolean.TRUE);
				...
				if (this.getProcessoSeletivoVO().getNrOpcoesCurso().equals("3")) {
					if (getCursoOpcao3() != null && getCursoOpcao3().getCodigo() != null && getCursoOpcao3().getCodigo() != 0) {
						if ((getInscricaoVO().getFormaIngresso().equals("PD") && getInscricaoVO().getProcSeletivo().getUploadComprovantePortadorDiploma()) || (getInscricaoVO().getFormaIngresso().equals("EN") && getInscricaoVO().getProcSeletivo().getUploadComprovanteEnem()) || (getInscricaoVO().getFormaIngresso().equals("TR") && getInscricaoVO().getProcSeletivo().getObrigarUploadComprovanteTransferencia()) ) {
							if (!getArquivoVO().getDescricao().equals("")) {
								setApresentarInscricao(true);
								setApresentarBotaoEtapa2(Boolean.TRUE);
...
//			montarListaSelectItemCursoOpcao();
	...
	public void emitirComprovanteInscricao() {
		try {
			setComprovanteInscricaoRelControle(null);
			setComprovanteInscricaoRelControle((ComprovanteInscricaoRelControle) context().getExternalContext().getSessionMap().get(ComprovanteInscricaoRelControle.class.getSimpleName()));
			if (getComprovanteInscricaoRelControle() == null) {
				setComprovanteInscricaoRelControle(new ComprovanteInscricaoRelControle());
				context().getExternalContext().getSessionMap().put(ComprovanteInscricaoRelControle.class.getSimpleName(), getComprovanteInscricaoRelControle());
			}
			if (!getInscricaoVO().getCodigo().equals(0)) {
				getComprovanteInscricaoRelControle().setInscricaoVO(getInscricaoVO());
				getComprovanteInscricaoRelControle().imprimirPDF();
			} else {
	...
	public void gravarInscricaoCandidato() throws Exception {
		try {
			if (getInscricaoVO().getFormaIngresso().equals("PD") || getInscricaoVO().getFormaIngresso().equals("EN") ) {
				getInscricaoVO().getItemProcessoSeletivoDataProva().setCodigo(getItemProcessoSeletivoMaiorDataProva());
			}
			getFacadeFactory().getInscricaoFacade().gravarInscricaoCandidato(getInscricaoVO(), getArquivoVO(), getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getInscricaoVO().getUnidadeEnsino().getCodigo()), getUsuarioLogado());
			setApresentarBotaoEtapa4(Boolean.FALSE);
			setApresentarBotaoConcluir3(Boolean.FALSE);
			setApresentarBotaoConcluir4(Boolean.FALSE);
			if (!getInscricaoVO().getContaReceber().equals(0)) {
				getFacadeFactory().getContaReceberFacade().executarRegerarBoletoVencidoInscricaoCandidato(getInscricaoVO(), getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getInscricaoVO().getUnidadeEnsino().getCodigo()), getUsuarioLogado());						
				setApresentarBotaoBoleto(getIsApresentarBotaEmitirBoleto());						
				if(getApresentarBotaoBoleto()){
					montarDadosPagamentoOnline();
				} else {
					setNegociacaoRecebimentoVO(null);
					setPermitirRecebimentoCartaoCreditoOnline(false);
					setConfiguracaoFinanceiroCartaoVOs(null);
				}
			}else{
				setApresentarBotaoBoleto(false);
				setPermitirRecebimentoCartaoCreditoOnline(false);
	...
	public Boolean getApresentarBotaoImprimirBoleto() {
		return Uteis.isAtributoPreenchido(getInscricaoVO()) && !getInscricaoVO().getSituacao().equals("CO") && Uteis.isAtributoPreenchido(getInscricaoVO().getContaReceber()) && !getInscricaoVO().getContaReceberVO().getEmissaoBloqueada();		
	}
	
	public Boolean getApresentarBotaoImprimirBoletoSemRegistroRemessa(){
		
		// Deverá emitir apenas boletos que possuam registro de remessa na tabela controleremessacontareceber.
		try {
		if (getInscricaoVO().getContaReceberVO().getContaCorrenteVO().getBloquearEmitirBoletoSemRegistroRemessa()) {
			return !getFacadeFactory().getControleRemessaContaReceberFacade().verificaContaReceberRegistrada(getInscricaoVO().getContaReceberVO());
		 }
		else {
			return true;
		}
	...
	public void consultarCandidatoPorCPF(String campoConsulta) throws Exception {
		...
		if (!pessoa.getCodigo().equals(0)) {
			...
			if (!getFacadeFactory().getInscricaoFacade().consultarSeExisteInscricaoParaPessoa(pessoa.getCodigo())) {
				...
				if (getInscricaoVO().getItemProcessoSeletivoDataProva().getDataProva().before(new Date())) {
					setPessoaVO(pessoa);
					setInscricaoVO(new InscricaoVO());
					setApresentarBotaoInscricao(Boolean.TRUE);
					setApresentarBotaoAlterarCandidato(Boolean.FALSE);
				} 
				else if(getInscricaoVO().getItemProcessoSeletivoDataProva().getDataLimiteApresentarDadosVisaoCandidato() != null && Uteis.getDataJDBC(new Date()).after(getInscricaoVO().getItemProcessoSeletivoDataProva().getDataLimiteApresentarDadosVisaoCandidato())){
					setPessoaVO(pessoa);
					setInscricaoVO(new InscricaoVO());
					setApresentarBotaoInscricao(Boolean.TRUE);
					setApresentarBotaoAlterarCandidato(Boolean.FALSE);
					...
					setApresentarBotaoInscricao(Boolean.FALSE);
					setApresentarBotaoAlterarCandidato(Boolean.TRUE);
					if (!getInscricaoVO().getContaReceber().equals(0)) {
						getFacadeFactory().getContaReceberFacade().executarRegerarBoletoVencidoInscricaoCandidato(getInscricaoVO(), getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getInscricaoVO().getUnidadeEnsino().getCodigo()), getUsuarioLogado());						
						setApresentarBotaoBoleto(getIsApresentarBotaEmitirBoleto());						
						if(getApresentarBotaoBoleto()){
	...
	public void consultarCandidatoPorCPFParaPreenchimentoComprovanteInscricao(String campoConsulta) throws Exception {
		...
		if (!pessoa.getCodigo().equals(0)) {
			if (!getFacadeFactory().getInscricaoFacade().consultarSeExisteInscricaoParaPessoa(pessoa.getCodigo())) {
				...
				setInscricaoVO(getFacadeFactory().getInscricaoFacade().consultarUltimaInscricaoPessoa(pessoa.getCodigo(), 0,false, Uteis.NIVELMONTARDADOS_TODOS, getUsuarioLogado()));
				if(getInscricaoVO().getItemProcessoSeletivoDataProva().getDataLimiteApresentarDadosVisaoCandidato() != null
						&& Uteis.getDataJDBC(new Date()).after(getInscricaoVO().getItemProcessoSeletivoDataProva().getDataLimiteApresentarDadosVisaoCandidato())){
					
					throw new Exception("Prezado(a) " + getInscricaoVO().getCandidato().getNome() + ", a inscrição de Nº "+ getInscricaoVO().getCodigo() +" não esta mais disponivel para consulta ");
					...
					setApresentarBotaoInscricao(Boolean.FALSE);
					setApresentarBotaoAlterarCandidato(Boolean.TRUE);
					setApresentarBotaoBoleto(Boolean.TRUE);
					setApresentarEtapa3NovaVisao(Boolean.TRUE);
					setApresentarBotaoInscricao(Boolean.FALSE);
					setFormacaoAcademicaVO(getFacadeFactory().getFormacaoAcademicaFacade().consultarPorPessoaEEscolaridade(getPessoaVO().getCodigo(), NivelFormacaoAcademica.MEDIO, false, getUsuarioLogado()));
					setConcluiuEnsinoMedio(getFormacaoAcademicaVO().getCodigo() > 0);
					getFacadeFactory().getContaReceberFacade().executarRegerarBoletoVencidoInscricaoCandidato(getInscricaoVO(), getConfiguracaoFinanceiroPadraoSistema(), getUsuarioLogado());
					}
				}
		} else {
			setInscricaoVO(null);
	...
	public void consultarInscricaoPorChavePrimaria() throws Exception {
		try {
			...
			if (getInscricaoVO().getCodigo() != null && getInscricaoVO().getCodigo() != 0) {
				...
				if(getInscricaoVO().getItemProcessoSeletivoDataProva().getDataLimiteApresentarDadosVisaoCandidato() != null && Uteis.getDataJDBC(new Date()).after(getInscricaoVO().getItemProcessoSeletivoDataProva().getDataLimiteApresentarDadosVisaoCandidato())){
					throw new Exception("Prezado(a) " + getInscricaoVO().getCandidato().getNome() + ", a inscrição de Nº "+ getInscricaoVO().getCodigo() +" não esta mais disponivel para consulta ");
				}else{
					setApresentarDadosDaInscricao(true);
					getFacadeFactory().getContaReceberFacade().executarRegerarBoletoVencidoInscricaoCandidato(getInscricaoVO(), getConfiguracaoFinanceiroPadraoSistema(), getUsuarioLogado());
					if(getIsApresentarBoletoVencido()){
						setMensagemID("msg_Inscricao_contaReceberVencida", Uteis.ALERTA);
					}
					if (Uteis.isAtributoPreenchido(getInscricaoVO().getContaReceber())) {
						consultarContaReceberPessoa();
					}
	...
	public String consultarResultadoPorChavePrimaria() throws Exception {
		try {
			...
			if(getInscricaoVO().getItemProcessoSeletivoDataProva().getDataLimiteApresentarDadosVisaoCandidato() != null && Uteis.getDataJDBC(new Date()).after(getInscricaoVO().getItemProcessoSeletivoDataProva().getDataLimiteApresentarDadosVisaoCandidato())){
				...
				if(getInscricaoVO().getItemProcessoSeletivoDataProva().getDataLiberacaoResultado().compareTo(new Date()) > 0) {				
				throw new Exception("Prezado(a) " + getInscricaoVO().getCandidato().getNome() + ", o resultado do processo seletivo ainda não está disponível. Tente após a data, " + Uteis.getDataComHora(getInscricaoVO().getItemProcessoSeletivoDataProva().getDataLiberacaoResultado()));
				//throw new Exception(UteisJSF.internacionalizar("msg_processoSeletivo_resultadoNaoDisponivel"));
				}
			}
			if (getInscricaoVO().getCodigo() != null && getInscricaoVO().getCodigo() != 0) {
	...
	public void inicializarResultadoProcSeletivoInscricao() throws Exception {
		ResultadoProcessoSeletivoVO resultadoProcessoSeletivo = getFacadeFactory().getResultadoProcessoSeletivoFacade().consultarPorCodigoInscricao_ResultadoUnico(getInscricaoVO().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado());
        if (!resultadoProcessoSeletivo.getCodigo().equals(0)) {
            setMensagem("");
            setMensagemID("");
        }
		this.setResultadoProcessoSeletivoVO(resultadoProcessoSeletivo);
		getResultadoProcessoSeletivoVO().setClassificacao(getFacadeFactory().getEstatisticaProcessoSeletivoRelFacade().verificarClassificacaoCandidado(getInscricaoVO()));
		getResultadoProcessoSeletivoVO().getSituacaoAprovacaoGeralProcessoSeletivo();
		if (!getOcultarChamadaCandidato()) {
			if (getInscricaoVO().getChamada() != null && getInscricaoVO().getChamada().intValue() > 0) {
				getResultadoProcessoSeletivoVO().setDescricaoClassificacaoCand("Convocado em " + getInscricaoVO().getChamada() + "? chamada");
			} else {
				getResultadoProcessoSeletivoVO().setDescricaoClassificacaoCand("N?o Convocado (Aguardar Convoca??o)");
			}
		} else {
			getResultadoProcessoSeletivoVO().setDescricaoClassificacaoCand("");
		}				
	}
	...
	public String getResultadoCandidato() throws Exception {
		if (getResultadoProcessoSeletivoVO().getResultadoPrimeiraOpcao().equals("AP")) {
			setNomeCursoAprovado(getInscricaoVO().getCursoOpcao1().getCurso().getNome());
			return "Candidato Aprovado ";
			// return "Candidato Aprovado para o Curso: ";
		} else if (getResultadoProcessoSeletivoVO().getResultadoSegundaOpcao().equals("AP")) {
			setNomeCursoAprovado(getInscricaoVO().getCursoOpcao2().getCurso().getNome());
			return "Candidato Aprovado  ";
			// return "Candidato Aprovado para o Curso: ";
		} else if (getResultadoProcessoSeletivoVO().getResultadoTerceiraOpcao().equals("AP")) {
			setNomeCursoAprovado(getInscricaoVO().getCursoOpcao3().getCurso().getNome());
			// return "Candidato Aprovado para o Curso: ";
			return "Candidato Aprovado ";
		} else {
			setNomeCursoAprovado("");
			return UteisJSF.internacionalizar("msg_processoSeletivo_candidatoReprovado");
		}
	}

	public List getListaCandidatosPorCurso() throws Exception {
		if (getResultadoProcessoSeletivoVO().getResultadoPrimeiraOpcao().equals("AP")) {
			return getFacadeFactory().getResultadoProcessoSeletivoFacade().consultarPorCodigoECurso(getInscricaoVO().getProcSeletivo().getCodigo(), getInscricaoVO().getCursoOpcao1().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSENTIDADESUBORDINADAS, getUsuarioLogado());
		} else if (getResultadoProcessoSeletivoVO().getResultadoSegundaOpcao().equals("AP")) {
			return getFacadeFactory().getResultadoProcessoSeletivoFacade().consultarPorCodigoECurso(getInscricaoVO().getProcSeletivo().getCodigo(), getInscricaoVO().getCursoOpcao2().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSENTIDADESUBORDINADAS, getUsuarioLogado());
		} else if (getResultadoProcessoSeletivoVO().getResultadoTerceiraOpcao().equals("AP")) {
			return getFacadeFactory().getResultadoProcessoSeletivoFacade().consultarPorCodigoECurso(getInscricaoVO().getProcSeletivo().getCodigo(), getInscricaoVO().getCursoOpcao3().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSENTIDADESUBORDINADAS, getUsuarioLogado());
		} else {
			return null;
		}
	}
	 ...
	 * apresentados no ComboBox correspondente
	 */
	public List consultarCursoPorUnidadeEnsino(Integer codigoPrm) throws Exception {
		List lista = getFacadeFactory().getProcSeletivoCursoFacade().consultarPorCodigoProcSeletivo(getProcessoSeletivoVO().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSENTIDADESUBORDINADAS, getUsuarioLogado());
		return lista;
	}
	...
	public void montarListaSelectItemOpcaoLinguaEstrangeira(String prm) throws Exception {
		List resultadoConsulta = consultarPorOpcaoLinguaEstrangeira();
		setListaSelectItemOpcaolinguaEstrangeira(UtilSelectItem.getListaSelectItem(resultadoConsulta, "codigo", "nome"));
	}
	...
	public List consultarPorOpcaoLinguaEstrangeira() throws Exception {
		...
		List<DisciplinasProcSeletivoVO> listaDisciplina = DisciplinasGrupoDisciplinaProcSeletivo.consultarDisciplinasGrupoDisciplinaProcSeletivosLinguaExtrangeira(getInscricaoVO().getProcSeletivo().getCodigo(), getUsuarioLogado());
		Iterator i = listaDisciplina.iterator();
		while (i.hasNext()) {
			DisciplinasProcSeletivoVO p = (DisciplinasProcSeletivoVO) i.next();
			if (p.getDisciplinaIdioma().booleanValue()) {
				lista.add(p);
			}
		}
		return lista;
	...
	public void montarListaSelectItemDataProva() {
		try {
			if (getProcessoSeletivoVO().getCodigo() != 0) {
				List<ItemProcSeletivoDataProvaVO> resultadoConsulta = getFacadeFactory().getItemProcSeletivoDataProvaFacade().consultarPorProcSelectivoAptoInscricao(getProcessoSeletivoVO().getCodigo(), true, false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado());
				setItemProcessoSeletivoMaiorDataProva(getFacadeFactory().getItemProcSeletivoDataProvaFacade().consultarMaiorDatas(getProcessoSeletivoVO().getCodigo()).getCodigo());
				List objs = new ArrayList(0);
				objs.add(new SelectItem("", ""));
				...
				if (!getApresentarDataProva() && Uteis.isAtributoPreenchido(resultadoConsulta)) {
	...
	public String getBoleto() throws Exception {
		if (getIsApresentarBotaEmitirBoleto()){			
			return "../BoletoBancarioSV?codigoContaReceber=" + getInscricaoVO().getContaReceber() + "&titulo=inscricao";
		}
		return "";
	}
	 ...
	 *            the resultadoProcessoSeletivoVO to set
	 */
	...
	public boolean getIsInscricaoComLinguaEstrangeira() {
		if (getInscricaoVO().getOpcaoLinguaEstrangeira().getCodigo() != 0) {
			return true;
		}
		return false;
	}
	...
	public ComprovanteInscricaoRelControle getComprovanteInscricaoRelControle() {
		return comprovanteInscricaoRelControle;
	}

	public void setComprovanteInscricaoRelControle(ComprovanteInscricaoRelControle comprovanteInscricaoRelControle) {
		this.comprovanteInscricaoRelControle = comprovanteInscricaoRelControle;
	}

	public boolean getApresentarBotaoEmitiBoleto() {
		return getIsApresentarBotaEmitirBoleto();
	}
	...
	public boolean getIsApresentarBotaEmitirBoleto() {
		return !getInscricaoVO().getSituacao().equals("CO") && Uteis.isAtributoPreenchido(getInscricaoVO().getContaReceber()) 
				&& getInscricaoVO().getContaReceberVO().getValor() > 0
				&& (Uteis.getDataJDBC(getInscricaoVO().getContaReceberVO().getDataVencimento()).compareTo(Uteis.getDataJDBC(new Date())) > 0
					|| Uteis.getData(getInscricaoVO().getContaReceberVO().getDataVencimento()).equals(Uteis.getData(new Date())));
	}
	
	public boolean getIsApresentarBoletoVencido() {
		return !getInscricaoVO().getSituacao().equals("CO") && Uteis.isAtributoPreenchido(getInscricaoVO().getContaReceber()) 
				&& getInscricaoVO().getContaReceberVO().getValor() > 0
				&& Uteis.getDataJDBC(getInscricaoVO().getContaReceberVO().getDataVencimento()).compareTo(Uteis.getDataJDBC(new Date())) < 0
				&& !Uteis.getData(getInscricaoVO().getContaReceberVO().getDataVencimento()).equals(Uteis.getData(new Date()));
	...
	public boolean getIsApresentarBoletoRecebido() {
		return getInscricaoVO().getSituacao().equals("CO") && (getInscricaoVO().getContaReceberVO().getSituacao().equals("RE") || getInscricaoVO().getContaReceberVO().getSituacao().equals("RE"));
	}
	...
	public void consultarContaReceberPessoa() {
		try {
			...
			getContaReceberVOs().add(getFacadeFactory().getContaReceberFacade().consultarPorChavePrimaria(getInscricaoVO().getContaReceber(), false, Uteis.NIVELMONTARDADOS_TODOS, getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getInscricaoVO().getUnidadeEnsino().getCodigo()), getUsuarioLogado()));
			for (ContaReceberVO obj : getContaReceberVOs()) {
				if(!obj.getSituacao().equals("RE")) {
					...
					contaReceberNegociacaoRecebimentoVO.setValorTotal(obj.getCalcularValorFinal(getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getInscricaoVO().getUnidadeEnsino().getCodigo()), getUsuarioLogado()));
					getNegociacaoRecebimentoVO().adicionarObjContaReceberNegociacaoRecebimentoVOs(contaReceberNegociacaoRecebimentoVO);
					getNegociacaoRecebimentoVO().setValorTotal(getNegociacaoRecebimentoVO().getValorTotal() + obj.getValorReceberCalculado());					
				} 
			}
			setPermitirRecebimentoCartaoCreditoOnline(getFacadeFactory().getConfiguracaoFinanceiroCartaoFacade().verificarExistenciaConfiguracaoFinanceiroCartaoPorCodigoConfiguracaoFinanceiro(getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getInscricaoVO().getUnidadeEnsino().getCodigo()).getCodigo(), getNegociacaoRecebimentoVO().getValorTotal(), "permitirecebimentocartaoonline"));
	...
	public void validarNumeroCartaoCredito() {
		try {
			FormaPagamentoNegociacaoRecebimentoVO formaPagamentoNegociacaoRecebimentoVO = (FormaPagamentoNegociacaoRecebimentoVO) context().getExternalContext().getRequestMap().get("formaPagamentoItens");
			formaPagamentoNegociacaoRecebimentoVO.setOperadoraCartaoVO(getFacadeFactory().getOperadoraCartaoFacade().consultarPorCodigoConfiguracaoFinanceiroCartao(formaPagamentoNegociacaoRecebimentoVO.getOperadoraCartaoVO().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado()));
	...
	public void realizarRecebimentoCartaoCredito() {
		try {
			...
			ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO =  getFacadeFactory().getConfiguracaoGeralSistemaFacade().consultarPorCodigoConfiguracoes(getNegociacaoRecebimentoVO().getConfiguracaoRecebimentoCartaoOnlineVO().getConfiguracaoFinanceiroVO().getConfiguracoesVO().getCodigo(), false, null, Uteis.NIVELMONTARDADOS_DADOSBASICOS);
			...
			getFacadeFactory().getNegociacaoRecebimentoFacade().realizarRecebimentoCartaoCreditoIncricao(getNegociacaoRecebimentoVO(), getInscricaoVO().getCandidato().getCodigo(), getContaReceberVOs().get(0).getUnidadeEnsino(), getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getInscricaoVO().getUnidadeEnsino().getCodigo()), usuarioVO);
			imprimirComprovanteRecebimento();
			setModalPagamentoOnline("RichFaces.$('panelPagamento').hide();");	
			setPagamentoRealizadoComSucesso(true);
			setInscricaoVO(getFacadeFactory().getInscricaoFacade().consultarPorChavePrimaria(getInscricaoVO().getCodigo(), Uteis.NIVELMONTARDADOS_TODOS, null));
	...
	public void validarMesCartaoCredito() {
		try {
			FormaPagamentoNegociacaoRecebimentoVO formaPagamentoNegociacaoRecebimentoVO = (FormaPagamentoNegociacaoRecebimentoVO) context().getExternalContext().getRequestMap().get("formaPagamentoItens");
			if(formaPagamentoNegociacaoRecebimentoVO.getFormaPagamentoNegociacaoRecebimentoCartaoCreditoVO().getMesValidade() > 12) {
				throw new Exception(UteisJSF.internacionalizar("msg_PagamentoOnline_mesInvalido"));				
			} else {
				setMensagemID("msg_PagamentoOnline_mesValido");
			}
		} catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage(), Uteis.ERRO);
		}
	...
	public void consultarConfiguracaoCartaoPermiteRecebimentoOnline() {
		try {
			...
			setConfiguracaoRecebimentoCartaoOnlineVO(getFacadeFactory().getConfiguracaoRecebimentoCartaoOnlineFacade().consultarConfiguracaoRecebimentoCartaoOnlineDisponivel(0, getInscricaoVO().getCursoOpcao1().getCurso().getCodigo(), "", getInscricaoVO().getUnidadeEnsino().getCodigo(), Uteis.NIVELMONTARDADOS_TODOS, null));
	...
	public void verificarContasRecbimentoOnline() {
		try {
			if(!getConfiguracaoRecebimentoCartaoOnlineVO().getCodigo().equals(0)) {
				...
				getFacadeFactory().getConfiguracaoRecebimentoCartaoOnlineFacade().verificarContasRecebimentoOnline(getNegociacaoRecebimentoVO().getContaReceberNegociacaoRecebimentoVOs(), getConfiguracaoRecebimentoCartaoOnlineVO(), consistirException, false, false, false, false, false, false, getUsuarioLogado());
	...
	public void realizarRecebimentoBoletoBancario() {
		try {
			List<BoletoBancarioRelVO> boletoBancarioRelVOs = getFacadeFactory().getBoletoBancarioRelFacade().emitirRelatorioLista(false, getInscricaoVO().getContaReceber(), "", "", "", "", 0, 0, null, null, 0, "", 0, getUsuarioLogado(), "boletoAluno", 0, getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getInscricaoVO().getUnidadeEnsino().getCodigo()), 0, false, null, null);
