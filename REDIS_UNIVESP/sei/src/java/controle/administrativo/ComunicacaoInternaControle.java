...
//			}
	...
	public String novoCRM(InteracaoWorkflowVO obj) throws Exception {
		registrarAtividadeUsuario(getUsuarioLogado(), "ComunicacaoInternaControle", "Nova Comunicação Interna", "Novo");
		...
		// definirApresentacao();
		setInformarEmailManualmente(false);
		// getComunicacaoInternaVO().getPessoa().setNome(obj.getProspect().getNome());
		getComunicacaoInternaVO().setComunicadoInternoDestinatarioVOs(obterListaDestinatarios(obj));
		setApresentarBotaoResponder(Boolean.FALSE);
		getComunicacaoInternaVO().setEnviarEmail(getLoginControle().getPermissaoAcessoMenuVO().getPermitirComunicacaoInternaEnviarCopiaPorEmail());
		setMensagemID("msg_entre_dados");
		return "editar";
	...
	public List<ComunicadoInternoDestinatarioVO> obterListaDestinatarios(InteracaoWorkflowVO obj) {
		List<ComunicadoInternoDestinatarioVO> listDestinatario = new ArrayList<ComunicadoInternoDestinatarioVO>();
		if (!obj.getProspect().getEmailPrincipal().trim().equals("")) {
			ComunicadoInternoDestinatarioVO destinatario = new ComunicadoInternoDestinatarioVO();
			destinatario.setCiJaLida(Boolean.FALSE);
			destinatario.getDestinatario().setNome(obj.getProspect().getNome());
			destinatario.getDestinatario().setEmail(obj.getProspect().getEmailPrincipal());
			destinatario.setEmail(obj.getProspect().getEmailPrincipal());
			destinatario.setNome(obj.getProspect().getNome());			
	...
	public void realizarUpload(FileUploadEvent uploadEvent) {
		...
		try {
			// if (uploadEvent.getUploadedFile().getName().contains(".jpg") ||
			// uploadEvent.getUploadedFile().getName().contains(".png") ||
			// uploadEvent.getUploadedFile().getName().contains(".gif")) {
	...
	public void gravarRecadoProfessor() {
		try {
			...
			setApresentarRichMensagem("RichFaces.$('panelMsgEnviada').show()");
...
//			}
	...
	public String editarComunicadoTopoVisao() {
		...
		if(getUsuarioLogado().getIsApresentarVisaoProfessor()) {			
			...
			return Uteis.getCaminhoRedirecionamentoNavegacao("recadoProfessor.xhtml");
		}
...
//		}
	...
	public String getAcaoModalMensagemMaketingOuLeituraObrigatoria(){
		if(getAbrirRichModalMarketing()){
			return "RichFaces.$('panelMarketing').show(); RichFaces.$('panelMensagem').hide();";
		}
		if(getAbrirRichModalMensagem()){
			return "RichFaces.$('panelMensagem').show(); RichFaces.$('panelMarketing').hide();";
		}
		return "RichFaces.$('panelMarketing').hide(); RichFaces.$('panelMensagem').hide();";
	}
...
//	}
	...
	public void montarListaSelectItemAreaConhecimento() {
		try {
			...
			// System.out.println("Erro montarListaSelectItemAreaConhecimento: " + e.getMessage());
		}
...
//	}
	...
	public List consultarProfessorPorAluno() throws Exception {
		...
		List lista = getFacadeFactory().getPessoaFacade().consultarProfessoresDoAluno(getRealizarValidacaoParaObterQualSeraUsuarioCorrente(getUsuarioLogado()).getPessoa().getCodigo(), getUnidadeEnsinoLogado().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, getConfiguracaoFinanceiroPadraoSistema(), getUsuarioLogado());
...
//	}
	...
	public void consultarAlunoPorMatricula() {
		try {
			...
			if (!getValorConsultaAluno().equals("")) {
				obj = getFacadeFactory().getMatriculaFacade().consultarAlunoPorMatricula(getValorConsultaAluno(), "AL", this.getUnidadeEnsinoLogado().getCodigo(), false, Uteis.NIVELMONTARDADOS_TODOS, getConfiguracaoFinanceiroPadraoSistema(), getUsuarioLogado());
			} else {
			}
	...
	public String getIsFecharModalTurma() {
		if (getFecharModalTurma()) {
			return "RichFaces.$('panelTurma').hide()";
		}
		return "";
	}
...
//	}
	...
	public UIDataScroller getComponente() {
		FacesContext context = FacesContext.getCurrentInstance();
		UIComponent htmlDataScroll = context.getViewRoot().findComponent("form:controlaLista");
		return (UIDataScroller) htmlDataScroll;
	}
...
//		}
	...
	public String getAbrirModalAnoSemestre() {
		if (getApresentarModalAnoSemestre()) {
			return "RichFaces.$('panelAnoSemestre').show();";
		}
		return "";
	}
	...
	public String getFecharModalAnoSemestre() {
		if (!getApresentarModalAnoSemestre()) {
			return "RichFaces.$('panelAnoSemestre').hide();";
		}
		return "";
	}
	...
	public Boolean verificarUsuarioPossuiPermissaoNovo(String identificadorAcaoPermissao) {
		...
		try {
			//ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico(identificadorAcaoPermissao, getUsuarioLogado());
			ComunicacaoInterna.incluir(identificadorAcaoPermissao, true, getUsuarioLogado());
			liberar = Boolean.TRUE;
		} catch (Exception e) {
			liberar = Boolean.FALSE;
		}
	...
	public Boolean getPermitirResponderComunicadoInternoSomenteLeitura() {
		if(permitirResponderComunicadoInternoSomenteLeitura == null){
			if(!getUsuarioLogado().getIsApresentarVisaoAdministrativa()){
				try{
					ControleAcesso.verificarPermissaoFuncionalidadeUsuario("ComunicacaoInterna_permitirResponderComunicadoInternoSomenteLeitura", getUsuarioLogado());
	...
	public void scrollListenerVisaoAluno(DataScrollEvent DataScrollEvent) {
		getControleConsultaOtimizado().setPaginaAtual(DataScrollEvent.getPage());
		getControleConsultaOtimizado().setPage(DataScrollEvent.getPage());
		try {
			consultarTodasEntradasAluno();
	...
	public void scrollListenerVisaoProfessor(DataScrollEvent DataScrollEvent) {
		getControleConsultaOtimizado().setPaginaAtual(DataScrollEvent.getPage());
		getControleConsultaOtimizado().setPage(DataScrollEvent.getPage());
		try {
			consultarTodasEntradasProfessor();
	...
	public void scrollListenerVisaoCoordenador(DataScrollEvent DataScrollEvent) {
		getControleConsultaOtimizado().setPaginaAtual(DataScrollEvent.getPage());
		getControleConsultaOtimizado().setPage(DataScrollEvent.getPage());
		try {
			consultarTodasEntradasCoordenador();
	...
	public void scrollListenerItemsSaida (DataScrollEvent DataScrollEvent) {
		getControleConsultaOtimizado().setPaginaAtual(DataScrollEvent.getPage());
		getControleConsultaOtimizado().setPage(DataScrollEvent.getPage());
		try {
			consultar();
...
//		}
	...
	public void adicionarTodosAlunosCursoTurmaAnoSemestre() {
		try {
			...
			if (!getTrazerApenasAlunosNaoRenovaram()) {
				...
				getFacadeFactory().getPessoaFacade().consultaRapidaAlunoPorCursoTurmaAnoSemestreSitacaoAcademica(getComunicacaoInternaVO().getUnidadeEnsino().getCodigo().intValue(), getListaPessoaDestinatario(), getComunicacaoInternaVO(), getComunicadoInternoDestinatarioVO(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado(), getFiltroRelatorioAcademicoVO(), getNivelEducacional(), getIdentificadorSalaAulaBlackboard(), getPercentualIntegralizacaoCurricularInicial(), getPercentualIntegralizacaoCurricularFinal(), getProgramacaoFormaturaVO(), getBimestre(), getTipoDeficiencia(), getDiaSemana(), getDisciplinaVO());
			} else {
	...
	public String getIsFecharModalFiltroAlunoAnoSemestre() {
		if (Uteis.isAtributoPreenchido(getComunicacaoInternaVO().getAno()) && getIsApresentarAno() && Uteis.isAtributoPreenchido(getComunicacaoInternaVO().getComunicadoInternoDestinatarioVOs() )) {
	        ...
	        limparCurso();
	        limparDadosTurma();
	        setFiltroRelatorioAcademicoVO(null);
			getComunicacaoInternaVO().setSemestre("");
			return "";
		...
		return "RichFaces.$('panelAlunoAnoSemestre').hide()";
	}
...
//		setTipoDeficiencia(TipoDeficiencia.NAO_DECLARADO);
	...
	public void paginarConsultaDestinatarios(DataScrollEvent DataScrollEvent){
		try {
			getComunicacaoInternaVO().getListaConsultaDestinatario().setPaginaAtual(DataScrollEvent.getPage());
			getComunicacaoInternaVO().getListaConsultaDestinatario().setPage(DataScrollEvent.getPage());
	...
	public List validarDataConsultaProgramacaoFormatura(List objs) throws Exception {
	   ...
	   public void realizarInicializacaoProgressBar(Boolean verificarTamanhoMensagem) {
			try {
				...
				if((Uteis.getQuantidadeMemoriaStringEmMega(comunicacaoInternaVO.getMensagem())) > getConfiguracaoGeralSistemaVO().getTamanhoMaximoCorpoMensagem() && verificarTamanhoMensagem
					...
					setOncompleteModal("RichFaces.$('panelConfirmacaoEnvio').show()");
					return;
				}
	   ...
	   public void scrollerListenerDestinatario(DataScrollEvent dataScrollerEvent) {
			try {
		...
		public void realizarUploadPlanilhaDestinatario (FileUploadEvent uploadEvent) {
			try {
				...
				if(Uteis.isAtributoPreenchido(getComunicacaoInternaVO().getErrosProcessamentoPlanilha())) {
					setOncompleteModal("RichFaces.$('panelErrosProcessamentoPlanilha').show()");
				}
			} catch (Exception e) {
				setOncompleteModal("");
				setMensagemDetalhada("msg_erro", e.getMessage());
