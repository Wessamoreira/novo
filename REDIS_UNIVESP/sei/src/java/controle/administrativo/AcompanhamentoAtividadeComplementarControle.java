...
public class AcompanhamentoAtividadeComplementarControle extends SuperControle implements Serializable {
	...
	public void consultarRegistroAtividadeComplementarPorAnoSemestre() {
		try {
			...
			this.setListaRegistroAtividadeComplementarMatriculaVOs(getFacadeFactory().getAcompanhamentoAtividadeComplementarFacade().consultar(0, 0, getCampoAno(), getCampoSemestre(), "", getVisaoAlunoControle().getMatricula().getMatricula(), 0, null, false, getVisaoAlunoControle().getMatricula().getGradeCurricularAtual().getCodigo(), getControleConsultaOtimizado(), getDataInicio(), getDataFim(), this.getUsuarioLogado()));
	...
	public void limparTurma() throws Exception {
		...
		this.setCampoAno(Constantes.EMPTY);
		this.setCampoSemestre(Constantes.EMPTY);
	}
		...
		try {
			...
			if (getCampoConsultaCurso().equals("nome")) {
				...
				if (getValorConsultaCurso().equals("")) {
					throw new IllegalArgumentException("O campo deve estar preenchido.");
				}
				if (!getValorConsultaCurso().matches("\\d+")) {
					throw new IllegalArgumentException("O campo deve conter apenas números.");
				}
				int valorInt = Integer.parseInt(getValorConsultaCurso());
				objs = getFacadeFactory().getCursoFacade().consultaRapidaPorCodigoEUnidadeDeEnsino(new Integer(valorInt), getUnidadeEnsinoLogado().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSMINIMOS, getUsuarioLogado());
			}
	...
	public void gravar() {
		try {
			...
			Ordenacao.ordenarListaDecrescente(getListaConsultaRegistroAtividadeComplementarMatriculaAguardandoDeferimentoVOs(), "registroAtividadeComplementar.data");
			novo();
			setOncompleteModal("RichFaces.$('panelIncluirAtividade').hide();");
		...
		if (permiteAlunoIncluirAtividade == null) {
			try {
				...
				if(Uteis.isAtributoPreenchido(getVisaoAlunoControle()) && (getUsuarioLogado().getIsApresentarVisaoAluno()|| getUsuarioLogado().getIsApresentarVisaoPais())){
					ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVO("AtividadeComplementarAluno_permitirIncluirAtividade", getUsuarioLogado());				
	...
	public List<SelectItem> getListaSelectItemSituacaoAtividadeComplementarMatricula() {
		if (listaSelectItemSituacaoAtividadeComplementarMatricula == null) {
			...
			listaSelectItemSituacaoAtividadeComplementarMatricula.add(new SelectItem(SituacaoAtividadeComplementarMatriculaEnum.DEFERIDO, SituacaoAtividadeComplementarMatriculaEnum.DEFERIDO.getValorApresentar()));
		}
	...
	public void realizarIndeferimentoAtividadeComplementarMatricula() {
		try {
			...
			realizarAtualizacaoInformacaoListaConsulta();
			setOncompleteModal("RichFaces.$('panelIndeferir').hide();");
			setMensagemID("msg_dados_deferido",  Uteis.SUCESSO);
		}catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage(), Uteis.ERRO);
		}
	...
	public void realizarAtualizacaoInformacaoListaConsulta() throws Exception {
		for(RegistroAtividadeComplementarMatriculaVO registroAtividadeComplementarMatriculaVO: getListaRegistroAtividadeComplementarMatriculaVOs()) {
			if(registroAtividadeComplementarMatriculaVO.getMatriculaVO().getMatricula().equals(getAtividadeComplementarMatricula().getMatriculaVO().getMatricula())) {
				List<RegistroAtividadeComplementarMatriculaVO> registroAtividadeComplementarMatriculaVOs = getFacadeFactory().getAcompanhamentoAtividadeComplementarFacade().consultar(null, null, "", "", "", getAtividadeComplementarMatricula().getMatriculaVO().getMatricula(), null, null, false, getAtividadeComplementarMatricula().getMatriculaVO().getGradeCurricularAtual().getCodigo(), getControleConsultaOtimizado(), getDataInicio(), getDataFim(), getUsuarioLogado());
	...
	public Boolean getPermitiDeferir() {
		if (permitiDeferir == null) {
			try {
				ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVO("RegistroAtividadeComplementarMatricula_permiteDeferir", getUsuarioLogado());
				permitiDeferir = true;
			}catch (Exception e) {
	...
	public Boolean getPermitiIndeferir() {
		if (permitiIndeferir == null) {
			try {
				ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVO("RegistroAtividadeComplementarMatricula_permiteIndeferir", getUsuarioLogado());
	...
	public void realizarAlteracaoCHAtividadeComplementarMatricula() {
		try {
			...
			consultar();
			setMensagemID("msg_dados_gravados", Uteis.SUCESSO);
			setOncompleteModal("RichFaces.$('panelEditarChConsiderada').hide(); RichFaces.$('panelDeferir').show(); ");
		}catch (Exception e) {
			setOncompleteModal("");
			setMensagemDetalhada("msg_erro", e.getMessage(), Uteis.ERRO);
		}
	...
	public Boolean getPermiteEditarCHConsiderada() {		
		if (permiteEditarCHConsiderada == null) {
			try {
				ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVO("RegistroAtividadeComplementarMatricula_permitirEditarCHConsiderada", getUsuarioLogado());
