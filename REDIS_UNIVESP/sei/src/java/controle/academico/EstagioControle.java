...
import negocio.comuns.bancocurriculum.enumeradores.AtividadeProfissionalExercicioEnum;
...
//	}
	...
	public void validarDadosAdicionarEstagio(UnidadeEnsinoVO unidadeEnsino, EstagioVO estagio, MatriculaVO matricula) throws Exception {		
		...
		try {
			...
			if (listaEstagioConcomitantes != null) {
				...
				setResultadoValidacao("RichFaces.$('panelErroCargaHorariaMaxima').show()");
	...
	public void consultarParceiro() {
		try {
			...
			setListaConsultaParceiro(new ArrayList(0));
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
	}
	...
	public void selecionarParceiro() {
		try {
			...
			obj = getFacadeFactory().getParceiroFacade().consultarPorChavePrimaria(obj.getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado());

			if (!obj.getConveniadaParaVagasEstagio()) {
				throw new Exception("Esta empresa selecionada não está definida como convêniada para estágio. É necessário alterar o cadastro da Empresa/Parceiro habilitando-a como Conveniada para Estágio.");
			}
			// anteriormente utiliza-se somente o campo nome para empresa.
			// depois passou-se a adotar um vinculo com o cadastro de parceiro
			// Assim, a partir do momento que seleciona-se um parceiro, então
			// zera-se o campo
			// nome antigo. Ficando somente o dado atual preenchido.
			setResultadoConsultaParceiro("RichFaces.$('panelParceiro').hide(); setFocus(formCadastro,'formCadastro:ano');");
			setListaConsultaParceiro(null);
			setMensagemID("msg_dados_selecionados");
		} catch (Exception e) {
			setResultadoConsultaParceiro("");
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
	}
	...
	/**
	 * @param listaConsultaAreaProfissional
	 *            the listaConsultaAreaProfissional to set
	 */
	...
	public void consultarAreaProfissional() {
		try {
			...
			setListaConsultaAreaProfissional(new ArrayList(0));
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
	}
	...
	public void selecionarAreaProfissional() {
		try {
			AreaProfissionalVO obj = (AreaProfissionalVO) context().getExternalContext().getRequestMap().get("areaProfissional");
	...
	public void imprimirTextoPadraoEstagioHtml() {
		try {
			...
			this.setCaminhoRelatorio(getFacadeFactory().getImpressaoDeclaracaoFacade().imprimirDeclaracao(textoPadraoImprimirVO.getCodigo(), impressaoContratoVO, impressaoContratoVO, textoPadraoImprimirVO.getTipo(), impressaoContratoVO.getMatriculaPeriodoVO().getTurma(), impressaoContratoVO.getDisciplinaVO(), getConfiguracaoGeralPadraoSistema(), getConfiguracaoFinanceiroPadraoSistema(), getUsuarioLogado(), true));
	...
	public void imprimirTextoPadraoEstagioDOC() {
		try {
			...
			String arquivo = (getFacadeFactory().getImpressaoDeclaracaoFacade().imprimirDeclaracao(textoPadraoImprimirVO.getCodigo(), impressaoContratoVO, impressaoContratoVO, textoPadraoImprimirVO.getTipo(), impressaoContratoVO.getMatriculaPeriodoVO().getTurma(), impressaoContratoVO.getDisciplinaVO(), getConfiguracaoGeralPadraoSistema(),getConfiguracaoFinanceiroPadraoSistema(), getUsuarioLogado(), true));
	...
	public void liberarGravarEstagioSemValidacaoDados() {
		...
		try {
			usuarioVerif = ControleAcesso.verificarLoginUsuario(this.getNomeUsuarioLiberar(), this.getSenhaUsuarioLiberar(), true, Uteis.NIVELMONTARDADOS_TODOS);
			usuarioValido = true;
		} catch (Exception e) {
		}
		boolean usuarioTemPermissaoLiberar = false;
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("Estagio_liberarValidacao", usuarioVerif);
	...
	public void montarListaSelectItemAreaProfissional(Integer parceiro, Integer estagio) {
		getListaSelectItemAreaProfissional().clear();
		List<AreaProfissionalVO> listaAreaProfissionalVOs = getFacadeFactory().getAreaProfissionalFacade().consultarPorCodigoEmpresaNivelComboBox(parceiro, estagio, getUsuarioLogado());
		List<SelectItem> itens = new ArrayList<SelectItem>(0);
		itens.add(new SelectItem("", ""));
		for (AreaProfissionalVO areaProfissionalVO : listaAreaProfissionalVOs) {
			itens.add(new SelectItem(areaProfissionalVO.getCodigo(), areaProfissionalVO.getDescricaoAreaProfissional()));
		}
		setListaSelectItemAreaProfissional(itens);
	}
	...
	public void verificarPermissaoImpressaoContratoObrigatorio() throws Exception {
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("Estagio_permitirImpressaoContratoEstagioObrigatorio", getUsuarioLogado());
	...
	public void verificarPermissaoImpressaoContratoNaoObrigatorio() throws Exception {
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("Estagio_permitirImpressaoContratoEstagioNaoObrigatorio", getUsuarioLogado());
	...
	public List<SelectItem> getListaSelectItemAtividadeProfissionalExcercicio() {
		return AtividadeProfissionalExercicioEnum.getListaSelectItemSituacaoResultadoProcessoSeletivo();
	}
	...
	public void verificarPermitirInformarSeguradoraEstagio() throws Exception {
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("Estagio_permitirInformarSeguradoraEstagio", getUsuarioLogado());
