...
import relatorio.controle.academico.AlunoBaixaFrequenciaRelControle;
...
import relatorio.controle.academico.HorarioDaTurmaRelControle;
import relatorio.controle.academico.PerfilTurmaRelControle;
import relatorio.controle.arquitetura.SuperControleRelatorio;
import relatorio.negocio.comuns.arquitetura.enumeradores.TipoRelatorioEnum;

@Controller("VisaoCoordenadorControle")
@Scope("session")
@Lazy
public class VisaoCoordenadorControle extends SuperControleRelatorio implements Serializable {

	private static final long serialVersionUID = 2547998241935069489L;
	...
	private Boolean uploadArquivosCoordenador;
	...
	private Boolean menuHorarioTurma;
	private Boolean menuSenha;
	private Boolean menuMeritoAcademico;
	...
	private Boolean menuCronogramaAula;

	private Boolean menuAlunoBaixaFrequencia;

	
	private Integer qtdeAtualizacaoRequerimentoCoordenador;
	private Boolean menuFrequenciaAluno;
	private Boolean menuPlanoEnsino;

	private Boolean visualizarMenuRelatorioSEIDecidir;
	private Boolean menuRelatorioSEIDecidirAtividadeExtraClasse;
	...
	public VisaoCoordenadorControle() {
...
//			inicializarBanner();
			inicializarRelatorioSEIDEcidir();
			inicializarMenuRegistroAula();
			isPermiteConfiguracaoSeiGsuite();
			verificarPrimeiroAcesso();
			consultarConfiguracaoDashboard();
			consultarQtdeAtualizacaoRequerimentoCoordenador();
	...
	public String inicializarMenuBiblioteca() {
		...
		setMenuSenha(Boolean.TRUE);
		setMensagemDetalhada("", "");
		return "buscaBibliotecaProfessor";
	}
		...
		if (getRelatorioSEIDecidirVO().getUnidadeEnsinoVOs().size() > 1) {
			...
			if (!getRelatorioSEIDecidirVO().getUnidadeEnsinoVOs().isEmpty()) {
				if (getRelatorioSEIDecidirVO().getUnidadeEnsinoVOs().get(0).getFiltrarUnidadeEnsino()) {
					getRelatorioSEIDecidirVO().setUnidadeEnsinoApresentar(
							getRelatorioSEIDecidirVO().getUnidadeEnsinoVOs().get(0).getNome());
	...
	public String editarDadosPessoa() throws Exception {
		try {
			registrarAtividadeUsuario(getUsuarioLogado(), "VisaoCoordenadorControle",
					"Inicializando Editar Dados Pessoa", "Editando");
			...
			registrarAtividadeUsuario(getUsuarioLogado(), "VisaoCoordenadorControle", "Finalizando Editar Dados Pessoa",
					"Editando");
			setMensagemID("msg_dados_editar");
			return Uteis.getCaminhoRedirecionamentoNavegacao("dadosPessoaisCoordenador.xhtml");
		} catch (Exception e) {
			setMensagemDetalhada(MSG_TELA.msg_erro.name(), e.getMessage());
		}
		return Uteis.getCaminhoRedirecionamentoNavegacao("dadosPessoaisCoordenador.xhtml");
	...
	public void montarListaSelectItemLayoutRelatorioSeiDecidir() {
		try {
			...
			List<LayoutRelatorioSEIDecidirVO> layoutRelatorioSEIDecidirVOs = getFacadeFactory()
					.getLayoutRelatorioSEIDecidirInterfaceFacade()
					.consultarPorModuloENivelDetalhamento(RelatorioSEIDecidirModuloEnum.RECURSOS_HUMANOS,
							RelatorioSEIDecidirNivelDetalhamentoEnum.ATIVIDADE_EXTRA_CLASSE_PROFESSOR,
			...
			for (LayoutRelatorioSEIDecidirVO layoutRelatorioSEIDecidirVO : layoutRelatorioSEIDecidirVOs) {
				getListaSelectItemLayoutRelatorioSeiDecidir().add(new SelectItem(
						layoutRelatorioSEIDecidirVO.getCodigo(), layoutRelatorioSEIDecidirVO.getDescricao()));

			}

		} catch (Exception e) {
			setMensagemDetalhada(MSG_TELA.msg_erro.name(), e.getMessage(), Uteis.SUCESSO);
		}
	}
	...
	public void gravarDadosPessoa() {
		try {
			executarValidacaoSimulacaoVisaoCoordenador();
			getFacadeFactory().getPessoaFacade().setIdEntidade("Coordenador");
	...
	/**
	 * Metodo que remover as fotos da pata temporaria no servidor, Deixando somente
	 * a pasta base com o numero do CPF
	 * 
	 * @throws Exception
	 */
	...
	public boolean permiteGerarRelatorioSEIDecidirAtividadeExtraClasse() {
		try {
			return ControleAcesso.verificarPermissaoFuncionalidadeUsuario(
					"RelatorioAtividadeExtraClasseProfessorVisaoCoordenador", getUsuarioLogado());
		} catch (Exception e) {
			setMensagemDetalhada(MSG_TELA.msg_erro.name(), e.getMessage());
			return false;
		}
	...
	public void inicializarMenuMeusHorarios() {
		...
		setMenuFrequenciaAluno(Boolean.FALSE);
	...
	public void inicializarMenuRelatorioSEIDecidir() {
		setMenuRecado(Boolean.FALSE);
		setMenuDisciplina(Boolean.FALSE);
		setMenuRequisicao(Boolean.FALSE);
		setMenuRelatorio(Boolean.FALSE);
		...
		setMenuSenha(Boolean.FALSE);
		setConfiguracao(Boolean.FALSE);
		setMenuRelatorioSEIDecidirAtividadeExtraClasse(Boolean.TRUE);
		setVisualizarMenuRelatorioSEIDecidir(Boolean.FALSE);
	...
	public String inicializarMenuConfiguracao() throws Exception {
		registrarAtividadeUsuario(getUsuarioLogado(), "VisaoCoordenadorControle", "Inicializando Menu Configuração",
				"Inicializando");
		...
		setLoginAnterior("");
		setSenhaAnterior("");
		return Uteis.getCaminhoRedirecionamentoNavegacao("configuracaoCoordenador.xhtml");
	}
	...
	public String inicializarMenuDisciplina() throws Exception {
		registrarAtividadeUsuario(getUsuarioLogado(), "VisaoCoordenadorControle", "Inicializando Menu Disciplina",
				"Inicializando");
		inicializarDisciplinaConsultar();
		setMensagemDetalhada("", "");
		return "disciplinaCoordenador";
	}

	public String inicializarMenuRequisicao() {
		inicializarRequisicaoConsultar();
		setMensagemDetalhada("", "");
		return Uteis.getCaminhoRedirecionamentoNavegacao("requisicaoCoordenador.xhtml");
	}
	...
	public String inicializarMenuRelatorio() throws Exception {
		registrarAtividadeUsuario(getUsuarioLogado(), "VisaoCoordenadorControle", "Inicializando Menu Relatório",
				"Inicializando");
		...
		setMenuHorarioTurma(Boolean.FALSE);
		setMenuCronogramaAula(Boolean.FALSE);
		setMenuAlunoBaixaFrequencia(Boolean.FALSE);
		setMenuMeritoAcademico(Boolean.FALSE);
		setApresentarIconeRelatorio(Boolean.TRUE);
		setMenuRelatorioSEIDecidirAtividadeExtraClasse(Boolean.FALSE);
		setVisualizarMenuRelatorioSEIDecidir(Boolean.TRUE);
		setMenuFrequenciaAluno(Boolean.FALSE);
		setMenuPlanoEnsino(Boolean.FALSE);
		setMenuRelatorioAulasRegistradasNaoRegistradas(Boolean.FALSE);
		setMensagemDetalhada("", "");
		return Uteis.getCaminhoRedirecionamentoNavegacao("relatorioCoordenador.xhtml");
	}

	public String inicializarMenuRegistroAtividadeComplementar() throws Exception {
		registrarAtividadeUsuario(getUsuarioLogado(), "VisaoCoordenadorControle",
				"Inicializando Menu Registro Atividade Complementar", "Inicializando");
		setMensagemDetalhada("", "");
		return Uteis.getCaminhoRedirecionamentoNavegacao("registroAtividadeComplementarConsCoordenador.xhtml");
	}

	public String inicializarMenuEstagio() throws Exception {
		registrarAtividadeUsuario(getUsuarioLogado(), "VisaoCoordenadorControle", "Inicializando Menu Estágio",
				"Inicializando");
		setMensagemDetalhada("", "");
		return Uteis.getCaminhoRedirecionamentoNavegacao("estagioCoordenadorCons.xhtml");
	}

	public String inicializarMenuAcompanhamentoAtividadeComplementar() throws Exception {
		registrarAtividadeUsuario(getUsuarioLogado(), "VisaoCoordenadorControle",
				"Inicializando Menu Acompanhamento Atividade Complementar", "Inicializando");
		setMensagemDetalhada("", "");
		return Uteis.getCaminhoRedirecionamentoNavegacao("acompanhamentoAtividadeComplementarCoordenador.xhtml");
	}

	public String inicializarMenuUploadProfessorRelatorio() throws Exception {
		setMenuRelatorio(Boolean.FALSE);
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
		setMenuHistoricoAluno(Boolean.FALSE);
		...
		setMenuRelatorioAulasRegistradasNaoRegistradas(Boolean.FALSE);
		setMensagemDetalhada("", "");
		return Uteis.getCaminhoRedirecionamentoNavegacao("uploadProfessorRelVisaoCoordenador.xhtml");
	}

	public void inicializarMenuDiario() {
		...
		setMenuPlanoEnsino(Boolean.FALSE);
		setMenuRelatorioAulasRegistradasNaoRegistradas(Boolean.FALSE);
		executarMetodoControle(DiarioRelControle.class.getSimpleName(), "montarListaSelectItemTurmaVisaoCoordenador");
	}

	public void inicializarMenuEspelhoDiario() {
		setMenuDiario(Boolean.FALSE);
		...
		setMenuPlanoEnsino(Boolean.FALSE);
		setMenuRelatorioAulasRegistradasNaoRegistradas(Boolean.FALSE);
		executarMetodoControle(EspelhoRelControle.class.getSimpleName(), "montarListaSelectItemTurmaVisaoCoordenador");
	}

	public void inicializarMenuHistoricoAluno() {
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
	...
	public void inicializarMenuHistoricoTurma() {
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
		setMenuHistoricoAluno(Boolean.FALSE);
		...
		setMenuPlanoEnsino(Boolean.FALSE);
		setMenuRelatorioAulasRegistradasNaoRegistradas(Boolean.FALSE);
		executarMetodoControle(HistoricoTurmaRelControle.class.getSimpleName(),
				"montarListaSelectItemTurmaVisaoCoordenador");
	}

	public void inicializarMenuDownloads() {
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
		setMenuHistoricoAluno(Boolean.FALSE);
		setMenuHistoricoTurma(Boolean.FALSE);
		...
		setMenuPlanoEnsino(Boolean.FALSE);
		setMenuRelatorioAulasRegistradasNaoRegistradas(Boolean.FALSE);
		executarMetodoControle(DownloadRelControle.class.getSimpleName(), "montarListaSelectItemTurmaVisaoCoordenador");
	}

	public void inicializarMenuPerfilTurma() {
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
		setMenuHistoricoAluno(Boolean.FALSE);
		setMenuHistoricoTurma(Boolean.FALSE);
		...
		setMenuPlanoEnsino(Boolean.FALSE);
		setMenuRelatorioAulasRegistradasNaoRegistradas(Boolean.FALSE);
		executarMetodoControle(PerfilTurmaRelControle.class.getSimpleName(),
				"montarListaSelectItemTurmaVisaoCoordenador");
	}

	public void inicializarMenuHorarioTurma() {
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
		setMenuHistoricoAluno(Boolean.FALSE);
		setMenuHistoricoTurma(Boolean.FALSE);
		setMenuDownload(Boolean.FALSE);
		setMenuPerfilTurma(Boolean.FALSE);
		setMenuHorarioTurma(Boolean.TRUE);
		setApresentarIconeRelatorio(Boolean.FALSE);
		setMenuFrequenciaAluno(Boolean.FALSE);
		setVisualizarMenuRelatorioSEIDecidir(Boolean.FALSE);
		setMenuPlanoEnsino(Boolean.FALSE);
		setMenuRelatorioAulasRegistradasNaoRegistradas(Boolean.FALSE);
		executarMetodoControle(HorarioDaTurmaRelControle.class.getSimpleName(),
				"montarListaSelectItemTurmaVisaoCoordenador");
	}

	public void inicializarMenuCronogramaAula() {
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
		setMenuHistoricoAluno(Boolean.FALSE);
		setMenuHistoricoTurma(Boolean.FALSE);
		...
		setMenuPerfilTurma(Boolean.FALSE);
		setMenuHorarioTurma(Boolean.FALSE);
		setMenuCronogramaAula(Boolean.TRUE);
		setMenuAlunoBaixaFrequencia(Boolean.FALSE);
		setMenuMeritoAcademico(Boolean.FALSE);
		setApresentarIconeRelatorio(Boolean.FALSE);
		setMenuFrequenciaAluno(Boolean.FALSE);
	...
	public void inicializarMenuMeritoAcademico() {
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
		setMenuHistoricoAluno(Boolean.FALSE);
		setMenuHistoricoTurma(Boolean.FALSE);
		...
		setMenuCronogramaAula(Boolean.FALSE);
		setMenuAlunoBaixaFrequencia(Boolean.FALSE);
		setMenuMeritoAcademico(Boolean.TRUE);
		setApresentarIconeRelatorio(Boolean.FALSE);
		setMenuFrequenciaAluno(Boolean.FALSE);
		setVisualizarMenuRelatorioSEIDecidir(Boolean.FALSE);
		setMenuPlanoEnsino(Boolean.FALSE);
	...
	public void inicializarMenuFrequenciaAluno() {
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
		setMenuHistoricoAluno(Boolean.FALSE);
		setMenuHistoricoTurma(Boolean.FALSE);
		...
		setMenuMeritoAcademico(Boolean.FALSE);
		setApresentarIconeRelatorio(Boolean.FALSE);
		setMenuFrequenciaAluno(Boolean.TRUE);
		setVisualizarMenuRelatorioSEIDecidir(Boolean.FALSE);
		setMenuPlanoEnsino(Boolean.FALSE);
		setMenuRelatorioAulasRegistradasNaoRegistradas(Boolean.FALSE);
		// executarMetodoControle(HorarioDaTurmaRelControle.class.getSimpleName(),
	...
	public void inicializarMenuPlanoEnsino() {
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
		setMenuHistoricoAluno(Boolean.FALSE);
		setMenuHistoricoTurma(Boolean.FALSE);
		...
		setMenuFrequenciaAluno(Boolean.FALSE);
		setVisualizarMenuRelatorioSEIDecidir(Boolean.FALSE);
		setMenuPlanoEnsino(Boolean.TRUE);
		setMenuRelatorioAulasRegistradasNaoRegistradas(Boolean.FALSE);
		// executarMetodoControle(HorarioDaTurmaRelControle.class.getSimpleName(),
		// "montarListaSelectItemTurmaVisaoCoordenador");
	}

	public void inicializarMenuAlunoBaixaFrequenciaAluno() {
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
		setMenuHistoricoAluno(Boolean.FALSE);
		setMenuHistoricoTurma(Boolean.FALSE);
		...
		setMenuHorarioTurma(Boolean.FALSE);
		setMenuCronogramaAula(Boolean.FALSE);
		setMenuAlunoBaixaFrequencia(Boolean.TRUE);
		setMenuMeritoAcademico(Boolean.FALSE);
		setApresentarIconeRelatorio(Boolean.FALSE);
		setMenuFrequenciaAluno(Boolean.FALSE);
		setMenuPlanoEnsino(Boolean.FALSE);
		setMenuRelatorioAulasRegistradasNaoRegistradas(Boolean.FALSE);
		executarMetodoControle(AlunoBaixaFrequenciaRelControle.class.getSimpleName(),
				"montarListaSelectItemTurmaVisaoCoordenador");
	}
	
	public void inicializarMenuRelatorioAulasRegistradasNaoRegistradas() {
		setMenuDiario(Boolean.FALSE);
		setMenuEspelhoDiario(Boolean.FALSE);
		setMenuHistoricoAluno(Boolean.FALSE);
		setMenuHistoricoTurma(Boolean.FALSE);
	...
	public void inicializarUploadArquivosCoordenador() {
		setUploadArquivosCoordenador(Boolean.TRUE);
		setMensagemDetalhada("", "");
	}
	...
	public void inicializarRelatorioSEIDEcidir() {
		if (permiteGerarRelatorioSEIDecidirAtividadeExtraClasse()) {
			setVisualizarMenuRelatorioSEIDecidir(Boolean.TRUE);
		}
		setMensagemDetalhada("", "");
	}

	public void verificarPermissaoParaGravarDadosPessoaisVisaoCoordenador() {
		try {
			if (getUsuarioLogado().getVisaoLogar().equals("coordenador")) {
				ControleAcesso.incluir("Coordenador", getUsuarioLogado());
	...
	public void limparDadosFuncionario() {
		try {
			getRelatorioSEIDecidirVO().setFuncionarioCargoVO(new FuncionarioCargoVO());
			getRelatorioSEIDecidirVO().setCursoVO(new CursoVO());
	...
	public void selecionarFuncionarioCargo() {
		try {
			...
			getRelatorioSEIDecidirVO().setFuncionarioCargoVO(obj);
	...
	/**
	 * Consulta responsavel por retornar os usuarios do popup de pesquisa do
	 * funcionario
	 */
	...
	/*
	 * public void montarListaSelectItemCurso() { setListaSelectItemCurso(new
	 * ArrayList<>()); List<CursoVO> cursoVOs = getFacadeFactory().getCursoFacade().
	 * consultarCursoAtividadeExtraClasseProfessor(getFuncionarioCargo().getCodigo()
	 * ); if (Uteis.isAtributoPreenchido(cursoVOs)) {
	 * getListaSelectItemCurso().add(new SelectItem("", "")); for (CursoVO cursoVO :
	 * cursoVOs) { getListaSelectItemCurso().add(new SelectItem(cursoVO.getCodigo(),
	 * cursoVO.getNome())); } } }
	 */

	public void montarListaSelectItemCurso() {
		setListaSelectItemCurso(new ArrayList<>());
		List<CursoVO> cursoVOs = getFacadeFactory().getCursoFacade().consultarCursoAtividadeExtraClasseProfessor(
				getRelatorioSEIDecidirVO().getFuncionarioCargoVO().getCodigo());
		if (Uteis.isAtributoPreenchido(cursoVOs)) {
	...
	public boolean apresentarBotaoPlanilha() {
		return getRelatorioSEIDecidirVO().getLayoutRelatorioSEIDecidirVO().getTipoGeracaoRelatorio()
				.equals(TipoRelatorioEnum.EXCEL) ? true : false;
	}

	public boolean apresentarBotaoPDF() {
		return getRelatorioSEIDecidirVO().getLayoutRelatorioSEIDecidirVO().getTipoGeracaoRelatorio()
				.equals(TipoRelatorioEnum.PDF) ? true : false;
	}
	...
	public void realizarGeracaoRelatorio(TipoRelatorioEnum tipoRelatorio) {
		try {
			...
			String caminhoRelatorio = getFacadeFactory().getRelatorioSeiDecidirFacade()
					.realizarGeracaoRelatorioSeiDecidir(getRelatorioSEIDecidirVO(), tipoRelatorio,
							getLogoPadraoRelatorio(), false, ";", "xlsx", true, getUsuarioLogado(),
							getConfiguracaoGeralSistemaVO(), null);
			this.setCaminhoRelatorio(caminhoRelatorio);
			limparMensagem();
		} catch (Exception e) {
	...
	public Boolean getUploadArquivosCoordenador() {
		return uploadArquivosCoordenador;
	}

	public void setUploadArquivosCoordenador(Boolean uploadArquivosCoordenador) {
		this.uploadArquivosCoordenador = uploadArquivosCoordenador;
	}
	...
	public Boolean getMenuHorarioTurma() {
		if (menuHorarioTurma == null) {
			menuHorarioTurma = Boolean.FALSE;
		}
		return menuHorarioTurma;
	}

	public void setMenuHorarioTurma(Boolean menuHorarioTurma) {
		this.menuHorarioTurma = menuHorarioTurma;
	}

	public Boolean getPermiteGravarVisaoCoordenador() {
		if (permiteGravarVisaoCoordenador == null) {
			verificarPermissaoParaGravarDadosPessoaisVisaoCoordenador();
		}
	...
	public Boolean getMenuCronogramaAula() {
		return menuCronogramaAula;
	}

	public void setMenuCronogramaAula(Boolean menuCronogramaAula) {
		this.menuCronogramaAula = menuCronogramaAula;
	}

	public Boolean getMenuAlunoBaixaFrequencia() {
		return menuAlunoBaixaFrequencia;
	}

	public void setMenuAlunoBaixaFrequencia(Boolean menuAlunoBaixaFrequencia) {
		this.menuAlunoBaixaFrequencia = menuAlunoBaixaFrequencia;
	}

	public Boolean getMenuMeritoAcademico() {
		return menuMeritoAcademico;
	}

	public void setMenuMeritoAcademico(Boolean menuMeritoAcademico) {
		this.menuMeritoAcademico = menuMeritoAcademico;
	}

	public String inicializarMenuAdvertencia() throws Exception {
		registrarAtividadeUsuario(getUsuarioLogado(), "VisaoCoordenadorControle", "Inicializando Menu Advertencia",
				"Inicializando");
		setMensagemDetalhada("", "");
...
//						+ "/visaoCoordenador/matriculaOnlineVisaoCoordenadorForm.xhtml?banner=" + politica.getCodigo()
//						+ "\" style=\"display:block;height:290px;width:100%\"  >");
//				html.append(" 	<img id=\"bannerMat"+politica.getCodigo()+"\" src=\"").append(url).append("\" height=\"290px\" title=\"" + politica.getNome() + "\"  >");
//				html.append("</a>");
//				html.append("</div>");
//			}
//			setCaminhoArquivoBanner(html.toString());
	...
	/**
	 * @return the qtdeAtualizacaoRequerimentoCoordenador
	 */
	public Integer getQtdeAtualizacaoRequerimentoCoordenador() {
		if (qtdeAtualizacaoRequerimentoCoordenador == null) {
			qtdeAtualizacaoRequerimentoCoordenador = 0;
		}
		return qtdeAtualizacaoRequerimentoCoordenador;
	}

	/**
	 * @param qtdeAtualizacaoRequerimentoProfessor the
	 *                                             qtdeAtualizacaoRequerimentoCoordenador
	 *                                             to set
	 */
	public void setQtdeAtualizacaoRequerimentoCoordenador(Integer qtdeAtualizacaoRequerimentoCoordenador) {
		this.qtdeAtualizacaoRequerimentoCoordenador = qtdeAtualizacaoRequerimentoCoordenador;
	}

	
	public void consultarQtdeAtualizacaoRequerimentoCoordenador() {
		try {
			setQtdeAtualizacaoRequerimentoCoordenador(getFacadeFactory().getRequerimentoFacade()
					.consultarQtdeRequerimentoAlunoVisaoProfessorCoordenador(getUsuarioLogadoClone()));
		} catch (Exception e) {
			setQtdeAtualizacaoRequerimentoCoordenador(0);
		}
	}
	...
	public Boolean getMenuFrequenciaAluno() {
		if (menuFrequenciaAluno == null) {
			menuFrequenciaAluno = Boolean.FALSE;
		}
		return menuFrequenciaAluno;
	}

	public void setMenuFrequenciaAluno(Boolean menuFrequenciaAluno) {
		this.menuFrequenciaAluno = menuFrequenciaAluno;
	}

	public Boolean getMenuPlanoEnsino() {
		if (menuPlanoEnsino == null) {
			menuPlanoEnsino = Boolean.FALSE;
		}
		return menuPlanoEnsino;
	}

	public void setMenuPlanoEnsino(Boolean menuPlanoEnsino) {
		this.menuPlanoEnsino = menuPlanoEnsino;
	}

	public Boolean getMenuRelatorioSEIDecidirAtividadeExtraClasse() {
		if (menuRelatorioSEIDecidirAtividadeExtraClasse == null) {
			menuRelatorioSEIDecidirAtividadeExtraClasse = Boolean.FALSE;
		}
		return menuRelatorioSEIDecidirAtividadeExtraClasse;
	}

	public void setMenuRelatorioSEIDecidirAtividadeExtraClasse(Boolean menuRelatorioSEIDecidirAtividadeExtraClasse) {
		this.menuRelatorioSEIDecidirAtividadeExtraClasse = menuRelatorioSEIDecidirAtividadeExtraClasse;
	}

	public Boolean getVisualizarMenuRelatorioSEIDecidir() {
		if (visualizarMenuRelatorioSEIDecidir == null) {
			visualizarMenuRelatorioSEIDecidir = Boolean.FALSE;
		}
		return visualizarMenuRelatorioSEIDecidir;
	}

	public void setVisualizarMenuRelatorioSEIDecidir(Boolean visualizarMenuRelatorioSEIDecidir) {
		this.visualizarMenuRelatorioSEIDecidir = visualizarMenuRelatorioSEIDecidir;
	}
	...
	public void alterarSenhaPrimeiroAcesso() {
		try {
			...
			setApresentarModalResetarSenha(Boolean.FALSE);
			setOncompleteModal("RichFaces.$('panelPrimeiroAcesso').hide()");
	...
	public void consultarConfiguracaoDashboard() {
		try {
			List<DashboardVO> dashboards = getFacadeFactory().getDashboardInterfaceFacade()
					.consultarDashboardPorUsuarioAmbiente(getUsuarioLogado(), TipoVisaoEnum.COORDENADOR);
			if (dashboards != null) {
	...
	public Boolean getMenuRelatorioAulasRegistradasNaoRegistradas() {
		if (menuRelatorioAulasRegistradasNaoRegistradas == null) {
			menuRelatorioAulasRegistradasNaoRegistradas = false;
		}
