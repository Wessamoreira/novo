...
public class VisaoAlunoControle extends SuperControleRelatorio implements Serializable, ActionListener {
	...
	private CurriculumPessoaVO curriculumPessoaVO;
...
//		}
	...
	public String getDownloadContrato() {
		...
		if (getApresentarModalDocumentoAssinado()) {
			retorno = "RichFaces.$('panelDocumentoContratoAssinado').show()";			
...
//			stopwatch.start();
			setHistoricoVOs(getFacadeFactory().getHistoricoFacade().executarMontagemListaHistoricoAluno(getMatricula(), 0, getConfiguracaoAcademico(), getMatricula().getCurso(), getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getMatricula().getUnidadeEnsino().getCodigo()), getFilterAnoSemestre(), true, true, false, getUsuarioLogado()));
...
//				setMatriculaPeriodoVO(getFacadeFactory().getMatriculaPeriodoFacade().consultaRapidaBasicaUltimaMatriculaPeriodoPorMatricula(getMatricula().getMatricula(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado()));
//				try {
//					ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComPerfilUsuarioVOEspecifico("MarcarReposicaoAula", getUsuarioLogado());
...
//					getMatricula().setSalaAulaBlackboardVO(getFacadeFactory().getSalaAulaBlackboardFacade().consultarSalaAulaBlackboardPorMatriculaPorTipoSalaBlackboard(getMatricula().getMatricula(), TipoSalaAulaBlackboardEnum.ESTAGIO, getUsuarioLogadoClone()));
	...
	public String realizarAcessoEstudoOnline(String telaAvancar) {
		try {
			...
			setModalPanelErroAcessoConteudoEstudo("RichFaces.$('panelErroAcessoConteudoEstudo').show()");
			return "";
		}
	...
	public void consultarMatriculaPeriodoTurmaDisciplinaOnline() {
		try {
			if(getAplicacaoControle().getMatriculaPeriodoTurmaDisciplinasCache().containsKey(getMatricula().getMatricula())) {
				...
				setListaMatriculaPeriodoTurmaDisciplinaVOs(getFacadeFactory().getMatriculaPeriodoTurmaDisciplinaFacade().consultarDisciplinaDoAlunoPorMatricula(getMatricula().getMatricula(), getFiltroAnoSemestreTelaInicial(), getLoginControle().getPermissaoAcessoMenuVO(), null, true, Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado()));
			}
...
//			comunicacaoInternaControle.setQtdMensagemCaixaEntradaAluno(getFacadeFactory().getComunicacaoInternaFacade().consultaRapidaComunicacaoInternaNaoLidasVisaoAluno(getAlunoPorResponsavelLegal().getPessoa().getCodigo()));
	...
	public String inicializarMenuMinhasNotasDestinoContrato() throws Exception {
		...
		setMatriculaPeriodoContratoVOs(getFacadeFactory().getMatriculaPeriodoFacade().consultarMatriculaPeriodosComContrato(getMatricula().getMatricula(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, getConfiguracaoFinanceiroPadraoSistema(), getUsuarioLogado(), true));
	...
	public void adicionarDadosComerciais() {
		try {
			...
			Integer numeroDadosComeciais = getPessoaVO().getDadosComerciaisVOs().size();
			getFacadeFactory().getDadosComerciaisFacade().adicionarObjDadosComerciaisVOs(getDadosComerciaisVO(), getPessoaVO());
			if (getPessoaVO().getDadosComerciaisVOs().size() > numeroDadosComeciais) {
				getFacadeFactory().getDadosComerciaisFacade().setIdEntidade("Pessoa");
				getFacadeFactory().getDadosComerciaisFacade().incluirDadosComerciais(getPessoaVO(), getPessoaVO().getDadosComerciaisVOs(), true);
			}
	...
	public void removerAreaProfissional() throws Exception {
		AreaProfissionalInteresseContratacaoVO obj = (AreaProfissionalInteresseContratacaoVO) context().getExternalContext().getRequestMap().get("apicItens");
		getFacadeFactory().getAreaProfissionalInteresseContratacaoFacade().excluirObjAreaProfissionalVOs(obj, getPessoaVO());
		getFacadeFactory().getAreaProfissionalInteresseContratacaoFacade().setIdEntidade("Pessoa");
		getFacadeFactory().getAreaProfissionalInteresseContratacaoFacade().alterarAreaProfissionalInteresseContratacao(getPessoaVO().getCodigo(), getPessoaVO().getAreaProfissionalInteresseContratacaoVOs(), getUsuarioLogado());
		setMensagemID("msg_dados_excluidos");
	}
...
//		return qtdeAtualizacaoDuvidaProfessor;
	...
	public void uploadArquivoCurriculum(FileUploadEvent upload) {
		try {
			getFacadeFactory().getPessoaFacade().uploadArquivoCurriculum(upload, getPessoaVO(), getCurriculumPessoaVO(), getConfiguracaoGeralPadraoSistema(), getUsuarioLogado());
		...
		if (apresentarBotaoNovoRequerimento == null) {
			...
			try {
				if (!getConfiguracaoGeralSistemaPrivilegiandoACfgDaUnidade().getPermitiAlunoPreMatriculaSolicitarRequerimento()) {
					MatriculaPeriodoVO matPer = getFacadeFactory().getMatriculaPeriodoFacade().consultaUltimaMatriculaPeriodoPorMatriculaConsultaBasica(getMatricula().getMatricula(), false, Uteis.NIVELMONTARDADOS_TODOS, getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(), getUsuarioLogado());
...
//		}
	...
	public void verificarApresentarGravarDadosPessoais() throws Exception {
		try {
			Pessoa.incluir(Pessoa.getIdEntidade(), true, getUsuarioLogado());
	...
	public void verificarApresentarAbaDadosPessoais() throws Exception {
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("MeusDadosAbaDadosBasicos", getUsuarioLogado());
	...
	public void verificarApresentarAbaDadosFormacaoAcademica() throws Exception {
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("MeusDadosAbaFormacaoAcademica", getUsuarioLogado());
	...
	public void verificarApresentarAbaExperienciasProfissionais() throws Exception {
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("MeusDadosAbaExperienciasProfissionais", getUsuarioLogado());
	...
	public void verificarApresentarAbaFormacaoExtraCurricular() throws Exception {
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("MeusDadosAbaFormacaoExtraCurricular", getUsuarioLogado());
...
//			montarListaSelectItemCursoAluno();
	...
	public void verificarApresentarAbaQualificaoesInformacoesComplementares() throws Exception {
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("MeusDadosAbaQualificaoesInformacoesComplementares", getUsuarioLogado());
	...
	public void verificarApresentarAbaOjetivo() throws Exception {
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("MeusDadosAbaObjetivo", getUsuarioLogado());
	...
	public void verificarApresentarAbaQuestionario() throws Exception {
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("MeusDadosAbaQuestionario", getUsuarioLogado());
...
//		}
	...
	public void realizarNavegacaoClassroomPorAgenda() {
		try {
			AgendaAlunoRSVO agendaAlunoRSVO = (AgendaAlunoRSVO) context().getExternalContext().getRequestMap().get("agendaAlunoRSVOItem");
			if (getCalendarioDataEventoRsVO().getListaPessoaGsuiteVO().size() > 0) {
				getFacadeFactory().getClassroomGoogleFacade().realizarVerificacaoSeAlunoEstaVinculadoAoClassroomGoogle(agendaAlunoRSVO.getGoogleMeetVO().getClassroomGoogleVO(), getCalendarioDataEventoRsVO().getListaPessoaGsuiteVO().get(0), getUsuarioLogadoClone());
			}
		} catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
	...
	public void verificarAlertarAlunoPendenciaDocumentacaoMatriculaLogar() {
		try {
			...
			if (Uteis.isAtributoPreenchido(getUsuarioLogado()) && ControleAcesso.verificarPermissaoFuncionalidadeUsuario("EntregaDocumentoAlertarAlunoPendenciaDocumentacaoMatriculaLogar", getUsuarioLogado()) && getMatricula().getSituacao().equals(SituacaoVinculoMatricula.ATIVA.getValor()) && getFacadeFactory().getDocumetacaoMatriculaFacade().consultarSeExistemDocumentosPendentesAluno(getMatricula().getMatricula(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, false, getUsuarioLogado())) {
	...
	public void verificarDocumentoContratoAssinadoPendente() {
		try {
			...
			if(Uteis.isAtributoPreenchido(getUsuarioLogado()) 
				...
				List<DocumentoAssinadoVO> listaDocumento = getFacadeFactory().getDocumentoAssinadoFacade().verificarGeracaoDocumentoAssinado(getUsuarioLogado().getPessoa().getCodigo(), getMatricula(), getMatriculaPeriodoVO(), getMatricula().getUnidadeEnsino(), getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getMatricula().getUnidadeEnsino().getCodigo()), getConfiguracaoGeralPadraoSistema(), Uteis.NIVELMONTARDADOS_TODOS, getUsuarioLogado());
...
//				}
	...
	public Boolean getPermitirUploadAnexo() {
		if (permitirUploadAnexo == null) {
			try {
				permitirUploadAnexo = ControleAcesso.verificarPermissaoFuncionalidadeUsuario("EntregaDocumentoPermiteAnexarArquivo", getUsuarioLogado());
			} catch (Exception e) {
	...
	public String visualizarEntregaRelatorioFinalFacilitador() {
		try {
			removerControleMemoriaFlashTela("RelatorioFinalFacilitadorControle");
			MatriculaPeriodoTurmaDisciplinaVO obj = (MatriculaPeriodoTurmaDisciplinaVO) context().getExternalContext().getRequestMap().get("matriculaPeriodoTurmaDisciplinaItem");
			context().getExternalContext().getSessionMap().put("matriculaPeriodoTurmaDisciplina", obj);
			if(obj.getApresentarConsultarRelatoriofinalfacilitador()) {
	...
	public void renderizarRelatoriofinalfacilitador() {
		try {
			for(MatriculaPeriodoTurmaDisciplinaVO matriculaPeriodoTurmaDisciplinaVO : getListaMatriculaPeriodoTurmaDisciplinaVOs()) {
				if(getFacadeFactory().getRelatorioFinalFacilitadorInterfaceFacade().realizarVerificacaoFacilitador(getUsuarioLogado().getPessoa().getCodigo(), matriculaPeriodoTurmaDisciplinaVO.getAno(), matriculaPeriodoTurmaDisciplinaVO.getSemestre())) {
					boolean calendarioRelatorioFinalFacilitadorVO = getFacadeFactory().getCalendarioRelatorioFinalFacilitadorInterfaceFacade().verificarCalendarioEmAbertoPorDisciplinaAnoSemestre(
					matriculaPeriodoTurmaDisciplinaVO.getDisciplina(), matriculaPeriodoTurmaDisciplinaVO.getAno(), matriculaPeriodoTurmaDisciplinaVO.getSemestre(),new Date());
					boolean relatorioFinalFacilitador = getFacadeFactory().getRelatorioFinalFacilitadorInterfaceFacade().validarRelatorioFinalFacilitador(matriculaPeriodoTurmaDisciplinaVO);
					if(calendarioRelatorioFinalFacilitadorVO && !relatorioFinalFacilitador) {
						matriculaPeriodoTurmaDisciplinaVO.setApresentarRelatoriofinalfacilitador(Boolean.TRUE);			
					}
					if(calendarioRelatorioFinalFacilitadorVO && relatorioFinalFacilitador) {
						matriculaPeriodoTurmaDisciplinaVO.setApresentarConsultarRelatoriofinalfacilitador(Boolean.TRUE);			
					}
				}
			}
		} catch (Exception e) {
