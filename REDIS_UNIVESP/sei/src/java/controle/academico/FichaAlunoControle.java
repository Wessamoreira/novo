...
import negocio.comuns.financeiro.ContaReceberNegativacaoVO;
...
public class FichaAlunoControle extends SuperControleRelatorio implements Serializable {
	...
	private List<InteracaoWorkflowHistoricoVO> listaInteracaoWorkflowHistoricoVOs;
	...
	private ProspectsVO prospectNovaInteracaoVO;

	private List<InscricaoVO> listaInscricaoVOs;
	
	private List<RegistroNegativacaoCobrancaContaReceberVO> listaNegativacaoCobrancaVOs;
	...
	private InteracaoWorkflowHistoricoVO interacao;
	...
	private List<DetalhamentoValorContaVO> centroResultadoOrigemVOs;
...
//		}
	...
	public void imprimirComprovanteMatricula() {
		...
		try {
			...
			listaObjetos = getFacadeFactory().getComprovanteRenovacaoMatriculaRelFacade().criarObjeto(getMatriculaPeriodoVO().getCodigo(), getMatriculaPeriodoVO().getMatricula(), getConfiguracaoFinanceiroPadraoSistema(), getUsuarioLogado());
	...
	public void realizarNavegacaoContaReceber() {
		...
		if (obj != null && !obj.getCodigo().equals(0)) {
			...
			context().getExternalContext().getSessionMap().put("contaReceberFichaAluno", obj);
		}
	}

	public void realizarNavegacaoRecebimento() {
		ContaReceberVO obj = (ContaReceberVO) context().getExternalContext().getRequestMap().get("contaReceberItem");
		if (obj != null && !obj.getCodigo().equals(0)) {
	...
	public void realizarVisualizacaoContaReceber() {
		MatriculaPeriodoVO obj = (MatriculaPeriodoVO) context().getExternalContext().getRequestMap().get("matriculaPeriodoItem");
		if (obj != null && !obj.getCodigo().equals(0)) {
			setListaContaReceberVOs(getFacadeFactory().getContaReceberFacade().consultarPorMatriculaPeriodoFichaAluno(obj.getCodigo(), getUsuarioLogado()));
			setMatriculaPeriodoVO(obj);
			setMensagemID("msg_dados_consultados");
		}
	}

	public void inicializarDadosContaReceber() {
		ContaReceberVO obj = (ContaReceberVO) context().getExternalContext().getRequestMap().get("contaReceberItem");
		if (obj != null && !obj.getCodigo().equals(0)) {
	...
	public void excluirContaReceber() {
		if (getContaReceberVO() != null && !getContaReceberVO().getCodigo().equals(0)) {
			Boolean verificarPermissao = Boolean.FALSE;
			try {
				registrarAtividadeUsuario(getUsuarioLogado(), "ContaReceberControle", "Iniciando Excluir Conta Receber", "Excluindo");
				if (getContaReceberVO().getTipoOrigem().equals("REQ")) {
				...
				if (Uteis.isAtributoPreenchido(getContaReceberVO().getNotaFiscalSaidaServicoVO().getCodigo())) {
				...
				if (getContaReceberVO().getSituacao().equals("AR")) {
					...
					try {
						matriculaPeriodoVencimentoVO = getFacadeFactory().getMatriculaPeriodoVencimentoFacade().consultarPorContaReceber(getContaReceberVO().getCodigo().intValue(), false, Uteis.NIVELMONTARDADOS_DADOSMINIMOS, getConfiguracaoFinanceiroPadraoSistema(), getUsuarioLogado());
						getFacadeFactory().getContaReceberFacade().excluir(getContaReceberVO(), matriculaPeriodoVencimentoVO, getConfiguracaoFinanceiroPadraoSistema(), verificarPermissao, getUsuarioLogado());
					} catch (Exception e) {
						getFacadeFactory().getContaReceberFacade().excluir(getContaReceberVO(), getConfiguracaoFinanceiroPadraoSistema(), true, getUsuarioLogado());
					}
					setListaContaReceberVOs(getFacadeFactory().getContaReceberFacade().consultarPorMatriculaPeriodoFichaAluno(getContaReceberVO().getMatriculaPeriodo(), getUsuarioLogado()));
	...
	public void cancelarContaReceber() {
		if (getContaReceberVO() != null && !getContaReceberVO().getCodigo().equals(0)) {
			try {
	...
	public void imprimirBoletoContaReceber() throws Exception {
		ContaReceberVO obj = (ContaReceberVO) context().getExternalContext().getRequestMap().get("contaReceberItem");
		setContaReceberVO(obj);
		...
		if ((getContaReceberVO().getContaReceberReferenteMatricula()) && (getConfiguracaoFinanceiroPadraoSistema().getBloquearPagarMatriculaRenovacaoVisaoAlunoInadimplente())) {
			if (veriricarAlunoPossuiContasReceberEmAberto(getContaReceberVO())) {
				// se entrar aqui é por que o aluno possui contas a receber em
				// aberto, diferente da parcela
				// de matrícula. Logo, como está se trabalhando com bloqueio
				// (conforme configuracao financeira ->
				// BloquearPagarMatriculaRenovacaoVisaoAlunoInadimplente) entao
				// temos que ver se o usuário tem permissão
		...
		try {
			if (getConfiguracaoFinanceiroVO().getCobrarReimpressaoBoletos() && getContaReceberVO().getImpressaoBoletoRealizada()) {
				setAbrirModalImpressaoBoleto("RichFaces.$('panelCobrarReimpressao').show()");
				 
			} else {

				setAbrirModalImpressaoBoleto("RichFaces.$('panelEmissaoBoleto').show()");
			}
			// Na conta corrente é permitido informar para bloquear emissão de
			// boletos, esse metodo realiza a validação dessa emissão.
		...
		try {
			...
			for (ContaReceberVO conta : contasAReceber) {
				if (!conta.getCodigo().equals(obj.getCodigo())) {
					// se entrar aqui é por que existe ao menos uma conta a
					// receber
					// com a situacao AR diferente da qual o aluno esta
					// imprimindo.
					// Logo, como o bloqueio para impressao do boleto da
					// matricula
					// está ativado, deve ser retornado true para ver se o
					// usuario
	...
	public String getBoletoConsultaContaReceber() {
		if (getApresentarImprimirBoletoConsultaContaReceber()) {
			try {
				...
				return "RichFaces.$('panelCobrarReimpressao').hide();  RichFaces.$('panelAvisoExclusaoContaReceber').show()";
			} catch (Exception e) {
				return "";
			}
		} else {
	...
	public void consultarDadosAbaNegativacaoCobranca() {
		try {
			getListaNegativacaoCobrancaVOs().clear();
			setListaNegativacaoCobrancaVOs(getFacadeFactory().getRegistroNegativacaoCobrancaContaReceberFacade().consultarContasReceberPorRegistroNegativacaoCobrancaPorPessoa(getAlunoVO().getCodigo(), getUsuarioLogado()));
			Map<String, ContaReceberNegativacaoVO> mapContaReceberNegativacao =  new HashMap<String, ContaReceberNegativacaoVO>();
			for(RegistroNegativacaoCobrancaContaReceberVO registroNegativacaoCobrancaContaReceberVO: getListaNegativacaoCobrancaVOs()) {
				for(RegistroNegativacaoCobrancaContaReceberItemVO registroNegativacaoCobrancaContaReceberItemVO: registroNegativacaoCobrancaContaReceberVO.getListaContasReceberCobranca()) {
					if(!mapContaReceberNegativacao.containsKey(registroNegativacaoCobrancaContaReceberItemVO.getDataRegistroApresentar())) {
						ContaReceberNegativacaoVO contaReceberNegativacaoVO = new ContaReceberNegativacaoVO();
						contaReceberNegativacaoVO.getRegistro().setDataGeracao(registroNegativacaoCobrancaContaReceberItemVO.getDataRegistro());
						mapContaReceberNegativacao.put(registroNegativacaoCobrancaContaReceberItemVO.getDataRegistroApresentar(), contaReceberNegativacaoVO);						
					}
					ContaReceberNegativacaoVO contaReceberNegativacaoVO  = mapContaReceberNegativacao.get(registroNegativacaoCobrancaContaReceberItemVO.getDataRegistroApresentar());
					contaReceberNegativacaoVO.getRegistroNegativacaoCobrancaContaReceberItemVOs().add(registroNegativacaoCobrancaContaReceberItemVO);					
				}
				registroNegativacaoCobrancaContaReceberVO.getListaContasReceberNegativacao().addAll(mapContaReceberNegativacao.values());
				Ordenacao.ordenarListaDecrescente(registroNegativacaoCobrancaContaReceberVO.getListaContasReceberNegativacao(), "registro.dataGeracao");
			}
		} catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
	...
	public void realizarNavegacaoRecebimentoReceberRequerimento() {
		RequerimentoVO obj = (RequerimentoVO) context().getExternalContext().getRequestMap().get("requerimentoItem");
		if (obj != null && !obj.getCodigo().equals(0)) {
			if (!obj.getContaReceber().equals(0)) {
				removerControleMemoriaFlashTela("NegociacaoRecebimentoControle");
				ContaReceberVO contaReceberReceberVO = new ContaReceberVO();
				contaReceberReceberVO.setCodigo(obj.getContaReceber());
				context().getExternalContext().getSessionMap().put("contaReceberFichaAluno", contaReceberReceberVO);
			}
		}
	}
	...
	public void excluirRequerimento() {
		try {
			...
			getFacadeFactory().getRequerimentoFacade().excluir(getRequerimentoVO(), getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getRequerimentoVO().getUnidadeEnsino().getCodigo()), getUsuarioLogado(), getConfiguracaoGeralPadraoSistema(), true);
			consultarRequerimentoPorMatriculaAluno();
...
//					obj.setValorMulta(getFacadeFactory().getMinhaBibliotecaInterfaceFacade().realizarCalculoMultaParcial(obj, TipoPessoa.getEnum(obj.getTipoPessoaEmprestimo()), obj.getConfiguracaoBibliotecaVO(), obj.getCidadeBibliotecaVO()));
	...
	public void consultarDadosAbaCRMPorAluno() {
		try {
			consultarCRMPorAluno();
			setListaSelectItemMesAnoInteracaoWorkflowVOs(getFacadeFactory().getMatriculaFacade().inicializarDadosListaSelectItemMesAnoBaseadoInteracaoWorkflow(getAlunoVO().getCodigo(), getUsuarioLogado()));
			montarListaSelectItemResponsavelInteracaoWorkFlow();
		} catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
	}

	public void consultarCRMPorAluno() {
		try {
			getListaInteracaoWorkflowHistoricoVOs().clear();
			setListaInteracaoWorkflowHistoricoVOs(getFacadeFactory().getMatriculaFacade().inicializarDadosCRMFichaAluno(getAlunoVO().getCodigo(), getResponsavelInteracaoWorkflowVO().getCodigo(), getMesAno(), getUsuarioLogado()));
		} catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
	}
	...
	public void inicializarProspectRealizarNavegacao() {
		try {
			setProspectNovaInteracaoVO(getFacadeFactory().getProspectsFacade().consultarPorCodigoPessoa(getAlunoVO().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, getUsuarioLogado()));
			removerControleMemoriaFlashTela("FollowUpControle");
		} catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
	}
	...
	public void consultarDadosAbaProcessoSeletivoPorAluno() {
		try {
			consultarProcessoSeletivoPorAluno();
		} catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
	}

	public void consultarProcessoSeletivoPorAluno() {
		try {
			getListaInscricaoVOs().clear();
			setListaInscricaoVOs(getFacadeFactory().getInscricaoFacade().inicializarDadosInscricaoProcessoSeletivoFichaAluno(getAlunoVO().getCodigo(), getUsuarioLogado()));
		} catch (Exception e) {
			setMensagemDetalhada("msg_erro", e.getMessage());
		}
	}

	public void realizarNavegacaoCandidatoTelaInscricao() {
		InscricaoVO obj = (InscricaoVO) context().getExternalContext().getRequestMap().get("inscricaoItem");
		if (obj != null && !obj.getCodigo().equals(0)) {
			removerControleMemoriaFlashTela("InscricaoControle");
			context().getExternalContext().getSessionMap().put("inscricaoFichaAluno", obj);
		}
	}

	public void realizarNavegacaoCandidatoTelaResultadoProcessoSeletivo() {
		InscricaoVO obj = (InscricaoVO) context().getExternalContext().getRequestMap().get("inscricaoItem");
		if (obj != null && !obj.getCodigo().equals(0)) {
			removerControleMemoriaFlashTela("ResultadoProcessoSeletivoControle");
			context().getExternalContext().getSessionMap().put("inscricaoFichaAluno", obj);
		}
	}
	...
	public void paginarComunicadoInterno(DataScrollEvent dataScrollEvent) {
		getControleConsultaComunicadoInterno().setPage(dataScrollEvent.getPage());
		getControleConsultaComunicadoInterno().setPaginaAtual(dataScrollEvent.getPage());
		consultarComunicacaoInternaPorAluno();	
	}
	...
	public String getOncompleteModalBloqueioMatricula() {
		if (getSituacaoBloqueioMatricula().equals("BLOQUEIO_REALIZADO_COM_SUCESSO")) {
			return "RichFaces.$('panelBloquearMatricula').hide()";
		}
		return "";
	}
	...
	public void verificarUsuarioPossuiPermissaoExcluirContaReceber() {
		try {
			ContaReceber.excluir(ContaReceber.getIdEntidade(),getUsuarioLogado());
			setApresentarBotaoExcluirContaReceber(true);
		} catch (Exception e) {
			setApresentarBotaoExcluirContaReceber(false);
		}
	}
	...
	public void verificarUsuarioPossuiPermissaoEditarRequerimento(){
		try {
			Requerimento.alterar(Requerimento.getIdEntidade(),getUsuarioLogado());
	...
	public void verificarUsuarioPossuiPermissaoAdicionarRequerimento(){
		try {
			Requerimento.incluir(Requerimento.getIdEntidade(),getUsuarioLogado());
	...
	public void verificarUsuarioPossuiPermissaoExcluirRequerimento(){
		try {
			Requerimento.excluir(Requerimento.getIdEntidade(),getUsuarioLogado());
	...
	public List<InteracaoWorkflowHistoricoVO> getListaInteracaoWorkflowHistoricoVOs() {
		if (listaInteracaoWorkflowHistoricoVOs == null) {
			listaInteracaoWorkflowHistoricoVOs = new ArrayList<InteracaoWorkflowHistoricoVO>(0);
		}
		return listaInteracaoWorkflowHistoricoVOs;
	}

	public void setListaInteracaoWorkflowHistoricoVOs(List<InteracaoWorkflowHistoricoVO> listaInteracaoWorkflowHistoricoVOs) {
		this.listaInteracaoWorkflowHistoricoVOs = listaInteracaoWorkflowHistoricoVOs;
	}
	...
	public ProspectsVO getProspectNovaInteracaoVO() {
		if (prospectNovaInteracaoVO == null) {
			prospectNovaInteracaoVO = new ProspectsVO();
		}
		return prospectNovaInteracaoVO;
	}

	public void setProspectNovaInteracaoVO(ProspectsVO prospectNovaInteracaoVO) {
		this.prospectNovaInteracaoVO = prospectNovaInteracaoVO;
	}

	public List<InscricaoVO> getListaInscricaoVOs() {
		if (listaInscricaoVOs == null) {
			listaInscricaoVOs = new ArrayList<InscricaoVO>(0);
		}
		return listaInscricaoVOs;
	}

	public void setListaInscricaoVOs(List<InscricaoVO> listaInscricaoVOs) {
		this.listaInscricaoVOs = listaInscricaoVOs;
	}
	...
	public void loginComSimulacaoAcessoVisaoAluno() {
		...
		try {
			if (!getAlunoVO().getCodigo().equals(0)) {
				usuarioVO = ControleAcesso.verificarLoginUsuarioSimulacaoVisaoAluno(getAlunoVO().getCodigo(), false);
	...
	public InteracaoWorkflowHistoricoVO getInteracao() {
		if(interacao == null){
			interacao = new InteracaoWorkflowHistoricoVO();
		}
		return interacao;
	}

	public void setInteracao(InteracaoWorkflowHistoricoVO interacao) {
		this.interacao = interacao;
	}
	
	public void selecionarInteracao(){
		setInteracao((InteracaoWorkflowHistoricoVO) getRequestMap().get("interacaoWorkflowHistoricoItem"));
	}
	...
	public void executarVisualizacaoRelatorioFichaAluno() {
		MatriculaVO obj = (MatriculaVO) context().getExternalContext().getRequestMap().get("matriculaItem");
		  try {
	            ...
	            List<FichaAlunoRelVO> fichaAlunoRelVOs = getFacadeFactory().getFichaAlunoRelFacade().criarObjeto(obj.getMatricula(),getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(obj.getUnidadeEnsino().getCodigo()), getUsuarioLogado());
	 ...
	 public void selecionarContaReceberEmissoBoletoPorRequerimento() {
		 try {
			 RequerimentoVO obj = (RequerimentoVO) context().getExternalContext().getRequestMap().get("requerimentoItem");
			 setContaReceberVO(obj.getContaReceberVO());
		 } catch (Exception e) {
			 setMensagemDetalhada("msg_erro", e.getMessage());
		 }   
	 }
	...
	public List<RegistroNegativacaoCobrancaContaReceberVO> getListaNegativacaoCobrancaVOs() {
		if (listaNegativacaoCobrancaVOs == null) {
			listaNegativacaoCobrancaVOs = new ArrayList<RegistroNegativacaoCobrancaContaReceberVO>();
		}
		return listaNegativacaoCobrancaVOs;
	}

	public void setListaNegativacaoCobrancaVOs(
			List<RegistroNegativacaoCobrancaContaReceberVO> listaNegativacaoCobrancaVOs) {
		this.listaNegativacaoCobrancaVOs = listaNegativacaoCobrancaVOs;
	}
	...
	public Boolean getPossuiPerfilAcessoDocumentacaoGED() {
		if (possuiPerfilAcessoDocumentacaoGED == null) {
			try {
				possuiPerfilAcessoDocumentacaoGED = ControleAcesso.verificarPermissaoFuncionalidadeUsuario("ApresentarConfiguracaoGEDAluno", getUsuarioLogado());
			} catch (Exception e) {
	...
	public void visualizarCentroResultadoOrigemDetalheContaReceber() {
		ContaReceberVO obj = (ContaReceberVO) context().getExternalContext().getRequestMap().get("contaReceberItem");
		setCentroResultadoOrigemVOs(null);
		if (Uteis.isAtributoPreenchido(obj) && Uteis.isAtributoPreenchido(obj.getDetalhamentoValorContaVOs())) {
			setCentroResultadoOrigemVOs(obj.getDetalhamentoValorContaVOs());
		}
	}

	public List<DetalhamentoValorContaVO> getCentroResultadoOrigemVOs() {
		if (centroResultadoOrigemVOs == null) {
			centroResultadoOrigemVOs = new ArrayList<>();
		}
		return centroResultadoOrigemVOs;
	}

	public void setCentroResultadoOrigemVOs(List<DetalhamentoValorContaVO> list) {
		this.centroResultadoOrigemVOs = list;
	}
