...
//			getTextoPadraoDeclaracaoVO().setTexto(getTextoPadraoDeclaracaoVO().getTexto().replaceAll("</body>", sb.toString()));
    ...
    public void realizarVisualizacaoTexto() {
    	try {    	
			...
			if(getTextoPadraoDeclaracaoVO().getTipoDesigneTextoEnum().isHtml()){
				String texto = getFacadeFactory().getTextoPadraoDeclaracaoFacade().removerBordaDaPagina(getTextoPadraoDeclaracaoVO().getTexto());
	    		texto = getFacadeFactory().getTextoPadraoDeclaracaoFacade().adicionarStyleFormatoPaginaTextoPadrao(texto, getTextoPadraoDeclaracaoVO().getOrientacaoDaPagina());
		        setCaminhoRelatorio(getFacadeFactory().getImpressaoDeclaracaoFacade().executarConversaoHtmlParaPdf(texto, getTextoPadraoDeclaracaoVO(), getUsuarioLogado()));	
			}else{
				if (getTextoPadraoDeclaracaoVO().getTipo().equals("CO")) {
					List<CartaCobrancaRelVO> objetos = new ArrayList<CartaCobrancaRelVO>();
					CartaCobrancaRelVO exemplo = new CartaCobrancaRelVO();
					exemplo.setDataPorExtenso(Uteis.getDataCidadeDiaMesPorExtensoEAno(exemplo.getNomeCidadeUnidadeEnsino(), new Date(), false));
					exemplo.setNomeResponsavel("Nome do Responsável");
					exemplo.setNomeAluno("Nome do Aluno");
					exemplo.setCpfAluno("XXX.XXX.XXX-XX");
					exemplo.setTurma("Nome da Turma");
					exemplo.setNomeCurso("Nome do Curso");
					exemplo.setDataEmissao(String.valueOf(Uteis.getData(new Date())));
					exemplo.setNomeCidade("Goiânia");
					exemplo.setEstadoResponsavel("Goiás");
					exemplo.setNumeroMatricula("XXXXXXXXXX");
					exemplo.setEnderecoResponsavel("Rua 1, 23");
					exemplo.setCepResponsavel("74000-000");
					exemplo.setSetor("Centro");
					ContaReceberVO contaExemplo = new ContaReceberVO();
					contaExemplo.setParcela("1/1");
					contaExemplo.setDataVencimento(new Date());
					contaExemplo.setValor(1.0);
					exemplo.getDebitosLista().add(contaExemplo);
					objetos.add(exemplo);
					getTextoPadraoDeclaracaoVO().setObjetos(objetos);
				}
    ...
    public void selecionarMarcadorAluno() {
        ...
        String texto = adicionarMarcador(getTextoPadraoDeclaracaoVO().getTexto(), obj.getTag());
    ...
    public void selecionarMarcadorOutras() {
        MarcadorVO obj = (MarcadorVO) context().getExternalContext().getRequestMap().get("marcadorOutrasItens");
        String texto = adicionarMarcador(getTextoPadraoDeclaracaoVO().getTexto(), obj.getTag());
        getTextoPadraoDeclaracaoVO().setTexto(texto);
    }
    ...
    public void selecionarMarcadorDisciplina() {
        MarcadorVO obj = (MarcadorVO) context().getExternalContext().getRequestMap().get("marcadorDisciplinaItens");
        String texto = adicionarMarcador(getTextoPadraoDeclaracaoVO().getTexto(), obj.getTag());
        getTextoPadraoDeclaracaoVO().setTexto(texto);
    }
    ...
    public void selecionarMarcadorDisciplinaDeclaracao() {
        MarcadorVO obj = (MarcadorVO) context().getExternalContext().getRequestMap().get("marcadorDisciplinaItens");
        String texto = adicionarMarcador(getTextoPadraoDeclaracaoVO().getTexto(), obj.getTag());
        getTextoPadraoDeclaracaoVO().setTexto(texto);
    }
	
    public void selecionarMarcadorEstagio() {
        MarcadorVO obj = (MarcadorVO) context().getExternalContext().getRequestMap().get("marcadorEstagioItens");
        String texto = adicionarMarcador(getTextoPadraoDeclaracaoVO().getTexto(), obj.getTag());
        getTextoPadraoDeclaracaoVO().setTexto(texto);
    }

    public void selecionarMarcadorInscProcSeletivo() {
    	MarcadorVO obj = (MarcadorVO) context().getExternalContext().getRequestMap().get("marcadorInscProcSeletivoItens");
    	String texto = adicionarMarcador(getTextoPadraoDeclaracaoVO().getTexto(), obj.getTag());
    	getTextoPadraoDeclaracaoVO().setTexto(texto);
    }
