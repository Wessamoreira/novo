...
public class ComprovanteRenovacaoMatriculaRel extends SuperRelatorio implements ComprovanteRenovacaoMatriculaRelInterfaceFacade {
    ...
    public List<ComprovanteRenovacaoMatriculaVO> criarObjeto(Integer matriculaPeriodo, String matricula, ConfiguracaoFinanceiroVO configuracaoFinanceiroVO, UsuarioVO usuarioVO) throws Exception {
        ...
        getFacadeFactory().getMatriculaPeriodoFacade().carregarDados(getComprovanteRenovacaoMatriculaVO().getMatriculaPeriodoVO(), NivelMontarDados.BASICO, configuracaoFinanceiroVO, usuarioVO);
        ...
        if(getComprovanteRenovacaoMatriculaVO().getMatriculaPeriodoVO().getCondicaoPagamentoPlanoFinanceiroCurso().getCodigo() > 0){
			...
			if (!getComprovanteRenovacaoMatriculaVO().getMatriculaPeriodoVO().getCondicaoPagamentoPlanoFinanceiroCurso().utilizaValorMatriculaMensalidadeFixo()) {
        		ContaReceberVO crmat = getFacadeFactory().getContaReceberFacade().consultarPrimeiraMensalidade(getComprovanteRenovacaoMatriculaVO().getMatriculaPeriodoVO(), "MA", true);
        		configuracaoFinanceiroVO = configuracaoFinanceiroVO == null ? getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(getComprovanteRenovacaoMatriculaVO().getMatriculaPeriodoVO().getMatriculaVO().getUnidadeEnsino().getCodigo(), usuarioVO) : configuracaoFinanceiroVO;
        		if (Uteis.isAtributoPreenchido(crmat)) {
        			getFacadeFactory().getContaReceberFacade().carregarDados(crmat, configuracaoFinanceiroVO, usuarioVO);
        			getComprovanteRenovacaoMatriculaVO().getMatriculaPeriodoVO().setValorMatriculaCheio(Uteis.arrendondarForcando2CadasDecimais(crmat.getValorTotalAPagarNaDataAtual()));
        		} else {
        			getFacadeFactory().getMatriculaPeriodoVencimentoFacade().consultarValorMatriculaMensalidadeMatriculaPeriodoVencimento(getComprovanteRenovacaoMatriculaVO().getMatriculaPeriodoVO(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, "MAT", usuarioVO);
        		}
        		ContaReceberVO cr = getFacadeFactory().getContaReceberFacade().consultarPrimeiraMensalidade(getComprovanteRenovacaoMatriculaVO().getMatriculaPeriodoVO());
        		if	(Uteis.isAtributoPreenchido(cr)) {
        			getFacadeFactory().getContaReceberFacade().carregarDados(cr, configuracaoFinanceiroVO, usuarioVO);
        			getComprovanteRenovacaoMatriculaVO().getMatriculaPeriodoVO().setValorMensalidadeCheio(Uteis.arrendondarForcando2CadasDecimais(cr.getValorTotalAPagarNaDataAtual()));
        		}else {
        			getFacadeFactory().getMatriculaPeriodoVencimentoFacade().consultarValorMatriculaMensalidadeMatriculaPeriodoVencimento(getComprovanteRenovacaoMatriculaVO().getMatriculaPeriodoVO(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, "ME", usuarioVO);
        		}
        	}
        }
