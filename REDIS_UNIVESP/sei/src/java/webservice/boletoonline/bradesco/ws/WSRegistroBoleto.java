...
import webservice.boletoonline.bradesco.BBRConfig;
import webservice.boletoonline.bradesco.BBRLoggable;
...
import negocio.comuns.utilitarias.boleto.BoletoHttpStatusEnum;
...
public class WSRegistroBoleto implements BBRLoggable {
    ...
    public RegistroRetornoBoleto enviarBoleto(final BBRConfig config, final RegistroEntradaBoleto registroEntrada) throws Exception {
        ...
        try (final CloseableHttpClient httpClient = HttpClients.custom().setSSLSocketFactory(sslSocketFactory).build()) {
            ...
            BoletoHttpStatusEnum.getBoletoHttpStatusEnumPorCodigo(response.getStatusLine().getStatusCode()).ifPresent(BoletoHttpStatusEnum::throwExceptionQuandoErro);
            
            this.getLogger().debug("Response status: {}", response.getStatusLine().getStatusCode());
            final String stringResposta = IOUtils.toString(response.getEntity().getContent(), StandardCharsets.UTF_8.toString());
    ...
    static RegistroRetornoBoleto converterJsonToRegistroRetorno(final String respostaJson) {
        String respostaJsonLimpa = respostaJson.substring(respostaJson.lastIndexOf(",") + 1).trim().equals("}") ? respostaJson.substring(0, respostaJson.lastIndexOf(",")) + "}" : respostaJson;
        respostaJsonLimpa = respostaJsonLimpa.replaceAll("\\\\", "");
