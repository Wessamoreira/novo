 ...
 */
public class CondicaoPagamentoUnidadeEnsinoSV extends HttpServlet {
	 ...
	 */
	public CondicaoPagamentoUnidadeEnsinoSV() {
		super();
...
//				turmaVO.setCodigo(codTurma);
//				getFacadeFactory().getTurmaFacade().carregarDados(turmaVO, null);
//				if (turmaVO.getUnidadeEnsino().getCodigo().intValue() != 18) {
//					throw new Exception("Unidade de Ensino não autenticada");
//				}
//			}
...
//				codUnidadeEnsino = 18;
//			}			
			//TurmaVO turmaVO = getFacadeFactory().getTurmaFacade().consultarPorChavePrimaria(codTurma, Uteis.NIVELMONTARDADOS_DADOSBASICOS, null);
			List<CondicaoPagamentoPlanoFinanceiroCursoVO> lista = getFacadeFactory().getCondicaoPagamentoPlanoFinanceiroCursoFacade().consultarPorCodigoUnidadeEnsino(codUnidadeEnsino, false, Uteis.NIVELMONTARDADOS_DADOSMINIMOS, null);
			// preenche o objeto
			Iterator i = lista.iterator();
			JSONObject my_obj_unid_nome = new JSONObject();
			while (i.hasNext()) {
				CondicaoPagamentoPlanoFinanceiroCursoVO uni = (CondicaoPagamentoPlanoFinanceiroCursoVO) i.next();
				JSONObject my_obj_unid_cod = new JSONObject();
				my_obj_unid_cod.put("codigoTurma", uni.getCodTurma());
				my_obj_unid_cod.put("codigoCondicaoPagamento", uni.getCodigo());
				my_obj_unid_cod.put("codigoPlanoFinanceiroCurso", uni.getPlanoFinanceiroCurso());
				my_obj_unid_cod.put("descricaoCondicaoPagamento", uni.getDescricao());
				my_obj_unid_cod.put("valorCondicaoPagamento", uni.getValorParcela());
				my_obj_unid_cod.put("qtdParcelasCondicaoPagamento", uni.getNrParcelasPeriodo());
				my_obj_unid_nome.put(uni.getCodTurma() + "_" + uni.getCodigo(), my_obj_unid_cod.toString().replace("\\", "").replace("\"{", "{").replace("}\"", "}"));				
			}
