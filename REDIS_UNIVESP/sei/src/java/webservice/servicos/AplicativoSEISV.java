...
//	private ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO;
	...
	public Response realizarAutenticacaoLoginSEI(UsuarioVO usuarioLogin) {
		try {
			UsuarioVO usuarioVO = ControleAcesso.verificarLoginUsuario(usuarioLogin.getUsername(), usuarioLogin.getSenha(), true, Uteis.NIVELMONTARDADOS_DADOSLOGIN);
	...
	private void realizarDefinicoesUsuarioLogado(UsuarioVO usuarioVO, MatriculaVO matriculaVO, ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO) throws Exception {
		...
		try {
			ControleAcesso.verificarPermissaoUsuarioFuncionalidadeComUsuarioVOEspecifico("PermitirAlunoVizualizarMateriaisPeriodoConcluido", usuarioVO);
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/carregarFinanceiroAluno/{matricula}/{codigoCurso}/{codigoUnidadeEnsinoMatricula}/{tipoNivelEducacional}/{codigoPerfilAcesso}/{limit}/{offset}/{visaoLogar}/{situacao}")
	public Response carregarFinanceiroAluno(@PathParam("matricula") final String matricula, @PathParam("codigoCurso") final Integer codigoCurso, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("tipoNivelEducacional") final String tipoNivelEducacional, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso, @PathParam("limit") final Integer limit, @PathParam("offset") final Integer offset, @PathParam("visaoLogar") final String visaoLogar, @PathParam("situacao") final String situacao) {
		try {
			UsuarioVO usuarioVO = autenticarConexao();
			...
			contaReceberVOs = getFacadeFactory().getContaReceberFacade().consultarContasAPagarVisaoAluno(configuracaoGeralSistemaVO, configuracaoFinanceiroVO, codigoCurso, matricula, situacao, usuarioVO, new HashMap<Integer, ConfiguracaoRecebimentoCartaoOnlineVO>(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO, true, limit, offset, new DataModelo());
			//getFacadeFactory().getContaReceberFacade().validarTipoImpressao(contaReceberVOs, usuarioVO);
			// Criado esse while, para limpar a linha digitavel do boleto, pois esse dado pode estar sendo apresentado errado, 
			// pois existe regras de modificação da linha digitavel e vencimento de boleto, que somente executam quando emite o PDF.
			// Inicialmente foi retirado o valor do objeto e posteriormente pensar em criar permissão para apresentar ou nao.
			Iterator i = contaReceberVOs.iterator();
			while (i.hasNext()) {
				ContaReceberVO conta = (ContaReceberVO)i.next();
				conta.setConfiguracaoFinanceiro(configuracaoFinanceiroVO);
				if (Uteis.isAtributoPreenchido(conta.getContaCorrenteVO().getMensagemCarteiraRegistrada())) {
					conta.getContaCorrenteVO().setMensagemCarteiraRegistrada(conta.getContaCorrenteVO().getMensagemCarteiraRegistrada().replaceAll("\\r\\n|\\r|\\n", " "));	
				}
				if (Uteis.isAtributoPreenchido(conta.getDescricaoPagamento())) {
					conta.setDescricaoPagamento(conta.getDescricaoPagamento().replaceAll("\\r\\n|\\r|\\n", " "));
				}
			}
			...
			//realizarVerificacaoContaVencida(contaReceberVOs, possuiPermissaoEmitirBoletoVencido);
			//getFacadeFactory().getContaReceberFacade().validarTipoImpressao(contaReceberVOs, usuarioVO);
			Collections.reverse(contaReceberVOs);
			return Response.status(Status.OK).entity(new GenericEntity<List<ContaReceberVO>>(contaReceberVOs) {}).build();
		} catch (Exception e) {
			ErrorInfoRSVO errorInfoRSVO = new ErrorInfoRSVO();
			errorInfoRSVO.setCodigo(Status.UNAUTHORIZED.name());
			errorInfoRSVO.setMensagem(e.getMessage());
	...
	public Response carregarDetalhesMinhasNotasAluno(@PathParam("matricula") final String matricula, @PathParam("tipoNivelEducacional") final String tipoNivelEducacional, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso, @PathParam("codigoHistorico") final Integer codigoHistorico) throws Exception {
		try {
			...
			if(historicoVOs != null && !historicoVOs.isEmpty()){
				...
				boolean permitirApresentarTodasNotasParametrizadasConfiguracaoAcademica = ControleAcesso.verificarPermissaoFuncionalidadeUsuario(PerfilAcessoPermissaoVisaoAlunoEnum.PERMITIR_APRESENTAR_TODAS_NOTAS_PARAMETRIZADAS_CONFIGURACAO_ACADEMICA.getValor(), 
						usuarioVO);
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/realizarImpressaoBoletoBancario/{matricula}/{codigoUnidadeEnsinoMatricula}/{codigoConta}")
	public Response realizarImpressaoBoletoBancario(@PathParam("matricula") final String matricula, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("codigoConta") final Integer codigoConta) {
		try {
			UsuarioVO usuarioVO = obterUsuarioWebService();
			ConfiguracaoFinanceiroVO configuracaoFinanceiroVO = getFacadeFactory().getConfiguracaoFinanceiroFacade().consultarConfiguracaoASerUsada(Uteis.NIVELMONTARDADOS_DADOSBASICOS, codigoUnidadeEnsinoMatricula, null);
			SuperParametroRelVO superParametroRelVO = new SuperParametroRelVO();
			List<BoletoBancarioRelVO> boletoBancarioRelVOs = getFacadeFactory().getBoletoBancarioRelFacade().emitirRelatorioLista(false, codigoConta, matricula, "", "", "", 0, 0, null, null, 0, "aluno", 0, usuarioVO, "boletoAluno", 0, configuracaoFinanceiroVO, 0, false, null, null);
			if(Uteis.isAtributoPreenchido(boletoBancarioRelVOs)) {
				getFacadeFactory().getBoletoBancarioRelFacade().validarImpressaoBoletoAluno(codigoConta, boletoBancarioRelVOs.get(0), "AplicativoSEISV - BoletoBancario ", usuarioVO.getCodigo());
				...
				getFacadeFactory().getInteracaoWorkflowFacade().persistirFollowUpContaReceber(boletoBancarioRelVOs.get(0), usuarioVO.getCodigo().toString(), "AplicativoSEISV - BoletoBancario ", superParametroRelVO.getNomeRelatorio());
				UrlRSVO url = new UrlRSVO();
				url.setUrl(getCaminhoWeb(usuarioVO)+"relatorio/"+renovarMatriculaControle.getCaminhoRelatorio());
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/realizarGeracaoPixContaCorrente/{matricula}/{codigoUnidadeEnsinoMatricula}/{codigoConta}")
	public Response realizarGeracaoPixContaCorrente(@PathParam("matricula") final String matricula, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("codigoConta") final Integer codigoConta) {
		try {
			...
			pixVO.setContaReceberVO(new ContaReceberVO());
			pixVO.getContaReceberVO().setCodigo(codigoConta);
			PixContaCorrenteVO pix = getFacadeFactory().getPixContaCorrenteFacade().realizarGeracaoPix(pixVO, configuracaoFinanceiroVO, usuarioVO);
			if(Uteis.isAtributoPreenchido(pix)) {
				PixRSVO pixRSVO = new PixRSVO();
				pixRSVO.setQrCode(pix.getQrCode());	
				return Response.status(Status.OK).entity(pixRSVO).build();
			}else {
				throw new Exception(UteisJSF.internacionalizar("msg_relatorio_sem_dados"));
			}
		} catch (Exception e) {
		...
		try {
			...
			for (RequerimentoVO requerimentoVO : requerimentoVOs) {
				...
				if (Uteis.isAtributoPreenchido(requerimentoVO.getContaReceberVO().getCodigo())) {
					requerimentoVO.setContaReceberVO(getFacadeFactory().getContaReceberFacade().consultarPorChavePrimaria(requerimentoVO.getContaReceberVO().getCodigo(), false, Uteis.NIVELMONTARDADOS_TODOS, configuracaoFinanceiroVO, usuarioVO));
					List<ContaReceberVO> contaReceberVOs = new ArrayList<ContaReceberVO>();
...
//		}
	...
	public List<ContaReceberVO> realizarValidarTipoImpressao(List<ContaReceberVO> contaReceberVOs, Boolean possuiPermissaoImprimirBoleto) throws Exception {
		for (ContaReceberVO contaReceberVO : contaReceberVOs) {
			if(Uteis.nrDiasEntreDatas(contaReceberVO.getDataVencimento(), new Date()) < 0) {
				contaReceberVO.setContaVencida(true);
			}
			contaReceberVO.setPermiteImprimirBoleto( (contaReceberVO.isHabilitarBotaoRecebimentoVisaoAluno() && !contaReceberVO.getHabilitarBotaoLinkBanco()) 
					|| 
				 (!contaReceberVO.isHabilitarBotaoRecebimentoVisaoAluno() //ok
					&& contaReceberVO.isHabilitarBotaoRecebimento() // ok
					&& !contaReceberVO.getHabilitarBotaoLinkBanco() //ok
					&& possuiPermissaoImprimirBoleto) );
...
//			System.out.println("MOBILE INFO: RETORNANDO " + historicoVOs.toString());
	...
	public Response consultarQuantidadeComunicadosExistentesProfessor(@PathParam("codigoPessoa") final Integer codigoPessoa, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso) {
		...
		@GET
		@Produces(MediaType.APPLICATION_JSON)
		@Path("/consultarHorarioProfessorDia/{codigoPessoa}/{codigoTurma}/{codigoDisciplina}/{ano}/{semestre}/{codigoPerfilAcesso}")
		public Response consultarHorarioProfessorDia(@PathParam("codigoPessoa") final Integer codigoPessoa, @PathParam("codigoTurma") 
		...
		final Integer semestre, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso) {
			...
			horarioProfessor.setHorarioProfessorDiaVOs(getFacadeFactory().getHorarioProfessorDiaFacade().consultarHorarioProfessorDia(0, 0, codigoPessoa, codigoDisciplina, codigoTurma, null, null, null, null, null, anoParametro, semestreParametro));
			for(HorarioProfessorDiaVO horarioProfessorDiaVO:horarioProfessor.getHorarioProfessorDiaVOs()) {
				horarioProfessorDiaVO.setData(Uteis.getDataAdicionadaEmHoras(Uteis.getDataComHoraSetadaParaPrimeiroMinutoDia(horarioProfessorDiaVO.getData()), 12));
			}
			return Response.status(Status.OK).entity(new GenericEntity<List<HorarioProfessorDiaVO>>(horarioProfessor.getHorarioProfessorDiaVOs()) {}).build();
		} catch (Exception e) {
			ErrorInfoRSVO errorInfoRSVO = new ErrorInfoRSVO();
			errorInfoRSVO.setCodigo(Status.UNAUTHORIZED.name());
			errorInfoRSVO.setMensagem(e.getMessage());
		...
		@GET
		@Produces(MediaType.APPLICATION_JSON)
		@Path("/realizarGeracaoRegistroAulaPeloHorarioProfessorDia/{horarioProfessorDia}/{codigoPessoa}/{codigoTurma}/{codigoDisciplina}/{ano}/{semestre}/{codigoPerfilAcesso}")
		public Response realizarGeracaoRegistroAulaPeloHorarioProfessorDia(@PathParam("horarioProfessorDia") final Integer horarioProfessorDia, @PathParam("codigoPessoa") final Integer codigoPessoa, @PathParam("codigoTurma") 
		final Integer codigoTurma, @PathParam("codigoDisciplina") final Integer codigoDisciplina, @PathParam("ano") final Integer ano, @PathParam("semestre") 
		final Integer semestre, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso) {
			try {
			UsuarioVO usuarioVO = autenticarConexao();
			...
			for (HorarioProfessorDiaVO horarioProfessorItem: horarioProfessor.getHorarioProfessorDiaVOs()) {
				horarioProfessorItem.setData(Uteis.getDataAdicionadaEmHoras(Uteis.getDataComHoraSetadaParaPrimeiroMinutoDia(horarioProfessorItem.getData()), 12));
				if (horarioProfessorItem.getCodigo().equals(horarioProfessorDia)){
					horarioProfessorDiaVO = (HorarioProfessorDiaVO) horarioProfessorItem.clone();
					horarioProfessorDiaVO.setData(Uteis.getDateSemHora(horarioProfessorItem.getData()));
					break;
				}
			}
			TurmaVO turmaVO = new TurmaVO();
			turmaVO.setCodigo(codigoTurma);
			DisciplinaVO disciplinaVO = getFacadeFactory().getDisciplinaFacade().consultarPorChavePrimaria(codigoDisciplina, Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO);
			...
			registroAulaVOs.addAll(getFacadeFactory().getRegistroAulaFacade().realizarGeracaoRegistroAulaPeloHorarioProfessorDia(horarioProfessorDiaVO, usuarioVO, turmaVO, disciplinaVO, usuarioVO.getPessoa(), anoParametro, semestreParametro, false));
			for (RegistroAulaVO registroAulaVO : registroAulaVOs) {
				registroAulaVO.setData(Uteis.getDataAdicionadaEmHoras(Uteis.getDataComHoraSetadaParaPrimeiroMinutoDia(registroAulaVO.getData()), 12));
				for (FrequenciaAulaVO frequenciaAulaVO : registroAulaVO.getFrequenciaAulaVOs()) {
					if (!frequenciaAulaVO.getNovoObj()) {
						frequenciaAulaVO.setNovoObjetoMobile(false);
					}
				}
			}
			return Response.status(Status.OK).entity(new GenericEntity<List<RegistroAulaVO>>(registroAulaVOs) {}).build();
		} catch (Exception e) {
		...
		try {
			...
			if (Uteis.isAtributoPreenchido(registroAulaVOs)) {
				for (RegistroAulaVO registroAulaVO : registroAulaVOs) {
					...
					for (FrequenciaAulaVO frequenciaAulaVO : registroAulaVO.getFrequenciaAulaVOs()) {
						if (!frequenciaAulaVO.getNovoObjetoMobile()) {
							frequenciaAulaVO.setNovoObj(false);
						}
					}
				}
...
//		}
		...
		try {
			...
			usuarioVO.getPerfilAcesso().setCodigo(codigoPerfilAcesso);
			usuarioVO.setVisaoLogar("professor");
			ControleAcesso.consultar("RegistroAula", true, usuarioVO);
			return Response.status(Status.OK).entity(usuarioVO).build();
		} catch (Exception e) {
			System.out.println("MOBILE ERROR: " + e.getMessage());
			ErrorInfoRSVO errorInfoRSVO = new ErrorInfoRSVO();
		...
		try {
			...
			usuarioVO.getPerfilAcesso().setCodigo(codigoPerfilAcesso);
			usuarioVO.setVisaoLogar("professor");
			ControleAcesso.consultar("LancamentoNota", true, usuarioVO);
			return Response.status(Status.OK).entity(usuarioVO).build();
		} catch (Exception e) {
			System.out.println("MOBILE ERROR: " + e.getMessage());
			ErrorInfoRSVO errorInfoRSVO = new ErrorInfoRSVO();
		...
		public Response consultarArquivosDownloadMaterial(@PathParam("unidadeEnsino") final Integer unidadeEnsino, @PathParam("matricula") final String matricula, @PathParam("ano") Integer ano, @PathParam("semestre") Integer semestre, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso, @PathParam("disciplina") final Integer disciplina, @PathParam("tipoArquivo") final String tipoArquivo) {
			try {
				...
				usuarioVO.getPerfilAcesso().setCodigo(codigoPerfilAcesso);
				usuarioVO.setVisaoLogar("aluno");
				ControleAcesso.consultar("DownloadArquivo", true, usuarioVO);
...
//						arquivoVO.setExtensao(ArquivoHelper.getExtensaoArquivo(arquivoVO.getNome()));
		...
		public Response registrarDownload(@PathParam("codigoArquivo") final Integer codigoArquivo, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("matricula") final String matricula) {
			try {
				...
				getFacadeFactory().getDownloadFacade().registrarDownload(arquivoVO, usuarioVO, new MatriculaPeriodoVO(), matriculaVO, configuracaoFinanceiroVO);
...
//			usuarioVO.setVisaoLogar("aluno");
		...
		public List<DocumetacaoMatriculaVO> consultarEntregaDocumentos(String matricula, Integer codigoPerfilAcesso)
			...
			if (!ControleAcesso.verificarPermissaoFuncionalidadeUsuario("EntregaDocumentoPermiteAnexarArquivo",
					usuarioVO)) {
			...
			realizarPreencherCaminhoAnexoImagem(documetacaoMatriculaVOs, configuracaoGeralSistemaVO);
		...
		public DocumentacaoMatriculaRSVO realizarUploadArquivoEspecificoValidandoSessaoNavegador(HttpServletRequest request) {
		    ...
		    if (ServletFileUpload.isMultipartContent(request)) {
		    	String cpfAluno = getFacadeFactory().getMatriculaFacade().consultarCPFalunoPreencherDocumentacaoMatriculaParaMatriculaOnline(documetacaoMatriculaVO.getMatricula());
		    	ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO = getFacadeFactory().getConfiguracaoGeralSistemaFacade().consultarConfiguracaoASerUsada(Uteis.NIVELMONTARDADOS_DADOSBASICOS_APLICATIVO, usuarioVO, null);
		    	if (frente) {
				            ...
				            try {
				            	...
				            	if (frente) {
				            		...
				            		if(Uteis.isAtributoPreenchido(documetacaoMatriculaVO.getArquivoVO().getCodigo()) ) {
				            			getFacadeFactory().getDocumetacaoMatriculaFacade().excluirDocumentacaoMatricula(documetacaoMatriculaVO,configuracaoGeralSistemaVO,false, usuarioVO);
				            			documetacaoMatriculaVO.getArquivoVO().setCpfAlunoDocumentacao(cpfAluno);				            			
				            		}
				            		
				            		getFacadeFactory().getArquivoHelper().upLoadDocumentacaoMatriculaRequerimentoAplicativo(item, documetacaoMatriculaVO.getArquivoVO(), documetacaoMatriculaVO.getArquivoVO().getCpfAlunoDocumentacao() + documetacaoMatriculaVO.getTipoDeDocumentoVO().getNome(), configuracaoGeralSistemaVO, PastaBaseArquivoEnum.DOCUMENTOS_TMP, usuarioVO);			            	
				            		String caminhoImagemAnexo = configuracaoGeralSistemaVO.getUrlExternoDownloadArquivo()+ File.separator+documetacaoMatriculaVO.getArquivoVO().getPastaBaseArquivo()  +  File.separator+documetacaoMatriculaVO.getArquivoVO().getNome();
				            		documetacaoMatriculaVO.getArquivoVO().setCaminhoImagemAnexo(caminhoImagemAnexo); 
				            		
				            	} else if (!frente) {				            		
				            		getFacadeFactory().getArquivoHelper().upLoadDocumentacaoMatriculaRequerimentoAplicativo(item, documetacaoMatriculaVO.getArquivoVOVerso(), documetacaoMatriculaVO.getArquivoVOVerso().getCpfAlunoDocumentacao() + documetacaoMatriculaVO.getTipoDeDocumentoVO().getNome(), configuracaoGeralSistemaVO, PastaBaseArquivoEnum.DOCUMENTOS_TMP, usuarioVO);
				            		String caminhoImagemAnexo = configuracaoGeralSistemaVO.getUrlExternoDownloadArquivo()+ File.separator+documetacaoMatriculaVO.getArquivoVOVerso().getPastaBaseArquivo()  +  File.separator+documetacaoMatriculaVO.getArquivoVOVerso().getNome();
				            		documetacaoMatriculaVO.getArquivoVOVerso().setCaminhoImagemAnexo(caminhoImagemAnexo);
				            	}
...
//				System.out.println(paramObject);
		...
		public RequerimentoVO novoRequerimentoAlunoV1(@PathParam("codigoPessoa") final Integer codigoPessoa, @PathParam("matricula") final String matricula, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso, @PathParam("codigoTipoRequerimento") final Integer codigoTipoRequerimento) {
			...
			try {
				...
				try {
					 if(ControleAcesso.verificarPermissaoFuncionalidadeUsuario("RequerimentoPermitirRequerenteAnexarArquivo", usuarioVO)) {
...
//				        }
		...
		@POST
		@Produces(MediaType.APPLICATION_JSON)
		@Path("/novoRecebimentoCartaoCredito")
		public NegociacaoRecebimentoRSVO novoRecebimentoCartaoCredito(final String paramObject) {
			...
			try {
				...
				Gson gson = new GsonBuilder().setExclusionStrategies(new ExcluirJsonStrategy())
						...
						.registerTypeAdapterFactory(new AlwaysListTypeAdapterFactory<>())
						.create();
				Type type = new TypeToken<ContaReceberVO>() {
				}.getType();
				contaReceberVO = gson.fromJson(paramObject, type);
				ConfiguracaoFinanceiroVO configuracaoFinanceiroVO = getFacadeFactory().getConfiguracaoFinanceiroFacade().consultarConfiguracaoASerUsada(Uteis.NIVELMONTARDADOS_DADOSBASICOS, contaReceberVO.getUnidadeEnsinoFinanceira().getCodigo(), usuarioVO);
				getFacadeFactory().getContaReceberFacade().carregarDados(contaReceberVO, configuracaoFinanceiroVO, usuarioVO);
				negociacaoRecebimentoRSVO.setContaReceber(contaReceberVO);
				CursoVO cursoVO = new CursoVO();
				if (Uteis.isAtributoPreenchido(contaReceberVO.getTipoOrigem()) && contaReceberVO.getTipoOrigem().equals(TipoOrigemContaReceber.INSCRICAO_PROCESSO_SELETIVO.getValor())) {
					InscricaoVO inscricaoVO = getFacadeFactory().getInscricaoFacade().consultarPorChavePrimaria(Integer.parseInt(contaReceberVO.getCodOrigem()), Uteis.NIVELMONTARDADOS_TODOS, usuarioVO);
					if (Uteis.isAtributoPreenchido(inscricaoVO.getCursoOpcao1().getCurso())) {
						cursoVO = inscricaoVO.getCursoOpcao1().getCurso();
					}
				} else if (Uteis.isAtributoPreenchido(contaReceberVO.getMatriculaAluno().getMatricula()) || Uteis.isAtributoPreenchido(contaReceberVO.getResponsavelFinanceiro().getCodigo())) {
					usuarioVO.setVisaoLogar("aluno");
					if (Uteis.isAtributoPreenchido(contaReceberVO.getMatriculaAluno().getCurso())) {
						cursoVO = contaReceberVO.getMatriculaAluno().getCurso();
					} else {
						cursoVO = getFacadeFactory().getCursoFacade().consultarCursoPorMatricula(contaReceberVO.getMatriculaAluno().getMatricula(), false, Uteis.NIVELMONTARDADOS_COMBOBOX, usuarioVO);
					}
				} else if (Uteis.isAtributoPreenchido(contaReceberVO.getTurma().getCodigo())) {
					usuarioVO.setVisaoLogar("aluno");
					cursoVO = getFacadeFactory().getCursoFacade().consultarCursoPorTurma(contaReceberVO.getTurma().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO);
				}
				ConfiguracaoRecebimentoCartaoOnlineVO configuracaoRecebimentoCartaoOnlineVO = getFacadeFactory().getConfiguracaoRecebimentoCartaoOnlineFacade().consultarConfiguracaoRecebimentoCartaoOnlineDisponivel(contaReceberVO.getTurma().getCodigo(), cursoVO.getCodigo(), TipoNivelEducacional.getEnum(cursoVO.getNivelEducacional()).toString(), contaReceberVO.getUnidadeEnsinoFinanceira().getCodigo(), Uteis.NIVELMONTARDADOS_TODOS, contaReceberVO.getTipoOrigem().equals(TipoOrigemContaReceber.INSCRICAO_PROCESSO_SELETIVO.getValor()) ? null : usuarioVO);
				...
				if (Uteis.isAtributoPreenchido(configuracaoRecebimentoCartaoOnlineVO.getApresentarOpcaoRecorrenciaAluno())) {
					negociacaoRecebimentoRSVO.setApresentarOpcaoRecorrenciaAluno(configuracaoRecebimentoCartaoOnlineVO.getApresentarOpcaoRecorrenciaAluno());			
				} else {
					negociacaoRecebimentoRSVO.setApresentarOpcaoRecorrenciaAluno(false);
				}
				if(Uteis.isAtributoPreenchido(contaReceberVO.getMatriculaAluno().getMatricula())) {
					...
					FormaPagamentoMatriculaOnlineExternaRSVO formaPagamentoMatriculaOnlineExternaRSVO = new FormaPagamentoMatriculaOnlineExternaRSVO();
					formaPagamentoMatriculaOnlineExternaRSVO.setOrientacaoRecorrenciaAluno(configuracaoRecebimentoCartaoOnlineVO.getOrientacaoRecorrenciaAluno());
					getFacadeFactory().getFormaPagamentoNegociacaoRecebimentoFacade().inicializarDadosCartaoCreditoRecorrenciaCadastrada(formaPagamentoNegociacaoRecebimentoVO, contaReceberVO.getMatriculaAluno().getMatricula(), usuarioVO);
					if (Uteis.isAtributoPreenchido(formaPagamentoNegociacaoRecebimentoVO.getFormaPagamentoNegociacaoRecebimentoCartaoCreditoVO().getNumeroCartao())) {
						formaPagamentoMatriculaOnlineExternaRSVO.setNumeroCartao(formaPagamentoNegociacaoRecebimentoVO.getFormaPagamentoNegociacaoRecebimentoCartaoCreditoVO().getNumeroCartao());
						formaPagamentoMatriculaOnlineExternaRSVO.setCodigoDeVerificacao(formaPagamentoNegociacaoRecebimentoVO.getFormaPagamentoNegociacaoRecebimentoCartaoCreditoVO().getCodigoVerificacao());
						formaPagamentoMatriculaOnlineExternaRSVO.setMesValidade(formaPagamentoNegociacaoRecebimentoVO.getFormaPagamentoNegociacaoRecebimentoCartaoCreditoVO().getMesValidade());
						formaPagamentoMatriculaOnlineExternaRSVO.setAnoValidade(formaPagamentoNegociacaoRecebimentoVO.getFormaPagamentoNegociacaoRecebimentoCartaoCreditoVO().getAnoValidade());
						formaPagamentoMatriculaOnlineExternaRSVO.setNomeNoCartao(formaPagamentoNegociacaoRecebimentoVO.getFormaPagamentoNegociacaoRecebimentoCartaoCreditoVO().getNomeCartaoCredito());
					}
					negociacaoRecebimentoRSVO.getFormaPagamentoMatriculaOnlineExternaRSVOs().add(formaPagamentoMatriculaOnlineExternaRSVO);
				}
				Date dataBase = new Date();
				...
				negociacaoRecebimentoRSVO.setQuantidadeParcelasAPagar(1);
				negociacaoRecebimentoRSVO.getBandeiraRSVOs().addAll(getFacadeFactory().getNegociacaoRecebimentoFacade().consultarBandeirasDisponiveisPagamentoOnline(configuracaoRecebimentoCartaoOnlineVO, contaReceberVO.getUnidadeEnsinoFinanceira(), negociacaoRecebimentoRSVO.getValorTotalAPagar()));
				List<TipoOrigemContaReceber> listaTipoOrigemContaReceber = new ArrayList<TipoOrigemContaReceber>();
				listaTipoOrigemContaReceber.add(negociacaoRecebimentoRSVO.getContaReceber().getTipoOrigemContaReceber());
				negociacaoRecebimentoRSVO.setQuantidadeLimiteParcela(configuracaoRecebimentoCartaoOnlineVO.getQtdeParcelasPermitida(negociacaoRecebimentoRSVO.getValorTotalAPagar(), contaReceberVO.getDataVencimento(), usuarioVO, listaTipoOrigemContaReceber));
			} catch (Exception e) {
				System.out.println("MOBILE ERROR: " + e.getMessage());
		...
		@POST
		@Produces(MediaType.APPLICATION_JSON)
		@Path("/gravarRecebimentoCartaoCredito")
		public NegociacaoRecebimentoRSVO gravarRecebimentoCartaoCredito(final String paramObject) {
			try {
				UsuarioVO usuarioVO = obterUsuarioWebService();
				NegociacaoRecebimentoRSVO negociacaoRecebimentoRSVO = new NegociacaoRecebimentoRSVO();
				Gson gson = new GsonBuilder().setExclusionStrategies(new ExcluirJsonStrategy())
						...
						.registerTypeAdapterFactory(new AlwaysListTypeAdapterFactory<>())
						.create();
				Type type = new TypeToken<NegociacaoRecebimentoRSVO>() {
				}.getType();
				negociacaoRecebimentoRSVO = gson.fromJson(paramObject, type);
				ConfiguracaoFinanceiroVO configuracaoFinanceiroVO = getFacadeFactory().getConfiguracaoFinanceiroFacade().consultarConfiguracaoASerUsada(Uteis.NIVELMONTARDADOS_DADOSBASICOS, negociacaoRecebimentoRSVO.getContaReceber().getUnidadeEnsinoFinanceira().getCodigo(), usuarioVO);
				if (Uteis.isAtributoPreenchido(negociacaoRecebimentoRSVO.getContaReceber().getMatriculaAluno().getMatricula())) {
					MatriculaVO matriculaVO = getFacadeFactory().getMatriculaFacade().consultarPorChavePrimaria(negociacaoRecebimentoRSVO.getContaReceber().getMatriculaAluno().getMatricula(), negociacaoRecebimentoRSVO.getContaReceber().getUnidadeEnsino().getCodigo(), Uteis.NIVELMONTARDADOS_TODOS, configuracaoFinanceiroVO, usuarioVO);
					negociacaoRecebimentoRSVO.getContaReceber().setMatriculaAluno(matriculaVO);
			   }
				UsuarioVO usuarioMatriculaVO = null;
				if(Uteis.isAtributoPreenchido(getAplicacaoControle().getConfiguracaoGeralSistemaVO(negociacaoRecebimentoRSVO.getContaReceber().getUnidadeEnsino().getCodigo() ,null).getUsuarioResponsavelOperacoesExternas())) {
					PessoaVO pessoaVO = getAplicacaoControle().getConfiguracaoGeralSistemaVO(negociacaoRecebimentoRSVO.getContaReceber().getUnidadeEnsino().getCodigo() ,null).getUsuarioResponsavelOperacoesExternas();
					 usuarioMatriculaVO = getFacadeFactory().getUsuarioFacade().consultarPorPessoa(pessoaVO.getCodigo(), false, Uteis.NIVELMONTARDADOS_COMBOBOX, null);
				}
				NegociacaoRecebimentoVO negociacaoRecebimentoVO = negociacaoRecebimentoRSVO.getContaReceber().gerarNegociacaoRecebimentoVOPreenchido(usuarioVO, negociacaoRecebimentoRSVO.getContaReceber().getPessoa(), negociacaoRecebimentoRSVO.getContaReceber().getResponsavelFinanceiro(), negociacaoRecebimentoRSVO.getContaReceber().getFuncionario(), negociacaoRecebimentoRSVO.getContaReceber().getParceiroVO(), configuracaoFinanceiroVO);
				negociacaoRecebimentoVO.setValorTotalRecebimento(negociacaoRecebimentoRSVO.getValorTotalAPagar());
				
				CursoVO cursoVO = new CursoVO();
				if (negociacaoRecebimentoRSVO.getContaReceber().getTipoOrigem().equals(TipoOrigemContaReceber.INSCRICAO_PROCESSO_SELETIVO.getValor())) {
					InscricaoVO inscricaoVO = getFacadeFactory().getInscricaoFacade().consultarPorChavePrimaria(Integer.parseInt(negociacaoRecebimentoRSVO.getContaReceber().getCodOrigem()), Uteis.NIVELMONTARDADOS_TODOS, usuarioVO);
					if (Uteis.isAtributoPreenchido(inscricaoVO.getCursoOpcao1().getCurso())) {
						cursoVO = inscricaoVO.getCursoOpcao1().getCurso();
					}
				} else if (Uteis.isAtributoPreenchido(negociacaoRecebimentoRSVO.getContaReceber().getMatriculaAluno().getMatricula()) || Uteis.isAtributoPreenchido(negociacaoRecebimentoRSVO.getContaReceber().getResponsavelFinanceiro().getCodigo())) {
					usuarioMatriculaVO.setVisaoLogar("aluno");
					if (Uteis.isAtributoPreenchido(negociacaoRecebimentoRSVO.getContaReceber().getMatriculaAluno().getCurso())) {
						cursoVO = negociacaoRecebimentoRSVO.getContaReceber().getMatriculaAluno().getCurso();
					} else {
						cursoVO = getFacadeFactory().getCursoFacade().consultarCursoPorMatricula(negociacaoRecebimentoRSVO.getContaReceber().getMatriculaAluno().getMatricula(), false, Uteis.NIVELMONTARDADOS_COMBOBOX, usuarioVO);
					}
				} else if (Uteis.isAtributoPreenchido(negociacaoRecebimentoRSVO.getContaReceber().getTurma().getCodigo())) {
					usuarioMatriculaVO.setVisaoLogar("aluno");
					cursoVO = getFacadeFactory().getCursoFacade().consultarCursoPorTurma(negociacaoRecebimentoRSVO.getContaReceber().getTurma().getCodigo(), false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO);
				}
				
				negociacaoRecebimentoVO.setConfiguracaoRecebimentoCartaoOnlineVO(getFacadeFactory().getConfiguracaoRecebimentoCartaoOnlineFacade().consultarConfiguracaoRecebimentoCartaoOnlineDisponivel(negociacaoRecebimentoRSVO.getContaReceber().getTurma().getCodigo(), cursoVO.getCodigo(), TipoNivelEducacional.getEnum(cursoVO.getNivelEducacional()).toString(), negociacaoRecebimentoRSVO.getContaReceber().getUnidadeEnsinoFinanceira().getCodigo(),Uteis.NIVELMONTARDADOS_TODOS, negociacaoRecebimentoRSVO.getContaReceber().getTipoOrigem().equals(TipoOrigemContaReceber.INSCRICAO_PROCESSO_SELETIVO.getValor()) ? null : usuarioMatriculaVO));
				...
				if (negociacaoRecebimentoVO.getConfiguracaoRecebimentoCartaoOnlineVO().getApresentarOpcaoRecorrenciaAluno() && negociacaoRecebimentoVO.getConfiguracaoRecebimentoCartaoOnlineVO().getUtilizarOpcaoRecorrenciaDefaulMarcado()) {
					negociacaoRecebimentoVO.setCriarRegistroRecorrenciaDCC(true);
				} else {
					negociacaoRecebimentoVO.setCriarRegistroRecorrenciaDCC(false);
				}
				List<ContaReceberVO> contaReceberVOs = new ArrayList<>();
				contaReceberVOs.add(negociacaoRecebimentoRSVO.getContaReceber());
				for (ContaReceberVO obj : contaReceberVOs) {
					...
					negociacaoRecebimentoVO.setValorTotal(Uteis.arrendondarForcando2CadasDecimais(obj.getValorReceberCalculado()));
				}
				...
				for (FormaPagamentoMatriculaOnlineExternaRSVO obj : negociacaoRecebimentoRSVO.getFormaPagamentoMatriculaOnlineExternaRSVOs()) {
					...
					if (obj.getParcela() > negociacaoRecebimentoRSVO.getQuantidadeLimiteParcela()) {
						throw new Exception("A quantidade de parcelas informada é acima do limite permitido de " + negociacaoRecebimentoRSVO.getQuantidadeLimiteParcela());
					}
					formaPagamentoNegociacaoRecebimentoVO.setQtdeParcelasCartaoCredito(obj.getParcela());
					...
					formaPagamentoNegociacaoRecebimentoVO.setUtilizarCartaoComoPagamentoRecorrenteProximaParcela(obj.getUtilizarCartaoComoPagamentoRecorrenteProximaParcela());
					formaPagamentoNegociacaoRecebimentoVO.setFormaPadraoPagamentoAutomaticoParcelaRecorrencia(obj.getFormaPadraoPagamentoAutomaticoParcelaRecorrencia());
					formaPagamentoNegociacaoRecebimentoVO.setDiaPadraoPagamentoRecorrencia(obj.getDiaPadraoPagamentoRecorrencia());
				...
				imprimirComprovanteRecebimento(negociacaoRecebimentoRSVO, negociacaoRecebimentoVO, negociacaoRecebimentoRSVO.getContaReceber().getMatriculaAluno(), usuarioMatriculaVO);
	...
	public void imprimirComprovanteRecebimento(NegociacaoRecebimentoRSVO negociacaoRecebimentoRSVO,	NegociacaoRecebimentoVO negociacaoRecebimentoVO, MatriculaVO matricula, UsuarioVO usuario) {
		try {
			ComprovanteRecebimentoRelControle comprovanteRecebimentoRelControle = new ComprovanteRecebimentoRelControle();
...
//					}
	...
	public String realizarEmissaoArquivoRequerimento(final String paramObject) {
		try {
			...
			if (Uteis.isAtributoPreenchido(requerimentoTurmaReposicaoRSVO.getRequerimentoVO().getMatriculaPeriodoVO())) {
				requerimentoTurmaReposicaoRSVO.getRequerimentoVO().setMatriculaPeriodoVO(getFacadeFactory().getMatriculaPeriodoFacade().consultarPorChavePrimaria(requerimentoTurmaReposicaoRSVO.getRequerimentoVO().getMatriculaPeriodoVO().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, getAplicacaoControle().getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(requerimentoTurmaReposicaoRSVO.getRequerimentoVO().getUnidadeEnsino().getCodigo()), usuarioVO));
			}
			ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO = getFacadeFactory().getConfiguracaoGeralSistemaFacade().consultarConfiguracaoASerUsada(Uteis.NIVELMONTARDADOS_DADOSBASICOS_APLICATIVO, usuarioVO, null);
			if (!getConfiguracaoGeralSistemaVO().getUrlAcessoExternoAplicacao().endsWith("/")) {
				getConfiguracaoGeralSistemaVO().setUrlAcessoExternoAplicacao(getConfiguracaoGeralSistemaVO().getUrlAcessoExternoAplicacao() + "/");
			...
			if (requerimentoTurmaReposicaoRSVO.getTipoArquivo().equals("VISUALIZACAO_CERTIFICADO")) {
				...
				caminhoArquivo = getFacadeFactory().getRequerimentoFacade().realizarImpressaoDeclaracaoRequerimento(requerimentoTurmaReposicaoRSVO.getRequerimentoVO(), contratoGravar, contratoGravar, configuracaoGeralSistemaVO, getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(requerimentoTurmaReposicaoRSVO.getRequerimentoVO().getUnidadeEnsino().getCodigo()), texto, true, usuarioVO);
	...
	public RequerimentoVO gravarRequerimentoV1(final String paramObject) {
		...
		try {
			...
			if (Uteis.isAtributoPreenchido(requerimentoVO.getMatriculaPeriodoVO())) {
				requerimentoVO.setMatriculaPeriodoVO(getFacadeFactory().getMatriculaPeriodoFacade().consultarPorChavePrimaria(requerimentoVO.getMatriculaPeriodoVO().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, configuracaoFinanceiroVO, usuarioVO));
			}
...
//				}				
			...
			getFacadeFactory().getRequerimentoFacade().persistirRequerimento(requerimentoVO, requerimentoHistoricoVO, Boolean.TRUE, requerimentoVO.getExigePagamento(), configuracaoGeralSistemaVO, configuracaoFinanceiroVO, requerimentoVO.getMatricula().getUnidadeEnsino(), usuarioVO);			
	...
	public RequerimentoVO editarRequerimento(Integer codigoRequerimento, Integer codigoUnidadeEnsinoMatricula) {
		try {
			...
			ConfiguracaoFinanceiroVO configuracaoFinanceiroVO = getFacadeFactory().getConfiguracaoFinanceiroFacade().consultarConfiguracaoASerUsada(Uteis.NIVELMONTARDADOS_DADOSBASICOS, codigoUnidadeEnsinoMatricula, null);
			ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO = getFacadeFactory().getConfiguracaoGeralSistemaFacade().consultarConfiguracaoASerUsada(Uteis.NIVELMONTARDADOS_DADOSBASICOS_APLICATIVO, usuarioVO, null);
			requerimentoVO = getFacadeFactory().getRequerimentoFacade().consultarPorChavePrimaria(codigoRequerimento, Uteis.NIVELMONTARDADOS_TODOS, usuarioVO, configuracaoFinanceiroVO);
			...
			try {
				 if(ControleAcesso.verificarPermissaoFuncionalidadeUsuario("RequerimentoPermitirRequerenteInteragirTramite", usuarioVO)) {
	...
	public Response excluirRequerimentoV2(@PathParam("codigoRequerimento") final Integer codigoRequerimento, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula) {		
		...
		try {
			...
			ConfiguracaoFinanceiroVO configuracaoFinanceiroVO = getFacadeFactory().getConfiguracaoFinanceiroFacade().consultarConfiguracaoASerUsada(Uteis.NIVELMONTARDADOS_DADOSBASICOS, codigoUnidadeEnsinoMatricula, null);
			requerimentoVO = getFacadeFactory().getRequerimentoFacade().consultarPorChavePrimaria(codigoRequerimento, Uteis.NIVELMONTARDADOS_TODOS, usuarioVO, configuracaoFinanceiroVO);
			getFacadeFactory().getRequerimentoFacade().excluir(requerimentoVO, configuracaoFinanceiroVO, usuarioVO, configuracaoGeralSistemaVO, true);			
		...
		public RequerimentoVO gravarInteracaoTramiteRequerimento(final String paramObject) {
			try {
				...
				requerimentoTurmaReposicaoRSVO.setRequerimentoVO(getFacadeFactory().getRequerimentoFacade().consultarPorChavePrimaria(requerimentoTurmaReposicaoRSVO.getRequerimentoVO().getCodigo(), Uteis.NIVELMONTARDADOS_TODOS, usuarioVO, configuracaoFinanceiroVO));
...
//				requerimentoVO = getFacadeFactory().getRequerimentoFacade().consultarPorChavePrimaria(requerimentoVO.getCodigo(), Uteis.NIVELMONTARDADOS_TODOS, usuarioVO, configuracaoFinanceiroVO);
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/consultarCursoProcessoSeletivo/{codigoProcessoSeletivo}/{codigoUnidadeEnsino}")
	public List<ProcSeletivoCursoVO> consultarCursoProcessoSeletivo(@PathParam("codigoProcessoSeletivo") final Integer codigoProcessoSeletivo , @PathParam("codigoUnidadeEnsino") final Integer codigoUnidadeEnsino) {
		
		try {
			autenticarTokenCliente();
			UsuarioVO usuarioVO = getFacadeFactory().getUsuarioFacade().obterUsuarioResponsavelOperacoesExternas();
			List<ProcSeletivoCursoVO> lista = new ArrayList<ProcSeletivoCursoVO>(0);
			//lista =  getFacadeFactory().getProcSeletivoCursoFacade().consultarPorCodigoProcSeletivoUnidadeEnsino(codigoProcessoSeletivo,codigoUnidadeEnsino,usuarioVO );
			lista =  getFacadeFactory().getProcSeletivoCursoFacade().consultarPorCodigoProcSeletivoUnidadeEnsinoOpcaoInscicao(codigoProcessoSeletivo,codigoUnidadeEnsino,new Date(), Uteis.NIVELMONTARDADOS_DADOSENTIDADESUBORDINADAS , usuarioVO);
			for(ProcSeletivoCursoVO proc : lista) {
		          List<DisciplinasGrupoDisciplinaProcSeletivoVO> lista2	 = proc.getGrupoDisciplinaProcSeletivo().getDisciplinasGrupoDisciplinaProcSeletivoVOs() ;
		          Iterator i = lista2.iterator();
		          while(i.hasNext()) {
		        	  DisciplinasGrupoDisciplinaProcSeletivoVO disc = (DisciplinasGrupoDisciplinaProcSeletivoVO) i.next();		        	  
		        	  if(!disc.getDisciplinasProcSeletivo().getDisciplinaIdioma()) {
		        		  i.remove();
		        	  }
		          }
			}
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/consultarDataProvaProcessoSeletivo/{codigoProcessoSeletivo}")
	public List<ItemProcSeletivoDataProvaVO> consultarDataProvaProcessoSeletivo(@PathParam("codigoProcessoSeletivo") final Integer codigoProcessoSeletivo) {					
		try {
			autenticarTokenCliente();
			UsuarioVO usuarioVO = getFacadeFactory().getUsuarioFacade().obterUsuarioResponsavelOperacoesExternas();		
			return getFacadeFactory().getItemProcSeletivoDataProvaFacade().consultarPorProcSelectivoAptoInscricao(codigoProcessoSeletivo, true, false, Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO);
		} catch (Exception e) {
			ErrorInfoRSVO errorInfoRSVO = new ErrorInfoRSVO();
			errorInfoRSVO.setCodigo(Status.UNAUTHORIZED.name());
			errorInfoRSVO.setMensagem(e.getMessage());
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)	
    @Path("/uploadArquivoInscricaoProcessoSeletivo")
	public ArquivoVO uploadArquivoInscricaoProcessoSeletivo(@Context HttpServletRequest request) {		
	 try {	
			autenticarTokenCliente();
			 UsuarioVO usuarioVO = getFacadeFactory().getUsuarioFacade().obterUsuarioResponsavelOperacoesExternas();
			ArquivoVO arquivoVO = new ArquivoVO();			
			request.getHeaderNames();		
			String objetoArquivo = request.getHeader("objeto") != null ? request.getHeader("objeto") : "";
			FileItemFactory factory = new DiskFileItemFactory();
			ServletFileUpload upload = new ServletFileUpload(factory);
			List<FileItem> items = null;
			try {
			...
			if (!Uteis.isAtributoPreenchido(objetoArquivo)) {
				if (items != null && items.stream().anyMatch(t -> t.getFieldName().equals("objeto"))) {
					objetoArquivo = items.stream().filter(t -> t.getFieldName().equals("objeto")).findFirst().get()
							.getString();
				}
			}
			Gson gson = new GsonBuilder()
					...
					.registerTypeAdapterFactory(new AlwaysListTypeAdapterFactory<>())
					.create();
			Type type = new TypeToken<ArquivoVO>() {}.getType();
			
			ArquivoVO arquivoJson  = gson.fromJson(objetoArquivo, type);

			if (Uteis.isAtributoPreenchido(arquivoJson)) {

				if (Uteis.isAtributoPreenchido(arquivoJson.getCodigo())
						&& Uteis.isAtributoPreenchido(arquivoJson.getCodOrigem())) {
					arquivoVO.setCodigo(arquivoJson.getCodigo());
					arquivoVO.setCodOrigem(arquivoJson.getCodOrigem());
		        ...
		        while (iter.hasNext()) {
		            ...
		            if (!item.isFormField() && item.getSize() > 0) {
			            try {
				            	...
				            	arquivoVO.setCpfAlunoDocumentacao(item.getFieldName());
				    			arquivoVO.setDescricao(item.getName().substring(0, item.getName().lastIndexOf(".")));		    			
				    		
				            		getFacadeFactory().getArquivoHelper().upLoadArquivoIncricao(item, arquivoVO,  configuracaoGeralSistemaVO, PastaBaseArquivoEnum.CERTIFICADOSINSCRICOES_TMP, usuarioVO);
				        			registrarAtividadeUsuario(usuarioVO, "InscricaoWebService", "Iniciando Adicionar Arquivo Inscricao", "Uploading");
				        			Uteis.checkState(!Uteis.isAtributoPreenchido(arquivoVO.getNome()), "Arquivo nao Encontrado por favor verificar as extensões ou local do arquivo");
				        			...
				        			arquivoVO.setPastaBaseArquivoEnum(PastaBaseArquivoEnum.CERTIFICADOSINSCRICOES_TMP);
				        				...
				        				if (!arquivoVO.getNome().equals("")) {
				        					if (!arquivoVO.getCodigo().equals(0)) {
				        						getFacadeFactory().getArquivoFacade().alterar(arquivoVO, usuarioVO, getConfiguracaoGeralPadraoSistema());
				        					} else {
				        						getFacadeFactory().getArquivoFacade().incluir(arquivoVO, usuarioVO, getConfiguracaoGeralPadraoSistema());
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/gravarInscricaoProcessoSeletivo")
	public InscricaoVO gravarInscricaoProcessoSeletivo(InscricaoVO inscricaoVO) {
		try {
			autenticarTokenCliente();
			UsuarioVO usuarioVO = getFacadeFactory().getUsuarioFacade().obterUsuarioResponsavelOperacoesExternas();		
			usuarioVO.setVisaoLogar("candidato");
			
			inscricaoVO.setProcSeletivo(getFacadeFactory().getProcSeletivoFacade().consultarPorChavePrimaria(inscricaoVO.getProcSeletivo().getCodigo(), Uteis.NIVELMONTARDADOS_TODOS, usuarioVO));
			if (inscricaoVO.getFormaIngresso().equals("PD") || inscricaoVO.getFormaIngresso().equals("EN")  || inscricaoVO.getFormaIngresso().equals("TR") || (  inscricaoVO.getProcSeletivo().getTipoAvaliacaoProcessoSeletivo().equals(TipoAvaliacaoProcessoSeletivoEnum.AVALIACAO_CURRICULAR))) {
				ItemProcSeletivoDataProvaVO   itemProcessoSeletivoMaiorDataProva = getFacadeFactory().getItemProcSeletivoDataProvaFacade().consultarMaiorDatas(inscricaoVO.getProcSeletivo().getCodigo());
				inscricaoVO.setItemProcessoSeletivoDataProva(itemProcessoSeletivoMaiorDataProva);
			}
			
			if (Uteis.isAtributoPreenchido(inscricaoVO.getCursoOpcao1().getCodigo())) {			
			        inscricaoVO.setCursoOpcao1(getFacadeFactory().getUnidadeEnsinoCursoFacade().consultarPorChavePrimaria(inscricaoVO.getCursoOpcao1().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO));
			}
			if (Uteis.isAtributoPreenchido(inscricaoVO.getCursoOpcao2().getCodigo())) {			
		        inscricaoVO.setCursoOpcao2(getFacadeFactory().getUnidadeEnsinoCursoFacade().consultarPorChavePrimaria(inscricaoVO.getCursoOpcao2().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO));
			}
			if (Uteis.isAtributoPreenchido(inscricaoVO.getCursoOpcao3().getCodigo())) {			
		        inscricaoVO.setCursoOpcao3(getFacadeFactory().getUnidadeEnsinoCursoFacade().consultarPorChavePrimaria(inscricaoVO.getCursoOpcao3().getCodigo(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO));
			}
			
			inscricaoVO = getFacadeFactory().getInscricaoFacade().gravarInscricaoCandidato(inscricaoVO, inscricaoVO.getArquivoVO(), getConfiguracaoFinanceiroPrevilegiandoUnidadeEnsino(inscricaoVO.getUnidadeEnsino().getCodigo()), getUsuarioLogado());
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)	
	@Path("/realizarImpressaoComprovanteInscricao/{codigoInscricao}")
	public Response realizarImpressaoComprovanteInscricao(@PathParam("codigoInscricao") final Integer codigoInscricao) {		
		try {
			autenticarTokenCliente();
			InscricaoVO inscricaoVO = new InscricaoVO();
			UsuarioVO usuarioVO = getFacadeFactory().getUsuarioFacade().obterUsuarioResponsavelOperacoesExternas();
			inscricaoVO = getFacadeFactory().getInscricaoFacade().consultarPorChavePrimaria(codigoInscricao,
					Uteis.NIVELMONTARDADOS_TODOS, usuarioVO);

			ComprovanteInscricaoRelControle comprovanteInscricaoRelControle = new ComprovanteInscricaoRelControle();	
			
			if (!inscricaoVO.getCodigo().equals(0)) {
				comprovanteInscricaoRelControle.setInscricaoVO(inscricaoVO);
				comprovanteInscricaoRelControle.imprimirPDF();
			} else {
				setMensagemID("msg_relatorio_sem_dados");
			}
			UrlRSVO url = new UrlRSVO();
			url.setUrl(getCaminhoWeb(usuarioVO)+"relatorio/"+comprovanteInscricaoRelControle.getCaminhoRelatorio());			
			return Response.status(Status.OK).entity(url).build();
		} catch (Exception e) {
			ErrorInfoRSVO errorInfoRSVO = new ErrorInfoRSVO();
			errorInfoRSVO.setCodigo(Status.UNAUTHORIZED.name());
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/gerarProvaInscricao/{codigoInscricao}/{codigoAutenticacao}/{codigoautenticacaoNavegador}/{navegador}")
	public InscricaoVO gerarProvaInscricao(@PathParam("codigoInscricao") final Integer codigoInscricao , @PathParam("codigoAutenticacao") final Integer codigoAutenticacao  ,  @PathParam("codigoautenticacaoNavegador") final String codigoautenticacaoNavegador ,  @PathParam("navegador") final String navegador) {
		InscricaoVO inscricaoVO = new InscricaoVO();
		try {
			autenticarTokenCliente();
			UsuarioVO usuarioVO = getFacadeFactory().getUsuarioFacade().obterUsuarioResponsavelOperacoesExternas();
			...
			getFacadeFactory().getInscricaoFacade().validarInscricaoAptoApresentarProvaProcessoSeletivoResultadoProcessoSeletivo(inscricaoVO,"PROVA_APLICADA");			
			...
			if (Uteis.isAtributoPreenchido(inscricaoVO.getProvaProcessoSeletivoVO().getCodigo())) {
				...
				throw new Exception("Não Foi Encontrado Uma Prova Para Essa Inscrição");	
			}
			inscricaoVO = getFacadeFactory().getResultadoProcessoSeletivoFacade().executarMontagemDadosResultadoProcessoSeletivoPorInscricaoVO(inscricaoVO, usuarioVO);
			if(Uteis.isAtributoPreenchido(navegador)) {
				inscricaoVO.setNavegadorAcesso(navegador);
			    inscricaoVO.getResultadoProcessoSeletivoVO().setNavegadorAcesso(navegador);
			}
			if(!Uteis.isAtributoPreenchido(inscricaoVO.getResultadoProcessoSeletivoVO())  && 
				Uteis.isAtributoPreenchido(inscricaoVO.getProvaProcessoSeletivoVO()) && 
				inscricaoVO.getFormaIngresso().equals(FormaIngresso.PROCESSO_SELETIVO.getValor()) && 
				inscricaoVO.getProcSeletivo().getRealizarProvaOnline() && 
				inscricaoVO.getProcSeletivo().getTipoAvaliacaoProcessoSeletivo().equals(TipoAvaliacaoProcessoSeletivoEnum.ON_LINE)){
				inscricaoVO.getResultadoProcessoSeletivoVO().setResultadoPrimeiraOpcao("AG");
				inscricaoVO.getResultadoProcessoSeletivoVO().setResultadoSegundaOpcao("AG");
				inscricaoVO.getResultadoProcessoSeletivoVO().setResultadoTerceiraOpcao("AG");
		     }
			getFacadeFactory().getResultadoProcessoSeletivoFacade().persistir(inscricaoVO.getResultadoProcessoSeletivoVO(), usuarioVO, false);			
			if (!Uteis.isAtributoPreenchido(inscricaoVO.getDataHoraInicio())) {
				inscricaoVO.setDataHoraInicio(Uteis.getDataJDBCTimestamp(new Date()));								
				getFacadeFactory().getInscricaoFacade().alterarDataHoraInicioProvaProcessoSeletivo(inscricaoVO, usuarioVO);				
			}	
			getFacadeFactory().getInscricaoFacade().validarSessaoNavegadorProvaOnline(inscricaoVO,inscricaoVO.getCodigoAutenticacaoNavegador(),codigoautenticacaoNavegador, true, usuarioVO);			
			...
			inscricaoVO.setTempoRestanteProva(UteisData.diferencaEmSegundosEntreDatas(new Date(), prazoProva));			 
				
			if (inscricaoVO.getTempoRestanteProva() > 0L) {				
				JobProvaProcessoSeletivo jobProva =  getAplicacaoControle().getJobProvaProcessoSeletivoPorInscricao(inscricaoVO.getCodigo());
				if(jobProva == null) {
					jobProva = new JobProvaProcessoSeletivo(inscricaoVO.getCodigo(), prazoProva.getTime());
					 Thread threadProvaProcessoSeletivo = new Thread(jobProva);
					 threadProvaProcessoSeletivo.start();
					 getAplicacaoControle().adicionarJobProvaProcessoSeletivo(inscricaoVO.getCodigo(), jobProva);
				}else {					
...
//			inscricaoVO.setClassificacao(getFacadeFactory().getEstatisticaProcessoSeletivoRelFacade().verificarClassificacaoCandidado(inscricaoVO));
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/consultarDisciplinasLinguaExtrangeiraProcessoSeletivo/{codigoProcessoSeletivo}")
	public List<DisciplinasProcSeletivoVO> consultarDisciplinasLinguaExtrangeiraProcessoSeletivo(@PathParam("codigoProcessoSeletivo") final Integer codigoProcessoSeletivo ) throws Exception {
		try {
		    autenticarTokenCliente();
			UsuarioVO usuarioVO = getFacadeFactory().getUsuarioFacade().obterUsuarioResponsavelOperacoesExternas();
			List<DisciplinasProcSeletivoVO> lista = new ArrayList<DisciplinasProcSeletivoVO>(0);	
		    return DisciplinasGrupoDisciplinaProcSeletivo.consultarDisciplinasGrupoDisciplinaProcSeletivosLinguaExtrangeira(codigoProcessoSeletivo, usuarioVO);
		} catch (Exception e) {
			ErrorInfoRSVO errorInfoRSVO = new ErrorInfoRSVO();
			errorInfoRSVO.setCodigo(Status.UNAUTHORIZED.name());
			errorInfoRSVO.setMensagem(e.getMessage());
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/alterarVencimentoContaReceber")
	public Response alterarVencimentoContaReceber(final String  paramObject, @Context final HttpServletRequest request) throws Exception {
		try {
			validarTokenIntegracao(request);
			ContaReceberVO contaReceberVO = getFacadeFactory().getContaReceberFacade().alterarVencimentoContaReceberWS(paramObject);
			return Response.status(Status.OK).entity(contaReceberVO).build();
		} catch (Exception e) {
			ErrorInfoRSVO errorInfoRSVO = new ErrorInfoRSVO();
			errorInfoRSVO.setCodigo(Status.NOT_FOUND.name());
			errorInfoRSVO.setMensagem(e.getMessage());
			throw new WebServiceException(errorInfoRSVO, Status.NOT_FOUND);
		}
	}
	...
	public Response carregarQuantidadeFinanceiroAluno(@PathParam("matricula") final String matricula, @PathParam("codigoCurso") final Integer codigoCurso, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("tipoNivelEducacional") final String tipoNivelEducacional, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso, @PathParam("visaoLogar") final String visaoLogar, @PathParam("situacao") final String situacao) {
		try {
			...
			ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO = getFacadeFactory().getConfiguracaoGeralSistemaFacade().consultarConfiguracaoASerUsada(Uteis.NIVELMONTARDADOS_DADOSBASICOS_APLICATIVO, usuarioVO, codigoUnidadeEnsinoMatricula);
			QuantidadeObjetoExistentesRSVO quantidadeExistentes = new QuantidadeObjetoExistentesRSVO();			
			quantidadeExistentes.setValor(getFacadeFactory().getContaReceberFacade().consultarQuantidadeContasAPagarVisaoAluno(configuracaoGeralSistemaVO, configuracaoFinanceiroVO, codigoCurso, matricula, situacao, usuarioVO, new HashMap<Integer, ConfiguracaoRecebimentoCartaoOnlineVO>(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO, true, null, null));
			return Response.status(Status.OK).entity(quantidadeExistentes).build();
		} catch (Exception e) {
			ErrorInfoRSVO errorInfoRSVO = new ErrorInfoRSVO();
			errorInfoRSVO.setCodigo(Status.UNAUTHORIZED.name());
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/carregarFinanceiroAluno/{matricula}/{codigoCurso}/{codigoUnidadeEnsinoMatricula}/{tipoNivelEducacional}/{codigoPerfilAcesso}")
	public Response carregarTodoFinanceiroAluno(@PathParam("matricula") final String matricula, @PathParam("codigoCurso") final Integer codigoCurso, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("tipoNivelEducacional") final String tipoNivelEducacional, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso) {
		try {
			UsuarioVO usuarioVO = autenticarConexao();
			if (usuarioVO.getPessoa().getPossuiAcessoVisaoPais() && !usuarioVO.getPessoa().getAluno() && !usuarioVO.getPessoa().getProfessor() && !usuarioVO.getPessoa().getFuncionario() && !usuarioVO.getPessoa().getCoordenador()) {
				usuarioVO.setVisaoLogar("pais");
			} else if (!usuarioVO.getPessoa().getAtivo() && usuarioVO.getPessoa().getCodigo() > 0 && !usuarioVO.getPessoa().getAluno() && usuarioVO.getPessoa().getPossuiAcessoVisaoPais()) {
				usuarioVO.setVisaoLogar("pais");
			}  else {
				usuarioVO.setVisaoLogar("aluno");
			}
			...
			usuarioVO.getPerfilAcesso().setCodigo(codigoPerfilAcesso);
			List<ContaReceberVO> contaReceberVOs = new ArrayList<ContaReceberVO>();
			contaReceberVOs = getFacadeFactory().getContaReceberFacade().consultarContasAPagarVisaoAluno(configuracaoGeralSistemaVO, configuracaoFinanceiroVO, codigoCurso, matricula, "MB", usuarioVO, new HashMap<Integer, ConfiguracaoRecebimentoCartaoOnlineVO>(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO, true, null, null, new DataModelo());
			//getFacadeFactory().getContaReceberFacade().validarTipoImpressao(contaReceberVOs, usuarioVO);
			// Criado esse while, para limpar a linha digitavel do boleto, pois esse dado pode estar sendo apresentado errado, 
			// pois existe regras de modificação da linha digitavel e vencimento de boleto, que somente executam quando emite o PDF.
			// Inicialmente foi retirado o valor do objeto e posteriormente pensar em criar permissão para apresentar ou nao.
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/carregarQuantidadeFinanceiroAluno/{matricula}/{codigoCurso}/{codigoUnidadeEnsinoMatricula}/{tipoNivelEducacional}/{codigoPerfilAcesso}")
	public Response carregarQuantidadeFinanceiroAlunoDepreciado(@PathParam("matricula") final String matricula, @PathParam("codigoCurso") final Integer codigoCurso, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("tipoNivelEducacional") final String tipoNivelEducacional, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso) {
		try {
			UsuarioVO usuarioVO = autenticarConexao();
			if (usuarioVO.getPessoa().getPossuiAcessoVisaoPais() && !usuarioVO.getPessoa().getAluno() && !usuarioVO.getPessoa().getProfessor() && !usuarioVO.getPessoa().getFuncionario() && !usuarioVO.getPessoa().getCoordenador()) {
				usuarioVO.setVisaoLogar("pais");
			...
			ConfiguracaoGeralSistemaVO configuracaoGeralSistemaVO = getFacadeFactory().getConfiguracaoGeralSistemaFacade().consultarConfiguracaoASerUsada(Uteis.NIVELMONTARDADOS_DADOSBASICOS_APLICATIVO, usuarioVO, codigoUnidadeEnsinoMatricula);
			QuantidadeObjetoExistentesRSVO quantidadeExistentes = new QuantidadeObjetoExistentesRSVO();			
			quantidadeExistentes.setValor(getFacadeFactory().getContaReceberFacade().consultarQuantidadeContasAPagarVisaoAluno(configuracaoGeralSistemaVO, configuracaoFinanceiroVO, codigoCurso, matricula, "MB", usuarioVO, new HashMap<Integer, ConfiguracaoRecebimentoCartaoOnlineVO>(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO, true, null, null));
			return Response.status(Status.OK).entity(quantidadeExistentes).build();
		} catch (Exception e) {
			ErrorInfoRSVO errorInfoRSVO = new ErrorInfoRSVO();
			errorInfoRSVO.setCodigo(Status.UNAUTHORIZED.name());
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/carregarFinanceiroAluno/{matricula}/{codigoCurso}/{codigoUnidadeEnsinoMatricula}/{tipoNivelEducacional}/{codigoPerfilAcesso}/{limit}/{offset}")
	public Response carregarFinanceiroAlunoDepreciado(@PathParam("matricula") final String matricula, @PathParam("codigoCurso") final Integer codigoCurso, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("tipoNivelEducacional") final String tipoNivelEducacional, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso, @PathParam("limit") final Integer limit, @PathParam("offset") final Integer offset) {
		try {
			UsuarioVO usuarioVO = autenticarConexao();
			if (usuarioVO.getPessoa().getPossuiAcessoVisaoPais() && !usuarioVO.getPessoa().getAluno() && !usuarioVO.getPessoa().getProfessor() && !usuarioVO.getPessoa().getFuncionario() && !usuarioVO.getPessoa().getCoordenador()) {
				usuarioVO.setVisaoLogar("pais");
			...
			usuarioVO.getPerfilAcesso().setCodigo(codigoPerfilAcesso);
			List<ContaReceberVO> contaReceberVOs = new ArrayList<ContaReceberVO>();
			contaReceberVOs = getFacadeFactory().getContaReceberFacade().consultarContasAPagarVisaoAluno(configuracaoGeralSistemaVO, configuracaoFinanceiroVO, codigoCurso, matricula, "MB", usuarioVO, new HashMap<Integer, ConfiguracaoRecebimentoCartaoOnlineVO>(), Uteis.NIVELMONTARDADOS_DADOSBASICOS, usuarioVO, true, limit, offset, new DataModelo());
			//getFacadeFactory().getContaReceberFacade().validarTipoImpressao(contaReceberVOs, usuarioVO);
			// Criado esse while, para limpar a linha digitavel do boleto, pois esse dado pode estar sendo apresentado errado, 
			// pois existe regras de modificação da linha digitavel e vencimento de boleto, que somente executam quando emite o PDF.
			// Inicialmente foi retirado o valor do objeto e posteriormente pensar em criar permissão para apresentar ou nao.
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/carregarQuantidadeFinanceiroAluno/{matricula}/{codigoCurso}/{codigoUnidadeEnsinoMatricula}/{tipoNivelEducacional}/{codigoPerfilAcesso}/{visaoLogar}")
	public Response carregarQuantidadeFinanceiroAlunoDepreciado(@PathParam("matricula") final String matricula, @PathParam("codigoCurso") final Integer codigoCurso, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("tipoNivelEducacional") final String tipoNivelEducacional, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso, @PathParam("visaoLogar") final String visaoLogar) {
		try {
			UsuarioVO usuarioVO = autenticarConexao();
			usuarioVO.setVisaoLogar(visaoLogar);
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/carregarFinanceiroAluno/{matricula}/{codigoCurso}/{codigoUnidadeEnsinoMatricula}/{tipoNivelEducacional}/{codigoPerfilAcesso}/{limit}/{offset}/{visaoLogar}")
	public Response carregarFinanceiroAlunoDepreciado(@PathParam("matricula") final String matricula, @PathParam("codigoCurso") final Integer codigoCurso, @PathParam("codigoUnidadeEnsinoMatricula") final Integer codigoUnidadeEnsinoMatricula, @PathParam("tipoNivelEducacional") final String tipoNivelEducacional, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso, @PathParam("limit") final Integer limit, @PathParam("offset") final Integer offset, @PathParam("visaoLogar") final String visaoLogar) {
		try {
			UsuarioVO usuarioVO = autenticarConexao();
			usuarioVO.setVisaoLogar(visaoLogar);
	...
	public MatriculaRSVO realizarGeracaoContratoMatriculaOnlineExterna(final MatriculaRSVO matriculaRSVO) {
		try {
			...
			getFacadeFactory().getMatriculaFacade().verificarExistenciaPreencherDadosPertinentesMatriculaOnlineRealizadaPendendeAssinaturaContratoEletronicoEntregaDocumento(matriculaRSVO,false, "CONTRATO"  ,usuarioVO, true, configuracaoGeralSistema, configuracaoFinanceiroVO, null);		
			return matriculaRSVO;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Response realizarAlteracaoSenha(String alterarSenha) {
		try {
			...
			ControleAcesso.verificarPermissaoFuncionalidadeUsuario("ConfiguracoesAlteracaoSenha", usuarioVO);
	...
	public Response realizarAlteracaoFoto(@Context HttpServletRequest request) {
		try {
			UsuarioVO usuarioVO = obterUsuarioWebService();		
			usuarioVO.setVisaoLogar("aluno");
			if(request.getHeader("perfilAcesso") == null || !Uteis.getIsValorNumerico(request.getHeader("perfilAcesso"))) {
				throw new Exception("Deve ser informado no header da requisção o código do perfil de acesso.");
			}
			usuarioVO.getPerfilAcesso().setCodigo(Integer.valueOf(request.getHeader("perfilAcesso")));
			ControleAcesso.verificarPermissaoFuncionalidadeUsuario("ConfiguracoesAlteracaoSenha", usuarioVO);
			if (ServletFileUpload.isMultipartContent(request)) {
				FileItemFactory factory = new DiskFileItemFactory();
				ServletFileUpload upload = new ServletFileUpload(factory);
				...
				if (items != null) {
					...
					while (iter.hasNext()) {
						...
						if (!item.isFormField() && item.getSize() > 0) {
							...
							if(Uteis.isAtributoPreenchido(usuarioVO.getPessoa().getArquivoImagem().getCodigo())) {
								arquivoOriginal =  new File(getConfiguracaoGeralSistemaVO().getLocalUploadArquivoFixo()+File.separator+usuarioVO.getPessoa().getArquivoImagem().getPastaBaseArquivo()+File.separator+usuarioVO.getPessoa().getArquivoImagem().getNome());
							}
							usuarioVO.getPessoa().getArquivoImagem().setPastaBaseArquivoEnum(PastaBaseArquivoEnum.IMAGEM_TMP);
							usuarioVO.getPessoa().getArquivoImagem().setCpfRequerimento(usuarioVO.getPessoa().getCPF());
							
							getFacadeFactory().getArquivoHelper().upLoadDocumentacaoMatriculaRequerimentoAplicativo(item, usuarioVO.getPessoa().getArquivoImagem(), "foto_"+usuarioVO.getPessoa().getCodigo()+"_"+Uteis.getDataComHora(new Date()),getConfiguracaoGeralSistemaVO(), PastaBaseArquivoEnum.IMAGEM_TMP, usuarioVO);
							usuarioVO.getPessoa().inicializarDadosNecessidadeGerarProspectPelaConfiguracaoGeralSistemaTipoOrigem(getConfiguracaoGeralPadraoSistema(), "ALUNO");
							getFacadeFactory().getPessoaFacade().alterarFoto(usuarioVO.getPessoa(), getUsuarioLogado(),getConfiguracaoGeralPadraoSistema());
							usuarioVO.setCaminhoFotoUsuarioAplicativo(getFacadeFactory().getArquivoHelper().renderizarFotoUsuario(usuarioVO.getPessoa().getArquivoImagem(), PastaBaseArquivoEnum.IMAGEM.getValue(), getConfiguracaoGeralSistemaVO(), getCaminhoWeb(usuarioVO), "foto_usuario.jpg", false));
	...
	public Response carregarMinhasNotasAlunoV2(@PathParam("matricula") final String matricula, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso, @PathParam("ano") final String ano, @PathParam("semestre") final String semestre) throws Exception {
		try {
			...
			usuarioVO.getPerfilAcesso().setCodigo(codigoPerfilAcesso);
			usuarioVO.setVisaoLogar("aluno");
			ControleAcesso.verificarPermissaoFuncionalidadeUsuario("MinhasNotas", usuarioVO);
			MatriculaVO matriculaVO = new MatriculaVO();
			matriculaVO.setMatricula(matricula);
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/realizarGeracaoRegistroAulaPeloHorarioProfessorDia/V2/{horarioProfessorDia}/{codigoPessoa}/{codigoTurma}/{codigoDisciplina}/{ano}/{semestre}/{codigoPerfilAcesso}")
	public Response realizarGeracaoRegistroAulaPeloHorarioProfessorDiaV2(@PathParam("horarioProfessorDia") final Integer horarioProfessorDia, @PathParam("codigoPessoa") final Integer codigoPessoa, @PathParam("codigoTurma") 
	final Integer codigoTurma, @PathParam("codigoDisciplina") final Integer codigoDisciplina, @PathParam("ano") final Integer ano, @PathParam("semestre") 
	final Integer semestre, @PathParam("codigoPerfilAcesso") final Integer codigoPerfilAcesso) {
		try {
		UsuarioVO usuarioVO = autenticarConexao();
		...
		for(RegistroAulaVO r: registroAulaVOs){
			UtilReflexao.removerCamposChamadaAPI(r.getTurma(), "codigo", "identificadorTurma", "anual", "semestral", "tipoTurma", "turmaAgrupada");
			UtilReflexao.removerCamposChamadaAPI(r.getProfessor(), "codigo", "nome");
			for(FrequenciaAulaVO f: r.getFrequenciaAulaVOs()){
				UtilReflexao.removerCamposChamadaAPI(f.getMatricula(), "matricula", "aluno", "gradeCurricularAtual", "situacao", "data", "situacaoFinanceira", "curso");
				UtilReflexao.removerCamposChamadaAPI(f.getMatricula().getAluno(), "codigo", "nome");
				UtilReflexao.removerCamposChamadaAPI(f.getMatricula().getGradeCurricularAtual(), "codigo");
				UtilReflexao.removerCamposChamadaAPI(f.getMatricula().getCurso(), "codigo", "nome", "configuracaoAcademico");
				UtilReflexao.removerCamposChamadaAPI(f.getMatricula().getCurso().getConfiguracaoAcademico(), "codigo", "nome");
				f.setHistoricoVO(null);
				f.getMatriculaPeriodoTurmaDisciplinaVO().setMatriculaObjetoVO(null);
				UtilReflexao.removerCamposChamadaAPI(f.getMatriculaPeriodoTurmaDisciplinaVO().getMatriculaPeriodoObjetoVO(), "codigo", "ano", "semestre", "situacao", "situacaoMatriculaPeriodo", "matriculaVO");					
				UtilReflexao.removerCamposChamadaAPI(f.getMatriculaPeriodoTurmaDisciplinaVO().getMatriculaPeriodoObjetoVO().getMatriculaVO().getAluno(), "codigo", "nome");					
				UtilReflexao.removerCamposChamadaAPI(f.getMatriculaPeriodoTurmaDisciplinaVO().getMatriculaPeriodoObjetoVO().getMatriculaVO().getGradeCurricularAtual(), "codigo");					
				UtilReflexao.removerCamposChamadaAPI(f.getMatriculaPeriodoTurmaDisciplinaVO().getMatriculaPeriodoObjetoVO().getMatriculaVO().getCurso(), "codigo", "nome");					
				UtilReflexao.removerCamposChamadaAPI(f.getMatriculaPeriodoTurmaDisciplinaVO().getDisciplina(), "codigo");
				UtilReflexao.removerCamposChamadaAPI(f.getMatriculaPeriodoTurmaDisciplinaVO().getTurma(),"codigo");
				if(Uteis.isAtributoPreenchido(f.getMatriculaPeriodoTurmaDisciplinaVO().getTurmaTeorica())) {
					UtilReflexao.removerCamposChamadaAPI(f.getMatriculaPeriodoTurmaDisciplinaVO().getTurmaTeorica(),"codigo");
				}else {
					f.getMatriculaPeriodoTurmaDisciplinaVO().setTurmaTeorica(null);
				}
				if(Uteis.isAtributoPreenchido(f.getMatriculaPeriodoTurmaDisciplinaVO().getTurmaPratica())) {
					UtilReflexao.removerCamposChamadaAPI(f.getMatriculaPeriodoTurmaDisciplinaVO().getTurmaPratica(),"codigo");
				}else {
					f.getMatriculaPeriodoTurmaDisciplinaVO().setTurmaPratica(null);
				}
			};
		};
		Gson gson = new GsonBuilder().setExclusionStrategies(new ExcluirJsonStrategy()).setDateFormat("MM-dd-yyyy HH:mm:ss").create();
		String json =	gson.toJson(registroAulaVOs);
...
//		return Response.status(Status.OK).entity(new GenericEntity<List<RegistroAulaVO>>(registroAulaVOs) {}).build();
	...
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/V2/gravarRegistroAula")
	public Response gravarRegistroAulaV2(final String listaRegistrosAula) {
		try {
			...
			gson = new GsonBuilder().setExclusionStrategies(new ExcluirJsonStrategy()).setDateFormat("MM-dd-yyyy HH:mm:ss").create();
    		String json =	gson.toJson(registroAulaVOs);
			return Response.status(Status.OK).entity(json).build();
		} catch (Exception e) {
			System.out.println("MOBILE ERROR: " + e.getMessage());
	...
	public Response consultarDocumentoDigitaisAluno(@PathParam("matricula") final String matricula, @PathParam("perfilAcesso") final Integer perfilAcesso,@PathParam("limite") final Integer limite, @PathParam("pagina") final Integer pagina) {
		try {
			...
			try {
				ControleAcesso.consultar(PerfilAcessoPermissaoVisaoAlunoEnum.MEUS_CONTRATOS.getValor(), true, usuarioVO);		
