...
public class AutenticadorWS extends ControleAcesso {
	...
	private UsuarioVO realizarAutenticacaoUsuario(final String username, final String password, final String naoEncryptar) {
		...
		try {
			if (naoEncryptar != "") {
				usuarioVO = ControleAcesso.verificarLoginUsuario(username, password, false, Uteis.NIVELMONTARDADOS_DADOSLOGIN);
			} else {
				usuarioVO = ControleAcesso.verificarLoginUsuario(username, password, true, Uteis.NIVELMONTARDADOS_DADOSLOGIN);
			}
		} catch (Exception e) {
			return usuarioVO;
		}
