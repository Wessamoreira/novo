...
public class NotaFiscalServicoEletronicaGoiania extends ControleAcesso implements NotaFiscalServicoEletronicaInterfaceFacade {
	/**
	 * 
	 */
	...
	public NotaFiscalServicoEletronicaGoiania() throws Exception {
		super();
		setIdEntidade("NotaFiscalServicoEletronicaGoiania");
	}
	...
	public static void setIdEntidade(String idEntidade) {
		NotaFiscalServicoEletronicaGoiania.idEntidade = idEntidade;
	}
	...
	public Rps montarXmlEnvio(NotaFiscalSaidaVO notaFiscalSaidaVO,UsuarioVO usuarioVO) throws Exception{
		...
		Integer codigoDMS = getFacadeFactory().getCidadeFacade().consultarCodigoDMS(notaFiscalSaidaVO.getPessoaVO().getCidade(), Uteis.NIVELMONTARDADOS_TODOS, usuarioVO);
		...
		.withEndereco(
				Nfse.endereco()
				.withCodigoMunicipio(codigoDMS.toString().trim())				
				.build()
		).build();
		if(notaFiscalSaidaVO.getTipoPessoa().equals(TipoPessoa.FORNECEDOR.getValor())) {
			...
			tomador.getEndereco().setCodigoMunicipio(notaFiscalSaidaVO.getParceiroVO().getCidade().getCodigoDMS().toString().trim());
...
//			}
		...
		Rps rps = Nfse.rps()
				...
				.build();	
		rps.getInfDeclaracaoPrestacaoServico().getRps().getIdentificacaoRps().setSerie(notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getAmbienteNfeEnum().equals(AmbienteNfeEnum.HOMOLOGACAO) ? Serie.TESTE : Serie.UNICA);		
		return  rps;
	}
