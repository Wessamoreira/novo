...
public class NotaFiscalServicoEletronicaAnapolis extends ControleAcesso implements NotaFiscalServicoEletronicaInterfaceFacade {
	/**
	 * 
	 */
	...
	public NotaFiscalServicoEletronicaAnapolis() throws Exception {
		super();
		setIdEntidade("NotaFiscalServicoEletronicaAnapolis");
	}
	...
	public static void setIdEntidade(String idEntidade) {
		NotaFiscalServicoEletronicaAnapolis.idEntidade = idEntidade;
	}
	...
	public LoteRps montarXmlEnvio(NotaFiscalSaidaVO notaFiscalSaidaVO,UsuarioVO usuarioVO) throws Exception{
		...
		if(notaFiscalSaidaVO.getTipoPessoa().equals(TipoPessoa.FORNECEDOR.getValor())) {
			...
			tomador.getEndereco().setEstado(notaFiscalSaidaVO.getFornecedorVO().getCidade().getEstado().getSigla());
			...
			tomador.getEndereco().setCidade(notaFiscalSaidaVO.getFornecedorVO().getCidade().getCodigoIBGE());
			...
			tomador.getEndereco().setEstado(notaFiscalSaidaVO.getParceiroVO().getCidade().getEstado().getSigla());
			...
			tomador.getEndereco().setCidade(notaFiscalSaidaVO.getParceiroVO().getCidade().getCodigoIBGE());
			...
			tomador.getEndereco().setEstado(notaFiscalSaidaVO.getResponsavelFinanceiro().getCidade().getEstado().getSigla());
			...
			tomador.getEndereco().setCidade(notaFiscalSaidaVO.getResponsavelFinanceiro().getCidade().getCodigoIBGE());
			...
			tomador.getEndereco().setEstado(notaFiscalSaidaVO.getPessoaVO().getCidade().getEstado().getSigla());
			...
			tomador.getEndereco().setCidade(notaFiscalSaidaVO.getPessoaVO().getCidade().getCodigoIBGE());
		...
		servico.getValores().setBaseCalculo(servico.getValores().getValorServicos() - servico.getValores().getValorDeducoes() - servico.getValores().getDescontoIncondicionado());
		servico.getValores().setValorLiquidoNfse(servico.getValores().getValorServicos() - servico.getValores().getValorPis() 
				- servico.getValores().getValorCofins() - servico.getValores().getValorInss() 
				- servico.getValores().getValorIr() - servico.getValores().getValorCsll() 
				- servico.getValores().getOutrasRetencoes() - servico.getValores().getDescontoCondicionado() 
				- servico.getValores().getDescontoIncondicionado());
		servico.setItemListaServico(notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getCodigoItemListaServico().replace(".", ""));
		servico.getValores().setIssRetido(SimNao.NAO);
		String cnpj = notaFiscalSaidaVO.getUnidadeEnsinoVO().getCNPJ().replace("-", "").replace("/", "").replace(".", "").trim();
...
//		if ((dataCompetencia.get(Calendar.MONTH) + 1) < 10) {
//			rps.getInfDeclaracaoPrestacaoServico().setCompetencia(dataCompetencia.get(Calendar.YEAR) + "0" + (dataCompetencia.get(Calendar.MONTH) + 1));			
//		} else {
//			rps.getInfDeclaracaoPrestacaoServico().setCompetencia(dataCompetencia.get(Calendar.YEAR) + "" + (dataCompetencia.get(Calendar.MONTH) + 1));			
//		}
		...
		rps.getInfDeclaracaoPrestacaoServico().getServico().setCodigoCnae(notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getCodigoNCM());
//		rps.getInfDeclaracaoPrestacaoServico().setRegimeEspecialTributacao(RegimeEspecialTributacao.ME_EPP_SIMPLES_NACIONAL);
		rps.getInfDeclaracaoPrestacaoServico().getServico().setMunicipioPrestacaoServico("5201108"); // CODIGO DE ANAPOLIS 5201108
		rps.getInfDeclaracaoPrestacaoServico().getIdentificacaoRps().setSerie("8");
...
//				.v2_02()
	...
	public void consultarLoteRps(NotaFiscalSaidaVO notaFiscalSaidaVO,ConfiguracaoGeralSistemaVO configuracaoRespositoriArquivo, ProgressBarVO progressBar, String autorizado, UsuarioVO usuarioVO) throws Exception{
		try {
			...
			notaFiscalSaidaVO.setWebServicesNFSEEnum(WebServicesNFSEEnum.ANAPOLIS_GO_CONSULTAR_SITUACAO_LOTE);
			...
			Thread.sleep(10000);
 			retornoSituacao = ConexaoNFSE.conexaoNFSEAnapolis(notaFiscalSaidaVO, xmlEnvioConsulta, 
					...
					notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getSenhaUnidadeCertificadora(),"ConsultaSituacaoLoteRPS", usuarioVO);
 			retornoSituacao = retornoSituacao.replace("&lt;", "<").replace("&gt;", ">");
			...
			} else if (retornoSituacao.contains("<ListaMensagemRetorno>")) {
				notaFiscalSaidaVO.setSituacao(SituacaoNotaFiscalSaidaEnum.REJEITADA.getValor());
				notaFiscalSaidaVO.setMensagemRetorno(retornoSituacao);
			...
			if (situacao.equals("4") || situacao.equals("3") ) {//retorno de processamento for igual a 4, significa que o lote foi processado com sucesso.
				...
				notaFiscalSaidaVO.setWebServicesNFSEEnum(WebServicesNFSEEnum.ANAPOLIS_GO_CONSULTAR_LOTE);
				retorno = ConexaoNFSE.conexaoNFSEAnapolis(notaFiscalSaidaVO, xmlEnvioConsulta, 
						UteisNfe.getCaminhoCertificado(notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO(), configuracaoRespositoriArquivo), 
						notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getSenhaCertificado(), configuracaoRespositoriArquivo.getLocalUploadArquivoFixo() + 
						File.separator + PastaBaseArquivoEnum.NFE.getValue() + File.separator + "ca.jks", 
						notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getSenhaUnidadeCertificadora(),"ConsultarLoteRps", usuarioVO);
	...
	public static void validarLoteRps(NotaFiscalSaidaVO notaFiscalSaidaVO,ConfiguracaoGeralSistemaVO configuracaoRespositoriArquivo, ProgressBarVO progressBar, String autorizado, UsuarioVO usuarioVO) throws Exception {
		try {
			notaFiscalSaidaVO.setSituacao(SituacaoNotaFiscalSaidaEnum.ENVIADA.getValor());
			getFacadeFactory().getNotaFiscalSaidaFacade().gravarSituacaoEnvio(notaFiscalSaidaVO, SituacaoNotaFiscalSaidaEnum.ENVIADA.getValor(), "", notaFiscalSaidaVO.getDataStuacao(), notaFiscalSaidaVO.getIdentificadorReceita(), notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO(), usuarioVO);
			if (notaFiscalSaidaVO.getMensagemRetorno().contains("<tc:CodigoVerificacao>")) {
				String numeroNota = notaFiscalSaidaVO.getMensagemRetorno().substring(notaFiscalSaidaVO.getMensagemRetorno().indexOf("<tc:Numero>"), notaFiscalSaidaVO.getMensagemRetorno().indexOf("</tc:Numero>"));
				numeroNota = numeroNota.replace("<tc:Numero>", "");
				String codigoVerificacao = notaFiscalSaidaVO.getMensagemRetorno().substring(notaFiscalSaidaVO.getMensagemRetorno().lastIndexOf("<tc:CodigoVerificacao>"), notaFiscalSaidaVO.getMensagemRetorno().indexOf("</tc:CodigoVerificacao>"));
				codigoVerificacao = codigoVerificacao.replace("<tc:CodigoVerificacao>", "");
				...
				notaFiscalSaidaVO.setLinkAcesso(notaFiscalSaidaVO.getWebServicesNFSEEnum().getValorApresentar());
			} 
		} catch (Exception e) {
			autorizado = e.getMessage();
			getFacadeFactory().getNotaFiscalSaidaFacade().alterarMensagemRetorno(notaFiscalSaidaVO, usuarioVO);
