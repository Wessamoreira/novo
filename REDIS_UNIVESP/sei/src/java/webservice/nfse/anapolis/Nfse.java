...
public class Nfse {
	...
	public String asGerarNFSEXML(NotaFiscalSaidaVO notaFiscalSaidaVO, ConfiguracaoGeralSistemaVO configuracaoRespositoriArquivo, String Id) throws Exception {
		...
		LOG.debug("XML:\n{}", xml);
		String pasta = configuracaoRespositoriArquivo.getLocalUploadArquivoFixo() + File.separator + PastaBaseArquivoEnum.NFE.getValue() + File.separator + PastaBaseArquivoEnum.NOTAS_ENVIADAS.getValue() + File.separator + notaFiscalSaidaVO.getUnidadeEnsinoVO().getCodigo();
		File p = new File(pasta);
		if (!p.exists()) {
			p.mkdirs();
		}
		String nomeArquivoXML = pasta + File.separator + Uteis.getMontarCodigoBarra(notaFiscalSaidaVO.getNumero().toString(), 14) + ".xml";		
		xml = Assinador.assinarNFSeInfRpsSemID(notaFiscalSaidaVO, configuracaoRespositoriArquivo, xml, "LoteRps", nomeArquivoXML,"");
		if(enableValidation) {
			try {
