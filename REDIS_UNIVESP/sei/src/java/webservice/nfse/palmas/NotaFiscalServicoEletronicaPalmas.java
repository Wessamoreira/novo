...
public class NotaFiscalServicoEletronicaPalmas extends ControleAcesso implements NotaFiscalServicoEletronicaInterfaceFacade {
	/**
	 * 
	 */
	...
	public NotaFiscalServicoEletronicaPalmas() throws Exception {
		super();
		setIdEntidade("NotaFiscalServicoEletronicaPalmas");
	}
	...
	public static void setIdEntidade(String idEntidade) {
		NotaFiscalServicoEletronicaPalmas.idEntidade = idEntidade;
	}
...
//			}
	...
	public LoteRps montarXmlEnvio(NotaFiscalSaidaVO notaFiscalSaidaVO,UsuarioVO usuarioVO) throws Exception{
		...
		Servico servico = Nfse.servico()
				...
				.withCodigoTributacaoMunicipio(notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getCodigoItemListaServico().replace(".", ""))
				.withItemListaServico(notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getCodigoItemListaServico().replace(".", ""))
				.withCodigoCnae(Integer.parseInt(notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getCodigoNaturezaOperacao()))
				.withCodigoMunicipio(notaFiscalSaidaVO.getUnidadeEnsinoVO().getCidade().getCodigoIBGE())
				.build();
	...
	public void consultarLoteRps(NotaFiscalSaidaVO notaFiscalSaidaVO,ConfiguracaoGeralSistemaVO configuracaoRespositoriArquivo, ProgressBarVO progressBar, String autorizado, UsuarioVO usuarioVO) throws Exception{
		try {
			...
			notaFiscalSaidaVO.setWebServicesNFSEEnum(WebServicesNFSEEnum.PALMAS_TO_CONSULTAR_SITUACAO_LOTE);
			...
			Thread.sleep(15000);
 			retornoSituacao = ConexaoNFSE.conexaoNFSEPalmasSoap11(notaFiscalSaidaVO, "ConsultarSituacaoLoteRps", xmlEnvioConsulta, 
 					...
 					notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getSenhaCertificado(), 
 					configuracaoRespositoriArquivo.getLocalUploadArquivoFixo() + File.separator + PastaBaseArquivoEnum.NFE.getValue() + File.separator + "ca.jks", 
 					notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getSenhaUnidadeCertificadora(), usuarioVO);
			...
			for (int i = 0; i < 2 && situacao.equals("2"); i++) {
				Thread.sleep(10000);
				retornoSituacao = ConexaoNFSE.conexaoNFSEPalmasSoap11(notaFiscalSaidaVO, "ConsultarSituacaoLoteRps", 
						xmlEnvioConsulta, UteisNfe.getCaminhoCertificado(notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO(), 
								configuracaoRespositoriArquivo), notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getSenhaCertificado(), 
						configuracaoRespositoriArquivo.getLocalUploadArquivoFixo() + File.separator + PastaBaseArquivoEnum.NFE.getValue() + File.separator + "ca.jks", 
						notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getSenhaUnidadeCertificadora(), usuarioVO);
				if (retornoSituacao.contains("<Situacao>")) {
					 situacao = retornoSituacao.substring(retornoSituacao.indexOf("<Situacao>")+"<Situacao>".length(), retornoSituacao.indexOf("</Situacao>"));
			...
			if (situacao.equals("4") || situacao.equals("3") ) {//retorno de processamento for igual a 4, significa que o lote foi processado com sucesso.
				...
				notaFiscalSaidaVO.setWebServicesNFSEEnum(WebServicesNFSEEnum.PALMAS_TO_CONSULTAR_LOTE);
				retorno = ConexaoNFSE.conexaoNFSEPalmasSoap11(notaFiscalSaidaVO, "ConsultarLoteRps", 
						xmlEnvioConsulta, UteisNfe.getCaminhoCertificado(notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO(), 
								configuracaoRespositoriArquivo), notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getSenhaCertificado(), 
						configuracaoRespositoriArquivo.getLocalUploadArquivoFixo() + File.separator + PastaBaseArquivoEnum.NFE.getValue() + File.separator + "ca.jks", 
						notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getSenhaUnidadeCertificadora(), usuarioVO);
	...
	public static void validarLoteRps(NotaFiscalSaidaVO notaFiscalSaidaVO,ConfiguracaoGeralSistemaVO configuracaoRespositoriArquivo, ProgressBarVO progressBar, String autorizado, UsuarioVO usuarioVO) throws Exception {
		try {
			...
			if (notaFiscalSaidaVO.getMensagemRetorno().contains("<CodigoVerificacao>")) {
				...
				getFacadeFactory().getNotaFiscalSaidaFacade().gravarSituacaoEnvio(notaFiscalSaidaVO, SituacaoNotaFiscalSaidaEnum.AUTORIZADA.getValor(), "", notaFiscalSaidaVO.getDataStuacao(), notaFiscalSaidaVO.getIdentificadorReceita(), notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO(), usuarioVO);
				notaFiscalSaidaVO.setSituacao(SituacaoNotaFiscalSaidaEnum.AUTORIZADA.getValor());
				notaFiscalSaidaVO.setLinkAcesso("https://www5.webiss.com.br/palmasto/");
			} 
		} catch (Exception e) {
			progressBar.setStatus("NF-e n° " + notaFiscalSaidaVO.getNumeroNota() + " REJEITADA ( " + progressBar.getProgresso() + " de " + progressBar.getMaxValue() + " ) ");
			autorizado = e.getMessage();
