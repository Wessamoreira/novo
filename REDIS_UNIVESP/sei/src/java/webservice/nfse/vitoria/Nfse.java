...
//		xml = Assinador.assinarNFSeInfRpsComIDSemQuebraLinha(notaFiscalSaidaVO, configuracaoRespositoriArquivo, xml, "EN", nomeArquivoXML,"");
		xml = Assinador.assinarNFSeInfRpsComIDSemQuebraLinha(notaFiscalSaidaVO, configuracaoRespositoriArquivo, xml, "GerarNfseEnvio", nomeArquivoXML,"");
		if(enableValidation) {
			try {
	...
	public String asEnviarLoteRpsEnvioNFSEXML(NotaFiscalSaidaVO notaFiscalSaidaVO, ConfiguracaoGeralSistemaVO configuracaoRespositoriArquivo, String Id) throws Exception {
		String xml = XML_HEADER	+ xstream.toXML(enviarLoteRpsEnvio);
		LOG.debug("Validation enabled? {}", enableValidation);
		LOG.debug("XML:\n{}", xml);
		String nomeArquivoXML = configuracaoRespositoriArquivo.getLocalUploadArquivoFixo() + File.separator + PastaBaseArquivoEnum.NFE.getValue() + File.separator + PastaBaseArquivoEnum.NOTAS_ENVIADAS.getValue() + File.separator + Uteis.getMontarCodigoBarra(notaFiscalSaidaVO.getNumero().toString(), 14) + ".xml";		
//		xml = Assinador.assinarNFSeInfRpsComIDSemQuebraLinha(notaFiscalSaidaVO, configuracaoRespositoriArquivo, xml, "EN", nomeArquivoXML,"");
		xml = Assinador.assinarNFSeInfRpsComIDSemQuebraLinha(notaFiscalSaidaVO, configuracaoRespositoriArquivo, xml, "EnviarLoteRpsEnvio", nomeArquivoXML,"");
//		xml = Assinador.assinarNFSe(notaFiscalSaidaVO, configuracaoRespositoriArquivo, xml, "EN", nomeArquivoXML,"");
	...
	public Nfse withGerarNfseLoteRps(LoteRps loteRps) {
		gerarNfse.setLoteRps(loteRps);
		return this;
	}
