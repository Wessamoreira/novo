...
public class NotaFiscalServicoEletronicaVitoria extends ControleAcesso implements NotaFiscalServicoEletronicaInterfaceFacade {
	/**
	 * 
	 */
	...
	public NotaFiscalServicoEletronicaVitoria() throws Exception {
		super();
		setIdEntidade("NotaFiscalServicoEletronicaVitoria");
	}
	...
	public static void setIdEntidade(String idEntidade) {
		NotaFiscalServicoEletronicaVitoria.idEntidade = idEntidade;
	}
	...
	public void enviarXmlEnvio(NotaFiscalSaidaVO notaFiscalSaidaVO, ConfiguracaoGeralSistemaVO configuracaoRespositoriArquivo, ProgressBarVO progressBar, String autorizado, UsuarioVO usuarioVO) throws Exception {
		try {
			...
			notaFiscalSaidaVO.setXmlEnvio(builder.withLoteRps(loteRps).asEnviarLoteRpsEnvioNFSEXML(notaFiscalSaidaVO, configuracaoRespositoriArquivo, "1"+ cnpj +Uteis.preencherComZerosPosicoesVagas(notaFiscalSaidaVO.getNumeroNota(), 16)));
			...
			if (notaFiscalSaidaVO.getMensagemRetorno().contains("<CodigoVerificacao>")) {
				...
				notaFiscalSaidaVO.setLinkAcesso("https://nfe.portoalegre.rs.gov.br/nfse/pages/security/consultaNFS-e_prestador.jsf");
			} else {
	...
	public LoteRps montarXmlEnvio(NotaFiscalSaidaVO notaFiscalSaidaVO,UsuarioVO usuarioVO) throws Exception{
		...
		Servico servico = Nfse.servico()
				...
				.withItemListaServico(notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getCodigoItemListaServico())
				.build();
		...
		Rps rps = Nfse.rps()
				...
				.semIncentivoFiscal()
				.build();
		rps.getInfDeclaracaoPrestacaoServico().getRps().getIdentificacaoRps().setNumero(notaFiscalSaidaVO.getNumero());
		rps.getInfDeclaracaoPrestacaoServico().setId(notaFiscalSaidaVO.getNumero().toString());
...
//		rps.getInfDeclaracaoPrestacaoServico().setIncentivadorCultural(SimNao.NAO);
		rps.getInfDeclaracaoPrestacaoServico().getRps().getIdentificacaoRps().setSerie(notaFiscalSaidaVO.getUnidadeEnsinoVO().getConfiguracaoNotaFiscalVO().getAmbienteNfeEnum().equals(AmbienteNfeEnum.HOMOLOGACAO) ? Serie.TESTE : Serie.NFE);
//		rps.getInfDeclaracaoPrestacaoServico().setRegimeEspecialTributacao("6");
		LoteRps loteRps = Nfse.loteNfse()
				.withCnpj(cnpj)
				.withIdLote("Lote"+notaFiscalSaidaVO.getNumero().toString())
