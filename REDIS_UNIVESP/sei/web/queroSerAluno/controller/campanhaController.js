...
var urlBase = "http://186.202.179.83:8080/SEI/webservice/campanhaRS";

campanhaModule.config(['$httpProvider', '$routeProvider', function ($httpProvider, $routeProvider) {
	$routeProvider.
		when('/', {
			templateUrl: 'queroSerAluno.html',
...
campanhaModule.controller('campanhaControle', function ($scope, $http, $location, Scopes, $timeout, cfpLoadingBar) {
	...
	$scope.gerarAgendaLigacaoReceptivaQueroSerAlunoCampanhaCRM = function gerarAgendaLigacaoReceptivaQueroSerAlunoCampanhaCRM(curso, turno) {
		cfpLoadingBar.start();
		...
		$http.post(urlBase + '/gerarAgendaLigacaoReceptivaQueroSerAlunoCampanhaCRM', parametros, { headers: { 'Content-Type': 'application/json' } }).success(function (data) {
	...
	$scope.irQueroSerAluno = function irQueroSerAluno() {
		$location.path('/queroSerAluno');
	};
